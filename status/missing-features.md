...
- [x] Top-level and block-scoped `typedef` aliases: `typedef int Count;`, `typedef char Byte;`, `typedef struct Point Point;`, and one-level pointer aliases such as `typedef int *IntPtr;`, `typedef char *CharPtr;`, and `typedef struct Point *PointPtr;` after a prior struct declaration resolve at parse time without runtime storage changes; aliases work in globals, locals, arrays, one-level pointer declarations, function prototypes/definitions/parameters/returns, scalar struct fields, pointer-alias declarations/parameters, and `sizeof(alias)` / `sizeof(pointer_alias)`, with block-local shadowing/scope expiry, exact malformed typedef diagnostics including pointer-to-pointer alias rejection, and C compiler-oracle coverage
