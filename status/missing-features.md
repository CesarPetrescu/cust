...
- [ ] Arrays of supported struct types as the next aggregate milestone; acceptance: `struct Point points[2];` stores zero-initialized struct elements with deterministic Cust layout, supports element field access `points[i].x`, brace initializers for arrays of structs, copy/by-value semantics without aliasing embedded arrays/pointers beyond C pointer-value copying, targeted bounds/initializer diagnostics, and compiler-oracle coverage that avoids native struct `sizeof` assumptions.
