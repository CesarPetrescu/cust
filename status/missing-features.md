...
- [ ] Pointer fields inside structs as the next scoped aggregate/pointer milestone; acceptance: update `docs/plans/struct-model.md`/`docs/plans/pointer-model.md` before code, support one-level scalar/struct pointer fields such as `int *next_value;` and `struct Node *next;` only with interpreter-owned pointer metadata, define copy/initializer/const semantics explicitly, add valid/invalid/compiler-oracle fixtures, and avoid native ABI layout assumptions.
