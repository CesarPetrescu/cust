...
- [ ] Const-qualified pointer semantics beyond size/type contexts; acceptance: design and implement a deliberately scoped model for `const int *p` versus `int * const p` (or explicitly document a narrower first milestone), preserve existing scalar/array const diagnostics, add pointer write/reassignment valid/invalid fixtures, and add C compiler-oracle coverage for the supported subset
