...
- [ ] Better parser recovery/error messages (broad track largely expanded; missing-`=` diagnostics, missing-name/type diagnostics, context-aware unterminated-block diagnostics, array length edge diagnostics, pointer-parameter malformed-list coverage, and explicit unsupported pointer-return/pointer-array diagnostics are covered; remaining work should be driven by newly discovered malformed programs beyond current exact-error coverage)
