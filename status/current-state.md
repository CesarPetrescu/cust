...
cargo test
cargo test --test interpreter typedef_aggregate -- --nocapture
cargo test --test c_compat -- --nocapture
docker compose run --rm test
docker compose run --rm cust
```

All passed after the 2026-05-08 autonomous typedef-aggregate-definition run. This run added top-level aggregate typedef definitions such as `typedef struct Point { int x; int y; } Point;` and `typedef union Number { int value; char tag; } Number;`, registering the named aggregate type and parser-only alias in one declaration while reusing Cust's existing safe struct/union runtime model. Block-scoped aggregate typedef definitions are deliberately rejected until aggregate tag scopes are designed, preventing parser type-name leakage from block scopes. Coverage includes `tests/fixtures/valid/typedef_aggregate_definitions.c`, invalid `tests/fixtures/invalid/block_typedef_aggregate_definition.c`, native C compiler-oracle fixture `tests/fixtures/compat/valid/typedef_aggregate_definitions.c`, README examples, and updated `docs/plans/typedef-model.md`. Docker Compose emitted non-fatal `Docker Compose requires buildx plugin to be installed` warnings and fell back to the classic builder; both required Docker commands exited 0.
