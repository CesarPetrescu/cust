...
Bounded Cust v0.35.0 release preparation packages the completed aggregate-field direct `double *` slice. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/image version `0.34.0`, then passed after Cargo/lock and both image tags moved to `0.35.0`. Cargo metadata, the local CLI, and both rebuilt images report `cust 0.35.0`; local and remote annotated-tag preflight found no conflicting `v0.35.0`. Executable target listing independently reconciles 2,057 tests: 1,921 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Candidate evaluation considered bounded v0.35.0 release closure, shared-storage union bytes, typedef aliases for supported double forms, and parser/property expansion. Release closure was selected because it is the sole unchecked authoritative package and atomically packages independently reviewed behavior with bounded metadata/docs risk. Fresh independent complete-release-diff review returned `APPROVED`; formatting, warning-denied Clippy, all local tests, a no-cache rebuild, all Docker tests, runtime output `10`, exact local/image version checks, diff hygiene, and added-line secret scans pass. Release commit push and exact `origin/main` acceptance remain before annotated-tag creation; direct double typedef aliases are the next concrete language package after publication.
