...
Candidate evaluation considered v0.31.0 release closure, direct aggregate `double` scalar fields, shared-storage union bytes, and parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed array slice, and had bounded metadata/docs risk with exact executable version tests. Direct aggregate scalar fields are now the sole next concrete language package.
