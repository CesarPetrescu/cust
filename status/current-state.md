...
The bounded character-storage raw-memory slice now includes exactly prototyped overlap-safe `void *memmove(void *, const void *, size_t)`. Destination, source, and count evaluate once; up to 4,096 exact character-cell values are snapshotted before any write; forward, backward, and self overlap therefore follow C's temporary-array semantics without host addresses, host libc, direction-dependent mutation, or narrowing negative `signed char` values. Destination identity plus interpreter-owned provenance/lifetime/read-only metadata are retained, while integer and aggregate object representations remain outside the slice.

Focused RED/GREEN first proved missing `memmove` dispatch, then the shared bounded-copy path passed forward/backward/self-overlap and exact stored-value fixtures. Supplementary focused coverage proves zero count, const source, destination/source capacity, count shape/range, non-character storage, expired owners, exact runtime/`sizeof` declarations, 30-level nested non-evaluation, and user-definition precedence. The registered warning-free native fixture passes. Independent read-only review returned `APPROVED` with no blocking or material findings.

Formatting, warning-denied Clippy, all 1,420 local tests (1,286 interpreter, 98 fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,420-test Docker gate, runtime output `10`, focused `memmove`/`memcpy`, the compiler oracle, and `git diff --check` pass. See `references/cust-bounded-memory-move-function.md`.

Work-package evaluation considered overlap-safe character-storage `memmove`, broad integer/aggregate object-representation serialization, and lower-priority parser/fuzz expansion. `memmove` was selected because it was first in both authoritative queues, completes a coherent adjacent intrinsic on the reviewed snapshot/provenance model, and avoids inventing host ABI layout. Next, implement bounded character-storage `memcmp`, followed by `memset`.
