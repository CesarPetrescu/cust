...
cargo test
cargo test --test interpreter supports_struct_pointer_arrow_field_arithmetic -- --nocapture
cargo test --test c_compat supported_programs_match_c_compiler_exit_codes -- --nocapture
docker compose run --rm test
docker compose run --rm cust
```

All passed after the 2026-05-08 autonomous struct-pointer arrow field arithmetic run. This run closed the `->` parity gap for pointer-valued struct fields reached through a struct pointer: expressions/statements such as `slot->p`, `slot->p += 3`, `slot->p--`, `++slot->p`, and `slot->p = slot->p - 2` now reuse Cust's interpreter-owned array-backed pointer arithmetic, const-conversion, pointee-type, bounds/null/out-of-scope, and field-mutability checks instead of treating `slot->p` as a scalar field. Coverage includes `tests/fixtures/valid/struct_pointer_arrow_field_arithmetic.c`, native C compiler-oracle fixture `tests/fixtures/compat/valid/struct_pointer_arrow_field_arithmetic.c`, and `tests/interpreter.rs` / `tests/c_compat.rs` wiring. Docker Compose emitted non-fatal `Docker Compose requires buildx plugin to be installed` warnings and fell back to the classic builder; both required Docker commands exited 0.
