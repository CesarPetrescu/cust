...
- Top-level/local supported aggregate declarations can use brace initializers (including designated struct/union-array elements such as `struct Point points[3] = {[2] = {.x = 5}, [0] = {1, 2}};`), same-type aggregate-returning call expressions such as `struct Point p = make_point(...);` and `union Number n = make_number(5);`, aggregate-valued conditional/comma expressions such as `struct Point p = flag ? high : low;` and `union Number n = (side_effect(), right);`, or aggregate-array pointer indexed values such as `struct Point copy = p[i];` / `union Number picked = n[i];`, preserving branch short-circuiting, by-value copies, and const binding enforcement after initialization.
...
- one-dimensional array parameters such as `int values[3]`, passed by reference to the same array storage; C-style unsized scalar/aggregate array parameter spellings such as `int values[]`, `char text[]`, `struct Point points[]`, and `union Number numbers[]` are parsed as pointer parameters and interoperate with existing array/string/aggregate-array decay; string literals can be passed to `char` array or unsized `char text[]` parameters as read-only NUL-terminated byte arrays
...
cargo test
cargo test --test interpreter scalar_cast -- --nocapture
cargo test --test interpreter aggregate_cast -- --nocapture
cargo test --test c_compat -- --nocapture
docker compose run --rm test
docker compose run --rm cust
```

All passed after the 2026-05-08 autonomous scalar-cast/docs-refresh run. This run added scalar cast expressions for supported scalar types and scalar typedef aliases, so `(int)expr`, `(char)expr`, and `(Count)expr` parse at unary precedence, preserve operand side effects, and evaluate over Cust's existing deterministic integer storage model. Unsupported aggregate casts now report `aggregate casts are not supported` at the cast type token, and pointer casts remain explicitly unsupported. Coverage includes `tests/fixtures/valid/scalar_cast_expressions.c`, invalid `tests/fixtures/invalid/aggregate_cast_unsupported.c`, native C compiler-oracle fixture `tests/fixtures/compat/valid/scalar_cast_expressions.c`, and `tests/interpreter.rs` / `tests/c_compat.rs` wiring. The run also refreshed `README.md` and `docs/v0.1.md` so release-oriented docs no longer claim structs/unions/enums/prototypes/void functions are missing and now call out current interpreter/native-oracle boundaries plus remaining limitations. Docker Compose emitted non-fatal `Docker Compose requires buildx plugin to be installed` warnings and fell back to the classic builder; both required Docker commands exited 0.
