...
Bounded Cust v0.12.0 is prepared for atomic publication. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.12.0`. The release packages explicitly prototyped C11 `exit`, `_Exit`, and `abort` with interpreter-owned unwinding and no host-process termination. The verified inventory remains 1,271 tests: 1,137 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.11.0`, then passed after Cargo/lock and both Compose images moved to `0.12.0`. Local and remote annotated-tag preflight found no `v0.12.0`. Independent diff-only review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,271 local tests, the rebuilt 1,271-test Docker gate, runtime output `10`, container/local version `cust 0.12.0`, Cargo metadata `0.12.0`, and `git diff --check` pass. The tag is reserved for this verified release commit and may be created only after `origin/main` accepts it; publication and remote peeled-target verification are not claimed by this release-preparation diff. The next concrete language package is deterministic C-locale `strcoll`/`strxfrm`; `strtol`/`strtoul` remain deferred behind safe pointer-to-pointer `endptr` support.
