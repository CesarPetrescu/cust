...
All passed after the 2026-05-07 autonomous union-pointer run. This run added one-level union pointers and pointer fields inside unions as a scoped follow-up to the first-pass union model: `union Number *p = &n`, `p->value`, `(*p).tag`, union-array element pointers, pointer parameters, self-referential union pointer fields, and scalar pointer fields inside unions now use Cust's interpreter-owned pointer metadata. Scalar writes through union pointers reuse the same logical scalar-field synchronization as direct union writes, so supported behavior remains deterministic and native-oracle-compatible. Pointer-to-pointer union fields now have targeted parser diagnostics. Coverage includes `tests/fixtures/valid/union_pointers.c`, `tests/fixtures/invalid/union_pointer_to_pointer_field.c`, and native C compiler-oracle fixture `tests/fixtures/compat/valid/union_pointers.c`. Docker Compose emitted non-fatal `Docker Compose requires buildx plugin to be installed` warnings and fell back to the classic builder; both required Docker commands exited 0.
