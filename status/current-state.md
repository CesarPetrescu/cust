...
The bounded Cust v0.8.0 package is prepared for atomic publication. Cargo package/lock metadata, exact CLI output, both Docker Compose image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized for `0.8.0`. The release packages the post-v0.7 bounded string expansion: `strncmp`, `strchr`, `strrchr`, `strpbrk`, `strspn`, `strcspn`, `strstr`, `strcpy`, `strcat`, `strncat`, and `strncpy`. The release inventory is 1,241 tests: 1,108 interpreter tests, 98 deterministic fuzz-safety tests, 31 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.7.0`, then passed after Cargo/lock and both Compose images moved to `0.8.0`. Local and remote `v0.8.0` tag preflight found no conflict. The annotated tag is reserved for the independently reviewed, canonically verified release commit and may be published only after `origin/main` accepts that commit.
