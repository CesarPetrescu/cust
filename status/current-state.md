...
Bounded v0.27.0 release closure is the next recommended package. Whole-aggregate bytes remain deferred because Cust has no aggregate-padding model, and union-backed bytes remain deferred because members use separate interpreter storage.
...
Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.24.0`, then passed after Cargo/lock and both image tags moved to `0.25.0`. Local/Cargo output reports `cust 0.25.0`; local and remote annotated-tag preflight found no `v0.25.0`. Work-package candidates were bounded v0.25.0 release closure, non-character scalar aggregate-field object bytes, two-dimensional non-character row bytes, and whole-aggregate/union layout. Release closure was selected because it is first in both authoritative queues, atomically packages the already reviewed standalone scalar slice, and has bounded consistency risk with executable version assertions. Non-character scalar struct fields are the next concrete vertical slice; two-dimensional non-character rows and whole aggregate/union storage remain later because their shape and aliasing fan-out is larger.
...
Work-package evaluation considered bounded v0.22.0 release closure, broad integer/aggregate raw object representations, deterministic model-based pointer-field coverage, and further parser-diagnostic expansion. Release closure was selected because it is first in both authoritative queues, atomically packages the independently reviewed five-function raw-memory family, and has bounded consistency risk with executable version assertions. Deterministic model-based coverage for pointer fields selected through ordinary aggregate-array elements is the next concrete work package; broader raw object representations remain separate design work.
...
Work-package evaluation considered bounded character-storage `memcmp`, bounded `memset`, broad integer/aggregate object-representation serialization, and lower-priority parser/fuzz expansion. `memcmp` was selected because it was first in both authoritative queues, completes the read-only comparison side of the reviewed byte model, and avoids inventing host ABI layout. Next, implement bounded character-storage `memset`; broader object representations remain separate design work.

The first bounded raw-memory intrinsic slice is complete: an exactly prototyped `void *memcpy(void *, const void *, size_t)` copies up to 4,096 character-storage cells between interpreter-owned character objects/arrays, snapshots source values before mutation, returns the destination identity as `void *`, and never exposes host addresses or invokes host libc. Each Cust `char` cell counts as one byte for range/capacity purposes, while the snapshot preserves its exact deterministic stored value so negative `signed char` and high-byte values are not corrupted; integer and aggregate object representations remain an explicit unsupported boundary.
...
Work-package evaluation considered the first bounded `memcpy` slice, broader all-object byte representations, and lower-priority parser/fuzz coverage. Character-storage `memcpy` was selected because it is the first unchecked roadmap item, reuses mature provenance/lifetime/const machinery without inventing host ABI layout, and forms a complete safe vertical slice. Next, extend the same byte model to overlap-safe `memmove` before considering integer/aggregate object representations.
...
Work-package evaluation considered bounded v0.20.0 release closure, immediate implementation of a first memory intrinsic, and lower-priority parser/fuzz coverage. Release closure was selected because it was first in both authoritative queues at selection time, atomically packages the already reviewed function-boundary vertical slice, and has bounded consistency risk with executable version assertions. A separately designed first raw-memory intrinsic is next; object-representation semantics must not be improvised inside the release run.
...
Work-package evaluation considered bounded v0.19.0 release closure, immediate expansion of one-level `void *` through function boundaries, and lower-priority parser/fuzz coverage. Release closure was selected because it was first in both authoritative queues, packages the already reviewed safe-object vertical slice, and has bounded metadata/docs risk with executable version assertions. With release preparation complete, one-level `void *` function return/prototype/call-result support is the sole next concrete prerequisite to bounded memory intrinsics.
