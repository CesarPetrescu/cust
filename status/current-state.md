...
Bounded v0.52.0 is published around the completed safe tracked unqualified `_Bool **` object/output-parameter slice. Candidate evaluation considered the queue-leading release closure, a first safe tracked `double **` extension, parser-diagnostic expansion, and broader property/CLI work. Release closure was selected because it atomically packages already reviewed interpreter behavior with bounded metadata risk; `double **` is the concrete post-publication package. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/images `0.51.0` and pass after Cargo/lock plus both image tags moved to `0.52.0`. Executable target listing independently reconciles 2,208 tests as 2,064 interpreter + 101 deterministic fuzz-safety + 33 CLI + 6 pointer-classifier parity + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes describe mutable `_Bool *` slot identity, null/default state, forwarding, typed normalization, owner/lifetime/const/static guarantees, pointee-driven diagnostics, nine focused regressions, and exact adjacent limitations. The first evidence-bearing review rejected premature in-repository approval claims; after correction, fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,208 local tests, a no-cache image rebuild, all 2,208 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.52.0` versions, static safety scan, and diff hygiene pass. Repeated local and remote `v0.52.0` tag preflight were empty. Release commit `22ca74f271898e408200138dad054535ea4d2491` reached exact `origin/main` before annotated tag creation; remote tag object `55ea846f80a9850bd7f265ec5d387be757179329` peels to that exact release commit.

The first safe tracked unqualified `_Bool **` object/output-parameter slice is complete. Recovery inspection found an inherited substantial implementation and selected finishing it over bounded v0.52.0 release closure, parser-diagnostic expansion, and broader property/CLI work. Local, file-global, and block-static objects support null/default or mutable unqualified `_Bool *` slot initialization, compatible forwarding, equality/truthiness, indirect pointer reads/writes, and scalar writes normalized to `0` or `1`; interpreter-owned owner/lifetime/const/static metadata and non-evaluating `sizeof` remain intact. Qualified, incompatible, deeper, array, address-taking, arithmetic, ordering, and compound-update forms retain exact boundaries. Nine focused boolean-output regressions and one registered warning-free native fixture cover the slice. The inherited conditional test first exposed stale integer wording and passed after type-driven labels; independent review then found stale character wording for aggregate equality beneath `sizeof`/`_Generic`, whose dedicated regression went RED then GREEN after the shared binary output-kind classifier was used. Fresh complete-diff re-review returned `APPROVED`; formatting, warning-denied Clippy, all 2,208 local and rebuilt-Docker tests, runtime output `10`, static scan, and diff hygiene pass. See `references/cust-boolean-pointer-objects-and-output-parameters.md`. At feature completion, bounded v0.52.0 release closure became next.

Bounded v0.51.0 is published around the completed safe tracked unqualified `int **` object/output-parameter slice. Candidate evaluation considered this queue-leading release closure, an immediate `_Bool **` extension, parser-diagnostic expansion, and broader property/CLI work; release closure was selected because it atomically packages already reviewed interpreter behavior with bounded metadata risk. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/images `0.50.0` and pass after Cargo/lock plus both image tags moved to `0.51.0`. Executable target listing independently reconciles 2,199 tests as 2,055 interpreter + 101 deterministic fuzz-safety + 33 CLI + 6 pointer-classifier parity + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes describe the tracked storage model, owner/lifetime/const/static guarantees, conversion-before-side-effect diagnostics, 62 focused regressions, and one concrete post-release `_Bool **` package. Initial independent review found stale 52-test wording; executable delta checks proved 62 new interpreter tests, all release/status claims were corrected, and fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,199 local tests, a no-cache image rebuild, all 2,199 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.51.0` versions, repeated tag preflight, static safety scan, and diff hygiene pass. Release commit `c4d75d723c84b39a2832a166485b6d06e1618ca1` reached exact `origin/main` before annotated tag creation; remote tag object `f6de73c38c53f3173031c046dae620b7865d6322` peels to that exact release commit. This later status-only publication-evidence update is not the tagged release commit and does not move or recreate `v0.51.0`.

The first safe tracked unqualified `int **` object/output-parameter slice is complete. Recovery inspection found an inherited substantial implementation and selected finishing it over immediate v0.51.0 release closure, parser-diagnostic expansion, and lower-priority property/CLI work. Local, file-global, and block-static objects support null/default or mutable unqualified `int *` slot initialization, compatible tracked-object forwarding, indirect pointer/scalar reads and writes, equality/truthiness, supported null and `void *` conversions, unqualified scalar-pointee aliases, and non-evaluating `sizeof`. Interpreter-owned slot identity, pointee owner/lifetime/const metadata, static-storage rules, source-ordered argument binding, and pre-evaluation type diagnostics are preserved; qualified `_Bool`/`double`/integer targets, deeper pointers, arrays, fields, pointer typedef aliases, return types, address-taking, arithmetic, ordering, and compound updates retain targeted boundaries.

Sixty-two interpreter regressions and one registered warning-free native fixture cover the slice. Independent reviews drove four closure rounds: post-call validation now runs after callee parameter-scope teardown; bounded integer-constant-expression validity is separated from short-circuit value evaluation; conditional pointer writes validate aggregate conditions before a side-effecting LHS; null `void *` constants work across automatic, static, assignment, and call routes; and const-qualified `void *` diagnostics survive initializer/assignment error translation. The latest two qualification regressions first produced generic pointer-object diagnostics and then passed after the narrow translation fix. Formatting and stale inventory findings from the first complete-diff review are resolved. Final review also exposed over-broad `_Generic` association rejection and skipped unselected output-assignment validation; RED/GREEN now separates pointer-output association values from forbidden controlling-expression types. The canonical gate passes with 2,199 tests: 2,055 interpreter + 101 deterministic fuzz-safety + 33 CLI + 6 pointer-classifier parity + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. See `references/cust-integer-pointer-objects-and-output-parameters.md`. At feature completion, bounded v0.51.0 release closure became next.

Bounded Cust v0.50.0 is published around completed pointer classifier/evaluator parity. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/images `0.49.0` and passed after Cargo/lock plus both image tags moved to `0.50.0`. Executable target listing independently reconciled 2,137 tests as 1,993 interpreter + 101 deterministic fuzz-safety + 33 CLI + 6 pointer-classifier parity + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes describe all 69 routes × eight consumers, seven const routes, lexical owner/lifetime guarantees, pre-evaluation conversion diagnostics, and the concrete next `int **` package. Initial independent review found an ambiguous README limitation that appeared to reject supported generic row-pointer forms; double-specific wording resolved it, and fresh complete-diff reviews returned `APPROVED`. Formatting, warning-denied Clippy, all 2,137 local tests, a no-cache image rebuild, all 2,137 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.50.0` versions, repeated empty pre-publication tag checks, static safety scan, and diff hygiene passed. Release commit `6335a1fdc0b4f67df47f36cc186edca25cb0cc77` reached `origin/main` before annotated tag `v0.50.0` was created and pushed; remote tag object `e6ae36718e014b010fa17026c7f5a762fdd4baff` peels to that exact release commit. This later status-only evidence commit does not move or recreate the release tag.

Deterministic pointer classifier/evaluator parity is complete in v0.50.0. Recovery inspection at feature-selection time found an inherited uncommitted implementation and selected it over immediate release closure, parser-diagnostic expansion, and broader pointer work because finishing existing substantial work was mandatory and the package closed the sole queue-leading item. The generated matrix now crosses 69 stable pointer-valued route identities with eight initializer/argument/return/`sizeof`/incompatible consumers: 552 exact programs plus seven const-preservation routes. Six dedicated Rust tests also cover two-dimensional-row `void *` conversion, static pointer-returning call non-evaluation, ordinary and five aggregate-field assignment RHS prevalidation, and static-local pointer initialization. Array compound literals now carry lexical owners for every scalar element type, so block-scope use is valid while escaped pointers diagnose their hidden out-of-scope root.

Independent review found and TDD closed aggregate-array element-address pointee/const omissions, evaluated incompatible assignment RHS expressions, and static-local pointer initializers that evaluated before type rejection. Focused regressions first produced `division by zero` or the wrong type/const result, then passed after metadata-only type checks were applied before evaluation across direct, aggregate-literal, direct-field, aggregate-array-element, struct-pointer, and embedded aggregate-array-element assignments. Final complete-diff review returned `APPROVED`. The executable inventory is 2,137 tests: 1,993 interpreter + 101 deterministic fuzz-safety + 33 CLI + 6 pointer-classifier parity + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. Formatting, warning-denied Clippy, all 2,137 local tests, all 2,137 rebuilt-Docker tests, runtime output `10`, and diff hygiene passed. The first safe tracked unqualified `int **` object/output-parameter slice is next.

Bounded v0.49.0 was published on 2026-09-02 around completed aggregate classifier/evaluator parity. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/images `0.48.0` and pass after Cargo/lock plus both image tags moved to `0.49.0`. Executable target listing independently reconciles 2,131 tests as 1,993 interpreter + 101 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes describe the exact 22-route × four-consumer matrix, deep-copy and one-time-evaluation guarantees, `_Generic` validation closure, and one concrete post-release pointer classifier/evaluator parity task. The initial pre-gate complete-diff review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 2,131 local tests, a no-cache image rebuild, all 2,131 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.49.0` versions, static safety scan, and diff hygiene pass. Release commit `e745ae0ac6202fdecd93bdb19e13bddfac1ea6c7` reached exact `origin/main` while local and remote `v0.49.0` refs were absent; annotated tag object `defd137bfc1c714f1839dd071a7ed494891f5522` was then created and pushed, and local/remote peeled targets equal the release commit. This later status-only publication-evidence update is not the tagged release commit and does not move or recreate the tag.

The deterministic aggregate classifier/evaluator parity package is complete. One generated fuzz-safety test crosses all 22 aggregate-valued `Expr` routes with declaration-initializer, function-argument, function-return, and exact nominal type-mismatch consumers: 88 exact programs with identity-keyed per-route/per-context totals, panic guards, nested fields, embedded aggregate-array elements, deep-copy isolation, and one-time marker evaluation. The initial focused RED stopped on `_Generic` aggregate arguments with `expected struct expression`; after argument-classifier parity, the same test exposed `Expr::StructPtrArrayGet` as type-classified but missing from `eval_struct_expr()`. Runtime evaluation now resolves the existing aggregate pointer once and deep-clones its field map. Independent review then found that direct `_Generic` selection skipped semantic validation of unselected associations; its focused regression first returned `Ok(7)` and now reports the undefined unselected call through the shared validation/depth wrapper. A second review found position-keyed matrix counters, stale inventory, and rustfmt drift; identity-keyed route/context IDs, exact `2,131 = 1,993 interpreter + 101 fuzz-safety + 37 other` accounting, and formatting closed those findings. Fresh complete-diff re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 2,131 local tests, all 2,131 Docker tests, rebuilt runtime output `10`, static safety scan, and diff hygiene pass. Bounded v0.49.0 now publishes this feature package above.

Bounded v0.48.0 is published around the deterministic carrierless scalar-union property matrix and the embedded aggregate-array-element aggregate-evaluation fix. Version-first TDD changed only exact CLI and Compose expectations: both failed against `0.47.0` and pass after Cargo/lock plus both image tags moved to `0.48.0`. Executable target listing independently reconciles 2,129 tests as 1,992 interpreter + 100 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations and changelog notes expose the exact 90-route matrix, aggregate-value deep-copy behavior, and unchanged union safety boundaries. Initial review found one stale README next-milestone instruction; after promoting the aggregate classifier/evaluator parity matrix, fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,129 local tests, a no-cache image rebuild, all 2,129 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.48.0` versions, repeated empty tag preflight, and diff hygiene pass. Release commit `bc353b96d05ed8b573f49eb23370b921f4ca257c` reached exact `origin/main` before annotated tag object `434a7dfd3733241a168c5b1868d1b232ce934366` was created and pushed; local and remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate the tag and is not the tagged release commit.

The underlying deterministic carrierless scalar-union object-byte matrix is implemented across 90 exact mixed-layout × owner × root × intrinsic cells. Direct, nested, and embedded aggregate-array-element roots cover selected members and whole union objects for `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`; fixed-seed models assert deterministic bytes, returned pointers, untouched storage, source-ordered one-time argument evaluation, deep-copy isolation, and exact all-const/pointer/`double`/array/nested-layout boundaries. The matrix exposed a real aggregate evaluator parity gap: `aggregate_expr_type_name()` classified `Expr::StructArrayGet` as an aggregate value, but `eval_struct_expr()` omitted it. A focused regression first failed with `expected struct expression`; evaluation now resolves the embedded aggregate-array element through its interpreter-owned pointer and deep-clones the selected fields. The expanded registered fixture remains ABI-independent and passes strict GCC/Clang execution plus the actual compiler-oracle harness. Fresh feature-diff review returned `APPROVED`; formatting, warning-denied Clippy, all 2,129 local and rebuilt-Docker tests, runtime output `10`, and diff hygiene passed before release preparation.

Bounded v0.47.0 is published around completed persistent carrierless scalar-union object bytes. Release-run candidates were this queue-leading closure, a deterministic carrierless-union property matrix, parser-diagnostic expansion, and immediate broader pointer work. Release closure was selected because it atomically packages already reviewed behavior with exact executable version assertions and bounded metadata risk. Version-first TDD changed only the exact CLI and Compose expectations: both failed against `0.46.0`, then passed after Cargo/lock and both image tags moved to `0.47.0`. Executable listing independently reconciles 2,126 tests: 1,990 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes describe both carrier-backed and hidden-byte scalar-union routes. Fresh independent complete-diff re-review returned `APPROVED` after two stale-history correction rounds. Formatting, warning-denied Clippy, all 2,126 local tests, a no-cache image rebuild, all 2,126 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.47.0` versions, repeated empty tag preflight, and diff hygiene pass. Release commit `bf74fea72b9960e517ad8875e04a64f484121ae7` reached exact `origin/main` while the tag was absent; annotated tag object `f260abcc31da20fe24c820ae15846247ffd73b77` was then created and pushed, and local and remote refs peel exactly to the release commit. This later status-only evidence update does not move or recreate the tag and is not the tagged release commit.

Persistent interpreter-owned bytes now cover every remaining mutable scalar-only non-`double` union layout that lacks a non-const full-width `int`/`char` carrier. The generalized admission retains the existing carrier path for layouts that already have one, allocates hidden maximum-layout bytes only for carrierless layouts, and chooses an actual mutable source member for canonical typed pointer routing. Selected-member and whole-object `memcpy`/`memmove`/`memcmp`/`memset`/`memchr` preserve offset-zero alias identity, raw bytes, deterministic little-endian `int` plus `char`/`_Bool` synchronization, member-local versus maximum-layout capacity, overlap, recursive const/lifetime, zero count, non-evaluation, deep-copy isolation, and aggregate-array-element isolation. All-const, pointer, `double`, array, and nested-aggregate layouts remain exact boundaries. The feature recovery run preserved and completed an inherited implementation rather than selecting a second package. Its original RED is unavailable and is not re-claimed; the inherited focused tests were GREEN on inspection. Fresh independent review exercised additional overlap, by-value copy, and const-owner probes and found no blockers. Formatting, warning-denied Clippy, all 2,126 local tests (1,990 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test), all 2,126 Docker tests, runtime output `10`, direct GCC/Clang warning-denied fixture execution, compiler-oracle comparison, and diff hygiene pass. Bounded v0.47.0 release closure followed as the next package and is published above.

Bounded v0.46.0 is published around persistent all-`_Bool` scalar-union bytes. Cargo/lock metadata, exact CLI output/tests, both Docker Compose image tags, README central inventory/limitations/roadmap, changelog notes, and the independently listed 2,124-test inventory agree (1,988 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test). Version-first TDD failed against `0.45.0` and passes at `0.46.0`; the first canonical attempt then exposed current Clippy's `collapsible_if` warning in the preceding persistent-byte implementation, and a semantics-preserving Rust 2024 let-chain made strict Clippy GREEN. Fresh independent complete-diff review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,124 local tests, a no-cache image rebuild, all 2,124 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.46.0` versions, and diff hygiene pass. Release commit `386205c1c8d5911459d33b5f569df60c28e057df` reached exact `origin/main` while the tag was absent; annotated tag object `5ad87d9e4f9c64176020c4e0561cc6d1bb65b169` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate `v0.46.0` and is not the tagged release commit. At v0.46.0 publication, persistent bytes for remaining mutable scalar-only non-`double` union layouts without a full-width writable carrier became the next package; that package is complete and published as v0.47.0 above.

Previous feature update (2026-08-31): interpreter-owned persistent object bytes admit all-`_Bool` scalar unions with at least one mutable member across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Selected-member and whole-object routes share canonical offset-zero storage; raw byte `2` remains visible to memory operations while both language-visible members normalize to `1`. Member/whole capacity, overlap, const/lifetime, typed results, zero count, non-evaluation, aggregate-array-element isolation, and existing pointer/double/array/nested/all-const boundaries remain exact. The focused regression first failed with `function 'memcpy' does not yet support union-backed scalar object storage for argument 1`; hidden deep-copied byte storage plus symmetric read/write/synchronization made three focused tests GREEN. Independent review found two stale all-`_Bool` rejection expectations; after exact reconciliation, fresh current-diff review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,124 local and rebuilt-Docker tests (1,988 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test), runtime output `10`, direct GCC/Clang warning-denied fixture execution, compiler-oracle comparison, and diff hygiene pass.

Bounded v0.45.0 is published around the completed whole-object scalar-union byte-range slice. Version-first exact CLI and Compose expectations failed against `0.44.0` and pass after Cargo/lock plus both image tags moved to `0.45.0`; executable target listing independently reconciles 2,121 tests: 1,985 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog notes advertise maximum-layout whole-union ranges while retaining selected-member capacity and targeted unsupported-layout boundaries. Fresh independent complete-diff release review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,121 local tests, a no-cache image rebuild, all 2,121 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.45.0` versions, and diff hygiene pass. Release commit `361b8653884182c45ee8c2b5b50caaccef9b41ad` reached exact `origin/main` while the tag was absent; annotated tag object `556f94e68324eab86ffcd6fed51958115b1c8d20` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate `v0.45.0` and is not the tagged release commit.

Whole-object byte ranges for v0.45.0's carrier-backed scalar-only unions are complete across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Whole unions use maximum-layout capacity and the deterministic full-width writable carrier; selected-member aliases stay synchronized, nested and aggregate-array-element routes retain canonical identity and isolation, and bounds/const/lifetime/overlap/zero-count/non-evaluation checks remain exact. At that release boundary, all-`_Bool`, pointer, `double`, array, nested-aggregate, and full-width-const-only layouts retained targeted diagnostics; mutable all-`_Bool` layouts are admitted by the current-main extension above. TDD began with the exact whole-union rejection, then independent review drove RED/GREEN closure for nested writable ranges, typed `memchr` canonicalization, stale whole-struct boundary tests, and same-type const-member ordering. Fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,121 local tests (1,985 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test), all 2,121 rebuilt-Docker tests, runtime output `10`, GCC/Clang warning-denied fixture execution, and diff hygiene pass. Bounded v0.45.0 release closure became next at that point and is published above.

Bounded v0.44.0 is published around the scalar-only shared-union object-byte slice. Exact CLI and Compose expectations went RED against `0.43.0` and GREEN after Cargo/lock plus both image tags moved to `0.44.0`; executable target listing independently reconciles 2,117 tests: 1,981 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. README central inventory/limitations/roadmap and changelog release notes advertise selected-member canonical identity, deterministic aliases, raw `_Bool` bytes, selected-member capacity, and targeted unsupported layouts. Fresh independent release review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 2,117 local tests, a no-cache image rebuild, all 2,117 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image `0.44.0` versions, repeated empty tag preflight, and diff hygiene pass. Release commit `5d0a53c5aa506342fe33c4380ecb7871f7ef4bf5` reached exact `origin/main` while the tag was absent; annotated tag object `d956372daad25e8fb6a14d8555552cd3f48a45df` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate `v0.44.0` and is not the tagged release commit. Whole-object byte ranges for already admitted scalar-only unions were next at publication time and are complete above.

The first shared-member scalar-union object-byte slice is implemented. Exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr` accept selected members of scalar-only non-`double` unions that contain at least one non-const `int` or `char` storage carrier as wide as the largest member. Canonical aggregate-root/offset-zero identity makes distinct member pointers equal and overlapping, while byte-based synchronization plus carrier-backed intrinsic reads/writes preserve deterministic little-endian `int`/`char`/`_Bool` aliasing instead of copying independent normalized values. Selected-member capacity, const/lifetime, zero-count, nested non-evaluation, nested struct and union-array-element routes, raw bytes beneath normalized `_Bool` views, and exact all-`_Bool`/pointer/double/array/nested-aggregate union boundaries are covered. The initial focused regression returned `1` because member pointers had path-based identity; after canonical identity it reached the expected runtime union rejection, then passed after bounded admission and byte synchronization. Reviewer-driven RED/GREEN preserved raw byte `2` beneath a true `_Bool` view, added array-element/isolation and layout-wide array-veto coverage, removed a native padding-byte assumption, and retained exact const/full-width-carrier boundaries. Final review found that a narrow mutable carrier could be selected behind a wider const member; its focused regression first returned `Ok(0)`, then passed after admission required a non-const carrier exactly as wide as the largest member. Const-member, unsupported-layout equality, same-width alias/memmove, and deterministic synchronization-seed coverage also pass. Fresh complete-diff re-review returned `APPROVED`; formatting, warning-denied Clippy, all 2,117 local and Docker tests, runtime output `10`, compiler-oracle comparison, and diff hygiene pass. This work is packaged in published v0.44.0; whole-object ranges over the admitted layouts were the follow-up and are complete above.

Bounded v0.43.0 is published around the selected-row short-circuit composition fix. Version-first TDD changed only the exact CLI and Compose expectations: both failed against `0.42.0`, then passed after Cargo/lock and both image tags moved to `0.43.0`. Executable target-by-target listing independently reconciles 2,111 tests: 1,975 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. README central inventory/roadmap and changelog release notes advertise source-ordered `||` composition while retaining adjacent-row, whole-object, union-backed, row-address, incompatible-width/cast, whole-aggregate, and unprovable-root boundaries. One stale historical queue instruction found by review was rewritten as explicitly completed; fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,111 local tests, a no-cache image rebuild, all 2,111 rebuilt-Docker tests, runtime output `10`, exact Cargo/local/runtime-image/test-image versions, diff hygiene, and repeated empty local/remote tag preflight pass. Release commit `e264f69b29ae079f6f54a75996b1b1a3419ad0c0` reached exact `origin/main` while the tag was absent; annotated tag object `d80f30a0512589071d2a989229f4ea18e21fc19d` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate `v0.43.0` and is not the tagged release commit. True shared-member union byte storage is the next implementation package.

The focused short-circuit selected-row composition gap is closed. A bounded-memory `memset(*row, ...)` return-value comparison can now be the left operand of `||` with a row-byte `memcmp(*row, ...)` on the right without non-evaluating alias validation forcing `PointerValue::Array2DRow` through the intentionally unsupported scalar-pointer decay path. The fix recognizes a live two-dimensional row directly as double-backed storage while retaining owner-liveness and element-type checks. A focused RED first reported `two-dimensional array 'combined_values' does not decay to a scalar pointer`; GREEN proves combined/sequential parity, one-time marker evaluation, source-ordered short-circuiting, and adjacent-row preservation. The registered native fixture now uses the combined expression and passes Cust, GCC, Clang, and the actual compiler-oracle harness. Independent review returned `APPROVED`. Executable listing reconciles 2,111 tests: 1,975 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test. Formatting, warning-denied Clippy, all local tests, all Docker tests, runtime output `10`, and diff hygiene pass. The bounded v0.43.0 release package is published above; true shared-member union byte storage follows.

Bounded v0.42.0 is published around the completed selected-row binary64 package. Version-first TDD changed only the exact CLI and Compose expectations: both failed against `0.41.0`, then passed after Cargo/lock and both image tags moved to `0.42.0`. Executable target-by-target listing independently reconciles 2,110 tests: 1,974 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. README central inventory/limitations/roadmap and changelog release notes distinguish selected row-local access from still-unsupported adjacent-row flattening, whole two-dimensional objects, union-backed storage, row addresses, incompatible widths/casts, one-dimensional-alias-derived multidimensional fields, pointer-to-row typedef/function signatures, whole aggregates containing double fields, and unprovable roots. Two review-fix cycles corrected stale aggregate-field and pointer-to-row claims; fresh final independent review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,110 local tests, a no-cache rebuild, all 2,110 rebuilt-Docker tests, runtime output `10`, exact local/runtime-image/test-image versions, and diff hygiene pass. Release commit `4c665b86cd85cef9bbbec76ba30776988cc7549a` reached exact `origin/main` while local/remote `v0.42.0` refs were absent; annotated tag object `f26eb3c98da0ebe566555800e4c2ed48898c6ca1` was then created and pushed, and local/remote refs peel exactly to the release commit. This status-only publication-evidence update is not the tagged release commit and does not move or recreate `v0.42.0`.

Selected two-dimensional binary64 row views are complete. All five exactly prototyped bounded raw-memory intrinsics now accept one selected row of supported direct, explicit row-pointer, adjusted direct/typedef-backed parameter, and supported aggregate-field `double[R][C]` storage without flattening adjacent rows. Row-local capacity/overlap, checked row and element arithmetic, owner/lifetime/recursive const, aligned/interior identity, zero count, non-evaluation, and exact cross-row/union/whole-aggregate/unprovable/row-address boundaries are preserved. Independent review found six blocking gaps; focused RED/GREEN closed indexed-address row-bound escape, adjusted-parameter pointer-slot versus pointee const, aggregate-compound-literal row-address erasure, incompatible cast metadata beneath `sizeof`, and evaluated/non-evaluating conditional row-width compatibility. The first full gate then exposed eager branch typing in unrelated nested `strtol` conditionals; row-shape guarding restored the existing linear regression without weakening width checks. The native fixture now verifies zeroed bytes with `memcmp` rather than reading a byte-zeroed `double`. The registered fixture passes Cust, GCC, Clang, and the actual compiler-oracle harness. Formatting, warning-denied Clippy, all 2,110 local tests (1,974 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test), all 2,110 rebuilt-Docker tests, runtime output `10`, and diff hygiene pass. At that point, the focused short-circuit row-call composition regression became the concrete post-release implementation task; it is complete and packaged in v0.43.0.

Bounded v0.41.0 is published around the completed aggregate-field binary64 object-byte slice. Version-first TDD changed only the exact CLI and Compose expectations: both failed against `0.40.0`, then passed after Cargo/lock and both image tags moved to `0.41.0`. Executable target-by-target listing independently reconciles 2,089 tests: 1,953 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. README central inventory/limitations/roadmap and changelog release notes distinguish supported scalar/one-dimensional-array double fields from still-unsupported two-dimensional, union-backed, pointer-object, unprovable-target, and whole-aggregate-with-double layouts. Fresh independent complete-diff review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 2,089 local tests, a no-cache rebuild, all 2,089 rebuilt-Docker tests, runtime output `10`, exact local/runtime-image/test-image versions, and diff hygiene pass. Release commit `24d41c563c5261344abd19ffb69ccf12f184e02d` reached exact `origin/main` before annotated tag creation; local and remote tag object `b469e7f5ef7900ec073e7153cd6db721a9abd688` peel exactly to that release commit. This later status-only publication-evidence update does not move or recreate `v0.41.0` and is not the tagged release commit.

Release-run candidate evaluation considered bounded v0.41.0 release closure, selected two-dimensional binary64 rows, shared union-byte storage, and parser/property expansion. Release closure was selected because it was the authoritative queue-leading package and atomically packages an already reviewed ten-test feature slice with exact executable version assertions and bounded metadata risk. One selected row of supported two-dimensional `double` storage is now the single next concrete implementation package; shared union bytes remain deferred because they require true aliased storage.

Deterministic IEEE-754 binary64 object bytes now cover scalar and fixed one-dimensional `double` array fields in supported non-union structs across exactly prototyped `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Direct, nested, struct-array-element, and arrow routes preserve field-local byte capacity/overlap, owner/path/identity, recursive const and lexical lifetime, aligned/interior `memchr` results, partial/full destination-aware writes, and non-evaluating behavior. Runtime admission now accepts aggregate-backed array/scalar pointers only outside union ancestry; metadata-only validation distinguishes direct `double[N]` fields from `double *` fields whose current target may be an unsupported two-dimensional row. Union-backed fields, dimensioned rows/objects, whole aggregates with unsupported layouts, and unprovable helper roots remain rejected.

Focused TDD first failed on direct scalar fields, then array/nested routes, and then non-evaluating direct/arrow roots. Independent review found two safety bypasses: a broad non-evaluating `double *` shortcut laundered two-dimensional row provenance through pointer-valued fields, and direct union-backed `double[]` fields bypassed runtime union checks beneath `sizeof`. Dedicated regressions first returned `Ok(8)` before field-kind-aware metadata and union ancestry restored the exact double-storage diagnostic. Pre-commit review then found missing `sizeof` parity for scalar and array fields selected from aggregate compound literals plus a non-portable native typed read after zero-byte `memset`; focused regressions failed with the exact double-storage diagnostic before metadata-only compound-field admission, while the oracle now inspects zeroed storage only as bytes. Re-review exposed that generic pointer-provenance analysis did not identify a direct union compound literal; a ninth focused test first returned `Ok(8)` before declared aggregate type/path ancestry restored scalar and array rejection. Final review then found `AddressOfAggregateField` skipped aggregate-initializer conversion validation beneath `sizeof`; the tenth focused test first returned `Ok(8)` before non-evaluating aggregate sizing restored the exact pointer-to-double initializer diagnostic. Ten focused aggregate-double tests, the updated scalar-field runtime/`sizeof` regression, and a registered ABI-independent compiler-oracle fixture cover deterministic binary64 identity, capacity, overlap, const, lifetime, union, 2D, whole-layout, initializer constraints, and compound-literal boundaries. See `references/cust-bounded-memory-aggregate-double-object-bytes.md`.

Candidate evaluation considered this queue-leading aggregate-field binary64 package, selected two-dimensional binary64 rows, shared union-byte storage, parser/property expansion, and immediate release closure. Aggregate fields were selected because they reuse mature field provenance and deterministic serialization while completing all five intrinsics as one bounded vertical slice. A bounded v0.41.0 release closure became next at that point; selected two-dimensional rows remained the adjacent implementation package after release, while union bytes still required architectural shared storage.

Bounded v0.40.0 is published around the completed standalone and one-dimensional-array `double` object-byte package. Cargo/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and all status ledgers target `0.40.0`. Version-first TDD failed against `0.39.0` in the exact CLI and Compose tests, then passed after metadata synchronization. Executable target-by-target listing independently reconciles 2,079 tests: 1,943 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Initial independent release review found missing whole-pointer-object and unprovable helper-result wording in the README limitation inventory; the corrected complete diff and final post-gate prose received fresh `APPROVED`. Formatting, warning-denied Clippy, all 2,079 local tests, a no-cache rebuild, all 2,079 rebuilt-Docker tests, runtime output `10`, exact local/runtime-image/test-image versions, and diff hygiene pass on the isolated final candidate. Release commit `a0cbac98f7b3f5b3a4204c76d8b9c73f6a8130ba` reached exact `origin/main` before annotated tag `v0.40.0`; local and remote tag object `86b83200dd5f0842d4508fff52c11b0572e84e8d` peel exactly to that release commit.

Candidate evaluation considered bounded v0.40.0 release closure, binary64 object bytes for supported aggregate fields, binary64 bytes for selected two-dimensional rows, shared union-byte storage, and parser/property expansion. Release closure was selected because it packages independently reviewed behavior with executable version assertions and bounded metadata risk. Aggregate-field binary64 object bytes are the one next implementation package because they reuse mature field provenance and deterministic serialization; two-dimensional rows are a later adjacent slice, while union bytes still require architectural aliasing.

Deterministic IEEE-754 binary64 object bytes are complete for interpreter-owned standalone scalar and one-dimensional-array `double` storage across the five exactly prototyped bounded raw-memory intrinsics. Cust serializes `f64::to_bits()` as exactly eight little-endian bytes and reconstructs partial/full writes with `f64::from_bits`, while the existing object-byte model preserves destination-aware typed coercion, zero count, const/lifetime/capacity/overlap/owner safety, aligned/interior `memchr` identity, and one-time ordered argument evaluation without host addresses or host-libc execution. Aggregate-field, two-dimensional, union-backed, pointer-object, and unprovable helper-returned double storage retain exact diagnostics.

This recovery run preserved the inherited production/test diff rather than selecting a second package. Its first focused execution was RED in nested non-evaluating validation: conditional, comma, selected `_Generic`, and assignment wrappers were detected as double storage but were not classified as provably supported standalone roots. Each structural wrapper route was added incrementally and rerun to GREEN. Three focused interpreter tests now cover full five-intrinsic behavior/safety, exact deterministic binary64 byte order plus partial reconstruction, and non-evaluating side effects/boundaries; one registered warning-free compiler-oracle fixture checks only ABI-independent relationships. The executable inventory is 2,079 tests: 1,943 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Independent review first found stale docs/status, then three narrow documentation inaccuracies; after correction, fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,079 local tests, no-cache rebuilt images, all 2,079 Docker tests, runtime output `10`, and diff hygiene pass. Bounded v0.40.0 release closure is next.

Candidate evaluation considered finishing the inherited binary64 object-byte package, immediate bounded v0.40.0 release closure, shared union-byte storage, and parser/property expansion. Recovery priority and correctness selected the existing coherent vertical slice. Release closure is next because it can package independently reviewed behavior with exact version/inventory metadata; shared union bytes still require architectural aliasing and parser/property expansion is lower impact.

Bounded Cust v0.39.0 is published around completed fixed two-dimensional `double` typedef aliases. Exact CLI and Compose expectations first failed against `0.38.0`, then passed after Cargo/lock and both image tags moved to `0.39.0`; Cargo metadata, the local CLI, and both rebuilt images report `cust 0.39.0`. Executable target listing independently reconciles 2,076 tests: 1,940 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Fresh complete-diff independent release review returned `APPROVED`; formatting, warning-denied Clippy, all 2,076 local and rebuilt-Docker tests, runtime output `10`, exact versions, and diff hygiene pass. Release commit `e220b5aa1cbdc334f01fe2bbc5eb5e73bd50792c` reached exact `origin/main` while remote `v0.39.0` refs were still absent; annotated tag object `d4ab3d74c4c36d91d4bdf23bede8957a3721d961` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate the tag. Deterministic IEEE-754 binary64 object-byte views became the next package at that point.

Fixed two-dimensional `double` typedef aliases are complete. `typedef double Matrix[R][C]`, alias chains, and alias-carried const now reuse typed dimensioned scalar-array storage across local/file-global/block-static objects, nested initialization/zero fill, all double-index scalar update forms, one-time indexes, declaration lists, scalar `_Generic`, and deterministic object/row/element/alias type queries. Direct and alias-spelled double-array parameters, array returns, pointer-to-row/array derivatives, aggregate fields, multidimensional compound literals, ranks above two, and raw-memory double storage retain exact diagnostics. The initial focused fixture was RED at the explicit typedef guard; focused parameter adjustment and array-return regressions then went RED before route-specific guards restored the boundaries and replaced an internal return-type panic with a source-located error.

Three focused interpreter tests and one registered GCC/Clang-warning-free compiler-oracle fixture bring the executable inventory to 2,076 tests: 1,940 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Fresh complete-diff independent re-review returned `APPROVED` after an earlier review found the stale rejection and array-return panic. Focused tests, direct GCC/Clang execution, the actual compiler oracle, formatting, warning-denied Clippy, all 2,076 local tests, all rebuilt-Docker tests, runtime output `10`, and diff hygiene pass. Bounded v0.39.0 release closure became next at that point.

Bounded Cust v0.38.0 is published around completed direct fixed two-dimensional `double[R][C]` objects. Exact CLI and Compose expectations first failed against `0.37.0`, then passed after Cargo/lock and both image tags moved to `0.38.0`; Cargo metadata, the local CLI, and both rebuilt images report `cust 0.38.0`. Executable target listing independently reconciles 2,073 tests: 1,937 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Fresh complete-diff independent release review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,073 local tests, rebuilt-Docker tests, runtime output `10`, exact versions, and diff hygiene pass. Release commit `94f68a54f242ff3563455746e9e99e136c5069ca` reached exact `origin/main` while remote `v0.38.0` refs were still absent; annotated tag object `228c0510957dc98bcd56b607877279cf33e9d591` was then created and pushed, and local/remote refs peel exactly to the release commit. This later status-only publication-evidence update does not move or recreate the tag. Fixed two-dimensional `double` typedef aliases are the sole next concrete package.

Direct fixed two-dimensional `double[R][C]` objects are complete at local, file-global, and block-static scope. The slice reuses typed dimensioned scalar-array storage for nested initialization/zero fill, double-index reads and all scalar update forms, one-time indexes, declaration lists, `_Generic`, and deterministic full-object/row/element `sizeof` plus `_Alignof` relationships. Exact const, row/column bounds, incomplete-rank, pointer-to-row, parameter, aggregate-field, compound-literal, three-dimensional, and raw-memory boundaries remain. Review-driven RED/GREEN additionally closed row-pointer arithmetic and non-evaluating row-address leaks, const increments beneath `sizeof`, unchecked integer-constant arithmetic, and host-panicking oversized 2D allocation. Six focused interpreter tests and one registered GCC/Clang-warning-free compiler-oracle fixture bring the executable inventory to 2,073 tests: 1,937 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Independent review findings are closed or disproved by exact runnable probes; formatting, warning-denied Clippy, all local tests, rebuilt-Docker tests, runtime output `10`, focused compiler-oracle verification, and diff hygiene pass. Bounded v0.38.0 release closure became next at that point.

Bounded Cust v0.37.0 is published around the completed direct and typedef-backed one-dimensional `double` array compound-literal slice. Exact CLI and Compose expectations first failed against `0.36.0`, then passed after Cargo/lock and both image tags moved to `0.37.0`. Cargo metadata and the built CLI report `cust 0.37.0`; executable target listing independently reconciles 2,067 tests: 1,931 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Candidate evaluation considered release closure, direct fixed two-dimensional `double` objects, deterministic raw-memory double bytes, and shared union-byte storage. Release closure was selected because it was the sole unchecked authoritative queue item at selection time, atomically packages the independently reviewed compound-literal slice, and has bounded consistency risk with exact executable version tests. Fresh complete-diff independent release review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,067 local tests, rebuilt-Docker tests, runtime output `10`, exact local/Cargo/image versions, and diff hygiene pass. Release commit `b61dc95167158b0b1c4d1fa2ba90a4cb0219ea51` reached exact `origin/main` before annotated tag publication. Local and remote tag object `978fe0b12d5e44b26d73965d7304e6b479d14698` both peel exactly to that release commit. This later status-only publication-evidence update does not move or recreate the tag. Direct fixed two-dimensional `double` objects became the sole next concrete language package at that point.

Direct and typedef-backed one-dimensional `double` array compound literals are complete. Fixed and inferred lengths, positional and designated initialization, zero fill, hidden-root decay/index/update, one-time evaluated initialization, const rejection, lexical lifetime, `_Generic`, `_Alignof`, and non-evaluating full-object/element `sizeof` all reuse the typed scalar-array model. Evaluated and non-evaluating multidimensional, whole-array-address/pointer-to-row, union-backed decay, and raw-memory double-storage boundaries remain exact. Eight focused interpreter tests, one registered compiler-oracle fixture, GCC and Clang under `-Wall -Wextra -Werror`, fresh independent `APPROVED` review, formatting, warning-denied Clippy, all 2,067 local tests (1,931 interpreter, 99 fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler oracle, and 1 license), the rebuilt Docker test image and completed Compose test run, runtime output `10`, and diff hygiene pass. Bounded v0.37.0 release closure became next at that point.

Bounded Cust v0.36.0 is published around the completed direct `double` typedef-alias slice. Version-first CLI and Compose expectations failed against `0.35.0` before Cargo/lock and both image tags moved to `0.36.0`, then passed. Cargo metadata, the local CLI, and both rebuilt images report `cust 0.36.0`; executable target listing independently reconciles 2,060 tests: 1,924 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Initial independent review found one stale README limitation covering typedef aliases and all array parameters; the corrected direct-declarator wording received fresh complete-diff `APPROVED`. Formatting, warning-denied Clippy, all 2,060 local tests, a no-cache Docker rebuild, all 2,060 rebuilt-Docker tests, runtime output `10`, exact versions, tag preflight, and diff hygiene pass. Release commit `6542f0b45777dda494d2ca5a6554665a04b2e26b` reached exact `origin/main` before annotated tag publication. Local and remote tag object `cec64ed7bbdd2b7ea2c5bbdf180cd35c085cfb65` both peel exactly to that release commit. Direct and typedef-backed one-dimensional double-array compound literals are the next concrete package.

Direct `double` typedef aliases are complete across scalar values, const values, fixed one-dimensional arrays, adjusted array parameters, direct function parameters/returns, aggregate scalar/array/pointer fields, and one-level pointers. Alias-carried value type, pointee qualification, pointer-slot const, owner, lifetime, bounds, and non-evaluating type metadata are preserved. Review-driven TDD closed six alias bypasses: pointer-to-row returns and aggregate fields, atomic pointer aliases, double-array alias compound literals, and top-level pointer-slot const leaking into function-return and cast pointee qualification. Exact deeper-pointer, pointer-array, multidimensional, union-backed pointer, raw-memory, and array-compound-literal boundaries remain. Three focused interpreter tests and one registered warning-free GCC/Clang compiler-oracle fixture were added; independent final review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,060 local tests (1,924 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), rebuilt Docker tests, runtime output `10`, and diff hygiene pass. Bounded v0.36.0 packages this work; double-array compound literals are next.

Bounded Cust v0.35.0 is published around the completed aggregate-field direct `double *` slice. Version-first CLI and Compose expectations failed against `0.34.0` and pass at `0.35.0`; Cargo metadata, the local CLI, and both rebuilt images report `cust 0.35.0`; and the independently reconciled inventory is 2,057 tests (1,921 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). Fresh complete-release-diff review returned `APPROVED`; formatting, warning-denied Clippy, all local tests, a no-cache rebuild, all Docker tests, runtime output `10`, exact version checks, diff hygiene, and added-line secret scans pass. Release commit `aa7fa4db3926c8add58d6d28f0a0c172ad293d3e` was accepted exactly on `origin/main` before annotated tag creation. Local and remote tag object `b44e45ac36e53cb5e4f656c6863dc1f8bba5b939` both peel exactly to that release commit. Direct double typedef aliases are the next concrete language package.

Direct one-level `double *` support now extends to scalar and one-dimensional-array fields in supported aggregates. Direct/indexed/arrow/nested field addresses and decay preserve owner/path/index identity through pointer fields, calls, returns, conditionals, comma/reverse subscripts, and non-evaluating type queries while retaining recursive const, lexical lifetime, same-array bounds, raw-memory, multidimensional, and unsupported-union boundaries. This recovery run preserved the inherited feature implementation and completed its pending verification rather than selecting a second package. Focused recovery first exposed one remaining static-local provenance failure: merging a union-backed unknown aggregate target with a concrete safe target incorrectly retained the concrete target, so a later parameter writeback laundered the union taint. A strict inherited regression was RED with `Ok(8)`; conservative target joins now clear identity whenever branch targets differ, including `None` versus concrete, and all 42 `union_provenance`, 33 `direct_double_aggregate_field`, and compiler-oracle tests are GREEN. Independent complete-diff review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,057 local tests (1,921 interpreter, 99 fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle, 1 license), rebuilt Docker tests, runtime output `10`, and focused compiler-oracle verification pass. Bounded v0.35.0 release closure is next.

Bounded Cust v0.34.0 is published around the safe one-level direct `double *` slice. Version-first TDD changed only exact CLI and Compose expectations: both failed against package/image version `0.33.0`, then passed after Cargo/lock and both image tags moved to `0.34.0`. Cargo metadata and the built CLI report `cust 0.34.0`; executable target listing independently reconciles 1,977 tests: 1,841 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Candidate evaluation considered bounded v0.34.0 release closure, aggregate-field double address/decay support, shared-storage union bytes, and parser/property expansion. Release closure was selected because it was the sole unchecked authoritative item at selection time and packages the independently reviewed direct-pointer work without widening runtime semantics. The first release review found an omitted pointer-to-row double limitation in two README inventories; the documentation-only correction added explicit `double (*)[N]` boundaries and fresh complete-diff re-review returned `APPROVED`. The canonical gate passes formatting, warning-denied Clippy, all 1,977 local tests, a no-cache Docker rebuild, all 1,977 rebuilt-container tests, runtime output `10`, local/Cargo/Compose/direct-image version `0.34.0`, diff checks, and a release-diff secret scan. Release commit `4793b13c9a700eec804a8f32221bf37a8eb041e5` reached exact `origin/main` before annotated tag object `0ccc3a011b082f288df0213b5315ccc733f1b872` was published; local and remote tag objects match and peel exactly to that release commit. This later status-only publication-evidence update does not move the tag. Aggregate-field double pointer routes are now the sole next concrete language package.

Fresh production-code review found two related provenance-laundering groups. Prefix-incremented pointer lvalue writes resolved against pre-effect aliases in direct field, dereference-expression, and dereference-statement routes, while comma/postfix intermediate values were lost by call arguments, pointer declarations/assignments/returns, and assignment-wrapped target lookup. Ten strict tests first returned `Ok(8)`, including paired postfix/plain-comma precision routes. A shared value-snapshot helper now reconstructs comma-right and assignment-value intermediate states and applies the same snapshot across every call/value/write consumer; plain aggregate-target lookup also unwraps comma expressions. A later independent review found aggregate by-value dereference dropped embedded double-storage provenance, postfix aggregate dereference classified the post-increment target, and a 500-operand comma expression inside a summarized callee could overflow the Rust host stack. Three focused regressions went RED; aggregate dereference now resolves pointed aggregate facts through pre-effect snapshots, and both direct and callee-summary non-evaluating expression walks stop at a deterministic depth diagnostic. Final independent follow-up review traced all 52 recursive updater edges through the restoring depth guard. The first canonical run then exposed three valid nested-intrinsic regressions from sharing the 32-level validator state. Dedicated 128-level validator/effect-walker depth states restored all three existing tests while preserving the 500-operand diagnostic; correction review returned `APPROVED`. All 190 `non_evaluating_memory_` tests, all 65 `direct_double_pointer` tests, and the one compiler-oracle test are GREEN. The corrected canonical gate passes formatting, warning-denied Clippy, all 1,977 local tests, rebuilt Docker tests, runtime smoke output `10`, and diff checks.

The inherited safe first one-level direct `double *` package remains the selected recovery work package after evaluating immediate v0.34.0 release closure, shared-storage union bytes, and parser/property expansion. After the absolute aggregate-array coordinate fixes, a fresh complete-diff review found four further blocking non-evaluating routes: recursive fallback omitted call effects, aggregate-valued calls/literals dropped embedded aggregate-pointer targets, local aggregate arrays skipped C array-to-pointer conversion in lexical `_Generic` controls, and binary generic controls fell through to absent runtime locals. Four strict focused regressions first produced `Ok(8)` or `undefined variable 'pointer'`, then went GREEN after bounded recursive effect summarization, returned aggregate pointer-target maps, aggregate-literal/call target propagation, aggregate-array pointer typing, and lexical typing for every supported binary operator family. Exact focused filters now pass 95 non-evaluating-memory tests, 62 direct-pointer tests, and the one compiler-oracle test. Fresh complete-diff re-review and the canonical local/rebuilt-Docker gate remain pending before commit.

The inherited safe first one-level direct `double *` package is in review closure. Recovery inspection considered finishing that package, immediate bounded v0.34.0 release closure, shared-storage union bytes, and parser/property expansion; recovery priority and the queue-leading correctness work selected the existing direct-pointer package. The original feature RED belongs to the inherited work and is not re-claimed. Its 54 focused direct-pointer tests and the compiler-oracle harness were GREEN before review.

Independent pre-commit review first blocked unchecked aggregate-pointer index addition, one scoped aggregate-field type fallback that reached runtime state during non-evaluating analysis, aggregate field-provenance losses across pointer parameters/aggregate returns/callee mutations/conditional pointer arguments, and control-flow false positives or laundering across guaranteed `do` overwrites, unconditional returns, returning branches, and `break`/`continue`. Three review rounds drove eleven focused regressions plus one pre-existing failing nested aggregate-array sibling regression. A fourth fresh review then found runtime pointer-index overflow, skipped setter/update side effects, stale assignment/`_Generic` aggregate targets, aliased-parameter write-order/cache defects, adjusted struct-array element collapse, missing global facts/effects, fully-returning-switch imprecision, and invalid automatic-storage static pointer initializers. Eleven additional focused tests went RED for those routes and are GREEN after checked runtime addition, exhaustive effect traversal, target-preserving summaries, alias-aware ordered writes, element-preserving adjustment, global fact/effect propagation, exact switch stopping, and initialization-time storage-duration validation. A later blocking review found one-pass `for`/`do-while` alias analysis, post-mutation argument-target capture, global/parameter alias write reversal, shadowed parameter return effects, dynamic local aggregate-index storage misses, and runtime fallback for lexical conditional `_Generic` typing. Six focused regressions went RED and are GREEN after fixed-point loop propagation, per-argument snapshots, shared global targets, fixed parameter-scope lookup, conservative dynamic-element inspection, and shared lexical conditional type combination. In the second review-fix cycle, blocker 1 was outside Cust's supported syntax and was removed without edits. Blockers 2–4 each received one strict vertical regression: aggregate call-result field assignment first returned `Ok(8)`, unary `_Generic` control first failed with `undefined variable 'marker'`, and recursive `int *` forwarding first falsely reported double storage. Returned aggregate targets now flow through call analysis, supported unary controls use lexical types, and recursive fallback is limited to double-capable return types. The prior `StructPtrGet`/`void *` aggregate-field finding was invalid because Cust rejects `void *` aggregate fields and supported substitute spellings are already stopped earlier; no code fix is claimed for it. A fresh complete-diff review then exposed stale aggregate element identity after direct struct-pointer `+=`/`-=`; the same root pattern also affected `++`/`--`. Two strict RED/GREEN regressions first returned `Ok(8)`, then passed after non-evaluating alias updates offset the stored aggregate target for both mutation families. The exact executable filters pass all 76 `non_evaluating_memory_` tests and all 59 `direct_double_pointer` tests; `cargo fmt` and the one-test `c_compat` compiler oracle also pass. Fresh complete-diff re-review and the canonical local/rebuilt-Docker gate remain required before commit.

Bounded Cust v0.33.0 is published around completed direct one-dimensional `double` arrays in supported aggregate fields. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and all status ledgers are synchronized for `0.33.0`. Executable target listing independently reconciles 1,715 tests: 1,580 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Exact CLI and Compose expectations failed against `0.32.0`, then passed after Cargo/lock and both image tags moved to `0.33.0`; Cargo metadata and the built CLI report `cust 0.33.0`. Independent release review required time-scoped queue wording before fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,715 local and rebuilt-Docker tests, runtime output `10`, local/Cargo/container version `cust 0.33.0`, and `git diff --check` passed. Release commit `6ebaba1503d7b2d0f95b06c3102438ad7feb63b6` reached exact `origin/main` before annotated tag object `297be74ebe81982627fd9230284393f043cb49ad` was published; local and remote refs peel exactly to the release commit. This later status-only publication-evidence commit does not move or recreate `v0.33.0`.

Candidate evaluation considered bounded v0.33.0 release closure, a first safe one-level direct `double *` slice, shared-storage union bytes, and parser/property expansion. Release closure was selected because it was the sole unchecked authoritative queue item at selection time, atomically packages the already reviewed aggregate-array-field slice, and has bounded consistency risk with exact executable version tests. A safe one-level direct `double *` package over interpreter-owned scalar and one-dimensional-array storage is preserved as the next concrete language task after publication; shared union bytes remain deferred until member storage aliases correctly.

Direct one-dimensional `double` arrays in supported aggregate fields are complete. This recovery run closed two independent-review findings with focused RED/GREEN regressions: reverse aggregate subscripts now obtain contextual pointer-field qualification from the aggregate-valued index expression, and non-evaluating mutable pointer-field paths now validate direct const-array fields plus nested const aggregate ancestors instead of returning early. The final 12-case assignment/compound/increment matrix covers evaluated and `sizeof` routes while existing const-owner/mutable-pointee success remains green. Fresh complete-diff re-review returned `APPROVED`. All 95 focused `direct_double_` tests, the actual compiler oracle, formatting, warning-denied Clippy, all local tests including 1,580 interpreter and 99 fuzz-safety tests, the rebuilt Docker test gate, runtime output `10`, and `git diff --check` pass.

Candidate evaluation for the feature run considered finishing the inherited aggregate-double package, bounded v0.33.0 release closure, direct `double *` support, shared-storage union bytes, and parser/property expansion. Recovery rules and correctness priority selected the existing package because it was already a substantial uncommitted vertical slice; bounded v0.33.0 release closure became the next package at that point.

Bounded Cust v0.32.0 is published around completed direct scalar `double` aggregate fields. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and authoritative status ledgers are synchronized for `0.32.0`. Executable target listing independently reconciles 1,642 tests: 1,507 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Exact CLI and Compose expectations failed against `0.31.0` before metadata moved to `0.32.0`, then passed. Independent release review required historical queue scoping and exact raw-memory/aggregate-array limitation wording; fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,642 local and rebuilt-Docker tests, runtime output `10`, local/Cargo/container version `cust 0.32.0`, and `git diff --check` passed. Release commit `97442a8ed589f167abc7691d190de574154267e0` reached exact `origin/main` before annotated tag object `04358d2ca26243b88e717c45234639fe031c57a5` was published; local and remote refs peel exactly to the release commit. This later status-only publication-evidence commit does not move or recreate `v0.32.0`.

Candidate evaluation considered bounded v0.32.0 release closure, direct one-dimensional double arrays in aggregate fields, direct double pointers, and parser/property expansion. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed aggregate-field slice, and had bounded consistency risk with exact executable version tests. Direct one-dimensional double arrays in supported aggregate fields are now the sole next coherent language package; direct double pointers remain a larger safety and metadata expansion.

Direct scalar `double` fields in supported structs and unions are complete. Positional/designated initialization, direct/indexed/arrow/nested reads, replacement/compound/prefix/postfix updates, aggregate copies, aggregate parameter/return boundaries, deterministic field `sizeof`, shared scalar-union bit storage, and recursive const protection preserve double metadata. Direct `double *`, double aggregate-array/multidimensional fields, unsafe character/raw-memory views, and unions that mix double storage with non-scalar fields retain exact boundaries. This run recovered an inherited implementation, so its original feature RED is unavailable and is not re-claimed. Focused inherited tests were GREEN; independent review then found nested aggregate fields on indexed and arrow routes were rejected at aggregate function boundaries. A focused regression failed with `expected struct expression`, then passed after `StructElementGet` and `StructPtrGet` gained aggregate type/evaluation routing. Fresh independent re-review returned `APPROVED`.

Candidate evaluation considered finishing the inherited direct-double-field package, immediate v0.32.0 release closure, extending double storage further, and parser/property expansion. Recovery priority and the queue-leading coherent language slice selected the inherited package; release closure became first at that point. Executable listing reconciles 1,642 tests: 1,507 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. The 47 focused `direct_double_` tests, actual compiler oracle, formatting, warning-denied Clippy, all 1,642 local tests, the rebuilt 1,642-test Docker gate, runtime output `10`, and `git diff --check` pass.

Bounded Cust v0.31.0 is published around completed direct one-dimensional `double` arrays. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and the authoritative status ledgers are synchronized for `0.31.0`. Executable target listing independently reconciles 1,598 tests: 1,463 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Exact CLI and Compose expectations first failed against `0.30.0`, then passed after metadata moved to `0.31.0`; local and remote annotated-tag preflight was empty. Independent review initially required zero-fill and compound-literal documentation, contextual parameter/compound-literal diagnostics, and historical queue wording fixes; focused RED/GREEN and fresh complete-diff re-review then returned `APPROVED`. Formatting, warning-denied Clippy, all 1,598 local tests, the rebuilt 1,598-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.31.0`, and `git diff --check` passed. Release commit `0444d2a0a95f5eed325922bd33e2eedc953cd25a` reached exact `origin/main` before annotated tag object `e7ca94cebaec359463b79e310acd50b674fce0f2` was published; local and remote refs peel exactly to the release commit. This later status-only publication-evidence commit does not move or recreate `v0.31.0`.

Candidate evaluation considered v0.31.0 release closure, direct aggregate `double` scalar fields, shared-storage union bytes, and parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed array slice, and had bounded metadata/docs risk with exact executable version tests. Direct aggregate scalar fields became the sole next concrete language package at that point.

Direct one-dimensional bounded `double` arrays are complete on current `main`. Fixed and inferred local/file-global/block-static arrays support positional/designated initialization, zero fill, direct and reverse indexed reads, replacement/compound/prefix/postfix updates, integer/double conversion, `_Generic`, and deterministic object/element/type-name `sizeof` plus `_Alignof` relationships. Exact const/bounds diagnostics remain, while array decay, dereference/address formation, double pointers, and multidimensional double arrays stay unsupported. The warning-free registered fixture passes Cust, GCC, Clang, and the actual compiler oracle.

This run recovered an inherited uncommitted implementation, so its original feature RED is unavailable and is not re-claimed. Inspection candidates were finishing that work, v0.31.0 release closure, direct aggregate double fields, and parser/fuzz expansion; recovery priority selected the existing array package. Focused inherited tests were initially GREEN, but the broader `double` slice exposed a regression where raw-memory intrinsic calls saw `double pointers are not supported` before their established unsupported-double-storage diagnostic. Independent review then found reverse-subscript double reads, `_Generic`, and increment results were typed/updated as integers. New focused regressions went RED for `index[values]`, then shared reverse-subscript pointee metadata and numeric update logic made them GREEN while restoring raw-memory diagnostic precedence. Fresh independent summary review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,598 local tests, the rebuilt 1,598-test Docker gate, runtime output `10`, the actual compiler-oracle harness, direct GCC/Clang fixture execution, and `git diff --check` pass.

Bounded Cust v0.30.0 is published around the completed direct `double` function-boundary slice. Cargo package/lock metadata, exact CLI output and tests, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and the authoritative status ledgers are synchronized for `0.30.0`. Executable target listing independently reconciles 1,594 tests: 1,459 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.29.0`, then passed after Cargo/lock and both image tags moved to `0.30.0`. Cargo metadata and the built CLI report `cust 0.30.0`; local and remote annotated-tag preflight found no conflicting `v0.30.0`. Candidate evaluation considered bounded v0.30.0 release closure, direct one-dimensional double arrays, shared-storage union bytes, and parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time and atomically packaged the independently reviewed function-boundary slice without widening runtime semantics. Direct one-dimensional double arrays became the next concrete language package at that point.

Independent release review initially found stale README return wording, two historical present-tense queue claims, and an omitted double-typedef limitation. After targeted documentation corrections, fresh complete-diff re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,594 local tests, the rebuilt 1,594-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.30.0`, repeated empty tag preflight, and `git diff --check` pass. A supplemental Cargo-metadata probe initially had a Python quoting error after every required gate had passed; the corrected read-only probe reports `cust 0.30.0` and the rebuilt container reports `cust 0.30.0`.

Release commit `86d0d470fc22825e096a3dbca638dde84ca5800b` reached exact `origin/main` before annotated tag publication. Remote annotated tag object `3aaf5af050ca4867e7e928e86d2516370aae3b61` peels exactly to that release commit; local tag type/object/peeled checks and remote branch/tag checks agree. This later status-only publication-evidence commit did not move or recreate `v0.30.0`. Direct one-dimensional double arrays became the sole next roadmap package at that point.

Direct bounded `double` function boundaries are complete. `double` parameters and returns now work in prototypes, definitions, recursive calls, implicit integer/double conversions, casts, `_Generic`, and non-evaluating `sizeof(call)` while preserving exact f64-bit metadata and one-time argument evaluation. Direct `double *`, `double[N]`, pointer-to-row `double (*)[N]`, and non-`int` `main` declarations remain exact source-located boundaries. A registered warning-free native fixture passes GCC, Clang, and the actual compiler-oracle harness. The final canonical local/rebuilt-Docker gate passes 1,594 executable tests: 1,459 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle, and 1 repository-license test; the rebuilt runtime prints `10`.

TDD started with the scalar boundary regression failing at `double function returns are not supported`; focused GREEN required enabling parser signatures and classifying `Expr::Call` from definition/prototype return metadata. A separate diagnostic matrix first RED-confirmed that direct double pointers and arrays could bypass the new function route. Independent review then found pointer-to-row return/parameter bypasses and raw IEEE exit bits from `double main`; focused regressions failed for those exact defects before parser guards and the entry-point rule made both focused double tests GREEN. Pre-commit review subsequently exposed a row-pointer `main` bypass because the entry-point check ran before return-declarator lowering; its focused regression failed with `missing main() function`, then passed after moving the shared check after row-pointer classification. Fresh independent re-review returned `APPROVED`. Candidate evaluation considered direct double function boundaries, union shared-byte storage, and parser/fuzz expansion; the queue-leading direct function package was selected as the coherent high-impact vertical slice.

Bounded Cust v0.29.0 is published around the completed first `double` runtime-value slice. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitation/roadmap text, changelog notes, and the authoritative status ledgers are synchronized for `0.29.0`. Executable target listing independently reconciles 1,592 tests: 1,457 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only exact CLI and Compose expectations first: both failed against package/image version `0.28.0`, then passed after Cargo/lock and both image tags moved to `0.29.0`. Cargo metadata and the built CLI report `cust 0.29.0`; local and remote annotated-tag preflight found no conflicting `v0.29.0`. Work-package candidates were bounded v0.29.0 release closure, direct double function-boundary support, a shared-storage union-byte model, and parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time and atomically packages the already reviewed double slice with executable metadata assertions.

Independent review initially blocked two diagnostic overclaims plus premature “released” and stale queue wording. After targeted documentation corrections, fresh complete-diff re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,592 local tests, the rebuilt 1,592-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.29.0`, repeated empty tag preflight, and `git diff --check` pass. Release commit `fc48c670f85e70e2939e9aa6f222e09f6826c8af` reached exact `origin/main` before annotated tag object `e86cb26d49c7d3f6f029259f81d9fc5c4f6eebbe` was published; local and remote tag refs peel exactly to the release commit. This later status-only publication-evidence commit does not move or recreate `v0.29.0`.
...
Release TDD changed only exact CLI and Compose expectations first: both failed against package/image version `0.27.0`, then passed after Cargo/lock and both image tags moved to `0.28.0`. Cargo metadata and the built CLI report `cust 0.28.0`; local and remote annotated-tag preflight found no conflicting `v0.28.0`. Work-package candidates were bounded v0.28.0 release closure, a shared-storage union-byte model, a first bounded `double` runtime slice, and further parser/fuzz expansion. Release closure was selected because it was the sole unchecked authoritative roadmap item at selection time and atomically packaged the independently reviewed whole-struct feature without widening runtime semantics.

Independent release review found two documentation consistency issues: “sole unchecked” needed explicit selection-time scope after the next feature was recorded, and the longer-term roadmap still duplicated floating-point work after `double` became the next milestone. Both were corrected and fresh complete-diff re-review returned `APPROVED` with no remaining findings. Formatting, warning-denied Clippy, all 1,534 local tests, the rebuilt 1,534-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.28.0`, empty tag preflight, and `git diff --check` passed. Release commit `02561fb29abd65339d5d01612649636855041955` reached exact `origin/main` before annotated tag object `b12de927c124c163962e43bd5b56b30e46d9557d` was published; local and remote tag refs peel exactly to the release commit. This later status-only publication-evidence commit does not move or recreate `v0.28.0`.
...
Release TDD changed only exact CLI and Compose expectations first: both failed against package/image version `0.26.0`, then passed after Cargo/lock and both image tags moved to `0.27.0`. Cargo metadata reports `cust 0.27.0`; local and remote annotated-tag preflight found no conflicting `v0.27.0`. Work-package candidates were bounded v0.27.0 release closure, deterministic whole-struct object bytes, union-backed bytes, and further parser diagnostics. Release closure was selected because it was the sole unchecked roadmap item at selection time and atomically packaged the already reviewed row slice with bounded consistency risk. Fresh independent complete-diff review returned `APPROVED`; formatting, warning-denied Clippy, all 1,479 local tests, doc tests, focused version/license/compiler-oracle tests, the rebuilt 1,479-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.27.0`, tag preflight, and `git diff --check` passed. Release commit `0afffbf5f06528a225d672d886f82af4bdbc3f50` reached exact `origin/main` before annotated tag object `b2b763214212b7bc6b3c678754384a68b97518d7` was published; local and remote tag refs peel exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.27.0`.

After v0.27.0 publication, deterministic whole-struct object bytes became the next concrete implementation package under Cust's existing no-padding aggregate size model. Union-backed storage remained a separate rejected boundary until Cust has shared member storage/aliasing semantics.

All five exactly prototyped bounded raw-memory intrinsics now accept one selected row of supported two-dimensional non-character scalar arrays. Direct row decay, adjusted parameters, explicit pointer-to-row indexing/dereference, and supported struct-field `int`/`_Bool` rows reuse the existing dimensioned scalar storage model. Byte capacity and overlap stop at the selected row boundary; owner, lifetime, recursive const, checked row-index overflow, aligned/interior identity, zero-count behavior, and non-evaluating calls remain exact. Whole aggregates and union-backed storage remain unsupported.

Work-package candidates were the queue-leading two-dimensional non-character row extension, bounded v0.27.0 release closure, whole-aggregate deterministic layout, and further parser diagnostics. The row package was selected because it was first in both authoritative queues and completed a coherent five-intrinsic slice without inventing host padding or union aliasing. Initial RED failed at the explicit two-dimensional non-character admission guard. Removing that obsolete guard made the existing row-local dimension/provenance model GREEN; adjusted, pointer-to-row, aggregate-field, safety, identity, zero-count, and non-evaluating routes then passed focused coverage. Four interpreter tests and one registered ABI-independent compiler-oracle fixture bring the executable inventory to 1,479 tests: 1,344 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. See `references/cust-bounded-memory-two-dimensional-scalar-object-rows.md`.

Independent read-only review returned `APPROVED` with no blocking correctness, safety, C-semantics, regression-risk, or test-completeness findings. The focused 14-test scalar-object suite and actual compiler-oracle harness pass. Formatting, warning-denied Clippy, all 1,479 local tests, the rebuilt 1,479-test Docker gate, runtime output `10`, and `git diff --check` pass.

Completion of the row slice promoted bounded v0.27.0 release closure as the next package at that point. Whole-aggregate bytes were deferred because no deterministic aggregate-byte traversal had been designed, and union-backed bytes remained deferred because members use separate interpreter storage.

Bounded Cust v0.26.0 is published around the completed non-character scalar struct-field object-byte package. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central inventory/limitations/roadmap text, changelog notes, and current status ledgers are synchronized for `0.26.0`. Executable listing independently reconciles 1,475 tests: 1,340 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test. This corrects the preceding feature-run prose, which reported the right 1,475 total but retained the old 1,335 interpreter subtotal.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.25.0`, then passed after Cargo/lock and both image tags moved to `0.26.0`. Local/Cargo output reports `cust 0.26.0`; local and remote annotated-tag preflight found no conflicting `v0.26.0`. Work-package candidates were bounded v0.26.0 release closure, two-dimensional non-character row bytes, whole-aggregate deterministic layout, and further parser diagnostics. Release closure was selected because it was first in both authoritative queues, atomically packages the already reviewed struct-field slice, and had bounded consistency risk with executable version assertions. Independent complete-diff review returned `APPROVED` with no findings after checking exact version consistency, inventory arithmetic, README/release boundaries, queue truth, pre-publication wording, tag ordering, and secret absence. Formatting, warning-denied Clippy, all 1,475 local tests, the rebuilt 1,475-test Docker gate, compiler-oracle comparisons, runtime output `10`, built/Cargo version `cust 0.26.0`, and `git diff --check` pass. Release commit `9bb4e0222904a5029cfbc5ada389e33c38b34063` reached `origin/main` before annotated tag object `eeeaeb69967188b558d3211e76b877da5d62d8de` was published; local and remote tag refs peel exactly to that release commit. Two-dimensional non-character row bytes became the next concrete implementation package at that point.

All five exactly prototyped bounded raw-memory intrinsics now extend Cust's deterministic `int`/`_Bool` object bytes from standalone storage into supported struct fields. Direct and arrow field addresses, named/anonymous/nested fields, one-dimensional scalar-array fields, and scalar fields reached through embedded aggregate-array elements reuse existing field storage and provenance. Field-local byte bounds/overlap, recursive const and lexical lifetime, canonical aligned/interior `memchr` results, typed-pointer coercion, `_Bool` normalization, zero-count behavior, and non-evaluating calls remain exact; whole aggregates, union-backed roots, and two-dimensional non-character rows were unsupported in that struct-field slice.

Work-package candidates were the queued non-character scalar struct-field extension, two-dimensional non-character rows, whole-aggregate deterministic layout, and further parser diagnostics. The struct-field package was selected because it was first in both authoritative queues, reused the reviewed scalar representation without inventing aggregate padding, and completed a coherent five-intrinsic vertical slice. Initial RED failed with the explicit standalone-only diagnostic; removing that obsolete admission guard made direct fields GREEN, while broader direct/arrow/nested/anonymous/embedded routes, safety boundaries, identity, and non-evaluation coverage passed through the shared storage model. Five interpreter tests and a registered ABI-independent compiler-oracle fixture bring the executable inventory to 1,475 tests. Independent complete-diff review returned `APPROVED`; formatting, warning-denied Clippy, all 1,475 local and rebuilt-Docker tests, the runtime fixture output `10`, security/diff checks, and compiler-oracle comparisons pass. Bounded v0.26.0 release closure became next at that point.
...
Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.24.0`, then passed after Cargo/lock and both image tags moved to `0.25.0`. Local/Cargo output reports `cust 0.25.0`; local and remote annotated-tag preflight found no `v0.25.0`. Work-package candidates were bounded v0.25.0 release closure, non-character scalar aggregate-field object bytes, two-dimensional non-character row bytes, and whole-aggregate/union layout. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed standalone scalar slice, and had bounded consistency risk with executable version assertions. Non-character scalar struct fields became the next concrete vertical slice at that point; two-dimensional non-character rows and whole aggregate/union storage were later slices because their shape and aliasing fan-out was larger.

Independent release review first found that the README diff did not itself expose the exact executable inventory and v0.25.0 limitation delta. The README now records both, and fresh complete-diff re-review returned `APPROVED` with no findings after independently confirming version consistency, inventory arithmetic, support boundaries, queue truth, pre-publication wording, commit-before-tag ordering, and secret absence. Canonical release preparation verification then passed formatting, warning-denied Clippy, all 1,470 local tests, the rebuilt 1,470-test Docker gate, compiler-oracle comparisons, runtime output `10`, local/Cargo/container version `cust 0.25.0`, the static release-surface safety scan, local/remote tag preflight, and `git diff --check`. At that preparation point, the annotated tag remained reserved for creation only after the verified release commit reached `origin/main`.

Bounded Cust v0.25.0 is published. Verified release commit `e0d097ed4e6fe676a05a2871ed5c2d4fda31cd53` reached `origin/main` before annotated tag creation. Remote annotated tag object `9f8dd68109c9b00d7f11a41dd3e2b7bb439080c7` peels exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.25.0` and is not the tagged release commit.

The five exactly prototyped bounded raw-memory intrinsics now support deterministic object-byte access to standalone `int`/`_Bool` scalars and one-dimensional arrays. Cust `int` uses an explicit eight-byte little-endian two's-complement representation; partial writes reconstruct the selected `i64` cell, `_Bool` writes normalize to canonical zero/one storage, character cells retain their established exact-value copy semantics, and whole aggregates, non-character aggregate fields, two-dimensional non-character rows, and union-backed roots were exact unsupported boundaries in that standalone slice.

`PointerValue::ObjectByte` preserves precise interior-byte results from `memchr` without exposing host addresses. Byte-scaled capacity, overlap, owner/lifetime/const checks, destination-aware aligned-result coercion, pointer equality/difference, indexed byte lvalues, subsequent raw-memory calls, source snapshots, and zero-count/non-evaluating behavior reuse interpreter-owned storage. Focused RED/GREEN covered initial non-character rejection, interior-result equality, pointer difference, aligned-array identity, aligned `char *` byte-view arithmetic, typed declaration/statement/expression-assignment/parameter/return/aggregate-field coercion, and rejection of non-character aggregate fields outside this standalone slice. Seven focused interpreter tests and the registered native fixture's ABI-independent full-object relationships pass. See `references/cust-bounded-memory-scalar-object-bytes.md`.

Historical work-package candidates were the first deterministic standalone integer object representation, union-backed raw storage, whole-aggregate layout, and further parser diagnostics. The integer slice was selected because it was first in both authoritative queues, completed all five intrinsics over a high-value scalar representation without host ABI inference, and had bounded acceptance tests. Whole aggregates and unions remain deferred because Cust does not model padding or union byte aliasing. Completion of this slice promoted the bounded v0.25.0 release closure selected above.

Final verification on 2026-08-08: independent review returned `APPROVED`; `cargo fmt --check`, warning-denied Clippy, all 1,470 local tests, rebuilt Docker tests, compiler-oracle comparisons, `docker compose run --rm cust` output `10`, and `git diff --check` pass.

Bounded Cust v0.24.0 is published around the completed two-dimensional character-row raw-memory extension. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central inventory/limitations/roadmap text, changelog notes, and all status ledgers are synchronized for `0.24.0`. The independently listed executable inventory is 1,463 tests: 1,328 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.23.0`, then passed after Cargo/lock and both image tags moved to `0.24.0`. Local/Cargo output reports `cust 0.24.0`; local/remote annotated-tag preflight found no `v0.24.0`. Work-package candidates were release closure, a deterministic standalone-integer raw-memory object representation, union-backed character storage, and further parser diagnostics. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed row extension, and had bounded consistency risk with executable version assertions.

Independent read-only release review returned `APPROVED` with no findings after independently confirming exact version consistency, the 1,463-test arithmetic and 36-test grouped remainder, README/changelog boundaries, pre-publication wording, release history, and one unambiguous next task. Canonical release preparation verification then passed formatting, warning-denied Clippy, all 1,463 local tests, the rebuilt 1,463-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.24.0`, the static secret/security scan, and `git diff --check`.

Verified release commit `ce7801d57bc4fe21e5b66474fd6015158b9e50f8` reached `origin/main` before annotated tag creation. Remote annotated tag object `55422dba7bdc8db55f5e91cff83c2f08e56d7ccd` peels exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.24.0` and is not the tagged release commit.

The next concrete package at that point was the deterministic standalone `int` object-representation slice for bounded raw-memory intrinsics completed above.

All five exactly prototyped bounded raw-memory intrinsics now accept scalar character-pointer views of existing two-dimensional `char` rows. Direct `matrix[i]`, adjusted array parameters, explicit pointer-to-row indexing/dereference, and supported aggregate-field rows preserve interpreter-owned identity, owner/lifetime/const metadata, row-local capacity and overlap, interior offsets, return identity, zero-count behavior, and non-evaluating `sizeof` without flattening across rows or inventing host layout.

Focused RED/GREEN first removed the raw-memory-only rejection of dereferenced rows while retaining row-local read/write capacity. A second RED/GREEN added direct single-index row-decay routing across runtime/pointee/const classifiers. Independent review then exposed unchecked row-index overflow and two nested aggregate const-propagation gaps; dedicated regressions now produce an exact overflow diagnostic and reject both const roots and const aggregate ancestors reached after embedded aggregate-array elements. The focused success/diagnostic suite, actual compiler oracle, and direct GCC/Clang C11 warnings-as-errors checks pass. See `references/cust-bounded-memory-two-dimensional-character-rows.md`.

Formatting, warning-denied Clippy, all 1,463 local tests (1,328 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,463-test Docker gate, runtime image output `10`, and `git diff --check` passed for the row feature. Historical work-package candidates were the queued two-dimensional row raw-memory extension, broader integer/aggregate object representations, further parser diagnostics, and bounded v0.24.0 release closure. The row package was selected because it was first in both authoritative queues, reused existing width-aware row views without host ABI assumptions, and completed a coherent five-intrinsic vertical slice; that completion promoted the v0.24.0 release closure selected above.

Bounded Cust v0.23.0 packages the completed struct-backed character-storage raw-memory extension. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central inventory/limitations/roadmap text, changelog notes, and all status ledgers are synchronized for `0.23.0`. The independently listed executable inventory is 1,460 tests: 1,325 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.22.0`, then passed after Cargo/lock and both image tags moved to `0.23.0`. Local/Cargo output reports `cust 0.23.0`, and local/remote annotated-tag preflight found no `v0.23.0` before publication.

Independent read-only release review found one stale historical v0.22.0 “is next” claim; after historical scoping, complete-diff re-review returned `APPROVED` with no remaining findings. Formatting, warning-denied Clippy, all 1,460 local tests, the rebuilt 1,460-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.23.0`, and `git diff --check` pass.

Bounded v0.23.0 is published. Verified release commit `184699131756878a5c675dc179f2817eac06634e` reached `origin/main` before annotated tag creation. Remote annotated tag object `a69bd951ce7d1359a06214d10aac25e43573b564` peels exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.23.0` and is not the tagged release commit.

Historical work-package evaluation considered bounded v0.23.0 release closure, two-dimensional character-row raw-memory access, broad integer/aggregate object-representation serialization, and further parser diagnostics. Release closure was selected because it was first in both authoritative queues, packaged the independently reviewed struct-backed five-intrinsic slice, and had bounded consistency risk with executable version assertions. Row-local raw-memory access through Cust's existing two-dimensional character-row views became the next concrete package at that point; broader object representations remained separate design work because Cust must not improvise host ABI layout.

The bounded five-function raw-memory family now accepts interpreter-owned character scalars and one-dimensional character arrays embedded in named, anonymous, and nested struct fields. Direct and arrow decay, interior offsets, field-local capacity/overlap, selected-root return identity, const/lifetime checks, zero-count calls, and non-evaluating `sizeof` reuse existing checked pointer storage. Scalar fields reached directly or through embedded aggregate-array elements have one-byte capacity and use checked dereference/write paths. Whole aggregates and non-character fields were rejected in that character-field slice.

The initial aggregate-array fixture passed immediately because existing array-field decay already retained its `Rc<ArrayValue>` storage identity. Focused RED/GREEN then removed the explicit scalar aggregate-field rejection, added capacity/read support for `StructField` and `StructFieldElementField`, and closed direct plus embedded-element scalar routes. Independent review exposed unsafe union-member admission and stale documentation. Review-driven RED/GREEN now recovers the containing aggregate type for scalar and array field pointers and rejects union-backed character storage exactly, because Cust does not yet model raw-byte aliasing between union members. A subsequent independent review found that shared two-dimensional row validation regressed five established string-operation tests and that clonable array-level union flags leaked through union-member-to-struct copies. Focused RED/GREEN moved row rejection into the raw-memory validator and replaced copied flags with live `Rc<ArrayValue>` container/ancestor lookup; string row behavior and copied struct independence are restored. Focused runtime/diagnostic tests and the registered GCC/Clang-warning-free compiler oracle pass, a fresh independent complete-diff re-review returned `APPROVED`, and the final 1,460-test local/rebuilt-Docker canonical gate passes. See `references/cust-bounded-memory-aggregate-character-fields.md`.
...
The initial coverage-only property test passed immediately because the underlying interpreter behavior was already supported. Independent review then exposed two test-quality defects: consecutive low LCG bits correlated route dimensions, and forwarding branch selection was correlated with route parity; explicit 32-cell tuple and 48-cell selected route×branch assertions went RED before structural enumeration made every cell GREEN. Review also found that Clang rejects mutation expressions beneath `sizeof` under warnings-as-errors, so the registered native fixture now uses side-effect-free ABI-independent type relationships while the interpreter property retains non-evaluation markers. Final independent re-review returned `APPROVED`; the focused property test, actual compiler-oracle harness, and direct GCC/Clang C11 warnings-as-errors fixture checks pass. Formatting, warning-denied Clippy, all 1,448 local tests (1,313 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,448-test Docker gate, runtime image output `10`, and `git diff --check` pass. See `references/cust-ordinary-named-aggregate-array-pointer-field-modeling.md`.

Work-package evaluation for the preceding model run considered the queued deterministic aggregate-array pointer-field model, broader integer/aggregate raw object representations, aggregate-backed character-storage memory intrinsics, and further parser-diagnostic discovery. The model package was selected because it was the sole unchecked authoritative item and closed a high-risk owner/path/index/type/const interaction with executable route accounting. At that point, the next package was the now-completed one-dimensional struct-backed character-storage extension recorded above; broader integer/aggregate object representations and multidimensional character storage remain separate work.

Bounded Cust v0.22.0 release preparation packages the completed five-function C11 character-storage raw-memory family: exactly prototyped `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr` over interpreter-owned standalone `char` scalars and one-dimensional `char` arrays. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and all status ledgers are synchronized for `0.22.0`. The independently reconciled executable inventory is 1,447 tests: 1,313 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.21.0`, then passed after Cargo/lock and both image tags moved to `0.22.0`. Local/Cargo output and the rebuilt runtime image report `cust 0.22.0`, and local/remote annotated-tag preflight found no `v0.22.0`. Independent read-only review first found stale queue wording; after historical scoping, re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,447 local tests, the rebuilt 1,447-test Docker gate, runtime output `10`, version output `cust 0.22.0`, and `git diff --check` pass. Release preparation completed before release-commit-first publication.

Bounded v0.22.0 is published. Verified release commit `86b5a5be1ea73e5cf1ba8eef0c77d9b0cf3f66e9` reached `origin/main` before annotated tag creation. Remote annotated tag object `68fbdd70f8cb7edf1b4212272ba1900ac968f7df` peels exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.22.0`.

Historical work-package evaluation considered bounded v0.22.0 release closure, broad integer/aggregate raw object representations, deterministic model-based pointer-field coverage, and further parser-diagnostic expansion. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the independently reviewed five-function raw-memory family, and had bounded consistency risk with executable version assertions. Deterministic model-based coverage for pointer fields selected through ordinary aggregate-array elements became the next concrete work package at that point; broader raw object representations remained separate design work.
...
Historical work-package evaluation considered exactly prototyped character-storage `memchr`, broad integer/aggregate raw object representations, immediate parser/fuzz expansion, and a later release closure. `memchr` was selected because it was first in both authoritative queues, completed the five-function C11 raw-memory family over the already reviewed bounded byte model, and avoided inventing host ABI layout. Bounded v0.22.0 release closure became next at that point; broader object representations remained separate design work.

Bounded Cust v0.21.0 release preparation packaged the completed character-storage raw-memory family: exactly prototyped `memcpy`, `memmove`, `memcmp`, and `memset` over interpreter-owned standalone `char` scalars and one-dimensional `char` arrays. Aggregate-backed character fields and multidimensional character arrays were exact unsupported boundaries in that release. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and all status ledgers were synchronized for `0.21.0`. The executable inventory was independently reconciled at 1,437 tests: 1,303 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.20.0`, then passed after Cargo/lock and both image tags moved to `0.21.0`. Independent read-only review first found that release docs overstated the accepted storage roots and used one premature “released” adjective; the corrected diff explicitly limits raw-memory calls to standalone character scalars/one-dimensional arrays, rejects aggregate-backed/multidimensional character storage, and received `APPROVED` on final re-review. Formatting, warning-denied Clippy, all 1,437 local tests, the rebuilt 1,437-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.21.0`, secret scan, and `git diff --check` pass.

Bounded v0.21.0 is published. Verified release commit `a2594b91e4356e339faac0da06a8ef09587f9574` reached `origin/main` before annotated tag creation. Remote annotated tag object `329e943ce94dd716e476dba562e6efb80a6d8c67` peels exactly to that release commit. This later status-only publication-evidence commit does not move or recreate `v0.21.0`.

Historical work-package evaluation considered bounded v0.21.0 release closure, broad integer/aggregate raw object representations, exactly prototyped character-storage `memchr`, and lower-priority parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time and atomically packaged four independently reviewed adjacent intrinsics with executable metadata assertions. Bounded `memchr` became the next concrete vertical slice at that point because it reused the completed byte model without inventing integer/aggregate or host ABI object representations.

The bounded character-storage raw-memory package now includes exactly prototyped `void *memset(void *, int, size_t)`. Destination, scalar fill value, and count evaluate once in source order; fill values convert modulo 256 as `unsigned char`; up to 4,096 interpreter-owned character cells are written; zero count leaves storage unchanged; and the original destination identity returns as declared `void *`. Const, lifetime, capacity, non-character and aggregate-backed storage, count/value shape, exact declaration, absent declaration, user-definition precedence, and direct/nested non-evaluating `sizeof` boundaries remain deterministic without host addresses or host libc.

Focused RED/GREEN first proved missing `memset` dispatch. A later nested/direct `sizeof` regression failed because exact intrinsic calls lacked the scalar-size fallback after recursive metadata validation; the focused fix preserves pointer size and linear non-evaluation. Nine focused interpreter tests cover low-eight-bit conversion, embedded NULs, offsets, zero count, one-time evaluation, safety diagnostics, prototype/absent-declaration rules, nested `sizeof`, aggregate-backed rejection, and user definitions. The registered warning-free native fixture and compiler oracle pass. Independent read-only review found no blocking correctness or safety issue; its low documentation-drift finding was resolved across README, changelog, and status.

Formatting, warning-denied Clippy, all 1,437 local tests (1,303 interpreter, 98 fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,437-test Docker gate, runtime output `10`, focused `memset`, the compiler oracle, and `git diff --check` pass.

Work-package evaluation considered bounded character-storage `memset`, broader integer/aggregate object-representation serialization, a v0.21.0 package release, and lower-priority parser/fuzz expansion. `memset` was selected because it was first in both authoritative queues, completed the coherent four-intrinsic character-storage raw-memory package, and avoided inventing host ABI layout. Bounded v0.21.0 release closure became next at that point; broader object representations remained separate design work.

The bounded character-storage raw-memory slice now includes exactly prototyped `int memcmp(const void *, const void *, size_t)`. Both inputs and the count evaluate once in source order; up to 4,096 character cells are read without mutation; values compare lexically as `unsigned char`; zero count returns zero; and only the sign of the first differing byte is exposed. Independent input-capacity, non-character-storage, owner/lifetime, count, exact-declaration, user-definition-precedence, and direct/nested non-evaluating `sizeof` boundaries remain deterministic without host addresses or host libc.

Focused RED/GREEN first proved missing `memcmp` dispatch, then separately exposed and fixed copy-oriented capacity and count wording so left/right input failures remain exact. Eight focused interpreter tests cover unsigned high-byte ordering, embedded NULs, offsets, zero count, one-time evaluation, safety diagnostics, prototype/`sizeof` rules, and user definitions; the registered warning-free native fixture passes. Independent read-only review returned `APPROVED` with no blocking or material findings.

Formatting, warning-denied Clippy, all 1,428 local tests (1,294 interpreter, 98 fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,428-test Docker gate, runtime output `10`, focused `memcmp`, the compiler oracle, and `git diff --check` pass. The first Docker build exposed a pre-existing linearity-test false positive: depth 40 is five times depth 8 but the test allowed only 4x. Raising the detector to 8x preserves rejection of quadratic 25x growth and passed 20 focused repetitions plus independent re-review. See `references/cust-bounded-memory-comparison-function.md`.

Historical work-package evaluation considered bounded character-storage `memcmp`, bounded `memset`, broad integer/aggregate object-representation serialization, and lower-priority parser/fuzz expansion. `memcmp` was selected because it was first in both authoritative queues, completed the read-only comparison side of the reviewed byte model, and avoided inventing host ABI layout. Bounded character-storage `memset` became next at that point; broader object representations remained separate design work.

The first bounded raw-memory intrinsic slice is complete: an exactly prototyped `void *memcpy(void *, const void *, size_t)` copies up to 4,096 character-storage cells between interpreter-owned character objects/arrays, snapshots source values before mutation, returns the destination identity as `void *`, and never exposes host addresses or invokes host libc. Each Cust `char` cell counts as one byte for range/capacity purposes, while the snapshot preserves its exact deterministic stored value so negative `signed char` and high-byte values are not corrupted; integer and aggregate object representations were an explicit unsupported boundary in that initial slice.

Focused RED/GREEN established intrinsic dispatch, overlap rejection, exact source-capacity diagnostics, incompatible-prototype handling in runtime and non-evaluating `sizeof`, and contextual non-character-storage rejection. Review-driven RED/GREEN then preserved negative `signed char` values across the snapshot/write path. Additional focused coverage proves destination const/capacity, count, lifetime, zero-count, user-definition precedence, and non-evaluation behavior. The warning-free native compiler-oracle fixture covers defined non-overlap behavior, embedded NUL bytes, offset pointers, signed high-bit values, and destination identity. See `references/cust-bounded-memory-copy-function.md`.

Final independent read-only review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,414 local tests (1,280 interpreter, 98 fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,414-test Docker gate, runtime output `10`, and `git diff --check` pass after the review-driven regression fix.

Historical work-package evaluation considered the first bounded `memcpy` slice, broader all-object byte representations, and lower-priority parser/fuzz coverage. Character-storage `memcpy` was selected because it was the first unchecked roadmap item at selection time, reused mature provenance/lifetime/const machinery without inventing host ABI layout, and formed a complete safe vertical slice. Overlap-safe `memmove` became next at that point before integer/aggregate object representations.

Bounded Cust v0.20.0 is published around the completed one-level C11 `void *` function-boundary package. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/roadmap text, changelog notes, and all five status files are synchronized for `0.20.0`. The independently derived executable inventory is 1,400 tests: 1,266 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test. Remote annotated tag object `b47cb12f0cff7562f424a7de441226d1fd06fa1c` peels exactly to verified release commit `8fec9ef9e0f251808cc16b7f43d560686e88d9a1`.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.19.0`, then passed after Cargo/lock and both image tags moved to `0.20.0`. Local and remote annotated-tag preflight found no `v0.20.0`. Independent read-only review returned `APPROVED`; formatting, warning-denied Clippy, all 1,400 local tests, the rebuilt 1,400-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.20.0`, secret scan, and `git diff --check` passed. Release commit `8fec9ef9e0f251808cc16b7f43d560686e88d9a1` reached `origin/main` before annotated tag publication; remote tag object `b47cb12f0cff7562f424a7de441226d1fd06fa1c` peels exactly to that commit.

Historical work-package evaluation considered bounded v0.20.0 release closure, immediate implementation of a first memory intrinsic, and lower-priority parser/fuzz coverage. Release closure was selected because it was first in both authoritative queues at selection time, atomically packaged the already reviewed function-boundary vertical slice, and had bounded consistency risk with executable version assertions. A separately designed first raw-memory intrinsic became next at that point; object-representation semantics were not improvised inside the release run.
...
Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.18.0`, then passed after Cargo/lock and both image tags moved to `0.19.0`. Local and remote annotated-tag preflight found no `v0.19.0`. Independent read-only review returned `APPROVED` with no findings and independently confirmed the inventory arithmetic. Formatting, warning-denied Clippy, all 1,394 local tests, the rebuilt 1,394-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.19.0`, and `git diff --check` pass. Release commit `19e8117688d2ee21323bab4705f9de8fd974b1e2` reached `origin/main` before annotated tag publication; remote tag object `7394660be0cc79b771747ca7643b904eb42301ce` peels exactly to that commit.

Historical work-package evaluation considered bounded v0.19.0 release closure, immediate expansion of one-level `void *` through function boundaries, and lower-priority parser/fuzz coverage. Release closure was selected because it was first in both authoritative queues, packaged the already reviewed safe-object vertical slice, and had bounded metadata/docs risk with executable version assertions. With release preparation complete, one-level `void *` function return/prototype/call-result support became the sole next concrete prerequisite to bounded memory intrinsics at that point.

The bounded first safe C11 `void *` object/conversion slice is complete. Local, global, block-static, `for`-initializer, and parameter objects support null/equality/truthiness, ordinary assignment, conditional/comma forwarding, compatible object-pointer conversions, qualification preservation, and non-evaluating type queries while retaining interpreter-owned owner/lifetime/read-only identity. Dereference, indexing, arithmetic, ordering, qualification discard, incompatible conversion, deeper pointers, pointer-to-array forms, and function-pointer forms retain exact diagnostics without host addresses.

This run inherited the core implementation and initial fixtures, so their original RED evidence is unavailable and is not re-claimed. Focused RED/GREEN closed the missing `for (void *...)` initializer route and invalid arithmetic, ordering, increment, compound assignment, and unary arithmetic beneath non-evaluating `sizeof`. Independent read-only review returned `APPROVED`; focused interpreter coverage, recursion-depth coverage, and the registered warning-free compiler-oracle fixture pass. The canonical gate verifies 1,394 tests: 1,260 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test. See `references/cust-void-pointer-diagnostics.md`.

Work-package evaluation considered the bounded first safe `void *` slice, immediate v0.19.0 release closure, and lower-priority fuzz/parser expansion. Finishing the inherited `void *` diff was mandatory and completed the sole unchecked language package with focused parser/runtime/type/lifetime acceptance coverage. Bounded v0.19.0 release closure became the next recommended package at that point.

Bounded Cust v0.18.0 is independently approved and canonically verified for atomic publication around the completed first-pass C11 `_Generic` package. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.18.0`. The executable inventory is independently reconciled at 1,389 tests: 1,255 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.17.0`, then passed after Cargo/lock and both image tags moved to `0.18.0`. Local and remote annotated-tag preflight found no `v0.18.0`. Independent read-only review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,389 local tests, the rebuilt 1,389-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.18.0`, and `git diff --check` pass. The annotated tag remains reserved for this verified release commit and may be created only after `origin/main` accepts it; publication and remote peeled-target verification are not claimed by this preparation diff.

Work-package evaluation considered bounded v0.18.0 release closure, an immediate first safe `void *` slice, and lower-priority parser/fuzz expansion. Release closure was selected because it was first in both authoritative queues at selection time, packaged the already reviewed `_Generic` vertical slice as a coherent user-visible milestone, and had bounded metadata/docs risk with executable version assertions. The first safe `void *` object/conversion slice became the next concrete language package at that point.
...
Work-package evaluation considered finishing the inherited first-pass `_Generic` implementation, preparing v0.18.0 immediately, and lower-priority parser/fuzz expansion. Finishing the inherited diff was mandatory under the maintenance recovery rules and also completed the sole unchecked language backlog item with the highest roadmap impact. Bounded v0.18.0 release closure became the next recommended package at that point.
...
Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.16.0`, then passed after Cargo/lock and both image tags moved to `0.17.0`. Local and remote annotated-tag preflight found no `v0.17.0`. Independent read-only review found one low stale-history wording issue; after historical queue claims were time-scoped, re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,358 local tests, the rebuilt 1,358-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.17.0`, and `git diff --check` pass. The annotated tag remains reserved for this verified release commit and may be created only after `origin/main` accepts it; publication and remote peeled-target verification are not claimed by this preparation diff.

Ordinary reassignment for narrow tracked unqualified `char **` objects is complete. Local, file-global, block-static, and parameter objects accept null, compatible mutable unqualified `char *` slot addresses, and compatible tracked output-object values. Assignment expressions preserve the selected outer-slot and pointee owner/lifetime/read-only identity through forwarding, equality, truthiness, indirect reads/writes, and initializer contexts with one-time source-order evaluation. Parameter reassignment remains local to the callee. Compound updates, increment/decrement, arithmetic, address-taking, incompatible/qualified/deeper/array sources, and selected expired owners retain exact diagnostics.

Non-evaluating assignment classification is structural: direct and nested `sizeof` validates assignment compatibility without mutation or stale-target observation, while runtime liveness checks occur only on the selected runtime path. Review-driven RED/GREEN covered comma-side-effect repair before stale-target observation, non-evaluating `strtol` `endptr` assignment, conditional indirect-assignment target compatibility, nested scalar-wrapper validation, and null/non-null assignment-result `_Bool` normalization. Final independent review returned `passed: true` with no security, logic, or suggestion findings. Formatting, warning-denied Clippy, all 1,358 local tests (1,224 interpreter, 98 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the canonical Docker test gate, runtime output `10`, all 45 focused character-pointer-object tests, recursion-depth coverage, and `git diff --check` pass. See `references/cust-character-pointer-objects.md`.

Bounded Cust v0.16.0 is published around safe `char **` expression parity. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.16.0`. The executable release inventory is 1,343 tests: 1,209 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test. Remote annotated tag object `35ddb340e9ffb0bce35a4384f782edd0a6df745b` peels exactly to release commit `8d1537d78bd310ef8c6621d5520cf067241c3f55`.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.15.0`, then passed after Cargo/lock and both image tags moved to `0.16.0`. Local and remote annotated-tag preflight initially found no `v0.16.0`. Independent read-only review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,343 local tests, the rebuilt 1,343-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.16.0`, security scan, and `git diff --check` passed. Release commit `8d1537d78bd310ef8c6621d5520cf067241c3f55` reached `origin/main`; remote annotated tag object `35ddb340e9ffb0bce35a4384f782edd0a6df745b` peels exactly to that commit.

Safe `char **` expression parity is complete for Cust's bounded two-level character-pointer model. Conditional and comma expressions preserve the selected outer-slot identity through forwarding, equality, truthiness, and control contexts with one-time source-order evaluation. Function parameters reject addresses of qualified `char *` slots exactly, relational ordering remains unsupported, and non-evaluating `sizeof` recognizes conditional output values, validates both branches and discarded comma operands, preserves ordinary call-arity constraints, and stays linear across nested output-valued conditions. No host addresses or deeper pointers are exposed.

TDD and review evidence: inherited conditional/comma/equality/qualification/ordering coverage passed focused checks; independent review then drove RED/GREEN regressions for non-null scalar branches under `sizeof`, invalid ordinary void-call arity hidden in a discarded comma operand, and nested conditional validation scaling. The scaling probe failed before the metadata-only recursion fix and passed repeatedly afterward. Final read-only re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,343 local tests (1,209 interpreter, 98 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt 1,343-test Docker gate, runtime output `10`, and `git diff --check` pass.

Bounded Cust v0.15.0 is published around ordinary tracked unqualified `char **` objects. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.15.0`. The release inventory is 1,333 tests: 1,199 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.14.0`, then passed after Cargo/lock and both image tags moved to `0.15.0`. Local and remote annotated-tag preflight found no `v0.15.0`. Independent read-only review identified three low stale temporal claims; after they were rewritten as historical progression, re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,333 release tests, the rebuilt 1,333-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.15.0`, and `git diff --check` passed. Release commit `d4f016fd69399bf16f1e0a7ac54c50e2d62ab153` reached `origin/main`; remote annotated tag object `b91ca378f5059ee27c47383103b7d20fff6086b3` peels exactly to that commit.
...
TDD and review evidence: the inherited object implementation and fixture passed their initial focused checks; its original RED evidence was unavailable and is not re-claimed. Independent review then drove focused RED/GREEN regressions for qualified `char *` target slots and both branch orders of incompatible conditional assignments beneath `sizeof`; a second review required exact deeper/array declarator and reassignment/increment coverage. Final independent review returned `APPROVED`. Formatting, warning-denied Clippy, all 1,333 local tests (1,199 interpreter, 98 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt Docker test gate, runtime output `10`, the compiler-oracle fixture, recursion-depth regression, security scan, and `git diff --check` pass. See `references/cust-character-pointer-objects.md`.

Bounded Cust v0.14.0 is published. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.14.0`. The release packages the narrow safe unqualified `char **` output-parameter foundation and exact-prototype base-aware `strtol`, `strtoll`, `strtoul`, and `strtoull`. The enumerated inventory is 1,310 tests: 1,176 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test. Remote annotated tag object `c6981f850ff6e95d08cc39addc430fa4b159b253` peels exactly to release commit `c3c0ff3bc029f538e673dc6bfe506084fe9f1c23`.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.13.0`, then passed after Cargo/lock and both Compose images moved to `0.14.0`. Local and remote annotated-tag preflight found no `v0.14.0`. Independent review found one stale next-task sentence; after its correction, read-only re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,310 local tests, the rebuilt 1,310-test Docker gate, runtime output `10`, local/container version `cust 0.14.0`, Cargo metadata `cust 0.14.0`, and `git diff --check` pass. The verified release commit reached `origin/main` before annotated-tag publication, and the remote peeled target was checked exactly.

Bounded explicitly prototyped C11 `strtol`, `strtoll`, `strtoul`, and `strtoull` are complete. Exact normalized declarations activate Cust-owned implementations only when no user definition takes precedence. The package supports base 0 and 2..=36 selection, C whitespace/sign/valid-prefix/maximal-digit behavior, no-digit and partial conversions, deterministic signed/unsigned 64-bit bounds, source-order one-time evaluation, 4,096-byte scans, and absent/null/non-null `endptr` routes without host libc or host addresses. Returned end pointers preserve read-only and tracked owner/lifetime metadata; mutable ownerless array compound literals cannot escape through `endptr`, and visible enum-zero constants are accepted as null outputs.

TDD and review evidence: the inherited feature fixture and tests already passed focused interpreter/compiler-oracle checks. Independent review then drove three RED/GREEN regressions for escaped ownerless compound-literal end pointers, undefined base identifiers hidden in non-evaluating `sizeof(call)`, and visible enum-zero null `endptr` constants. The fixes preserve linear nested validation and enum/object shadowing. Final independent re-review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,310 local tests (1,176 interpreter, 98 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt Docker test gate, runtime output `10`, the compiler-oracle fixture, recursion-depth regression, security scan, and `git diff --check` pass.
...
Bounded explicitly prototyped C11 `strcoll` and `strxfrm` are complete on top of the published v0.12.0 package. Exact normalized declarations activate Cust-owned intrinsics only when no user definition takes precedence. `strcoll` uses deterministic unsigned-byte C-locale ordering. `strxfrm` returns the full transformed length excluding NUL, writes at most `count` bytes, permits a null destination only for a zero-count query, and enforces the shared 4,096-byte source/count ceiling plus destination capacity, overlap, const, owner, lifetime, and row safety.

TDD and review evidence: the registered warning-free C-locale fixture first failed with `undefined function 'strcoll'`; eight focused interpreter tests cover ordering/transform writes, normalized bytes, full-length/zero-count behavior, one-time source-order evaluation, exact declaration/arity/shape/user-definition boundaries, null/capacity/overlap/const/lifetime/row limits, nested non-evaluating `sizeof`, and exact 4,096-byte bounds. Independent review exposed exponential nested transform validation; a depth-6/depth-18 RED timed out after 300 seconds, the root-cause fix made validation linear, and final re-review returned `APPROVED`. Formatting, strict Clippy, all 1,279 local tests, the rebuilt Docker test gate, runtime output `10`, the compiler-oracle harness, and `git diff --check` pass.

Bounded v0.13.0 is published: annotated tag `v0.13.0` peels to release commit `4cc728e6a11aba9578683f73eda4d9184d8d88b9` on `origin`. The subsequent safe `char **` and base-aware conversion work is packaged by the prepared v0.14.0 release above.

Bounded explicitly prototyped C11 `exit`, `_Exit`, and `abort` is complete. Exact void signatures activate only when no user definition takes precedence; `exit`/`_Exit` evaluate one scalar status once and unwind to Cust's interpreted result, while `abort` unwinds to the recoverable `program aborted` error. Interpreter-owned markers never terminate the host process and are consumed only at the top-level run boundary. Direct, nested, global-initializer, scalar/pointer/aggregate value-context, declaration/arity/shape, user-definition, CLI, and non-evaluating constraint-aware `sizeof` routes are covered. Review-driven RED/GREEN fixed premature unwinding from scalar, aggregate, and pointer value contexts. The registered warning-free compiler-oracle fixture uses only status relationships and exits 23 under both native C and Cust. Independent final review returned `APPROVED`; formatting, warning-denied Clippy, all 1,271 local tests (1,137 interpreter, 98 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), the rebuilt Docker test gate, runtime output `10`, and `git diff --check` pass.

Bounded Cust v0.11.0 is prepared for atomic publication. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.11.0`. The release packages explicitly prototyped deterministic C11 `rand`/`srand`. The verified inventory is 1,259 tests: 1,126 interpreter tests, 98 deterministic fuzz-safety tests, 31 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.10.0`, then passed after Cargo/lock and both Compose images moved to `0.11.0`. Local and remote annotated-tag preflight found no `v0.11.0`. Independent diff-only review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,259 local tests, the rebuilt 1,259-test Docker gate, runtime output `10`, container version `cust 0.11.0`, and `git diff --check` pass. The tag is reserved for this verified release commit and may be created only after `origin/main` accepts it; publication and remote peeled-target verification are not claimed by this release-preparation diff.

Bounded explicitly prototyped C11 `rand`/`srand` is complete on top of the published v0.10.0 package. Exact normalized `int rand(void)` and `void srand(unsigned int)` declarations activate deterministic per-interpreter state only when no user definition exists. The default state matches seed 1, equal reseeds reproduce sequences, representative seeds diverge, values remain in `0..=32767`, seed expressions evaluate once, and runtime plus nested non-evaluating `sizeof` retain exact declaration, arity, scalar, and void-shape diagnostics without host libc.
...
Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.8.0`, then passed after Cargo/lock and both Compose images moved to `0.9.0`. Independent diff-only review returned `APPROVED` with no blocking/high/medium/low findings. The canonical gate passes: formatting, warning-denied Clippy, all 1,252 local tests, the rebuilt 1,252-test Docker gate, runtime output `10`, container version `cust 0.9.0`, and `git diff --check`. Release commit `9eaf5d266e5330547c999be6e59061141ef26fbe` was accepted by `origin/main` before annotated tag publication; remote tag object `70f5c8151b8938ae79e42ac6b524734748eb7959` peels exactly to that release commit.

The bounded explicitly prototyped C11 `<ctype.h>` classification package is complete. Exact retained `int function(int)` declarations activate Cust-owned `isalnum`, `isalpha`, `isblank`, `iscntrl`, `isdigit`, `isgraph`, `islower`, `isprint`, `ispunct`, `isspace`, `isupper`, and `isxdigit` intrinsics only when no user definition exists. Each call validates one scalar argument without evaluating it, then evaluates and converts it exactly once. Cust deliberately fixes the locale to deterministic ASCII/C-locale classes, returns canonical nonzero `1` or zero, accepts EOF (`-1`) and every unsigned-character value `0..=255`, and rejects other integers with an exact diagnostic. Missing/incompatible declarations, arity, pointer/aggregate/void shapes, direct runtime calls, nested calls, and non-evaluating `sizeof` all retain constraint-aware behavior without host libc.

TDD and review evidence: the registered warning-free fixture first failed with `undefined function 'isalnum'` before the shared classification dispatch was implemented. Three focused interpreter tests cover all twelve classes, category boundaries, EOF/high-byte handling, one-time nested-call evaluation, exact activation and user-definition precedence, unsupported declarations, arity/type/value diagnostics, and non-evaluating `sizeof`. The strict native fixture independently returned 0. Independent review found only Rustfmt blockers on its first pass; formatting was corrected and final re-review returned `APPROVED` with no logic, security, C-semantics, precedence, or dispatch blockers. Final `cargo fmt --check`, strict Clippy, all 1,252 local tests (1,119 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

Bounded explicitly prototyped C11 `strtok` is complete. Only the exact `char *strtok(char *, const char *)` declaration activates Cust's intrinsic; user definitions win and missing/incompatible declarations retain exact diagnostics. Both arguments evaluate once in source order before validation. Non-null calls skip delimiter runs, return only nonempty tokens, replace each token-ending delimiter in mutable interpreter storage with NUL, and install an owner-preserving continuation pointer; null calls continue that state and may use a different delimiter set. Exhaustion returns null and clears state. Both character sequences are normalized and independently bounded at 4,096 bytes. Null/type/const/read-only/expired-owner/two-dimensional-row and ownerless hidden-storage boundaries are deterministic. Direct, dereferenced, and nested `sizeof` stay non-evaluating while checking declaration, arity, pointer shape, and first-parameter qualification.

TDD and review evidence: the warning-free behavior fixture first failed with `undefined function 'strtok'`. Focused RED/GREEN then exposed continuation pointers escaping direct local arrays without lexical owners; attaching the existing array-owner metadata fixed that root cause. Independent review rejected a broad scalar-array-compound-literal registration workaround because it changed C block-lifetime semantics and retained every hidden array in scope; that change was removed, and ownerless mutable storage is now an explicit safe-subset runtime boundary. Review also drove typed-null runtime/`sizeof` regressions. Eight focused interpreter tests cover leading/trailing delimiter runs, changed delimiters, in-place mutation, reset/exhaustion, source-order evaluation, exact prototypes and user definitions, non-evaluation, 4,096-byte source/delimiter limits, const/read-only/null/type/row/lifetime diagnostics, and the tracked-owner boundary. The registered strict native fixture and all 1,249 local tests (1,116 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test) pass; final independent review reported no blocking security or logic errors.

The bounded Cust v0.8.0 package and annotated tag are published. Remote tag object `255460a2d335e613cffea674db327251d692810c` peels to release commit `b9abbcb46075153d83c872ccd1e354df784c92ae`.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.7.0`, then passed after Cargo/lock and both Compose images moved to `0.8.0`. The reviewed release commit was accepted by `origin/main`; annotated-tag publication and remote peeled-target verification are complete.

Bounded explicitly prototyped C11 `strncpy` is complete. Only the supported `char *strncpy(char *, const char *, size_t)`-equivalent signature under Cust's deterministic integer lowering activates the Cust-owned intrinsic; user definitions win and missing/incompatible declarations retain exact diagnostics. Destination, source, and count evaluate once in source order before validation. Cust rejects negative or above-4,096 counts, validates both character pointers at count zero, copies exactly the requested destination byte count, truncates without adding NUL when the source prefix fills the count, and pads the remaining destination range with NUL bytes after an early source terminator. Source values normalize to deterministic bytes, destination capacity and full read/write overlap are preflighted before mutation, and the original destination identity returns with exact row/owner/lifetime/const safety. Runtime, pointer classification, and direct/dereferenced non-evaluating `sizeof` preserve declaration, arity, and shape constraints without a host libc path.

TDD and review evidence: the focused behavior test first failed with `undefined function 'strncpy'`, then passed after the shared counted-copy dispatch and exact-write construction were added. Six focused interpreter tests cover NUL padding, no-NUL truncation, exact 4,096 and above-limit counts, normalized bytes, source-order one-time evaluation, destination identity, user-definition precedence, capacity/overlap/const/lifetime/two-dimensional-row safety, and direct/dereferenced non-evaluation. Independent review returned `APPROVED` with no blocking/high/medium findings; its sole low coverage note produced an early-NUL padding-capacity regression before the canonical gate. The registered warning-free native fixture passes; its zero-count self-copy probe remains interpreter-only because GCC rejects that form under the harness's `-Werror=stringop-truncation`. Final `cargo fmt --check`, strict Clippy, all 1,241 local tests (1,108 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

Bounded explicitly prototyped C11 `strncat` is complete. Only the supported `char *strncat(char *, const char *, size_t)`-equivalent signature under Cust's deterministic integer lowering activates the Cust-owned intrinsic; user definitions win and missing/incompatible declarations retain exact diagnostics. Destination, source, and count evaluate once in source order before validation. Cust rejects negative or above-4,096 counts, validates both character pointers even at count zero, scans the existing destination under the shared 4,096-byte bound, reads no more than the requested normalized source prefix without crossing two-dimensional row subobjects, appends one terminating NUL, preflights total capacity plus full read/write overlap before mutation, and returns the original destination identity. Exact null/type/lifetime/read-only/unterminated/capacity/overlap/row boundaries remain deterministic, including source prefixes without NUL when the count is reached.

TDD and review evidence: the registered behavior fixture first failed with `undefined function 'strncat'`; runtime pointer-count coverage then failed with the generic scalar diagnostic before exact shape-aware evaluation was added. Six focused interpreter tests cover zero/partial/full/early-NUL counts, exact 4,096 and above-limit bounds, normalized bytes, source-order one-time evaluation, return identity, declaration/user-definition precedence, capacity/overlap/const/lifetime/row safety, and direct/dereferenced non-evaluating `sizeof`. Independent review found missing early-source-NUL coverage; an exact-capacity focused regression now protects early termination and its capacity accounting, and final re-review reported no blocking findings. The warning-free fixture is registered in the native compiler-oracle harness; no host libc runtime path is used. Final `cargo fmt --check`, strict Clippy, all 1,235 local tests (1,102 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

Bounded explicitly prototyped C11 `strcat` is complete. The exact `char *strcat(char *, const char *)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Destination and source expressions evaluate once left-to-right before validation; both NUL-terminated sequences are independently bounded at 4,096 non-NUL bytes, source bytes are normalized, destination capacity is preflighted for `strlen(destination) + strlen(source) + 1`, and the original destination pointer is returned. Null, pointee, lifetime, read-only, unterminated, capacity, full append-range overlap, and two-dimensional row-boundary failures are deterministic. Runtime, direct/dereferenced/nested non-evaluating `sizeof`, and pointer classifiers preserve exact declaration/arity/shape constraints without a host runtime path.

TDD and review evidence: initial behavior failed with `undefined function 'strcat'`; a targeted overlap regression then proved that checking only the source-length range from the destination start missed source bytes intersected by the append range, and the shared overlap helper now accepts independent destination/source lengths. Nine focused interpreter tests and one registered warning-free native fixture cover success, empty/offset/scalar storage, return identity, source-order one-time evaluation, normalized source bytes, null/type/lifetime/read-only/termination/4,096-byte/capacity/overlap/row diagnostics, enum-shadowed row metadata, and non-evaluation. Independent review requested formatting and fixture staging; both were resolved and re-review returned `APPROVED`. Final `cargo fmt --check`, strict Clippy, all 1,229 local tests (1,096 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

Bounded explicitly prototyped C11 `strcpy` is complete. The exact `char *strcpy(char *, const char *)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Destination and source expressions evaluate once left-to-right before validation; the source is normalized and bounded at 4,096 non-NUL bytes, copied with its first NUL into mutable interpreter-owned character storage, and the original destination pointer is returned with owner/lifetime identity. Null, pointee, lifetime, read-only, unterminated, capacity, overlap, and two-dimensional row-boundary failures are deterministic. Runtime, direct `sizeof(call)`, dereferenced/wrapped non-evaluating pointer contexts, and pointer classifiers preserve exact declaration/arity/shape constraints without a host runtime path.

TDD and review evidence: initial behavior failed with `undefined function 'strcpy'`; focused RED/GREEN then covered arity-before-shape validation, enum shadowing of pointer-to-row metadata, row-pointer compound-assignment wrappers, overlap rejection, cross-row two-dimensional pointer arithmetic, and matching-prototype user-definition precedence. Nine focused interpreter tests and one registered warning-free native fixture cover successful copy/return identity, normalized bytes, source-order one-time evaluation, null/type/lifetime/read-only/termination/4,096-byte/capacity/overlap/row diagnostics, and direct/nested non-evaluation. Four independent review rounds drove the safety regressions and the final review returned `APPROVED`. Final `cargo fmt --check`, strict Clippy, all 1,220 local tests (1,087 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

The bounded C11 substring-search slice is implemented: the exact manually declared `char *strstr(const char *, const char *)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Both pointer expressions evaluate once left-to-right before validation; both interpreter-owned NUL-terminated sequences are normalized and independently bounded at 4,096 non-NUL bytes. Cust returns the first matching pointer into the original haystack, null when absent, and the unchanged haystack pointer for an empty needle. Returned offsets retain owner, lifetime, storage identity, and read-only enforcement. Runtime, pointer classifiers, direct `sizeof(call)`, nested `sizeof(*call)`, and calls beneath non-evaluating scalar wrappers enforce declaration, arity, and pointer-shape constraints without evaluating operands or using a host runtime path.

TDD and review evidence: the first focused behavior test failed with `undefined function 'strstr'`; empty-needle coverage then exposed Rust's invalid zero-width `windows(0)` path and received an explicit offset-zero fix. Eight focused interpreter tests now cover first/offset/absent/longer/empty matches, mutation and read-only storage, pointer truthiness/arithmetic, one-time source-order evaluation, normalized bytes, owner/lifetime/null/type/termination/4,096-byte diagnostics, exact activation and user-definition precedence, and direct/nested non-evaluating constraints. Independent review found nested `strstr` calls under scalar `sizeof` wrappers bypassed validation; a focused RED/GREEN regression generalized the shared recursive validator, and final re-review returned `APPROVED` with no blocking/high/medium finding. Final `cargo fmt --check`, strict Clippy, all 1,211 local tests (1,078 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

The bounded C11 string-span slice is implemented: exact manually declared integer-returning `strspn(const char *, const char *)` and `strcspn(const char *, const char *)` signatures activate Cust-owned intrinsics while user definitions win and missing/incompatible declarations remain explicit boundaries. Both pointer expressions evaluate once left-to-right before validation; both interpreter-owned NUL-terminated sequences are normalized and independently bounded at 4,096 non-NUL bytes. `strspn` returns the initial accepted-byte count and `strcspn` the initial non-rejected-byte count, including exact empty-text/set behavior. Runtime and direct/nested non-evaluating `sizeof` enforce declaration, arity, and pointer-shape constraints without a host runtime path.

TDD and review evidence: minimal `strspn` and `strcspn` tests first failed with `undefined function`; direct `sizeof` first failed similarly. Seven focused interpreter tests now cover standard behavior, empty/offset cases, strict fixture parity, one-time source-order evaluation, normalized bytes, null/type/termination/4,096-byte boundaries, user-definition precedence, and exhaustive direct/nested non-evaluating constraints. Independent review found unary/binary and then assignment/nested-`sizeof` traversal gaps; focused RED/GREEN expanded the validator across every expression and initializer child, and final re-review approved the complete diff with no blocking/high/medium finding. Final `cargo fmt --check`, strict Clippy, all 1,203 local tests (1,070 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime image prints `10`.

The bounded C11 character-set-search slice is complete: the exact manually declared `char *strpbrk(const char *, const char *)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Cust evaluates both pointer expressions once left-to-right before validation, validates and fully bounds both interpreter-owned NUL-terminated sequences independently, normalizes stored values to deterministic bytes, and returns the first pointer from the first sequence whose byte occurs in the accept sequence or null when absent/empty. Returned offsets preserve owner, lifetime, storage, and read-only enforcement; offset-zero matches retain the original pointer identity. Runtime, pointer classifiers, direct `sizeof(call)`, and nested `sizeof(*call)` routes enforce declaration, arity, and pointer-shape constraints without evaluating call operands or using a host runtime path.

TDD and review evidence: the warning-free behavior fixture first failed with `undefined function 'strpbrk'`. Six focused interpreter tests cover exact activation and user-definition precedence, first/absent/empty/offset results, mutable and read-only storage, one-time source-order evaluation before validation, normalized bytes, owner/lifetime/null/type/termination diagnostics, independent 4,096-byte bounds, pointer truthiness/arithmetic, and direct/nested non-evaluating `sizeof`. Independent review exposed two nested-`sizeof(*call)` constraint gaps; focused RED/GREEN regressions now preserve malformed-argument plus missing/incompatible-declaration diagnostics, and final review approved the code/test diff with no blocking/high/medium finding. The registered strict native C11 fixture, formatting, strict Clippy, all 1,196 local tests, and both Docker Compose gates pass; the runtime image prints `10`.

The bounded C11 reverse-character-search slice is complete: the exact manually declared `char *strrchr(const char *, int)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Cust evaluates the character pointer and search value once in source order before pointer validation, normalizes both to deterministic byte values, scans through the terminating NUL, and returns the last matching interpreter pointer including NUL or null when absent. Returned offsets preserve storage/owner/lifetime/read-only metadata; offset-zero NUL matches retain scalar-character pointer identity. A terminating NUL at offset 4,096 is valid, while any 4,097th non-NUL byte is rejected even if an earlier match exists. Runtime and non-evaluating `sizeof(call)` routes enforce exact declaration, arity, and shape constraints without a host runtime path.

TDD and review evidence: the warning-free behavior fixture first failed with `undefined function 'strrchr'`; incompatible runtime/`sizeof` declarations then failed with the generic undefined-function diagnostic before a focused RED/GREEN fix. Seven focused interpreter tests cover last/absent/NUL/offset results, normalized bytes, source-order one-time evaluation, pointer classifiers, mutable and read-only storage, owner/lifetime propagation, exact 4,096-byte boundaries, scalar NUL identity, user-definition precedence, and non-evaluation. Independent review approved the code/test diff with no blocking/high/medium findings; its sole low formatting finding was fixed before the canonical gate. Final `cargo fmt --check`, strict Clippy, all 1,190 local tests (1,057 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime container prints `10`.

The bounded C11 character-search slice is complete: the exact manually declared `char *strchr(const char *, int)` signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Cust evaluates the character pointer and search value once in source order before pointer validation, normalizes both to deterministic byte values, returns the first match including the terminating NUL or null when absent, preserves interpreter pointer owner/storage/read-only identity, and caps traversal at 4,096 bytes. Runtime and non-evaluating `sizeof(call)` routes enforce exact arity/type/declaration constraints; no host runtime path is used.

TDD and review evidence: focused behavior first failed with `undefined function 'strchr'`; incompatible declarations and `sizeof` constraints each received focused RED/GREEN fixes, and a scalar-character pointer regression first failed with unsupported scalar-pointer arithmetic before offset-zero matches were corrected. Seven focused interpreter tests and the registered warning-free native fixture cover first/absent/NUL/offset results, byte normalization, one-time source-order evaluation, pointer truthiness/arithmetic, mutation/read-only behavior, lifetime propagation, exact 4,096-byte boundaries, user-definition precedence, and non-evaluation. Independent final review approved the complete code/test diff with no blocking/high/medium findings. Final `cargo fmt --check`, strict Clippy, all 1,183 local tests (1,050 interpreter, 98 deterministic fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), both Docker Compose gates, and `git diff --check` pass; the runtime container prints `10`.

The annotated `v0.7.0` release tag is published on `origin`: tag object `61cfe4f003a81177408a537539095e537c844deb` peels to release commit `9492683140610f3ed35521186751132ccbe6967f`.

The bounded C11 prefix-comparison slice is complete: an exact manually declared `int strncmp(const char *, const char *, unsigned long int)`-compatible deterministic signature activates a Cust-owned intrinsic while user definitions win and missing/incompatible declarations remain explicit boundaries. Cust evaluates left pointer, right pointer, and count once in source order before validating either pointer. It rejects negative counts and counts above 4,096, validates both pointers even for a zero count, then reads normalized unsigned bytes lockstep and stops immediately at the first mismatch, shared NUL, or requested count. Storage exhaustion is diagnosed only when comparison reaches the unavailable byte. `sizeof(strncmp(...))` remains non-evaluating while validating declaration, arity, pointer shapes, and direct or wrapped void/aggregate/pointer count shapes. No host runtime path is used.

TDD and review evidence: the compiler-oracle behavior test first failed with `undefined function 'strncmp'`. Independent review then exposed eager independent prefix reads that rejected valid early-mismatch/NUL cases and a conditional-void count accepted by unevaluated `sizeof`; focused RED/GREEN regressions moved comparison to per-byte lockstep reads and reused `expr_is_void_value`. Exact 4,096-count and both zero-count null-pointer routes are covered. Six focused `strncmp` interpreter tests and the registered warning-free native fixture pass. The first full-suite gate exposed host-stack exhaustion at the previous 32-call guard after the interpreter call frame grew; focused recursion RED/GREEN lowered the deterministic guard to 24, preserving 22 recursive calls and the exact over-depth diagnostic. Independent final review approved the complete fix with no blocking/high/medium findings. `cargo fmt --check`, strict Clippy, all 1,176 local tests, both Docker Compose gates, and `git diff --check` pass; the runtime container prints `10`.

The Cust v0.7.0 package is published. Cargo package/lock metadata, exact CLI output, both Docker Compose image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized for `0.7.0`. The release packages explicitly prototyped bounded `abs`/`labs`/`llabs`, `atoi`/`atol`/`atoll`, `strcmp`, and `strlen` intrinsics without a host runtime path. The release inventory is 1,170 tests: 1,037 interpreter tests, 98 deterministic fuzz-safety tests, 31 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.6.0`, then passed after Cargo/lock and both Compose images moved to `0.7.0`. Independent review, the canonical local/Docker gate, acceptance of release commit `9492683140610f3ed35521186751132ccbe6967f` by `origin/main`, annotated-tag publication, and remote peeled-target verification are complete.
...
TDD and review evidence: four focused tests first failed with `undefined function 'strlen'`; the strict native C11 fixture independently returned 0. The initial review exposed an unregistered compiler-oracle fixture and an incompatible-declaration `sizeof` path that fell through to `undefined function`; registration plus a focused failing regression fixed both, and independent re-review returned `APPROVED`. Focused `strlen`, adjacent `strcmp`, and the registered compiler-oracle harness pass. Final `cargo fmt --check`, warning-denied Clippy, all 1,170 listed local tests, `docker compose run --rm test`, and `docker compose run --rm cust` pass; the runtime smoke test prints `10`.

The bounded C11 string-comparison slice is complete: an exact manually declared `int strcmp(const char *, const char *)` prototype activates a Cust-owned intrinsic, while user definitions win and missing/incompatible declarations remain explicit boundaries. Both pointer expressions evaluate once and left-to-right before runtime validation. Cust then validates interpreter-owned character storage and lexical owners/lifetimes, normalizes values to unsigned bytes before NUL detection, compares lexically with sign-only `-1`/`0`/`1` results, and independently caps each non-NUL scan at 4,096 bytes. Null, wrong-pointee, expired-owner, unterminated, and over-bound arguments have exact argument-specific diagnostics. `sizeof(strcmp(...))` remains non-evaluating while checking arity and pointer shape. No host C runtime is used.

TDD and review evidence: the focused behavior test first failed with `undefined function 'strcmp'`; incompatible declaration and unevaluated-`sizeof` tests each produced focused RED/GREEN fixes. Independent review then found raw `i64` NUL detection before unsigned-byte normalization and argument-1 validation before argument-2 evaluation; both received focused failing regressions and minimal fixes, and the final review returned `APPROVED`. The warning-free compiler-oracle fixture covers equality, lexical ordering, offset views, unsigned-byte ordering, one-time calls, and `sizeof` non-evaluation. Final `cargo fmt --check`, `cargo clippy -- -D warnings`, all 1,166 local tests (1,033 interpreter, 98 fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), `docker compose run --rm test`, and `docker compose run --rm cust` (output `10`) pass.

The bounded C11 integer-string conversion slice is complete: exact manually declared `atoi(const char *)`, `atol(const char *)`, and `atoll(const char *)` prototypes activate Cust-owned intrinsics while user definitions retain precedence and missing/incompatible declarations stay explicit boundaries. The calls evaluate one pointer expression once, read interpreter-owned scalar arrays/string literals/offset views, preserve lexical owner metadata through scalar and aggregate pointer slots, parse C whitespace plus an optional sign and decimal prefix, and require a NUL terminator within 4,096 bytes. Cust defines deterministic null, wrong-pointee, escaped-owner, unterminated, over-bound, and `i64` overflow diagnostics; no-digit inputs return zero. `sizeof(call)` remains non-evaluating while validating call arity and pointer shape, including C's literal `0` null-pointer constant.

TDD and review evidence: focused conversion behavior first failed with `undefined function 'atoi'`; incompatible pointee qualification, bounded scans, deterministic overflow, escaped pointer slots, aggregate-field ownership, and unevaluated `sizeof` call checks each had focused RED/GREEN coverage. Independent review found lost owner metadata in outer scalar and aggregate pointer slots plus missing unevaluated arity/type/null-constant checks; each finding received a focused regression and the final review returned `APPROVED`. Final `cargo fmt --check`, `cargo clippy -- -D warnings`, all 1,158 local tests (1,025 interpreter, 98 fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), `docker compose run --rm test`, and `docker compose run --rm cust` (output `10`) pass.

The first bounded v0.7 language milestone is complete: explicitly prototyped C11 `abs`, `labs`, and `llabs` calls now execute inside Cust without a host-runtime shortcut. Top-level function prototypes are retained as runtime metadata, exact deterministic-integer signatures activate the three intrinsics, user definitions still take precedence, and missing or incompatible prototypes remain explicit boundaries. Calls evaluate one scalar argument exactly once, return the nonnegative Cust integer value, preserve non-evaluating scalar `sizeof`, and convert Cust's otherwise-undefined minimum-integer case into the exact diagnostic `integer absolute value overflow in function '<name>'` rather than a Rust host panic.

TDD evidence: the warning-free valid fixture first failed with `undefined function 'abs'`; minimum-value coverage then reproduced a Rust `attempt to negate with overflow` panic; incompatible declarations first produced the generic undefined-function error. Focused GREEN covers six interpreter tests for all three names, one-time argument evaluation, zero/positive/negative values, `sizeof` non-evaluation, prototype requirements, incompatible signatures, arity/scalar-shape boundaries, and checked overflow. The canonical compiler-oracle harness passes the new fixture, and independent review reported no blocking/high/medium findings. Final `cargo fmt --check`, `cargo clippy -- -D warnings`, all 1,150 local tests (1,017 interpreter, 98 fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test), `docker compose run --rm test`, `docker compose run --rm cust` (output `10`), and `git diff --check` pass.

The annotated `v0.6.0` tag is published on `origin`; its peeled target is release commit `40f721ad0a88a545443eb991002369d89732912e`, matching the local release commit inspected at startup.

The Cust v0.6.0 package was prepared and published atomically. Cargo package/lock metadata, exact CLI output, both Docker Compose image tags, README support/limitations/roadmap text, changelog release notes, and all five status files were synchronized for `0.6.0`. The release packages C11 null directives, opened-identity `#pragma once`, and macro-expanded `_Pragma("once")`. Focused version assertions went RED on `0.5.0` and GREEN on `0.6.0`; the annotated `v0.6.0` tag is published and peels to release commit `40f721ad0a88a545443eb991002369d89732912e` on `origin`.

Historical release-preparation evidence included clean baseline `cargo test` (1,144 tests), focused CLI and Compose metadata RED/GREEN, `cargo check`, a release-surface audit against v0.5.0, and local/remote non-conflicting tag preflight; independent review, the canonical local/Docker gate, release-commit push, tag publication, and remote peeled-target verification subsequently completed.

Bounded C11 `_Pragma("once")` support is complete for file-aware entry points. Direct operators and object/function-like macro-produced operators execute after replacement/rescanning, including operators whose four preprocessing tokens arrive from separate macros. Ordinary and `L`/`u`/`U`/`u8` string spellings are destringized by removing the encoding prefix and unescaping `\\"` / `\\\\`; inactive groups remain inert. The result reuses `#pragma once` opened-file identity, secure include ordering, and recursive suppression. Exact malformed/non-string/unsupported diagnostics pass. Translation-unit payload bytes and per-payload tokens are bounded before unbounded work, pending large literals ignore whitespace/empty expansions in linear time, and batches of operators use a single deque pass. The warning-free native fixture, Windows GNU cross-target check, independent approval, canonical local gate, and both Docker gates pass with 1,144 tests (1,011 interpreter, 98 fuzz-safety, 31 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test).

Verified commands: clean-baseline `cargo test`; focused CLI and compiler-oracle RED/GREEN; review-driven split-token, cumulative-byte, large-whitespace, and empty-expansion RED/GREEN regressions; bounded-token and many-operator coverage; recursion-depth regression; strict native C11 compiler-oracle execution through `c_compat`; official N1570 §6.10.9 review; GCC/Clang probes; four independent Codex review passes ending in approval; Windows GNU cross-target check; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded `#pragma once` support is complete for Linux file-aware entry points. Active direct and `%:pragma once` directives mark the current opened source identity; repeated includes, symlink aliases, hard-link aliases, and recursive self-includes are skipped before depth/source-byte accounting, while inactive directives have no effect. The secure `openat2` containment/opened-handle checks still run before suppression. Exact missing-name, malformed-name, unsupported-pragma, and trailing-token diagnostics pass. Review-driven RED/GREEN replaced canonical-path cache keys with opened `(device, inode)` identities. A warning-free native project fixture returns 0, independent review approved the final diff, Windows GNU cross-target checks pass, and the canonical local/Docker gates pass with 1,139 tests (1,011 interpreter, 98 fuzz-safety, 26 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test).

Verified commands: clean-baseline `cargo test`; focused CLI/compiler-oracle RED on unsupported `#pragma`; focused direct/digraph/inactive/recursive/symlink GREEN; exact-diagnostic RED/GREEN; review-driven hard-link identity RED/GREEN; strict native C11 compiler-oracle execution; canonical `c_compat`; recursion-depth regression; official GCC CPP pragma documentation review; two independent Codex reviews ending in approval; Windows GNU cross-target check; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded C11 null preprocessing directives are complete. Active `#` and `%:` markers followed only by preprocessing whitespace/comments now consume their logical line with no effect; direct, digraph, line/block-comment, LF-spliced, active/inactive conditional, and included-header routes pass. Malformed non-null operands and mid-line placement retain exact diagnostics. A warning-free native C11 fixture returns 0, independent review found no blocking correctness/security issue, and the canonical local/Docker gates pass with 1,135 tests (1,011 interpreter, 98 fuzz-safety, 22 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). The already-published annotated `v0.5.0` tag was rechecked and peels to release commit `4a7ec79486b4c86ecdfea2524aa3e5c854ceba70` on `origin`.

Verified commands: clean-baseline `cargo test`; focused direct RED on `expected preprocessor directive name after '#'`; focused three-test GREEN; strict native `cc -std=c11 -Wall -Wextra -Werror` fixture execution; canonical `c_compat`; official WG14 N1570 §6.10.7 review; independent Codex review plus review-suggested malformed-after-comment coverage; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

The Cust v0.5.0 package was prepared and published atomically. Cargo package/lock metadata, exact CLI output, Docker Compose runtime/test image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized at `0.5.0`. The release packages direct-source C11 digraphs, macro-expanded quoted includes, dynamic `__FILE__`/`__LINE__`, bounded `#error`, and bounded `#line`. Focused version assertions went RED on `0.4.0` and GREEN on `0.5.0`; the release baseline had 1,132 tests (1,008 interpreter, 98 fuzz-safety, 22 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). The annotated `v0.5.0` tag is published and peels to the verified release commit on `origin`.

Verified commands before publication: clean-baseline `cargo test`; focused CLI and Compose metadata RED/GREEN; `cargo check`; `cargo run --quiet -- --version`; `cargo metadata --no-deps --format-version 1`; local/remote tag preflight; independent pre-commit review; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; `docker compose run --rm cust --version`; `git diff --check`; and remote peeled-target verification.
...
Verified commands: clean-baseline `cargo test`; focused direct RED/GREEN; macro-operand RED followed by corrected oracle and GREEN; physical-splicing RED/GREEN; exact operand diagnostics; review-driven 4 KiB source-name RED/GREEN hardening; focused include restoration; canonical `c_compat`; recursion-depth regression; independent Codex review with no correctness findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded active C11 `#error` diagnostics are complete. `#error` and `%:error` retain the raw, unexpanded preprocessing-token message while replacing comments and whitespace runs with one separator outside string/character literals. Empty messages, escaped literals, physical splicing, inactive conditional suppression, and nested included-header origins have exact coverage. A 1 MiB UTF-8 message cap rejects growth before formatting; its overflow path uses a 256-character context window so the diagnostic itself remains bounded even for multi-megabyte source lines. The canonical local/Docker gates pass with 1,125 tests (1,001 interpreter, 98 fuzz-safety, 22 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-bounded-error-directives.md`.

Verified commands: clean-baseline `cargo test`; direct active-message RED/GREEN; comment/whitespace normalization RED/GREEN; message-byte-limit RED/GREEN; review-driven bounded-diagnostic-output RED/GREEN; empty/digraph/spliced/inactive/literal/header-origin focused coverage; official N1570 §6.10.5 review; GCC/Clang strict-C11 probes; independent Codex review and focused re-review; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded C11 predefined `__FILE__` and `__LINE__` semantics are complete. Both names expand dynamically at the physical use-site line through ordinary expressions, object/function forwarding, argument prescan, two-level stringification, `#if`/`#elif`, `defined`, `#ifdef`, and `#ifndef`. String-only APIs use `<input>`; file-aware APIs use normalized project-relative logical names across primary and nested included sources. Valid preprocessing spelling escapes quotes, backslashes, and control characters while retaining the decoded logical filename value. Shared depth/work/token budgets, spelling/separation/source metadata, and exact reserved-name redefinition/undefinition diagnostics remain deterministic. The warning-free native fixture returns 0, and the canonical local/Docker gates pass with 1,121 tests (997 interpreter, 98 fuzz-safety, 22 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-predefined-file-line-macros.md`.

Verified commands: clean-baseline `cargo test`; focused direct-value RED/GREEN; focused condition/forwarding/stringification RED/GREEN; exact reserved-name diagnostic RED/GREEN; exact primary/header logical-name coverage; strict native C11 fixture; canonical `c_compat`; recursion-depth regression; independent Codex review that exposed and then approved fixes for the Clippy argument threshold and control-character preprocessing spelling; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded macro-expanded quoted-header operands are complete for Linux file entry points. Active `#include` and `%:include` operands may expand object-like, function-like, nested, and stringifying macros to exactly one ordinary unprefixed string-literal preprocessing token before reusing the existing secure logical-path/project-root resolver. Expansion shares translation-unit depth/token/work/generated-byte budgets; inactive malformed and recursive operands remain untouched. Exact diagnostics cover non-string, multi-token, empty, wide, system, malformed-invocation, unsafe-path, and nested-header routes. Direct no-whitespace include coverage remains, the warning-free native project fixture returns 0, and the canonical local/Docker gates pass with 1,115 tests (993 interpreter, 98 fuzz-safety, 20 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-macro-expanded-quoted-header-includes.md`.

Verified commands: clean-baseline `cargo test`; focused macro-expanded include RED/GREEN; exact success/diagnostic/shared-budget/nested-origin CLI tests; strict native C11 fixture; canonical `c_compat`; recursion-depth regression; GCC/Clang raw header-spelling probes; independent Codex review with no blocking/high findings and resolved medium/low coverage/docs findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Direct-source C11 digraph punctuator parity is complete. `<:`, `:>`, `<%`, and `%>` lower to semantic bracket/brace tokens while retaining exact source spelling; `%:` introduces every existing preprocessing directive and remains the stringification operator in macro replacements; `%:%:` remains the spelling-aware token-pasting operator. Physical-line splicing, active/inactive conditional directive recognition, same-line placement safety, source locations, stringification, and spelling-sensitive macro redefinition behavior remain intact. The warning-free native fixture returns 0, and the canonical local/Docker gates pass with 1,113 tests (993 interpreter, 98 fuzz-safety, 18 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test).

Verified commands: clean-baseline `cargo test`; strict native C11 digraph probe; focused direct-source digraph RED/GREEN; three exact interpreter regressions; canonical `c_compat`; official N1570 §6.4.6 review; independent Codex review ending in approval; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

The bounded Cust v0.4.0 release package is published. Cargo package/lock metadata, exact CLI output, Docker Compose runtime/test image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized at `0.4.0`. The release captures the completed bounded preprocessing roadmap: object/function/variadic macros, definition lifetime, conditionals, physical-line splicing, secure project-relative quoted headers, stringification, and token pasting. The annotated `v0.4.0` tag and its peeled target both resolve to verified release commit `6ffb364be549e4240fb33f2841e7f67020f56bda` on `origin`.

Verified commands: clean-baseline `cargo test`; focused release-version CLI RED/GREEN; focused Compose image-tag RED/GREEN; `cargo check`; `cargo run --quiet -- --version`; `cargo metadata --no-deps --format-version 1`; non-conflicting local/remote tag preflight; independent pre-commit review; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; `docker compose run --rm cust --version`; and `git diff --check`.

Bounded C11 macro token pasting is complete for function-like and object-like replacement lists. `##` and `%:%:` consume raw adjacent argument tokens, preserve multi-token argument boundaries, model empty arguments as placemarkers, reduce chained operators, validate each nonempty result as exactly one preprocessing token, and rescan the replacement. Identifier, keyword, pp-number, punctuator, variadic, `#if`, object-like, digraph, stringification-follow-up, and empty-chain routes pass. Generated `<:`, `:>`, `<%`, `%>`, `%:`, `##`, and `%:%:` punctuators retain their one-token identity and spelling through parsing or later stringification. Exact diagnostics cover edge/consecutive/invalid-result operators and a translation-unit-wide 1 MiB generated-paste byte bound before allocation; definition equality retains operator spelling and whitespace separation. The warning-free Cust/GCC/Clang fixture passes, and the canonical local/Docker gates pass with 1,110 tests (990 interpreter, 98 fuzz-safety, 18 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-macro-token-pasting.md`.

Verified commands: clean-baseline `cargo test`; focused raw/prescan/rescan token-pasting RED/GREEN; warning-free compiler-oracle `c_compat`; exact operator/result diagnostics; empty/variadic/digraph/object/stringification routes; native GCC/Clang redefinition and generated-punctuator probes; review-driven RED/GREEN fixes for lost paste-operator whitespace/spelling equality, generated-byte capacity, and valid multi-character punctuator identity; final independent Codex review with no blocking findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

Bounded C11 variadic function-like macros are complete. Definitions accept final `...`, fixed-plus-variable and variable-only parameter lists, and `__VA_ARGS__` substitution with balanced comma preservation, argument prescan, replacement/source rescanning, temporary self-disable, and shared nesting/token/work bounds. Prescan is lazy for unused named and variable arguments, while raw reserved tokens remain validated and substituted replacement vectors are bounded before allocation. Reserved-identifier, malformed ellipsis, arity, recursion, conflicting whitespace-sensitive redefinition, and generic resource diagnostics retain exact source context. Inactive nested conditional directives now process structural names while ignoring operands/trailing preprocessing tokens and preserving comment-aware lexical errors. The warning-free native C11 fixture passes, and the canonical local/Docker gates pass with 1,094 tests (974 interpreter, 98 fuzz-safety, 18 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-variadic-function-like-macro-preprocessing.md`.

Verified commands: resumed inherited-diff inspection; focused variadic, macro, malformed-conditional, arity, recursion-depth, pre-rescan token-amplification, and canonical `c_compat` tests; direct `cc`/GCC/Clang strict-C11 fixture execution; independent Codex reviews that exposed eager unused-argument prescan, inactive nested-conditional operand validation, empty-invocation arity reporting, reserved `__VA_ARGS__` bypass through an unused macro argument, and unbounded substituted-replacement allocation; targeted RED/GREEN fixes; final independent review with no blocking/high/medium findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output `10`); and `git diff --check`. The inherited variadic implementation's original RED evidence was unavailable and is not re-claimed.

Bounded project-relative quoted header inclusion is complete for Linux file entry points. The CLI and new `interpret_file`/`format_file_*` APIs share macro state across nested headers, search beside the logical including path before the project root, preserve normalized project-relative token/parser/runtime origins, permit guarded self-inclusion, and reject unguarded cycles. A 32-header depth bound and 1 MiB cumulative included-source bound constrain recursion and allocation. System headers, macro-expanded operands, missing/unsafe paths, non-regular files, symlink escapes/loops, and malformed directives have exact diagnostics. Linux `openat2(RESOLVE_BENEATH)` anchors file opening below the project root before nonblocking regular-file validation; opened dev/inode checks retain path/handle identity. Other platforms fail closed for quoted inclusion. The warning-free native C11 fixture passes, and the canonical local/Docker gates pass with 1,089 tests (969 interpreter, 98 fuzz-safety, 18 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-bounded-quoted-header-inclusion.md`.

Verified commands: resumed inherited-diff inspection; focused CLI include/path/location/cycle/depth/source-size RED/GREEN; canonical `c_compat`; recursion-depth regression; three independent Codex reviews that exposed semantic symlink lookup, FIFO hangs, opened-path TOCTOU, recursion/AST regressions, ambiguous origins, primary-file validation, and portability gaps; targeted RED/GREEN fixes; Windows GNU cross-check; final review with no blocking/high findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output `10`); and `git diff --check`. The inherited include implementation's original RED evidence was unavailable and is not re-claimed.

Bounded function-like macro preprocessing is complete. Definitions accept named parameters and balanced zero/empty/nested invocations; arguments are prescanned, substituted, and rescanned through object/function aliases, replacement/source call seams, ordinary C contexts, and `#if`/`#elif`. Temporary self-disable preserves native behavior for routes such as `F(F)(1)`, while direct/indirect replacement recursion remains deterministic. A separate 128-level expansion nesting guard plus shared translation-unit work/emitted-token quotas bound deeply nested arguments. Exact diagnostics cover duplicate/malformed parameters, arity, recursion, conflicting definitions, and unsupported variadic/stringification/token-pasting forms. `tests/fixtures/compat/valid/function_like_macros.c` passes Cust and the warning-free native C11 oracle. The canonical local and Docker gates pass with 1,078 tests (968 interpreter, 98 fuzz-safety, 8 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-function-like-macro-preprocessing.md`.

Verified commands: clean baseline `cargo test`; focused valid function-macro RED/GREEN; focused alias/following-call rescanning RED/GREEN; exact invalid diagnostics; canonical `c_compat`; independent Codex review that exposed a stale unsupported-feature regression, nested-argument stack growth, and temporary-self-disable semantics; targeted review-driven RED/GREEN fixes; final independent review with no blocking/high findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output `10`); and `git diff --check`.
...
Verified commands: resumed inherited-diff inspection; focused `#if`/`#elif`, macro, malformed-condition, unsigned-conversion, token/depth-bound, and compiler-oracle tests; independent Codex review; targeted RED/GREEN fixes for mixed-case doubled-long suffix rejection and internal preprocessing-number leakage; final independent review with no blocking findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`. The inherited feature's original RED evidence was unavailable and is not re-claimed.

Bounded nested `#ifdef` / `#ifndef` preprocessing is complete. Branches select from the current object-macro table, one `#else` is supported per group, inactive C tokens and non-structural directives are skipped without lexing or mutating macro state, and nested conditional directive names remain structurally tracked while their operands are ignored under inactive parents. A 128-frame stack bounds nesting; unmatched, duplicate, unterminated, active malformed/trailing, unsupported `#if`/`#elif`, and line-continuation paths retain exact source context. Comment-aware scanning preserves logical-line directive eligibility across chained multiline comments, while multiline object-macro replacement spelling/rescanning is source-correct and linear. `tests/fixtures/compat/valid/object_like_macro_conditionals.c` passes Cust and the warning-free native C11 compiler oracle. The canonical local and Docker gates pass with 1,061 tests (951 interpreter, 98 fuzz-safety, 8 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-conditional-preprocessing-state-machine.md`.

Verified commands: resumed-diff inspection; focused conditional, macro, and `#undef` regressions; targeted RED/GREEN fixes for mid-line structural recognition, inactive structural validation, multiline locations, replacement rescanning, and chained comments; a 20,000-token linearity probe; canonical `c_compat`; repeated independent Codex review ending with no blocking findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`. The inherited feature's original RED evidence was unavailable and is not re-claimed.

Bounded one-line `#undef` preprocessing is complete. A valid identifier removes its current object-like macro definition before subsequent source tokens are lexed; unknown names are harmless; aliases rescan against the current macro table; and a removed name may be redefined with a different replacement list. Directive whitespace accepts C horizontal whitespace plus line/block comments, including trailing comments. Missing and non-identifier names and trailing preprocessing tokens report exact source-line/caret diagnostics. `tests/fixtures/compat/valid/object_like_macro_undef.c` passes Cust and the warning-free native C11 compiler oracle. The canonical local and Docker gates pass with 1,053 tests (943 interpreter, 98 fuzz-safety, 8 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-object-like-macro-undef.md`.

Verified commands: clean-baseline `cargo test`; focused valid `#undef` RED/GREEN; focused malformed-name RED/GREEN; focused trailing-token RED/GREEN; focused `undef` regression; canonical `c_compat`; independent Codex review with no blocking findings; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

The first bounded v0.4 preprocessing slice now supports one-line object-like `#define` directives. The lexer stores replacement token kinds, expands nested macro names at subsequent identifier uses across declaration/type, integer-constant-expression, initializer, and runtime-expression contexts, and leaves comments plus string/character literal contents untouched. Direct/indirect recursion, conflicting replacement-token redefinitions, malformed names, unsupported replacement `#`, function-like macros, `#include`, physical-line continuations, excessive nesting, emitted-token growth, and empty-expansion work have exact source-context diagnostics. Translation-unit-wide depth/token/work limits keep expansion panic-free and bounded; line/block comments plus C horizontal whitespace are accepted around directives. `tests/fixtures/compat/valid/object_like_macros.c` passes Cust and a warning-free native C11 compiler oracle. The canonical local and Docker gates pass with 1,050 tests (940 interpreter, 98 fuzz-safety, 8 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). See `references/cust-object-like-macro-preprocessing.md`.

Previous autonomous verification: released and tagged Cust `v0.3.0` after completing the bounded two-dimensional scalar-array and safe pointer-to-row milestone. `Cargo.toml`/`Cargo.lock`, exact CLI version assertions, Docker image tags, README/changelog/release notes, compose metadata assertions, and status/backlog files are synchronized at `0.3.0`; `CHANGELOG.md` records 1,039 passing tests at release time (929 interpreter, 98 fuzz-safety, 8 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). The annotated `v0.3.0` tag was pushed to the same verified commit as `origin/main`. The next milestone remains untargeted (`v0.4` or later) while preprocessing and other explicit subset boundaries are scoped. See `references/cust-versioned-release-closure.md`.

All required verification passed for the 2026-07-27 v0.3.0 release package. Cargo package/lock metadata, exact local CLI output, Docker Compose runtime/test image tags, README support/limitations, and `CHANGELOG.md` release notes now agree on `0.3.0`. The release captures the completed fixed two-dimensional scalar-array and interpreter-owned pointer-to-row roadmap: objects, aggregate fields, typedefs, declaration lists, adjusted parameters, explicit row pointers, function returns, row arithmetic, and generic double-index expressions retain deterministic type/width/const/owner/lifetime/bounds behavior. The suite remains 1,039 tests: 929 interpreter, 98 deterministic fuzz-safety, and 12 CLI/Docker/compiler-oracle/repository tests.

Verified commands: clean-baseline `cargo test`; focused CLI release-version RED/GREEN; focused Docker image-tag RED/GREEN; `cargo run --quiet -- --version` (output `cust 0.3.0`); `cargo metadata --no-deps --format-version 1` (package `cust 0.3.0`); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output `10`); `docker compose run --rm cust --version` (output `cust 0.3.0`); and `git diff --check`.
...
Verified commands: resumed inherited-diff inspection; focused five-test pointer-to-row typedef sweep; direct native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-26 two-dimensional row-pointer return and generic-read run. Cust now parses explicit scalar `T (*function(params))[C]` prototypes and definitions, including unnamed row-pointer prototype parameters, and stores element type, column width, and pointee constness in function signatures. Returned interpreter-owned row views preserve backing-array identity, offsets, lexical-owner lifetime, const qualification, and width checks. Direct calls plus conditional/comma row-pointer expressions support C-style double-index reads by decaying the selected row to its first scalar element without host addresses. Exact regressions cover width mismatch, const discard, and escaped-local lifetime diagnostics; a warning-free compiler-oracle fixture returns 0. Pointer-to-row typedefs/declaration lists, generic row-expression lvalue updates, and non-variable 2D roots remain follow-up slices. The suite now has 915 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,024 total).

Verified commands: focused row-return RED/GREEN; unnamed-prototype RED/GREEN; focused generic call/conditional/comma double-index RED/GREEN; focused three-test row-return sweep; focused width/const/lifetime diagnostics; existing function-pointer and parenthesized-pointer diagnostic regressions; direct native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); canonical `c_compat`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-26 explicit two-dimensional row-pointer declaration/parameter run. Scalar `T (*row)[C]` objects now parse at global, file-static, block-static, and automatic scope, while named/unnamed explicit row-pointer parameters and compatible prototypes normalize to the same adjusted 2D signature metadata. Initializers retain interpreter-owned row identity, row-scaled arithmetic/difference, caller mutation, pointer-slot versus pointee const qualification, lexical-owner lifetime checks, exact column-width/deeper-rank/type/const diagnostics, and pointer-size `sizeof`. The run also fixed a RED-exposed const bug by separating a row pointer slot's constness from its pointee qualification during element writes. A warning-free compiler-oracle fixture returns 0. Generic double indexing over conditional/comma/call row-pointer expressions, explicit row-pointer function returns, pointer-to-row typedefs, declaration lists, and non-variable 2D roots remain follow-up slices. The suite now has 911 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,020 total).

Verified commands: clean-baseline `cargo test`; focused declaration RED/GREEN; focused parameter RED/GREEN; const-pointee write RED/GREEN; focused four-test explicit-row-pointer sweep; focused six-test row-pointer diagnostic sweep; direct native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-26 two-dimensional scalar-array row-pointer arithmetic/forwarding run. Two-dimensional array objects and adjusted `T matrix[][C]` parameters now decay to interpreter-owned row pointers that preserve backing-array identity, current row, column width, const qualification, lexical owner, and lifetime. Same-root row addition/subtraction, difference, equality, and ordering work; offset row views can be forwarded into adjusted parameters, mutate caller-owned rows, and retain exact row/column bounds and scalar-pointer incompatibility diagnostics. A warning-free compiler-oracle fixture returns 0. Explicit `T (*row)[C]` declarations, generic single-index row decay, returned row pointers, and non-variable 2D storage roots remain the next slice. The suite now has 907 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,016 total).

Verified commands: clean-baseline `cargo test`; focused offset-row-argument RED/GREEN; focused row-bounds diagnostic RED/GREEN; focused 23-test `two_dimensional_array` regression; focused row-pointer fixture; canonical `c_compat`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-26 adjusted fixed two-dimensional scalar-array parameter run. Direct `T matrix[R][C]` / `T matrix[][C]` and typedef-backed 2D parameters now adjust into interpreter-owned caller-array views across named/unnamed prototypes and definitions. Parameter signatures preserve scalar type and column width while intentionally ignoring the first bound; calls preserve caller storage identity, callee double-index reads and scalar lvalue updates, recursive helper forwarding, const-row qualification, and exact element/column-width/two-dimensional-shape/bounds diagnostics. A warning-free compiler-oracle fixture returns 0. The suite now has 904 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,013 total).

Verified commands: clean-baseline `cargo test`; focused direct-parameter RED/GREEN; focused const-row write RED/GREEN; focused const-discard RED/GREEN; focused 29-test `two_dimensional` sweep; updated rank-three diagnostics; direct native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-26 comma-separated fixed two-dimensional scalar-array declaration-list run. Direct `T first[R][C], second[R2][C2]` and typedef-backed `Matrix first, second` declarations now reuse ordinary same-scope declaration-list lowering at global, local, file-static, and block-static scope. Each declarator retains independent dimensions, nested initialization/zero fill, const metadata, exact bounds, and full-object/element `sizeof`; aggregate field declaration lists use the same completed 2D field metadata. Focused tests prove direct and alias parsing, one-time source-order initialization, exact pointer-to-row/rank-three diagnostics, const and bounds behavior, and a warning-free compiler-oracle fixture returning 0. The suite now has 898 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,007 total).

Verified commands: focused direct-declaration RED/GREEN; focused typedef-declaration RED/GREEN; focused five-test `two_dimensional_array_declaration_lists` coverage; canonical `c_compat`; native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-26 fixed two-dimensional scalar-array typedef run. `typedef int Matrix[R][C]` and `typedef char Labels[R][C]` now preserve both dimensions and the scalar element type through local/global/file-static/block-static object declarations, named aggregate fields, nested initialization/zero fill, double-index reads and scalar lvalue updates, const alias metadata, and deterministic full-object/element `sizeof`/`_Alignof`. Exact boundaries reject third dimensions, object suffixes beyond rank two, pointer-to-array typedef aliases at the `*`, and multidimensional parameters. A warning-free native compiler-oracle fixture returns 0 under Cust and `cc`. The suite now has 893 interpreter tests, 97 fuzz-safety tests, and 12 CLI/Docker/compiler-oracle/license tests (1,002 total).

Verified commands: focused five-test `two_dimensional_array_typedef` RED/GREEN coverage; focused array-typedef regression coverage; canonical `c_compat`; native `cc -std=c11 -Wall -Wextra -Werror` fixture execution (0); recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.
...
Verified commands: clean-baseline `cargo test`; focused read RED/GREEN; focused static/update/`sizeof` RED/GREEN; focused third-dimension diagnostic RED/corrected GREEN; focused const/bounds coverage; focused scalar-pointer-decay RED/GREEN; focused row-without-second-index RED/GREEN; focused fixture; canonical `cargo test --test c_compat -- --nocapture`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-25 v0.2.0 release-metadata run. `Cargo.toml`/`Cargo.lock`, Docker Compose runtime/test image tags, exact CLI output, README current-subset/limitation claims, and `CHANGELOG.md` release notes now agree on `0.2.0`. The release notes summarize the aggregate, pointer, declaration, expression, diagnostic, and deterministic robustness work completed since v0.1. A new Compose metadata regression joins the exact CLI release-version assertion, raising the suite to 984 tests: 875 interpreter, 97 fuzz-safety, and 12 CLI/Docker/compiler-oracle/license tests. Local and rebuilt Docker binaries both print `cust 0.2.0`.

Verified commands: clean-baseline `cargo test`; focused CLI release-version RED/GREEN; focused Docker image-tag RED/GREEN; `cargo run --quiet -- --version`; `cargo metadata --no-deps --format-version 1`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); `docker compose run --rm cust --version` (output `cust 0.2.0`); and `git diff --check`.

All required verification passed for the 2026-07-25 bounded six-token adjacent-string conformance run. The former nested three/four/five-token renderer chain is replaced by one slice-based renderer that independently emits every literal token, trivia boundary, token-plus-EOF location, expected concatenated value, and runtime checksum. Four valid rotations and five multiple-malformed masks cross ten balanced five-boundary trivia compositions and all three pointer/inferred-array/fixed-array routes for 420 unique programs. The 120 valid programs preserve exact per-token values/NULs, concatenated AST bytes with exactly one final NUL, and weighted runtime order; the 300 invalid programs make each position 0 through 4 the first malformed token and preserve its exact opening-quote source context. All rotation/family/position/mask/trivia-boundary/route counters pass, as do all eight numeric-escape fuzz regressions. The suite now has 97 fuzz-safety tests and 875 interpreter tests (983 total).

Verified commands: resumed focused six-token GREEN after repairing the interrupted test harness; focused eight-test numeric-escape regression; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-25 deterministic five-token multiple-malformed numeric-escape precedence run. Four pair masks and three triple masks make positions 0, 1, 2, and 3 each the first malformed literal; two rotations balance `\x` and `\8`; ten cyclic four-boundary trivia compositions balance spaces, multibyte block comments, LF, CRLF, and terminated multibyte line comments at every boundary; and all three pointer/inferred-array/fixed-array routes run. All 420 sources are unique. Both public token and AST APIs remain panic-free and report the independently modeled first malformed literal with its exact opening-quote line/column and source-line/caret context. Exact composition, mask, rotation, malformed-family, first-error-family, malformed-position, first-error-position, arity, trivia-composition, per-boundary trivia-family, and route counters pass. Existing lexer behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 96 fuzz-safety tests and 875 interpreter tests (982 total).

Verified commands: focused five-token coverage (immediate GREEN); focused seven-test numeric-escape regression; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-25 deterministic four-token multiple-malformed numeric-escape precedence run. Five selected pair/triple position masks make positions 0, 1, and 2 each the first malformed literal; two rotations balance `\x` and `\8`; ten cyclic three-boundary trivia compositions balance spaces, multibyte block comments, LF, CRLF, and terminated multibyte line comments at every boundary; and all three pointer/inferred-array/fixed-array routes run. All 300 sources are unique. Both public token and AST APIs remain panic-free and report the independently modeled first malformed literal with its exact opening-quote line/column and source-line/caret context. Exact composition, mask, rotation, malformed-family, first-error-family, malformed-position, first-error-position, arity, trivia-composition, per-boundary trivia-family, and route counters pass. Existing lexer behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 95 fuzz-safety tests and 875 interpreter tests (981 total).

Verified commands: clean-baseline `cargo test`; focused four-token coverage (immediate GREEN); focused three-test adjacent-string regression; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-25 deterministic multiple-malformed-token numeric-escape precedence run. Three two-token position masks and one all-three-token mask rotate `\x` and `\8` across active positions, then cross all 25 independently composed space/comment/LF/CRLF/terminated-line-comment trivia pairs and three pointer/inferred-array/fixed-array routes. All 600 sources are unique. Both public token and AST APIs remain panic-free and report the first malformed literal in source order with its exact opening-quote line/column and source-line/caret context. Exact eight-composition, mask, family-rotation, malformed-family, malformed-position, first-error-position, arity, trivia-pair, and route counters pass. Existing lexer behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 94 fuzz-safety tests and 875 interpreter tests (980 total).

Verified commands: clean-baseline `cargo test`; focused multiple-malformed-token coverage (immediate GREEN); focused three-test adjacent-string regression; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-25 deterministic simultaneous multi-token numeric-escape concatenation run. Three two-token position masks and one all-three-token mask each rotate across all four bounded octal/hex families, then independently cross both boundaries over five space/comment/LF/CRLF/terminated-line-comment forms and three pointer/inferred-array/fixed-array routes. All 1,200 sources are unique: 900 pair routes and 300 triple routes preserve exact independently modeled per-token values and trailing NULs, token-plus-EOF locations, one concatenated AST byte vector with intermediate NULs removed, weighted runtime byte order, and one final NUL. Exact 16-composition, position-mask, family-rotation, escape-family, token-position, arity, trivia-pair, and route counters pass. Existing lexer/parser/runtime behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 93 fuzz-safety tests and 875 interpreter tests (979 total).

Verified commands: clean-baseline `cargo test`; focused simultaneous-composition coverage (immediate GREEN); focused three-test adjacent-string regression; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.
...
Verified commands: clean-baseline `cargo test`; focused adjacent-string RED and corrected GREEN; focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic numeric-escape/literal-fragment robustness run. Eleven valid string families cover one/two/three-digit octal runs, one/multi-digit hexadecimal runs, ordinary terminators, `/` and `*` delimiter adjacency, and multibyte terminators; five valid character families cover bounded octal/hex runs; and eight malformed families cover missing hex digits plus unsupported numeric, letter, uppercase-`X`, and multibyte escapes in both string and character literals. Crossing LF/CRLF and four expression/initializer before/after routes produces 256 unique sources: all 128 valid routes preserve exact independently modeled decoded token values, every token-plus-EOF character location, parsed ASTs, and interpreted values; all 128 malformed routes preserve exact diagnostics at the opening quote with source-line/caret context from both token and AST APIs. Exact literal/family/line-ending/route counters pass. Existing lexer/parser/runtime behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 90 fuzz-safety tests and 875 interpreter tests (976 total).

Verified commands: clean-baseline `cargo test`; focused numeric-escape coverage (immediate GREEN); focused Clippy; `cargo fmt --check`; canonical verification gate; and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic composed literal-fragment-run robustness run. All 25 ordered pairs and five balanced cyclic triples of `/`, `*`, escaped-quote, escaped-backslash, and multibyte atoms cross LF/CRLF plus four expression/initializer before/after routes for 240 valid string programs. Five valid and four malformed character forms add 72 routes, and interrupted LF/CRLF strings add eight malformed routes, for 320 unique sources total. Public token/AST APIs remain panic-free; all 280 valid routes preserve exact decoded token values, every token-plus-EOF character location, parsed ASTs, and interpreted values; all 40 malformed routes preserve the exact opening-quote line/column, source line, and caret; and exact composition/atom/character/line-ending/route counters pass. Existing lexer/parser/runtime behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 89 fuzz-safety tests and 875 interpreter tests (975 total).

Verified commands: clean-baseline `cargo test`; focused composed-literal coverage (immediate GREEN); focused Clippy; `cargo fmt --check`; canonical verification gate; and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic comment-delimiter/literal-boundary robustness run. String and character literals cross ten bounded `/`, `*`, `//`, `/*`, `*/`, escaped-quote, escaped-backslash, multibyte, unterminated, and embedded-line-ending mutations; LF/CRLF; and four expression/initializer before/after boundaries with adjacent multibyte block-comment trivia. All 160 sources are unique. Public `format_tokens` and `format_ast` calls remain panic-free; 104 valid routes preserve exact independently modeled token kinds and every token-plus-EOF character location; 56 malformed routes preserve the exact opening-quote line/column, source line, and caret; and exact literal/mutation/line-ending/boundary/source counters pass. Existing lexer/parser behavior matched immediately after correcting a test-helper `matches!` syntax error, so this is deliberate robustness coverage without production changes. The suite now has 88 fuzz-safety tests and 875 interpreter tests (974 total).

Verified commands: clean-baseline `cargo test`; focused literal-boundary coverage; focused Clippy; `cargo fmt --check`; canonical verification gate; and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic malformed composed-comment-run robustness run. Eight bounded mutation families (`/`, `*`, line-comment open, unterminated block-comment open, block-comment close, `/=`, `*=`, and a closed multibyte block comment) cross LF/CRLF at six representative start/interior/end boundaries over four declaration, expression, initializer, and control-flow bases. All 384 sources are unique. Public `format_tokens` and `format_ast` calls remain panic-free; all 48 unterminated block-comment routes preserve the exact opening line/character column, source line, and caret; every other parser error retains a bounded 1-based location; and exact base/mutation/line-ending/boundary/source counters pass. Existing behavior matched immediately, so this is deliberate robustness coverage without production changes. The suite now has 87 fuzz-safety tests and 875 interpreter tests (973 total).

Verified commands: clean-baseline `cargo test`; focused malformed-comment coverage (immediate GREEN); focused Clippy; `cargo fmt --check`; canonical verification gate; and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic composed-trivia-run conformance run. Every ordered pair of eight trivia atoms plus eight selected triples covers spaces, tabs, LF, CRLF, adjacent multibyte block comments, multiline multibyte block comments, and LF/CRLF-terminated line comments. The 72 compositions run at six representative start/interior/end boundaries over each of four declaration, expression, initializer, and control-flow bases for 1,728 isolated programs; eight seeded whole-program layouts add 32 programs and 888 boundary assignments, with every composition appearing 10-13 times. All 1,760 sources are unique. Public `format_tokens`, `format_ast`, and `interpret` calls remain panic-free; token kinds, ASTs, and results match baselines; exact independently modeled character-based locations include every token plus EOF; and exact base/composition/pair-triple/boundary/layout/source counters pass. Existing lexer/parser/runtime behavior matched immediately, so this is deliberate conformance coverage without production changes. The suite now has 86 fuzz-safety tests and 875 interpreter tests (972 total).

Verified commands: clean-baseline `cargo test`; focused composed-trivia coverage (immediate GREEN); focused Clippy; `cargo fmt --check`; canonical verification gate; and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic whole-program mixed-trivia layout conformance run. Four verified declaration, expression, initializer, and control-flow bases each run under five fixed cyclic layouts that place spaces, tabs, newlines, multibyte block comments, and newline-terminated multibyte line comments across every start/interior/end token boundary simultaneously. All 20 unique programs contain every trivia form; 555 boundary assignments are balanced at 111 per trivia form, and exact 20/515/20 start/interior/end counters pass. Public `format_tokens`, `format_ast`, and `interpret` calls remain panic-free; token kinds, ASTs, and results match each baseline, and every token plus EOF has the exact independently modeled character-based line/column. The first focused RED exposed only that the test oracle omitted Cust's formatted EOF token location; after adding that expected location, existing lexer/parser/runtime behavior passed without production changes. The suite now has 85 fuzz-safety tests and 875 interpreter tests (971 total).

Verified commands: clean-baseline `cargo test`; focused mixed-trivia RED and corrected GREEN; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.

All required verification passed for the 2026-07-24 deterministic token-preserving trivia-splice conformance run. Four verified declaration, expression, initializer, and control-flow bases contribute 111 start/interior/end token boundaries; spaces, tabs, newlines, block comments, and newline-terminated line comments produce 555 unique valid programs. Public `format_tokens`, `format_ast`, and `interpret` calls remain panic-free; token kinds and ASTs match each baseline, lexer locations remain bounded and 1-based, results remain identical, and exact base/trivia/boundary-class/source counters pass. Existing lexer/parser/runtime behavior matched immediately, so this is deliberate conformance coverage without production changes. The suite now has 84 fuzz-safety tests and 875 interpreter tests (970 total).

Verified commands: clean-baseline `cargo test`; focused trivia-splice coverage (immediate GREEN); focused Clippy; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (output 10); and `git diff --check`.
...
All required verification passed for the 2026-07-23 C `_Bool` conversion/storage-normalization run. Every supported conversion boundary now stores exactly `0` for scalar zero/null pointers and `1` for every nonzero scalar/non-null pointer: automatic/global/static declarations, plain and expression assignments, scalar parameters/returns, casts and compound literals, scalar arrays and aggregate scalar/array fields, direct and pointer-mediated writes, compound assignments, and prefix/postfix updates. Destination metadata is resolved before evaluating pointer-valued right-hand sides, preserving one-time evaluation and existing shape/type/const diagnostics; low-level scalar/field/array writers defensively normalize stored values. `sizeof(_Bool)` remains 1. Five focused interpreter regressions and a warning-free compiler-oracle fixture return 27 under Cust, GCC, and Clang. The suite now has 78 fuzz-safety tests and 874 interpreter tests.

Verified commands: resumed-diff inspection and `git diff --check`; focused `bool_conversion`, `normalizes_bool`, cast/compound-literal, and parameter/return tests; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (example output 10).

All required verification passed for the 2026-07-23 C99 predefined function-name run. Every function call now exposes `__func__` as a read-only NUL-terminated `char[]` backed by one static interpreter-owned array per function. Indexing, array decay, pointer returns, and non-evaluating `sizeof(__func__)` reuse existing array semantics; recursive calls share the same static identity, nested calls resolve their own lexical names, direct writes report the read-only-array diagnostic, and file-scope expression use reports an exact source-located diagnostic. The warning-free fixture returns 73 under Cust, GCC, and Clang. RED first established three missing-feature failures; a strengthened recursive pointer-return check then exposed and fixed incorrect lexical-owner attachment to static `__func__` storage. The suite now has 78 fuzz-safety tests and 869 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused three-test RED/GREEN `predefined_func_name` run; direct warning-free GCC/Clang fixture execution (73); canonical `c_compat`; recursion-depth regression; official N1570 §6.4.2.2/§6.3.1.2 research; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-23 freshly composed distinct-root result reselection run. Nine thousand two hundred sixteen generated scalar/named-aggregate programs pass the fresh wrapped/offset/helper/returned pointer and its preserved rejected input through one more copied-parameter selector. The matrix balances both pointee kinds, all four primary direct/captured root families, both fresh source roots, every preceding wrapper/offset/placement/helper/return route, final argument order, final selection outcome, and one/two-hop selector depth. The selected result preserves either the fresh selected-root-relative offset or the rejected root unchanged; all fresh/rejected/final and inherited slots, 17 exact bounds/const/cross-root/type/lifetime checks, and balanced counters pass. The warning-free direct/captured fixture returns 63 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this is deliberate conformance/property closure without production changes. The suite now has 78 fuzz-safety tests and 866 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 9,216-case generated matrix plus 17 exact diagnostic/lifetime checks; focused interpreter fixture (63); direct warning-free GCC/Clang fixture execution (63); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-23 final-reselected distinct-root composition run. One thousand one hundred fifty-two generated scalar/named-aggregate programs apply every fresh conditional/comma wrapper and nonzero same-root offset before or after one/two-hop const-preserving helpers, then cross one/two return boundaries. The matrix balances both final reselection outcomes, both pointee kinds, all four primary direct/captured root families, wrapper/offset/placement/helper/return routes, preserved earlier slots, selected-root owner/path/base/index/type/qualification/lifetime behavior, and 17 exact bounds/const/cross-root/type/lifetime checks. The expanded warning-free direct/captured fixture returns 51 under Cust, GCC, and Clang. RED exposed two test-oracle mistakes (the added assertion count and the alternate-root-relative final delta); existing runtime behavior then matched without production changes. The suite now has 77 fuzz-safety tests and 866 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused RED/GREEN 1,152-case generated matrix plus 17 exact diagnostic/lifetime checks; focused interpreter fixture (51); direct warning-free GCC/Clang fixture execution (51); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-23 returned distinct-root result reselection run. Thirty-six thousand eight hundred sixty-four generated scalar/named-aggregate programs re-enter the post-composition returned pointer and preserved unselected root through a final copied-parameter selector. The matrix balances both pointee kinds, all four primary direct/captured root families, both carried roots, every prior wrapper/offset/helper/return route, final argument order, final selection outcome, and one/two-hop selector depth. The final result either preserves the composed selected-root offset or restores the untouched alternate root; both input slots, owner/path/base/index/type/qualification/lifetime behavior, balanced counters, and 17 exact bounds/const/cross-root/type/lifetime checks pass. A warning-free direct/captured fixture returns 39 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this is deliberate conformance/property closure. The suite now has 76 fuzz-safety tests and 866 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 36,864-case generated matrix plus 17 exact diagnostic/lifetime checks; focused interpreter fixture (39); direct warning-free GCC/Clang fixture execution (39); canonical `c_compat`; preceding 4,608-case distinct-root carry regression; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-23 distinct-root selected-result carry run. Four thousand six hundred eight generated scalar/named-aggregate programs compose the selected distinct-root pointer with all conditional/comma wrappers and nonzero offset spellings before or after one/two-hop const-preserving helpers, then cross one/two return boundaries. Both selected roots, all four primary direct/captured root families, both pointee kinds, both original root slots, owner/path/base/index/type/qualification/lifetime behavior, balanced dimensions, and 17 exact bounds/const/cross-root/type/lifetime checks pass. A warning-free direct/captured fixture returns 31 under Cust, GCC, and Clang. Existing runtime behavior matched after fixing one missing test-generator marker declaration, so this is deliberate conformance/property closure. The suite now has 75 fuzz-safety tests and 865 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated RED (`undefined variable 'carry_selected'`) followed by 4,608-case GREEN after fixing the test generator; 17 exact diagnostic/lifetime checks; focused interpreter fixture (31); direct warning-free GCC/Clang fixture execution (31); canonical `c_compat`; preceding distinct-root reselection regression; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.
...
Verified commands: clean-baseline `cargo test`; focused 64-case generated test and nine exact diagnostics; focused interpreter fixture (48); direct warning-free GCC/Clang fixture execution (48); preceding 18,432-case post-selection re-forward/return regression; canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-22 post-selection re-forward/final-return conformance run. Eighteen thousand four hundred thirty-two generated programs carry the nested-selected and wrapped/offset-composed scalar/named-aggregate inner `const` pointer through one/two-hop const-preserving helpers and one/two final return boundaries across all four direct/captured roots. Selected/composed/re-forwarded/returned owner/path/base/index/type/qualification/lifetime identity, original slots, exact selector/wrapper/helper/return counts, balanced dimensions, seven exact bounds/cross-root/type/lifetime diagnostics, and two const-write diagnostics pass. A warning-free direct-scalar/captured-aggregate fixture returns 38 under Cust, GCC, and Clang. The RED failures were test-generator insertion and bounds-oracle mistakes; existing runtime behavior then passed without production changes. The suite now has 72 fuzz-safety tests and 862 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 18,432-case generated test and nine exact diagnostics; focused interpreter fixture (38); direct warning-free GCC/Clang fixture execution (38); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-22 post-selection wrapper/offset conformance run. Four thousand six hundred eight generated programs compose the ultimate nested-selected scalar/named-aggregate inner `const` pointer with conditional/comma wrappers before or after a nonzero same-array offset across all four direct/captured roots, both selector stages, three offset spellings, and both placements. Selected/composed owner/path/base/index/type/qualification/lifetime identity, original final/base/first-stage slots, exact selector counts, one-time wrapper markers, balanced route counters, seven exact bounds/cross-root/type/lifetime diagnostics, and two const-write diagnostics pass. A warning-free direct-scalar/captured-aggregate fixture returns 40 under Cust, GCC, and Clang. The generated matrix passed immediately; the fixture's initial expected score of 41 was corrected after all three implementations returned 40. The suite now has 71 fuzz-safety tests and 861 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 4,608-case generated test; focused interpreter fixture (40); direct warning-free GCC/Clang fixture execution (40); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed for the 2026-07-22 nested const-parameter return-selection conformance run. Two hundred fifty-six generated programs feed the first selected scalar/named-aggregate inner `const` pointer plus its alternate same-array input through a second one/two-hop selector across both argument orders, both selections, both helper depths, and all four direct/captured roots. Ultimate/intermediate reads, equality/difference/ordering, original final/base slots, owner/path/index/type/qualification/lifetime identity, balanced route counters, and seven exact nested const-write/cross-root/type/lifetime diagnostics pass. A warning-free direct-scalar/captured-aggregate fixture returns 30 under Cust, GCC, and Clang. The initial generated RED was a test-side case-count miscalculation (512 versus 256), not a runtime mismatch; existing behavior then passed. The suite now has 70 fuzz-safety tests and 860 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 256-case generated test; focused interpreter fixture (30); direct warning-free GCC/Clang fixture execution (30); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-22 const-parameter return-selection conformance run. Thirty-two generated programs select and return either a final-received derived inner `const` pointer or its pre-final same-array base through one/two-hop helpers across scalar/named-aggregate pointees and all four direct/captured roots. Selected reads/equality/difference/ordering, original caller slots, owner/path/index/type/qualification/lifetime identity, helper counts, and seven exact const-write/cross-root/type/lifetime diagnostics remain stable. A warning-free direct-scalar/captured-aggregate fixture returns 24 under Cust, GCC, and Clang. Existing runtime behavior matched immediately; this was conformance/property closure. The suite now has 69 fuzz-safety tests and 859 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 32-case generated test; focused interpreter fixture (24); direct warning-free GCC/Clang fixture execution (24); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-22 copied const-parameter re-entry conformance run. Sixteen generated programs pass final-received derived inner `const` pointers and their pre-final bases through one/two-hop helpers across both pointee kinds and all four direct/captured root families. Same-root reads/equality/difference/ordering, caller-versus-callee slot locality, owner/path/index/type/qualification/lifetime identity, and seven exact const-write/cross-root/type/lifetime diagnostics remain stable. A warning-free direct/captured fixture returns 33 under Cust, GCC, and Clang. Existing runtime behavior matched immediately; this was conformance/property closure. The suite now has 68 fuzz-safety tests and 858 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 16-case generated test; focused interpreter fixture (33); direct warning-free GCC/Clang fixture execution (33); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-22 final receiving-caller re-forward conformance run. Two thousand five hundred ninety-two balanced generated programs carry outer-caller-returned derived inner `const` pointers through another one/two-hop const-preserving helper placed before/after a final conditional/comma wrapper or nonzero offset. Both pointee kinds, all four direct/captured roots, owner/path/base/index/type/qualification/lifetime identity, separate marker sets, copied slots, and eleven exact const/bounds/cross-root/type/lifetime diagnostics remain stable. A warning-free direct/captured fixture returns 30 under Cust, GCC, and Clang. Existing runtime behavior matched immediately; this was conformance/property closure. The suite now has 67 fuzz-safety tests and 857 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 2,592-case generated test; focused interpreter fixture (30); direct warning-free GCC/Clang fixture execution (30); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-22 outer-caller return-boundary conformance run. Two thousand five hundred ninety-two balanced generated programs carry caller-re-forwarded derived inner `const` pointers across one/two additional outer return boundaries while crossing scalar and named-aggregate pointees; direct aggregate-array compound literals plus captured named, anonymous, and union-containing field roots; callee promotion/re-forward placement; one/two initial return boundaries; and every caller helper-depth, wrapper, and nonzero-offset route. Values at promoted/base/final indexes, hidden or containing root/path/base/index identity, concrete pointee type, qualification, lifetime provenance, separate marker sets, and copied slots remain stable. Eleven exact const/bounds/cross-root/type/lifetime regressions pass across the new boundary. A warning-free direct/captured fixture returns 20 under Cust, GCC, and Clang. Existing runtime behavior matched immediately; this was conformance/property closure. The suite now has 66 fuzz-safety tests and 856 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 2,592-case generated test; focused interpreter fixture (20); direct warning-free GCC/Clang fixture execution (20); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-22 caller-side derived inner-pointer const re-forward/return conformance run. Two thousand five hundred ninety-two balanced generated programs cross scalar and named-aggregate inner pointees; direct aggregate-array compound literals plus captured named, anonymous, and union-containing field roots; every callee promotion and caller re-forward placement; balanced callee promotion/re-forward wrappers, offsets, and one/two-hop helpers; one/two callee return boundaries; and every caller conditional/comma wrapper, nonzero offset spelling, and one/two-hop const-preserving helper. Promoted/base/final values, hidden or containing root/path/nonzero-outer-base/inner-index identity, concrete pointee type, const qualification, separate callee/caller/helper/root markers, helper call counts, and copied caller slots remain stable. Eleven caller-helper exact const/bounds/cross-root/type/lifetime regressions remain green. A warning-free direct/captured scalar/aggregate fixture returns 28 under Cust, GCC, and Clang. The initial generated RED exposed only an imbalanced base-3 route counter (`[891, 891, 810]`); deriving that route from balanced loop indexes corrected the generator, and all runtime routes then passed without production changes. The suite now has 65 fuzz-safety tests and 855 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated RED then 2,592-case GREEN; all 65 `fuzz_safety` tests; focused interpreter fixture (28); direct GCC/Clang fixture execution (28); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-22 callee-internal derived inner-pointer const re-forward/return conformance run. Two thousand five hundred ninety-two generated programs balance scalar and named-aggregate inner pointees; direct aggregate-array compound literals plus captured named, anonymous, and union-containing field roots; every initial promotion placement; one/two-hop promotion and const re-forward helpers; every second re-forward placement before a wrapper, after that wrapper, or after its nonzero offset; first/second conditional/comma wrappers and offset spellings; one/two return boundaries; and every caller wrapper/offset. Promoted/base/returned values, hidden or containing root/path/nonzero-outer-base/inner-index identity, concrete pointee type, const qualification, separate stage/caller/root markers, and copied pointer slots remain stable. The adjacent eleven exact const/bounds/cross-root/type/lifetime regressions remain green. A warning-free direct/captured scalar/aggregate fixture returns 24 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this is deliberate conformance/property closure. The suite now has 64 fuzz-safety tests and 854 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 2,592-case generated matrix (immediate GREEN conformance closure); focused interpreter fixture (24); direct GCC/Clang fixture execution (24); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-21 callee-internal derived inner-pointer promotion/return conformance run. Three thousand eight hundred eighty-eight generated programs exhaustively cross scalar and named-aggregate inner pointees; direct aggregate-array compound literals plus captured named, anonymous, and union-containing field roots; promotion before the callee wrapper, after that wrapper, or after the callee's nonzero inner offset; every callee conditional/comma wrapper and offset spelling; one/two return boundaries; and every caller wrapper/offset. Hidden or containing root/path/nonzero-outer-base/inner-index identity, concrete pointee type, const qualification, callee/caller/root marker evaluation, values, and copied pointer slots remain stable. The adjacent eleven exact const/bounds/cross-root/type/lifetime regressions remain green. A warning-free direct/captured scalar/aggregate fixture returns 20 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this is deliberate conformance/property closure. The suite now has 63 fuzz-safety tests and 853 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused 3,888-case generated matrix (immediate GREEN conformance closure); focused interpreter fixture (20); canonical `c_compat`; direct GCC/Clang fixture execution (20); recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-21 derived inner const-pointer callee-return boundary run. One hundred forty-four generated valid programs cross scalar and named-aggregate inner pointees; direct aggregate-array compound literals plus captured named, anonymous, and union-containing field roots; all caller conditional/comma wrappers; all caller `pointer + 1`, `1 + pointer`, and `&pointer[1]` offsets; and one/two-hop adjusted-parameter pointer-return callees. Six identity/read assertions per route preserve hidden or containing root/path, nonzero outer base, inner index, concrete pointee type, const qualification, caller marker evaluation, and local pointer-slot copies. Eleven panic-guarded invalid programs retain exact const-discard/write, inner-bounds, cross-root, concrete-type, and out-of-scope lifetime diagnostics. RED exposed a real lifetime bug: scalar embedded-array pointers were represented only by retained `Rc` storage, so a pointer returned through an adjusted parameter could outlive a local containing aggregate. Returned scalar-array pointers now attach their owning scope/root, propagate it through arithmetic/indexing, and reject dereference/index/type use after the owner scope exits. The suite now has 62 fuzz-safety tests and 852 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated-model RED (`pointer to out-of-scope variable 'local'` expected but value returned) followed by GREEN after owner tracking; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-21 captured-field derived inner-pointer post-promotion const re-forwarding run. Seventeen thousand four hundred ninety-six generated cases exhaustively cross scalar and named-aggregate inner pointees; named, anonymous, and union-containing captured paths; all three initial promotion placements; first and second conditional/comma wrappers; first and second `pointer + 1`, `1 + pointer`, and `&pointer[1]` offsets; one/two-hop promotion helpers; all three const re-forwarding placements; and one/two-hop const-preserving helpers. Containing hidden-root/recursive-path/nonzero-outer-base/inner-index identity, six captured roots, separate first/second wrapper markers, adjusted/local copied slots, legal const reads, and local const-pointer-slot reassignment remain stable. One hundred eight panic-guarded mutable-rebinding/write checks preserve exact const diagnostics. The warning-free named/anonymous/union fixture covers both pointee kinds and all promotion/re-forward stages and returns 58 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this was deliberate conformance/property closure. The suite now has 61 fuzz-safety tests and 852 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated model and exact diagnostics (immediate GREEN conformance closure); focused interpreter fixture coverage (immediate GREEN); direct Cust/GCC/Clang fixture execution (58); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-21 direct-literal derived inner-pointer post-promotion const re-forwarding run. Five thousand eight hundred thirty-two generated cases exhaustively cross scalar and named-aggregate inner pointees; all three initial promotion placements; first and second conditional/comma wrappers; first and second `pointer + 1`, `1 + pointer`, and `&pointer[1]` offsets; one/two-hop promotion helpers; all three const re-forwarding placements; and one/two-hop const-preserving helpers. Hidden-root/nonzero-outer-base/inner-index identity, separate initializer/first-wrapper/second-wrapper markers, adjusted/local copied slots, legal const reads, and local const-pointer-slot reassignment remain stable. Thirty-six panic-guarded mutable-rebinding/write checks preserve exact const diagnostics. The warning-free direct-literal fixture covers both pointee kinds and all three promotion/re-forward placements and returns 39 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this was deliberate conformance/property closure. The suite now has 60 fuzz-safety tests and 851 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated model and exact diagnostics (immediate GREEN conformance closure); focused interpreter fixture coverage; direct Cust/GCC/Clang fixture execution (39); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-21 direct aggregate-array compound-literal derived inner-pointer qualification-transition run. One hundred eight generated cases exhaustively cross scalar and named-aggregate inner pointees; promotion before the inner wrapper, after the wrapper, or after the nonzero inner offset; conditional-true, conditional-false, and comma wrappers; `pointer + 1`, `1 + pointer`, and `&pointer[1]`; and one/two-hop const-returning helpers. Hidden-root/nonzero-outer-base/inner-index identity, one-time initializer/wrapper markers, copied adjusted/local slots, legal const reads, and local const-pointer-slot reassignment remain stable. Twelve panic-guarded mutable-rebinding/write checks preserve exact const diagnostics. The warning-free direct-literal fixture covers both pointee kinds and all three promotion stages and returns 21 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this was deliberate conformance/property closure. The suite now has 59 fuzz-safety tests and 850 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated model and exact diagnostics (immediate GREEN conformance closure); focused interpreter fixture coverage (immediate GREEN); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (21); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-21 derived inner-pointer qualification-transition run. Three hundred twenty-four generated cases exhaustively cross scalar and named-aggregate inner pointees; named, anonymous, and union-containing captured outer paths; promotion before the inner wrapper, after the wrapper, or after the nonzero inner offset; conditional-true, conditional-false, and comma wrappers; `pointer + 1`, `1 + pointer`, and `&pointer[1]`; and one/two-hop const-returning helpers. Containing root/path/nonzero outer base and inner index identity, root/wrapper one-time markers, helper/caller copied slots, legal const reads, and local const-pointer-slot reassignment remain stable. Thirty-six panic-guarded mutable-rebinding/write checks preserve exact const diagnostics. The warning-free fixture covers all three promotion stages across named/anonymous/union paths and returns 31 under Cust, GCC, and Clang. Existing runtime behavior matched immediately, so this was deliberate conformance/property closure. The suite now has 58 fuzz-safety tests and 849 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused interpreter fixture coverage (immediate GREEN conformance closure); focused generated model and exact diagnostics (immediate GREEN); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (31); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-21 mutable-to-const re-forwarded captured aggregate-compound-literal field adjusted-parameter run. Two thousand five hundred ninety-two generated cases exhaustively cross scalar/named-aggregate inner pointees; named, anonymous, and union-containing captured paths; qualification promotion before or after the post wrapper; all inner/post wrappers and nonzero offsets; one/two-hop initial and const re-forwarding helpers; and re-forwarding before/after offset formation. Captured containing roots, recursive paths, absolute bases, one-time root/wrapper markers, inner-pointer identities, and copied parameter slots remain stable. Twelve panic-guarded mutable-rebinding/write checks preserve exact const diagnostics. The warning-free fixture covers all three path families and both promotion stages, returning 29 under Cust, GCC, and Clang. The suite now has 57 fuzz-safety tests and 848 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated model RED on an incorrect expected score followed by GREEN after correcting the independent test oracle; focused fixture coverage (immediate GREEN conformance closure); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (29); canonical `c_compat`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (exit 0, output `10`).

All required verification passed after the 2026-07-21 mutable-to-const re-forwarded direct aggregate-array compound-literal adjusted-parameter run. Four hundred thirty-two generated cases exhaustively cross qualification promotion before or after the post wrapper; inner and post conditional/comma wrappers; all three nonzero offsets; one/two-hop initial and const re-forwarding helpers; and const re-forwarding before or after offset formation. Mutable hidden roots retain absolute base and scalar/named-aggregate inner-pointer identity after promotion, selected/comma markers execute once, adjusted/copy pointer slots remain local, and const reads remain legal. Four exact panic-guarded mutable-rebinding and write diagnostics pass. The warning-free fixture covers both promotion stages around wrappers/offsets and returns 37 under Cust, GCC, and Clang. The suite now has 56 fuzz-safety tests and 847 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused static fixture coverage (immediate GREEN conformance closure); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (37); focused generated model/diagnostics (immediate GREEN); canonical `c_compat`; and `cargo fmt`. The recursion-depth regression and full required local/Docker gates were run after this status update; see the final run report.
...
Verified commands: clean-baseline `cargo test`; focused generated model/diagnostic RED on an incorrect expected matrix count followed by GREEN after correcting the test oracle; focused fixture coverage; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (45); canonical `c_compat`; and `cargo fmt`. The recursion-depth regression and full required local/Docker gates were run after this status update; see the final run report.

All required verification passed after the 2026-07-20 post-forward wrapper captured aggregate-compound-literal field adjusted-parameter run. Two thousand five hundred ninety-two generated scalar/named-aggregate two-writer/const-reader cases exhaustively cross named, anonymous, and union-containing field paths; same-element, same-field-distinct, cross-field, and separate-root relationships; inner and post-forward conditional/comma wrappers; all three nonzero offsets; before/after-offset wrapper placement; and one/two-hop mutable/const outer helpers. Separate inner/post markers, six captured roots, inner forwarding, and copied slots preserve root/path/base/outer/inner identity. Fourteen panic-guarded inner/absolute bounds, recursive const, cross-root, concrete type, and out-of-scope lifetime diagnostics pass. The warning-free fixture returns 45 under Cust, GCC, and Clang. The suite now has 53 fuzz-safety tests and 844 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused generated model/diagnostic coverage (immediate GREEN conformance closure); focused fixture coverage; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (45); canonical `cargo test --test c_compat supported_programs_match_c_compiler_exit_codes -- --nocapture`; and `cargo fmt`. The recursion-depth regression and full required local/Docker gates were run after this status update; see the final run report.

All required verification passed after the 2026-07-20 post-forward wrapper direct aggregate-array literal adjusted-parameter run. Six hundred forty-eight generated scalar/named-aggregate two-writer/const-reader cases cross same-element, same-array-distinct, and separate-root relationships; inner and post-forward conditional/comma wrappers; all three nonzero offsets; wrapper placement before/after the offset; and one/two-hop outer helpers. A separate 108-case const-array-typedef matrix covers every inner-wrapper/post-wrapper/offset/placement/helper-depth route. Distinct inner/post selected, unselected, and comma markers, hidden roots, inner forwarding, and copied slots preserve root/base/outer/inner identity. Fourteen panic-guarded inner/absolute bounds, const, cross-root, concrete type, and out-of-scope lifetime diagnostics pass. The warning-free fixture returns 49 under Cust, GCC, and Clang. The suite now has 52 fuzz-safety tests and 843 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused fixture coverage (immediate GREEN conformance closure); focused generated model/diagnostic coverage (immediate GREEN); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (49); recursion-depth regression; canonical `cargo test --test c_compat supported_programs_match_c_compiler_exit_codes -- --nocapture`; `cargo fmt`; and `git diff --check`. The full required local and Docker gates were run after this status update; see the final run report.

All required verification passed after the 2026-07-20 outer-pointer-forwarded wrapped captured field-offset adjusted-parameter run. One hundred forty-four generated scalar/named-aggregate two-writer/const-reader cases balance named, anonymous, and union-containing paths; same-element, same-field-distinct, cross-field, and separate-root relationships; conditional/comma wrappers; all three nonzero offsets; and one/two-hop `struct Item *` / `const struct Item *` helpers before and after offset formation. Inner forwarding, selected/unselected/comma markers, six captured roots, and copied slots preserve root/path/base/outer/inner identity. Fourteen panic-guarded inner/absolute bounds, recursive const, cross-root, concrete type, and out-of-scope lifetime diagnostics pass. The warning-free fixture returns 44 under Cust, GCC, and Clang. The suite now has 50 fuzz-safety tests and 841 interpreter tests.

Verified commands: clean-baseline `cargo test`; focused fixture coverage (immediate GREEN conformance closure); focused generated diagnostic RED/GREEN; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (44); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-20 conditional/comma-wrapped captured field-offset adjusted-parameter run. Seventy-two generated scalar/named-aggregate two-writer/const-reader cases balance named, anonymous, and union-containing paths; same-element, same-field-distinct, cross-field, and separate-root relationships; conditional-true, conditional-false, and comma wrappers; and `field + 1`, `1 + field`, and `&field[1]` offsets. Distinct wrapper markers prove selected/unselected/comma evaluation, while six captured roots, one/two-hop inner-pointer forwarding, and copied slots preserve root/path/base/outer/inner identity. Thirteen panic-guarded inner/absolute upper/lower bounds, recursive const, concrete type, and cross-root diagnostics pass. The warning-free fixture returns 44 under Cust, GCC, and Clang. The suite now has 49 fuzz-safety tests and 840 interpreter tests.

Verified commands: focused fixture coverage (immediate GREEN conformance closure); generated model RED on an incomplete comma-route marker oracle followed by GREEN after correcting the test generator; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (44); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-20 captured aggregate-compound-literal field-offset adjusted-parameter run. Fifty-four generated scalar/named-aggregate two-writer/const-reader cases balance same-element, same-array-distinct, and cross-field relationships across named, anonymous, and union-containing paths composed with `field + 1`, `1 + field`, and `&field[1]`. Six enclosing literals are captured once per program; one/two-hop forwarding and copied slots retain base-relative outer/inner identity. Thirteen panic-guarded inner/absolute upper/lower bounds, recursive const, concrete type, and cross-path diagnostics pass. The warning-free fixture proves `items[-1]` reaches the preceding field element and returns 34 under Cust, GCC, and Clang. The suite now has 48 fuzz-safety tests and 839 interpreter tests.

Verified commands: focused generated field-offset model; focused fixture; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (34); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-20 wrapped nonzero-offset direct aggregate-array literal adjusted-parameter run. Fifty-four generated scalar/named-aggregate two-writer/const-reader cases balance same-element, same-array-distinct, and separate-literal relationships across conditional-true, conditional-false, and comma roots composed with `literal + 1`, `1 + literal`, and `&literal[1]`. Selected/unselected/comma markers prove one-time hidden-root creation; one/two-hop forwarding and copied slots retain base-relative outer/inner identity. Nine const-array-typedef wrapper combinations and 13 panic-guarded inner/upper/lower bounds, const, type, and cross-root diagnostics pass. The warning-free fixture additionally proves `items[-1]` reaches the preceding element from the adjusted nonzero base and returns 37 under Cust, GCC, and Clang. The suite now has 47 fuzz-safety tests and 838 interpreter tests.

Verified commands: clean baseline `cargo test`; focused generated offset model; focused fixture; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (37); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust` (exit 0, output `10`); and `git diff --check`.

All required verification passed after the 2026-07-20 conditional/comma-wrapped direct aggregate-array literal adjusted-parameter run. Thirty-six generated scalar/named-aggregate two-writer/const-reader cases balance same-element, same-array-distinct, and separate-literal relationships across conditional-true, conditional-false, and comma routes. Selected/unselected/comma markers prove short-circuiting and one-time hidden-root creation; one/two-hop forwarding and copied slots retain recursive identity. Six const-array-typedef wrapper checks and 11 panic-guarded diagnostics preserve qualification, bounds, concrete type, and cross-root behavior. The warning-free fixture returns 69 under Cust, GCC, and Clang; the suite now has 46 fuzz-safety tests and 837 interpreter tests.

Verified commands: focused generated model and fixture coverage (immediate GREEN conformance closure after correcting harness helper names/counts); direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (69); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (exit 0, output `10`).
...
Verified commands: focused fixture coverage (immediate GREEN conformance closure); generated model coverage with an initial const-fixture setup RED followed by GREEN; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (67); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (exit 0, output `10`).

All required verification passed after the 2026-07-19 nested adjusted-aggregate-parameter alias-modeling run. Forty-eight fixed scalar/named-aggregate two-writer/const-reader cases now balance direct/reverse addresses and one/two-hop forwarding across root, direct-field, nested named, nested anonymous, and union-containing outer arrays. Recursive owner/path/outer-index/inner-index identity preserves ordered same-element writes, isolates different path/owner storage, and keeps copied callee slots local. Twenty-three targeted panic-guarded checks retain exact inner/outer bounds, recursive const, concrete aggregate type, and cross-path subtraction diagnostics. A warning-free fixture covers all path families and returns 32 under Cust, GCC, and Clang; the suite now has 43 fuzz-safety tests and 834 interpreter tests.

Verified commands: focused nested-path fixture coverage (immediate GREEN conformance closure); generated alias-model coverage-counter RED/GREEN; focused nested diagnostic model; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (32); recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (exit 0, output `10`).

All required verification passed after the 2026-07-19 adjusted-aggregate-parameter ordered alias-mutation run. Forty-eight fixed scalar/named-aggregate cases now cover two writers plus a `const` reader across root and field-decayed outer arrays, six owner/path/index relationships, balanced direct/reverse addresses, balanced one/two-hop forwarding, ordered observations, caller-visible mutations, and caller-invisible parameter-slot reassignment. RED exposed that `StructFieldArrayElementGet` only classified ordinary pointer fields as pointer-valued: nested scalar/aggregate arrays reached through outer aggregate-array fields therefore failed with `expected pointer expression`. Pointer classification and evaluation now route `Array`/`StructArray` fields through array-base decay before falling back to ordinary pointer-field reads. The warning-free fixture returns 40 under Cust, GCC, and Clang; the suite now has 42 fuzz-safety tests and 833 interpreter tests.

Verified commands: focused direct embedded-array decay RED/GREEN; focused generated adjusted-parameter alias model; focused alias fixture; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (40); canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust` (exit 0, output 10).

All required verification passed after the 2026-07-19 deterministic adjusted-aggregate-parameter identity-model run. A fixed matrix now checks 60 scalar/aggregate embedded-field pointer cases across root aggregate-array arguments and arrays decayed from aggregate fields. Six independently modeled relationships cover same elements, same embedded arrays at distinct indexes, distinct outer elements, distinct field paths, distinct owners, and root-versus-field storage; five operations cover reads, writes through captured addresses, equality, difference, and ordering. Direct/reverse routes are balanced at 60 each, outer/inner index expressions execute once, and 14 targeted checks preserve inner/outer bounds, const-ancestor writes, concrete aggregate pointer types, and panic freedom. Existing runtime behavior matched after correcting the test harness to initialize through captured adjusted-parameter pointers rather than an unrelated direct field-backed scalar write. A warning-free root/field identity fixture returns 35 under Cust, GCC, and Clang; the suite now has 41 fuzz-safety tests and 830 interpreter tests.

Verified commands: focused generated adjusted-parameter model; focused recursive-identity fixture; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (35); canonical `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (830 interpreter tests, 41 fuzz-safety tests, and all other targets); `docker compose run --rm test`; `docker compose run --rm cust` (10); and `git diff --check`.

All required verification passed after the 2026-07-19 embedded-array-field parity run for aggregate arrays adjusted to parameters. `items[i].nested.values[j]` and `items[i].nested.points[j]` now resolve scalar and aggregate embedded-array storage when `items` is a C-adjusted aggregate pointer parameter, including direct/reverse reads, scalar replacement/compound/increment writes, aggregate copy assignment/consumers, element addresses, deep-copy isolation, and one-time outer/index/RHS evaluation. A new recursive `NestedStructArrayElement` pointer target preserves parent pointer + field path + element identity for outer arrays decayed from aggregate fields; an adjacent RED identity regression returned 5 instead of 6 until `pointer_eq()` and pointer difference compared recursive interpreter-owned identity. Const ancestors, const final fields, outer/inner bounds, aggregate type diagnostics, and non-evaluating `sizeof` remain exact. The warning-free fixture returns 82 under Cust, GCC, and Clang; thirteen new focused tests raise the interpreter suite to 829 tests.

Verified commands: focused embedded-array-parameter sweep; focused pointer-identity RED/GREEN; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (82); `cargo test --test c_compat -- --nocapture` through the full suite; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (829 interpreter tests, 40 fuzz-safety tests, and all other targets); `docker compose run --rm test`; `docker compose run --rm cust` (10); and `git diff --check`.

All required verification passed after the 2026-07-19 adjusted aggregate-array-parameter pointer-field slot update run. `items[i].nested.pointer_field` now supports replacement, `+=`/`-=`, and prefix/postfix `++`/`--` when `items` is bound as an adjusted aggregate pointer parameter rather than stored `Value::StructArray`. RED isolated the direct-array-only mutating resolver: it called `find_struct_element_pointer()` after enforcing variable-slot mutability, so adjusted parameters failed with `variable 'choices' is not a struct array`. The resolver now distinguishes adjusted pointers from stored arrays, rejects const aggregate pointees without conflating them with const parameter pointer slots, and delegates to established field slot/type/const checks. Named-holder, anonymous-inner, and union routes preserve pointer-valued results, concrete pointee qualification, one-time index/RHS evaluation, exact const diagnostics, and non-evaluating `sizeof`. The warning-free fixture returns 63 under Cust, GCC, and Clang; six new focused tests raise the interpreter suite to 816 tests.

Verified commands: focused replacement RED/GREEN; focused compound/increment RED/GREEN; focused 10-test aggregate-array-parameter sweep; qualifier/diagnostic/`sizeof` tests; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture execution (63); `cargo test --test c_compat supported_programs_match_c_compiler_exit_codes -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (816 interpreter tests, 40 fuzz-safety tests, and all other targets); `docker compose run --rm test`; `docker compose run --rm cust` (10); and `git diff --check`.

All required verification passed after the 2026-07-19 nested-holder and adjusted aggregate-array-parameter pointer-field subscript run. Named nested and anonymous-inner holder arrays plus union arrays now have warning-free coverage for direct/reverse scalar reads, replacement/compound/increment writes, element addresses, aggregate argument/return/conditional/comma/discard/postfix consumers, deep-copy isolation, one-time evaluation, exact const/bounds/type diagnostics, and non-evaluating `sizeof`. RED isolated a real adjusted-parameter gap: `choices[i].nested.pointer_field[j]` parsed as `StructElementArrayGet` but runtime and metadata helpers assumed `choices` was a stored `Value::StructArray`; array parameters are `Value::Pointer` after C adjustment. A shared indexed-element resolver now handles both storage forms, declaration metadata recognizes aggregate pointers, and type/const/`sizeof` inference stays non-evaluating. The warning-free fixture returns 32 under Cust, GCC, and Clang; seven focused tests raise the interpreter suite to 810 tests.

Verified commands: focused RED/GREEN aggregate-array-parameter read, update, address, diagnostic, and `sizeof` tests; focused 11-test ordinary named aggregate-array pointer-field sweep; `cargo test --test c_compat -- --nocapture`; direct Cust/GCC/Clang fixture execution (32); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (810 interpreter tests, 40 fuzz-safety tests, and all other targets); `docker compose run --rm test`; `docker compose run --rm cust` (10); and `git diff --check`.
...
Verified commands: focused scalar and aggregate pointer-field subscript RED/GREEN tests; focused aggregate copy-read and copy-assignment RED/GREEN tests; recursion-depth regression; `cargo test --test c_compat -- --nocapture`; direct `cc -std=c11 -Wall -Wextra -Werror` fixture execution (exit 80); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (797 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 ordinary named aggregate-array element pointer-field run. `StructElementGet` now distinguishes stored pointer fields from embedded array decay; matching `StructElementSet`/`StructElementCompoundSet` and increment/decrement routes support replacement, `+=`/`-=`, and prefix/postfix results. Shared field metadata preserves scalar and named-aggregate pointee types, pointee constness, const pointer slots, one-time index/RHS evaluation, and non-evaluating `sizeof`. A dedicated mutating element resolver also closes a RED-exposed const-root bug while keeping mutable pointees reachable through const pointer-field slots. The warning-free compiler-oracle fixture returns 17 under Cust and native `cc`; thirteen focused tests raise the interpreter suite to 782 tests.

Verified commands: clean-baseline `cargo test`; four operation-specific RED/GREEN tests; focused const-root RED/GREEN; `cargo test --test interpreter named_aggregate_array -- --nocapture` (12 tests before the final focused bounds regression); focused exact-bounds regression; recursion-depth regression; `cargo test --test c_compat -- --nocapture`; direct `cc -std=c11 -Wall -Wextra -Werror` fixture execution (exit 17); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (782 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 embedded aggregate-array element pointer-field update run. Pointer-valued fields selected by `StructFieldArrayElementGet`/`Set`/`CompoundSet` now classify and evaluate as pointers across direct, nested, and scalar-field reverse-subscript routes. Replacement, `+=`/`-=`, and prefix/postfix updates preserve pointer-valued results, concrete pointee types, pointee constness, const pointer slots, one-time target/RHS evaluation, and non-evaluating `sizeof`. Pointer-field reads now use a read-only target resolver, so const containing aggregates remain readable while mutation stays rejected. RED also exposed and fixed increment-result const metadata leakage. A warning-free compiler-oracle fixture returns 9 under Cust and native `cc`; twelve focused interpreter regressions/fixtures raise the interpreter suite to 769 tests.

Verified commands: clean-baseline `cargo test`; focused replacement, compound-assignment, increment, reverse/nested, const-read, qualifier-leak, one-time evaluation, diagnostic, and `sizeof` RED/GREEN tests; `cargo test --test interpreter embedded_aggregate_array_element -- --nocapture` (28 tests before the final focused additions); direct `cc -std=c11 -Wall -Wextra -Werror` fixture execution (exit 9); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (769 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 embedded aggregate-array element field assignment run. `StructFieldArrayElementSet` and `StructFieldArrayElementCompoundSet` now resolve direct embedded-array and scalar-field reverse-subscript targets to one interpreter-owned aggregate-element pointer before mutation. Replacement and arithmetic compound assignment support `int`/`char` fields, preserve expression results, evaluate direct indexes/reverse pointer expressions/RHS expressions once, and retain exact const-container/const-field/const-root, bounds, and non-aggregate diagnostics. Non-evaluating `sizeof` retains lvalue result types without running markers or mutations. Independent pre-commit review found and the run fixed an adjacent aggregate-pointer-field const regression: `const T *field` writes are rejected while `T * const field` element writes remain legal. The warning-free compiler-oracle fixture returns 16 under Cust, GCC, and Clang; eight new interpreter tests raise the suite to 757 tests.

Verified commands: clean-baseline `cargo test`; focused reverse replacement RED/GREEN (`struct variable 'points' used as scalar` before GREEN); focused direct compound-assignment single-evaluation RED/GREEN (result check 2/5 before GREEN); focused reverse compound-assignment RED/GREEN; focused aggregate-pointer-field const RED/GREEN (const-pointee write returned 3 before GREEN); focused assignment/diagnostic/`sizeof` tests; recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; direct GCC and Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution (exit 16); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (757 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 embedded aggregate-array element field increment/decrement run. `StructFieldArrayElementGet` is now a supported scalar increment/decrement lvalue for direct embedded-array routes such as `++box.points[i].tag` and scalar-field reverse aggregate subscripts such as `++selector.value[points].tag`. Prefix/postfix `++`/`--` preserve `int`/`char` results, evaluate side-effecting direct indexes and reverse pointer expressions once, mutate the captured interpreter-owned aggregate element, and retain root/field const, bounds, and non-aggregate pointer diagnostics. `sizeof` reuses the target metadata without evaluating indexes or mutating storage. The warning-free compiler-oracle fixture returns 14 under Cust, GCC, and Clang; five new interpreter tests raise the interpreter suite to 749 tests.

Verified commands: clean-baseline `cargo test`; focused parser RED/GREEN for direct and reverse prefix increment; focused single-evaluation RED/GREEN for the complete direct/reverse prefix/postfix fixture; focused const/bounds/type and non-evaluating `sizeof` tests; recursion-depth regression; canonical `cargo test --test c_compat -- --nocapture`; direct GCC and Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution (exit 14); `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (749 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`. The full Clang `c_compat` sweep remains blocked by the pre-existing `inline_enum_conditional_type_definitions.c` `-Wconstant-logical-operand` warning, but the new fixture passes Clang directly and the canonical/default-compiler gate passes.

All required verification passed after the 2026-07-18 reverse-subscript lvalue-result `sizeof` run. Assignment and compound-assignment AST routes now retain their reverse-subscript index metadata instead of sizing the syntactic scalar left operand as an array. `sizeof` classifies scalar-variable and scalar aggregate-field offsets over `int`/`char` arrays and pointers, plus scalar fields selected from reverse-indexed named aggregate arrays/pointers; prefix increment results reuse the same non-evaluating target metadata. Eleven assignment/compound/increment type relationships and two explicit marker checks prove pointer/index/RHS expressions are not evaluated. Four focused tests were observed RED with the legacy `variable ... is not an array` / `struct field ... is not an array` / `variable ... is not a struct array` diagnostics before GREEN; the complete fixture returns 13 under Cust and warning-free GCC. Clang correctly compiles the C forms only when its `-Wunevaluated-expression` warning is not promoted to an error, so this side-effecting fixture remains interpreter-focused rather than entering the compiler-neutral `c_compat` list.

Verified commands: clean-baseline `cargo test`; four focused RED/GREEN tests; `cargo test --test interpreter reverse_subscript -- --nocapture` (14 tests); `cargo test --test c_compat -- --nocapture`; `CC=gcc cargo test --test c_compat -- --nocapture`; direct GCC `-std=c11 -Wall -Wextra -Werror` fixture build/execution (exit 13); documented Clang `-Werror,-Wunevaluated-expression` boundary probe; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (744 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 non-evaluating scalar reverse-subscript `sizeof` run. `sizeof(index[array])`, `sizeof(index[pointer])`, `sizeof(index[aggregate_array])`, `sizeof(index[aggregate_pointer])`, selected aggregate fields, and scalar aggregate-field offset equivalents now classify the pointer operand's pointee type without evaluating either subscript operand. The metadata path preserves `char` versus Cust `int` sizes, concrete aggregate field sizes, and exact two-scalar / scalar-pointer-field diagnostics. The same audit fixed direct `sizeof(aggregate_pointer[index].field)`, which previously assumed every specialized `StructElementGet` base was an aggregate array. Three focused RED/GREEN regressions plus paired fixtures cover thirteen ABI-independent direct/reverse relationships; the Cust-only fixture also proves side effects remain suppressed and returns 16, while the warning-free compiler-oracle fixture returns 13 under Cust, GCC, and Clang.

Verified commands: clean-baseline `cargo test`; focused RED `cargo test --test interpreter sizeof_scalar_variable -- --nocapture` (all three new tests failed with the expected legacy metadata diagnostics); focused GREEN `cargo test --test interpreter sizeof_scalar_variable -- --nocapture`; focused reverse-subscript regressions; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC/Clang builds and exit-13 execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (739 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-18 scalar-variable reverse-subscript run. Runtime subscript resolution now distinguishes scalar left operands from array/pointer bases after lookup, so `index[array]`, `index[pointer]`, and `index[aggregate_pointer].field` support reads, replacement/compound/increment writes, address-of, and const/bounds/type checks without replacing the specialized direct-index AST paths. Scalar aggregate fields also work as reverse-subscript offsets. Seven focused regressions (six exact diagnostic boundaries plus one scalar-field lvalue route) and a warning-free compiler-oracle fixture cover scalar and named-aggregate arrays/pointers; the fixture returns 220 under Cust, GCC, and Clang. An adjacent probe found that non-evaluating `sizeof(index[array])` still classifies the syntactic left variable as an array base; that follow-up became the first unchecked backlog item at that point.

Verified commands: clean-baseline `cargo test`; focused RED/GREEN scalar-variable, scalar-field, aggregate, const, bounds, type, and missing-bracket tests; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (736 interpreter tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.
...
Verified commands: clean-baseline `cargo test`; focused generated valid/mismatch initializer-element models; focused interpreter fixture test; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic declaration-initializer and assignment-RHS context classification run. `tests/fuzz_safety.rs` adds 58 valid scalar/pointer/aggregate route-context cases plus 24 result-shape, pointer qualification/type, and aggregate-type mismatch cases under panic guards. Declaration and later-assignment contexts cover direct, conditional, comma, assignment, compound-assignment, call, cast, pointer arithmetic/field assignment, aggregate copy/literal/dereference, direct/arrow aggregate-field assignment, root aggregate-array assignment, and embedded aggregate-array assignment routes with exact side-effect and single-evaluation checks. Existing Cust behavior matched immediately, so this was deliberate property/conformance coverage with no production-code change. The warning-free one-expression-per-statement fixture returns 180 under Cust, GCC, and Clang.

Verified commands: clean-baseline `cargo test`; focused generated valid/mismatch initializer/RHS models; focused interpreter fixture test; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic function-argument context classification run. `tests/fuzz_safety.rs` adds 28 valid scalar/pointer/aggregate argument routes plus 12 result-shape, pointer qualification/type, and aggregate-type mismatch cases under panic guards. Direct, conditional, comma, assignment, compound-assignment, call, cast, compound-literal, dereference, direct/arrow aggregate-field assignment, root aggregate-array assignment, and embedded aggregate-array assignment routes prove per-argument side effects and single evaluation. RED showed `eval_struct_argument()` rejected every aggregate-valued assignment result because its aggregate-expression allowlist omitted `Assign`, `DerefSet`, `StructSet`, `StructPtrSet`, `ArraySet`, and `StructArraySet`; all six now route through established deep-copy/type/const evaluation. Exact mismatch diagnostics remain unchanged. The warning-free one-argument-at-a-time fixture returns 173 under Cust, GCC, and Clang.

Verified commands: clean-baseline `cargo test`; six focused aggregate-assignment argument RED/GREEN cycles; focused generated valid/invalid argument models; focused compiler-oracle interpreter test; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic return-context classification run. `tests/fuzz_safety.rs` adds 24 valid scalar/pointer/aggregate return routes plus 15 value/empty mismatch and pointer qualification/type cases under panic guards. Direct expressions, assignments, compound assignments, calls, casts, conditionals, commas, aggregate copies/literals/dereferences, selected-branch side effects, exact diagnostics, and single evaluation are covered. RED showed value-bearing returns in void functions were incorrectly scalar-evaluated, so pointer, aggregate, and void expressions leaked result-shape errors before the established `void function '<name>' returned a value` diagnostic. `eval_return_value()` now evaluates void-return expressions exactly once in discard context before existing return validation, while non-void routing preserves established exact diagnostics. The warning-free GCC/Clang fixture returns 199 like Cust.

Verified commands: clean-baseline `cargo test`; focused void-return RED/GREEN; focused generated return and discard models; focused return interpreter regressions; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic discard-context classification run. `tests/fuzz_safety.rs` adds 81 fixed route/context cases across expression statements, explicit `(void)` casts, and comma-left operands for scalar, pointer, aggregate, and void results. Assignment/compound-assignment, direct/arrow/aggregate-literal pointer fields, calls, conditionals, commas, pointer casts, aggregate values/copies, and void expressions retain exact scalar-use diagnostics, single evaluation, and panic freedom. RED showed pointer-valued comma expressions retried after scalar evaluation and executed their left side effects twice, aggregate dereference copy assignment speculatively evaluated its pointer twice, direct aggregate values were rejected as scalars, and void-valued commas leaked scalar-use errors. `eval_discard()` now classifies void, pointer, aggregate, and scalar result shapes from metadata before selecting exactly one evaluator. All 27 fuzz-safety tests remain sub-second locally; the warning-free GCC/Clang fixture returns 240 like Cust.

Verified commands: clean-baseline `cargo test`; focused pointer-comma, aggregate-value, void-comma, and aggregate-dereference RED/GREEN regressions; focused generated discard model; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic truthiness/logical-classification run. `tests/fuzz_safety.rs` adds 144 fixed-seed scalar and pointer cases balanced across direct `if` truthiness, unary `!`, `&&`, `||`, conditional conditions, and loop conditions. Routes include nested scalar array-field reads, scalar wrappers and pointer differences, pointer arithmetic/address/calls, direct and arrow pointer fields, pointer-field assignment results, conditionals, commas, casts, null/non-null values, aggregate/void diagnostics, short-circuit side-effect counts, and panic freedom. RED showed `eval_truthy()`'s hand-maintained pointer list omitted arrow pointer-field assignment results, so `view->primary = (int *)0` in a loop condition leaked `pointer value used as scalar`. Truthiness now classifies the whole expression with `expr_is_pointer_value()` before evaluation, then evaluates pointer-valued results exactly once with `eval_pointer()`. Balancing null/non-null values across every generated pointer route exposed a second seam: `(int *)(int)0` leaked `expected pointer expression`; pointer casts now evaluate scalar wrappers once, accept zero as null, and preserve the existing nonzero diagnostic. All 26 fuzz-safety tests complete in about 0.85 seconds; the warning-free GCC/Clang fixture returns 227 like Cust.

Verified commands: clean-baseline `cargo test`; focused generated truthiness RED/GREEN; focused interpreter regression; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic relational-ordering classification run. `tests/fuzz_safety.rs` adds 96 fixed-seed scalar/scalar, pointer/pointer, and mixed pointer/scalar cases across `<`, `<=`, `>`, and `>=`, nested scalar array-field reads, literal/cast/unary/pointer-difference wrappers, array/indexed-address routes, direct/arrow pointer fields, pointer-returning calls, conditionals, commas, pointer casts, same/cross-root pointers, null, exact diagnostics, side-effect counts, and panic freedom. RED showed the ordering branch speculatively called `eval_pointer()` for both operands whenever either was pointer-valued, so an addressable scalar nested field read was mistaken for a pointer and mixed ordering leaked `cannot compare pointers to different arrays`. `eval_ordering()` now classifies both result shapes before evaluation, evaluates scalar/scalar and pointer/pointer operands exactly once through the correct evaluator, and rejects mixed shapes without speculative evaluation. The warning-free GCC/Clang fixture returns 130 like Cust.

Verified commands: clean-baseline `cargo test`; focused generated ordering RED/GREEN; focused valid and invalid interpreter regressions; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 deterministic equality-classification model run. `tests/fuzz_safety.rs` adds 96 fixed-seed scalar/scalar, pointer/pointer, and mixed pointer/integer cases across nested scalar array-field reads, literals/casts/unary wrappers, pointer differences, array/indexed-address routes, direct/arrow pointer fields, pointer-returning calls, conditionals, commas, pointer casts, null, same/cross-root identity, exact nonzero/type diagnostics, side-effect counts, and panic freedom. RED showed mixed equality still speculatively called `eval_pointer()` on scalar operands and only recognized raw numeric literals: `pointer == +0` leaked `expected pointer expression`. `eval_equality()` now classifies both operand shapes first, evaluates each operand exactly once in its scalar or pointer context, treats wrapped zero as null, preserves the established nonzero diagnostic, and reports aggregate operands from scalar context. The warning-free GCC/Clang fixture returns 129 like Cust.

Verified commands: clean-baseline `cargo test`; focused generated equality RED/GREEN; focused valid and invalid interpreter regressions; canonical `cargo test --test c_compat -- --nocapture`; direct warning-free GCC and Clang fixture builds/execution; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-17 nested/anonymous aggregate-compound-literal field-pointer alias run. `tests/fuzz_safety.rs` adds 96 fixed-seed two-writer/const-reader cases over `int`, `char`, named `struct`, and named `union` array fields reached through named nested and anonymous-inner aggregate paths. Four captured compound-literal roots are evaluated exactly once per case; one/two-hop helpers plus arithmetic, indexed-address, conditional, and comma wrappers preserve path + literal root + field + index + concrete type + qualification identity. Six balanced alias relationships feed an independent weighted model over all 32 cells, with ordered reader observations, caller identities, and recursive top-level const, cross-path/root, bounds, and type diagnostics. RED exposed scalar equality's speculative `eval_pointer()` probe: nested scalar array-field element reads through struct pointers were mistaken for pointers and `field[index] == integer` reported `cannot compare pointer with nonzero integer`. `eval_equality()` now uses `expr_is_pointer_value()` before evaluation when both operands are scalar. The warning-free GCC/Clang fixture returns 20 like Cust, and six previously unregistered field-backed compiler-oracle fixtures are now part of the canonical `c_compat` corpus.

Verified commands: clean-baseline `cargo test`; focused RED/GREEN interpreter fixture; focused generated model; pointer/nonzero regression; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-16 aggregate-compound-literal field-pointer alias-mutation model run. `tests/fuzz_safety.rs` now adds 96 fixed-seed two-writer/const-reader cases over captured `int`/`char` scalar array fields and embedded named `struct Point`/`union Number` array fields. Each hidden field root is evaluated once, then one/two-hop helpers plus arithmetic, indexed-address, conditional, and comma wrappers preserve root/field/index/type/qualification identity. Same-element, same-root distinct-element, different-field-root, and separate-literal relationships feed an independent weighted model over all sixteen cells; ordered writes, intermediate const-reader observations, caller pointer identities, and initializer execution counts all match. RED exposed that const-qualified aggregate compound literals lost top-level qualification before field-array decay; `Expr::AggregateLiteral` and `AddressOfAggregateLiteral` now carry `read_only` metadata into aggregate-field and address-of pointer inference, preserving pointer-field pointee semantics while rejecting const-array-field discard/write routes. Exact cross-root, hidden-field bounds, and concrete-type diagnostics remain green. All twenty-two fuzz-safety tests complete in about 0.85 seconds locally, and warning-free GCC/Clang C11 fixtures return 20 like Cust.

Verified commands: clean baseline `cargo test`; focused RED/GREEN aggregate-literal field model; focused interpreter fixture; all twenty-two fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; and `git diff --check`. The full required local and Docker gates were run after the status update; see the final run report.

All required verification passed after the 2026-07-16 nested/anonymous field-backed returned-pointer alias-mutation model run. `tests/fuzz_safety.rs` now adds 96 fixed-seed two-writer/const-reader cases over `int`/`char` scalar array fields and embedded named `struct Point`/`union Number` array fields inside both nested named holders and anonymous holder objects. Direct/arrow paths and one/two-hop pointer-returning helpers retain containing-path, owner, field, index, concrete pointee type, and qualification identity. Ordered replacement/compound writes cover same-element, same-field distinct-element, cross-path, and different-owner relationships; an independent position-weighted model verifies all 32 cells plus intermediate const-reader observations and unchanged caller pointer identities. Exact nested/anonymous const-container, cross-path subtraction, bounds, and type diagnostics remain green. Existing Cust behavior matched immediately, so this was deliberate property/conformance coverage with no production-code change. All twenty-one fuzz-safety tests complete in about 0.49 seconds locally, and the warning-free GCC/Clang C11 fixture returns 36 like Cust.

Verified commands: clean baseline `cargo test`; focused generated nested/anonymous alias-mutation and interpreter fixture tests; all twenty-one fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; and `git diff --check`. The full required local and Docker gates were run after the status update; see the final run report.

All required verification passed after the 2026-07-16 field-backed returned-pointer alias-mutation model run. `tests/fuzz_safety.rs` now generates 96 fixed-seed cases over `int`/`char` scalar array fields and embedded named `struct Point`/`union Number` array fields. Two mutable pointers and one const reader retain containing owner, field path, index, concrete pointee type, and qualification through balanced direct/arrow arguments and one/two-hop pointer-returning helpers. Ordered replacement/compound writes cover same-element, same-field distinct-element, different-field, and different-owner relationships; an independent position-weighted model verifies all sixteen cells across both fields in both objects, intermediate reader observations, and unchanged caller pointer identities after callee-local slot reassignment. Exact const-return write/discard, bounds, and concrete-type diagnostics remain green. Existing Cust behavior matched immediately, so this was deliberate property/conformance coverage with no production-code change. All twenty fuzz-safety tests complete in about 0.83 seconds locally, and the warning-free GCC/Clang C11 fixture returns 36 like Cust.

Verified commands: clean baseline `cargo test`; focused generated alias-mutation and interpreter fixture tests; all twenty fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (704 interpreter tests and twenty fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-16 field-backed pointer-return/forwarding model run. `tests/fuzz_safety.rs` now generates 128 fixed-seed mutable/const pointer expressions over `int`/`char` scalar array fields and embedded named `struct Point`/`union Number` array fields. Direct and arrow field arguments flow through one/two-hop pointer-returning helpers plus arithmetic, conditional, comma, and indexed-address wrappers while an independent model preserves containing owner, field path, element index, concrete pointee type, and qualification. Exact cross-field/root, bounds, const-discard, const-write, const-containing-object, and type diagnostics remain green. Existing Cust behavior matched every semantic assertion; after balancing the test generator's qualification routes, all nineteen fuzz-safety tests complete in about 0.74 seconds locally. The warning-free GCC and Clang C11 fixture returns 160, matching Cust.

Verified commands: clean baseline `cargo test`; focused field-backed forwarding model and interpreter fixture tests; all nineteen fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; and `git diff --check`. The full required local and Docker gates were run after the status update; see the final run report.

All required verification passed after the 2026-07-16 field-backed mixed-qualification pointer-parameter model run. `tests/fuzz_safety.rs` now generates 96 fixed-seed `T *writer` / `const T *reader` pairs over `int`/`char` scalar array fields and embedded named `struct Point`/`union Number` array fields. The independent model distinguishes containing owner, field path, element index, and direct versus arrow route; it checks same-element aliases, same-field distinct elements, different fields, and different containing objects. Weighted storage, post-write reader observations, copied parameter slots, and unchanged caller pointers all match. Exact const-containing-object, field-bounds, and concrete pointee-type diagnostics remain green. All eighteen fuzz-safety tests complete in about 0.70 seconds locally. Existing Cust behavior matched immediately, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 28, matching Cust.

Verified commands: clean baseline `cargo test`; focused field-backed mixed-qualification model and interpreter fixture tests; all eighteen fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (702 interpreter tests and eighteen fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-16 mixed-qualification pointer-parameter alias model run. `tests/fuzz_safety.rs` now generates 90 fixed-seed `T *writer` / `const T *reader` pairs over mutable global/file-static scalar and named `struct`/`union` arrays: 30 same-element aliases, 30 same-root distinct-index pairs, and 30 cross-root pairs. An independent weighted-storage model verifies writer mutations, post-write reader observations, and caller-visible root/index identity while both callee slots are reassigned locally. Exact const-reader write, const-storage-to-writer, bounds, and concrete-type diagnostics remain green. All seventeen fuzz-safety tests complete in about 0.80 seconds locally. Existing Cust behavior matched, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 63, matching Cust.

Verified commands: clean baseline `cargo test`; focused mixed-qualification alias model, mutation-diagnostic, and interpreter fixture tests; all seventeen fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (701 interpreter tests and seventeen fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-16 two-parameter pointer-alias mutation model run. `tests/fuzz_safety.rs` now generates 90 fixed-seed scalar/named-struct/named-union pointer pairs over mutable global/file-static arrays: 30 same-element aliases, 30 same-root distinct-index pairs, and 30 cross-root pairs. Callees apply an ordered replacement through the first parameter and compound update through the second, then reassign both local parameter slots; an independent weighted-storage model verifies final root/index contents and unchanged caller pointer identities. Existing exact const-write, bounds, and concrete-type mutation diagnostics remain green. All sixteen fuzz-safety tests complete in about 0.49 seconds locally. Existing Cust behavior matched, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 54, matching Cust.

Verified commands: clean baseline `cargo test`; focused two-parameter alias model, mutation-diagnostic, and interpreter fixture tests; all sixteen fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (700 interpreter tests and sixteen fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-16 pointer-parameter mutation model run. `tests/fuzz_safety.rs` now generates 120 fixed-seed scalar/named-struct/named-union pointer cases over mutable global/file-static array roots. Callees mutate selected pointees, reassign their local parameter slots, and return slot-local identity checks; the independent model verifies caller-visible replacement values, retained caller pointer root/index identity, and one- versus two-hop helper behavior. Nine targeted cases retain exact const-pointee write, out-of-bounds, and concrete pointee-type diagnostics. All fifteen fuzz-safety tests complete in about 0.70 seconds locally and 0.77 seconds in Docker. Existing Cust behavior matched after correcting low-bit bias in the test generator, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 35, matching Cust.

Verified commands: focused mutation-model and diagnostic tests; focused interpreter fixture; all fifteen fixed-seed `fuzz_safety` tests; recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (699 interpreter tests and fifteen fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-16 pointer-parameter forwarding model run. `tests/fuzz_safety.rs` now generates 144 fixed-seed scalar/named-struct/named-union pointer expressions flowing through one or more mutable/const pointer parameters and pointer-returning helper calls, plus 39 targeted cross-root, parameter/return type, const, bounds, and out-of-scope checks. The independent model separates storage constness from the forwarded expression's pointee qualification, tracks global/file-static root identity, index/value, and concrete pointee type, and covers nested forwarding inside arithmetic, indexed-address, conditional, and comma expressions. All thirteen fuzz-safety tests complete in about 0.43 seconds locally. Existing Cust behavior matched, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 121, matching Cust.

Verified commands: clean baseline `cargo test`; focused pointer-forwarding model and interpreter tests; all thirteen fixed-seed `fuzz_safety` tests; focused pointer-return and recursion-depth regressions; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (698 interpreter tests and thirteen fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.
...
Verified commands: clean baseline `cargo test`; focused hidden aggregate-literal pointer-model test; all eleven fixed-seed `fuzz_safety` tests; focused interpreter fixture and recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (696 interpreter tests and eleven fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-15 hidden scalar-array literal pointer-model run. `tests/fuzz_safety.rs` now generates 240 fixed-seed expression cases plus 37 cross-root, const, read-only, and type checks over mutable `int`/`char` array compound literals, const array-typedef compound literals, and read-only string literals. The independent model checks hidden storage identity, element index/value, pointee type, const/read-only metadata, exact bounds/cross-root/const-discard/write/type diagnostics, and panic freedom through arithmetic, pointer difference, equality, ordering, conditional, and comma expressions. All ten fuzz-safety tests complete in about 0.77 seconds locally and 0.49 seconds in Docker. Existing Cust behavior matched, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 65, matching Cust.

Verified commands: clean baseline `cargo test`; focused generated hidden-literal pointer model test; all ten fixed-seed `fuzz_safety` tests; focused interpreter fixture; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (695 interpreter tests and ten fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-15 scalar array-field pointer-model run. `tests/fuzz_safety.rs` now generates 768 fixed-seed expressions plus 128 route-specific type/read-only checks for `int` and `char` array fields in named and anonymous struct/union containers. Direct decay, indexed address-of, arrow, nested-field, and aggregate-compound-literal routes flow through pointer arithmetic, differences, conditionals, and comma expressions; the independent model checks owner/path/index identity, scalar pointee type, const propagation, and exact cross-owner, bounds, const-discard, read-only-write, and type-conversion diagnostics. All nine fuzz-safety tests complete in about 0.80 seconds locally. Existing Cust behavior matched, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 56, matching Cust.

Verified commands: clean baseline `cargo test`; focused generated scalar-field pointer model test; all nine fixed-seed `fuzz_safety` tests; focused interpreter fixture; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; full required local and Docker gates (see final run report).

All required verification passed after the 2026-07-15 embedded aggregate-array field pointer-model run. `tests/fuzz_safety.rs` now generates 768 fixed-seed expression cases plus route-specific type checks over named and anonymous struct/union containers whose fields are struct/union arrays. Direct decay, indexed address-of, arrow, nested-field, and aggregate-compound-literal routes flow through pointer arithmetic, differences, conditionals, and comma expressions; the independent model checks storage owner/path/index identity, concrete pointee types, const propagation, exact field/literal bounds diagnostics, cross-owner subtraction, and panic freedom. All eight fuzz-safety tests complete in about 0.71 seconds locally. Existing Cust behavior matched after correcting test-model expectations, so this was deliberate property/conformance coverage with no production-code change. The warning-free GCC and Clang C11 fixture returns 246, matching Cust.

Verified commands: clean baseline `cargo test`; focused generated embedded-pointer model test; all eight fixed-seed `fuzz_safety` tests; focused compiler-oracle interpreter fixture and recursion-depth regression; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-15 aggregate-pointer expression model run. `tests/fuzz_safety.rs` now generates fixed-seed bounded struct/union pointer trees over named aggregate arrays and direct, arrow, and aggregate-compound-literal pointer-field read/assignment/compound-assignment routes. The independent model checks selected storage/index/type/const behavior, exact bounds/type/const/cross-array diagnostics, evaluation order, and panic freedom through pointer arithmetic, differences, conditionals, and comma expressions. RED exposed missing pointee/const metadata for arrow and aggregate-literal pointer-field assignment results and for const-pointee fields read through `->`; metadata inference now covers direct/arrow/aggregate read, assignment, and compound-assignment result variants without confusing a const containing-aggregate view with a const pointee. The warning-free GCC and Clang C11 fixture returns 12, matching Cust.

Verified commands: clean baseline `cargo test`; focused RED/GREEN assignment-result const regression; all seven fixed-seed `fuzz_safety` tests; focused aggregate pointer-field and recursion-depth regressions; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (692 interpreter tests and seven fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-15 deterministic pointer-expression model run. `tests/fuzz_safety.rs` now generates fixed-seed bounded expression trees over mutable/const scalar-array bases, pointer-plus/minus-scalar, pointer differences, conditional expressions, and comma expressions; Cust results, exact const-discard/cross-array diagnostics, and panic freedom are checked against an independent test-side model. The RED run exposed speculative pointer evaluation that misclassified scalar wrappers ending in zero and masked an earlier cross-array error. `eval_pointer_arithmetic()` now classifies operands before evaluation, and pointer-expression classification covers addressable literals plus direct/arrow/aggregate pointer-field assignment results. The warning-free GCC and Clang C11 fixture returns 50, matching Cust.

Verified commands: clean baseline `cargo test`; focused RED/GREEN model tests in `tests/fuzz_safety.rs`; minimized reverse-addition and cross-array diagnostic regressions; focused pointer-difference, pointer-arithmetic, aggregate pointer-field-lvalue, and recursion-depth regressions; direct GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (691 interpreter tests and four fuzz-safety tests); `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.
...
Verified commands: clean baseline `cargo test`; focused RED/GREEN `cargo test --test interpreter supports_addresses_of_aggregate_pointer_indexed_elements -- --nocapture` and `cargo test --test interpreter aggregate_pointer_index_address -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 chained const-qualified array-typedef compound-literal run. Scalar (`int`/`char`), named-struct, and enum arrays retain alias-carried `const` through an additional typedef layer, lower to read-only compound-literal storage, preserve full-array and indexed-element `sizeof`, flow into const pointers/parameters, support pointer arithmetic, and reject mutable pointer conversion and writes exactly. Inner mutable array typedefs shadow the outer const aliases and produce writable compound literals. The run fixed metadata-only pointee inference for scalar and aggregate array compound literals, so `sizeof(((Alias){...})[0])` now reports the element type without evaluating the literal. Warning-free GCC and Clang C11 binaries return 127, matching Cust.

Verified commands: clean baseline `cargo test`; focused RED/GREEN `cargo test --test interpreter chained_const_array_typedef_compound_literal -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; array-compound-literal `sizeof` and recursion-depth regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 chained qualified fixed-array-alias parameter conformance run. Cust preserves scalar, named-struct, and enum array element identity plus alias-carried `const` through an additional typedef layer and comma-separated mixed declarator lists. Unnamed prototypes remain compatible with named definitions, adjusted parameter pointer slots remain assignable, calls and pointer arithmetic preserve const pointees, and lexical shadowing remains correct. Exact regressions reject writes and mutable pointer declarations/arguments. Warning-free GCC and Clang C11 binaries return 127, matching Cust. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused valid/write/const-discard `cargo test --test interpreter chained_qualified_array_alias -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 named-type qualified fixed-array-alias parameter conformance run. The prior `parse_params()` correctness fix generalizes across const-qualified named struct, union, and enum array typedefs: unnamed prototypes remain compatible with named definitions, array parameters adjust to pointers whose pointees stay const, the adjusted pointer slots remain assignable inside callees, and calls preserve concrete pointee identity. Exact regressions reject writes and mutable pointer decay/arguments for all three type families; lexical alias shadowing, pointer arithmetic, `sizeof`, and `_Alignof` relationships are covered. GCC and Clang warning-free C11 binaries return 127, matching Cust. This was deliberate conformance coverage with no further production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter qualified_named_type_array_alias -- --nocapture` (first run exposed and corrected the enum-array test's expected existing diagnostic, then all three tests passed); local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds/execution; `cargo test --test c_compat -- --nocapture`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 qualified fixed-array-alias correctness run. Cust now preserves alias-carried `const` as pointee qualification when a fixed-array typedef parameter undergoes C array-to-pointer adjustment, while keeping the adjusted pointer slot assignable. This fixes false `cannot discard const qualifier from pointer target` failures for `typedef const T ConstArray[N]` parameters and applies across scalar and aggregate element types. Dedicated anonymous struct/union atomic fixtures cover const/volatile array aliases across globals, static/automatic locals, aggregate fields, unnamed prototypes plus named definitions, array decay, pointer views/arithmetic, lexical shadowing, `sizeof`, and `_Alignof`; exact negative tests retain const-write, mutable-pointer decay/argument, and multidimensional-alias diagnostics. GCC and Clang warning-free binaries return 255 for the atomic fixture and 48 for the general const-array typedef fixture, matching Cust.

Verified commands: clean baseline `cargo test`; RED/GREEN `cargo test --test interpreter atomic_anonymous_aggregate_qualified_array_aliases -- --nocapture`; focused general regression `cargo test --test interpreter supports_const_typedef_aliases -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` builds/execution for both changed compiler-oracle fixtures; `cargo test --test c_compat -- --nocapture`; recursion-depth regression; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 qualified-value-alias conformance run for anonymous aggregate atomic value aliases. Cust already preserves the original anonymous aggregate identity plus inherited `const`, `volatile`, and atomic metadata for `typedef const AtomicAnonValue ConstAtomicAnonValue`, chained aliases, and matching anonymous-union forms across globals/locals, fixed arrays, aggregate fields, named/unnamed parameters with compatible prototypes/definitions, pointer views, lexical shadowing, `sizeof`, and `_Alignof`. Exact regressions reject direct const object/array writes, pointer-view writes, const-discard conversions, and `_Atomic(QualifiedAlias)` at the alias token. GCC and Clang both execute the warning-free fixture with exit code 255, matching Cust. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter atomic_anonymous_aggregate_qualified_value_aliases -- --nocapture`; focused `_Atomic(QualifiedAlias)` rejection; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds and execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 const-qualified pointer-view conformance run for anonymous aggregate atomic value aliases. Cust already preserves the original anonymous aggregate identity plus atomic and const pointee metadata for `typedef const AtomicAnonValue *ConstAtomicAnonView` and matching anonymous-union forms across globals/locals, aggregate fields, named/unnamed parameters, compatible prototypes/definitions, mutable-to-const conversions, `sizeof`, and `_Alignof`. Exact regressions reject const-to-mutable declarations/assignments/arguments, writes through const views, const pointer-slot reassignment, pointer-to-pointer aliases, and pointer-array aliases. GCC and Clang both execute the warning-free fixture with exit code 255, matching Cust. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter atomic_anonymous_aggregate_const_pointer -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds and execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.

All required verification passed after the 2026-07-14 derived typedef-alias conformance run for anonymous aggregate atomic value aliases. Cust already preserves the original anonymous aggregate identity when pointer and fixed-array declarators are captured as `AtomicAnonPtr` / `AtomicAnonArray` aliases, including globals/locals, aggregate fields, named/unnamed parameters, compatible prototypes/definitions, pointer arithmetic, array-parameter decay, `sizeof`, and `_Alignof`. Atomic qualification remains on the pointer pointee, so `_Atomic(AtomicAnonPtr)` is valid while later const pointer-slot aliases remain non-reassignable. Exact regressions retain pointer-to-pointer, pointer-array, and multidimensional-array typedef diagnostics. GCC and Clang both execute the warning-free fixture with exit code 255, matching Cust. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter atomic_anonymous_aggregate_derived -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` fixture builds and execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; `docker compose run --rm cust`; and `git diff --check`.
...
All required verification passed after the 2026-07-13 anonymous-aggregate atomic-pointer typedef-alias conformance run. Cust already preserves one anonymous pointee identity when `_Atomic(struct { ... } *)` or `_Atomic(const union { ... } *)` is captured in a typedef alias, including comma-separated aliases, named/unnamed function parameters, compatible prototypes/definitions, global/local objects, `sizeof`, `_Alignof`, const-pointee write protection, const pointer-slot aliases, and lexical shadowing. Aliases retain top-level atomic qualification, so wrapping one in a second `_Atomic(...)` reports the exact alias-token diagnostic. GCC and Clang both compile and execute the warning-free fixture with exit code 12. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter atomic_anonymous_aggregate_pointer -- --nocapture`; focused const-slot, const-pointee, and double-atomic alias tests; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` differential probes and direct fixture execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 anonymous-aggregate atomic-pointer conformance run. Cust already accepted `_Atomic(struct { ... } *)` and `_Atomic(const union { ... } *)` across global/local declarations, aggregate fields, function parameters, `sizeof`, and `_Alignof`; the existing recursive atomic parser preserves unique anonymous pointee identities and const-pointee metadata. Focused coverage now locks in exact source-located second-star, pointer-array, and function-suffix diagnostics. GCC and Clang accept the warning-free object/field/type-query fixture with ABI-independent pointer size/alignment relationships; anonymous parameter spellings remain interpreter-only because native compilers emit visibility warnings under `-Werror`. This was deliberate conformance coverage with no production-code change.

Verified commands: clean baseline `cargo test`; focused `cargo test --test interpreter atomic_anonymous_aggregate_pointer -- --nocapture`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` differential probes and direct fixture execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 anonymous-aggregate atomic type-definition run. Cust now accepts `_Atomic(struct { ... })` and `_Atomic(union { ... })` across global/local objects, aggregate fields, function parameters, `sizeof`, and `_Alignof`. The parser reuses its existing anonymous aggregate definition path, preserving a unique internal identity for every spelling, exact qualified/nested atomic diagnostics, and deterministic no-op atomic aggregate semantics. GCC and Clang differential probes accept warning-free object/field/type-query forms; parameter-list anonymous aggregates remain interpreter-only because both native compilers emit visibility warnings under `-Werror`.

Verified commands: clean baseline `cargo test`; GCC/Clang `-std=c11 -Wall -Wextra -Werror` probes; focused RED/GREEN `cargo test --test interpreter atomic_anonymous_aggregate -- --nocapture`; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 inline named-aggregate atomic type-definition conformance run. GCC and Clang C11 probes accept `_Atomic(struct Tag { ... })` and `_Atomic(union Tag { ... })` across global/local objects, aggregate fields, function parameters, `sizeof`, and `_Alignof`; parameter-list tags are function-scoped but trigger native visibility warnings under the repository's `-Werror` flags. Cust's existing recursive atomic/aggregate parser and aggregate-tag scope stack already preserve file/block/function visibility, exact qualified/nested diagnostics, and deterministic no-op atomic aggregate behavior, so this run closes the backlog item as deliberate conformance coverage without production changes. The warning-free compiler-oracle fixture uses ABI-independent relationships between atomic objects and their same-tag aggregate types.

Verified commands: clean baseline `cargo test`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` differential probes and direct fixture execution; focused `cargo test --test interpreter atomic_inline_aggregate -- --nocapture`; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 inline-enum atomic type-definition conformance run. GCC and Clang C11 probes accept `_Atomic(enum Tag { ... })` for global/local objects, aggregate fields, and type queries; parameter-list forms are accepted but warn under the repository's `-Werror` flags. Cust's existing recursive atomic type parser and pending-inline-enum declaration routing already preserve initializer visibility, function-body parameter enumerators, block/file lexical scope, exact qualified/nested diagnostics, and deterministic enum-as-int atomic behavior, so this run closes the backlog item as deliberate conformance coverage without production changes. The compiler-oracle fixture uses warning-free object/field/type-query forms and ABI-independent relationships between equivalent atomic enum spellings.

Verified commands: clean baseline `cargo test`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` differential probes and direct fixture execution; focused `cargo test --test interpreter atomic_inline_enum -- --nocapture`; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 direct/typedef-backed atomic enum conformance run. Local GCC/Clang C11 probes confirmed that direct `_Atomic(enum State)` and typedef-backed `_Atomic(State)` forms are valid across objects, aggregate fields, function parameters, `sizeof`, and `_Alignof`; qualified enum aliases and aliases of atomic enum types remain invalid when wrapped in another `_Atomic(...)`. Cust already preserved the exact alias-token diagnostics, lexical shadowing, and deterministic enum-as-int behavior, so this run closes the backlog item as deliberate conformance coverage without production changes. The warning-free compiler-oracle fixture uses only equivalent atomic enum `sizeof`/`_Alignof` relationships because GCC and Clang differ on atomic enum compound-literal initialization.

Verified commands: clean baseline `cargo test`; local GCC/Clang `-std=c11 -Wall -Wextra -Werror` differential matrix and direct fixture execution; focused `cargo test --test interpreter atomic_enum -- --nocapture`; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 aggregate atomic-alias field/return conformance run. Local GCC/Clang C11 probes confirmed that `_Atomic(ConstPoint)` and `_Atomic(AtomicPoint)` are invalid in aggregate fields and function return declarations, while `_Atomic(View)` remains valid for an unqualified pointer alias to a const aggregate pointee. Cust already preserved the exact alias-token diagnostics and valid boundaries, so this run closes the work as deliberate conformance coverage. The interpreter-only fixture also locks in Cust's deterministic no-op treatment of supported atomic aggregate/pointer return types; the compiler-oracle fixture is restricted to warning-free aggregate-field type/size relationships because native GCC rejects atomic-qualified function returns under `-Werror=ignored-qualifiers`.

Verified commands: local GCC/Clang `-std=c11 -Wall -Wextra -Werror -fsyntax-only` differential matrix; focused aggregate atomic-alias field/return and valid-boundary tests; direct native fixture compilation/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 qualified aggregate atomic-alias signature conformance run. A local C11 differential matrix confirmed that `_Atomic(ConstPoint)` is invalid when `ConstPoint` aliases a top-level-qualified aggregate, including named and unnamed function parameters, while `_Atomic(View)` remains valid for an unqualified pointer alias to a const aggregate pointee. Aliases of `_Atomic(struct Point)` are valid ordinary signature components but invalid when wrapped in a second `_Atomic(...)`, and inner unqualified aggregate aliases correctly shadow outer qualified aliases. The focused tests passed immediately for exact invalid diagnostics, while a warning-free compiler-oracle fixture closed the valid parameter/prototype, pointer-to-atomic-aggregate, `sizeof`, and shadowing coverage without production changes.

Verified commands: clean-target CLI regression after removing a stale moved-workspace Cargo artifact; local `cc -std=c11 -Wall -Wextra -Werror -fsyntax-only` differential matrix; focused aggregate atomic-alias signature tests; direct native fixture compilation/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-13 atomic typedef-chain conformance run. Local C11 differential probes confirmed that `_Atomic(Alias)` remains invalid through chained `const`/`volatile` scalar aliases, chained qualified pointer-slot aliases, and aliases of `_Atomic(type-name)`, while mixed `typedef const int Value, *View;` correctly leaves `View` as an unqualified pointer to a qualified pointee and inner unqualified typedefs shadow outer qualified aliases. Focused tests passed immediately after correcting one expected source column, confirming the previous scope-aware metadata implementation already covered these chains; this run closes the backlog item with exact diagnostics and a compiler-oracle fixture rather than inventing production changes.

Verified commands: clean baseline `cargo test`; local `cc -std=c11 -Wall -Wextra -Werror -fsyntax-only` differential matrix; focused atomic typedef-chain and mixed-declarator tests; direct native fixture compilation/execution; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.
...
Verified commands: clean baseline `cargo test`; local `cc -std=c11 -Wall -Wextra -Werror -fsyntax-only` differential matrix; focused RED/GREEN tests for nested/leading-qualified and postfix-qualified atomic types; focused atomic regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-12 direct atomic-array type diagnostic run. Native C differential probes confirmed that direct `_Atomic(int[2])` and `_Atomic(struct Point[2])` forms are invalid, while Cust previously parsed only the nested base type and fell through to `expected ')' after _Atomic type, found LBracket`. TDD RED covered scalar and aggregate declarations, `sizeof`, `_Alignof`, and compound-literal routing. GREEN now rejects the first direct array suffix with `array _Atomic types are not supported at line ..., column ...`, while array-typedef atomic diagnostics and supported atomic pointer forms remain unchanged.

Verified commands: clean baseline `cargo test`; local `cc -std=c11 -Wall -Wextra -Werror -fsyntax-only` differential probes; focused RED/GREEN `cargo test --test interpreter rejects_direct_atomic_array_types_with_context -- --nocapture`; focused atomic-array and atomic-pointer regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-12 atomic array-typedef argument diagnostic run. Native C differential probes confirmed that `_Atomic(Scores)` is invalid when `Scores` is an array typedef, while Cust previously accepted it as an ordinary array in declarations, `sizeof`, `_Alignof`, and compound-literal routing. TDD RED captured all four accepted-invalid forms. GREEN now rejects the atomic array argument at the alias token with `array _Atomic types are not supported at line ..., column ...`, while ordinary array aliases and supported `_Atomic(T *)` forms remain unchanged.

Verified commands: clean baseline `cargo test`; local `cc -std=c11 -Wall -Wextra -Werror -fsyntax-only` differential probes for declaration, `sizeof`, `_Alignof`, and compound-literal forms; focused RED/GREEN `cargo test --test interpreter rejects_atomic_array_typedef_arguments_with_context -- --nocapture`; focused atomic-pointer and ordinary-array-alias regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-12 C11 atomic-pointer type-specifier run. Deterministic suffix probing found that native C accepts `_Atomic(int *)`, while Cust stopped at the inner scalar type and reported `expected ')' after _Atomic type, found Star`. TDD RED captured the missing pointer type. GREEN now parses one-level scalar and aggregate pointers inside `_Atomic(type-name)`, preserves `const` pointee metadata, supports aliases/parameters/declarations plus `sizeof` and `_Alignof`, and gives source-located diagnostics for pointer-to-pointer, pointer-array, and function suffixes inside `_Atomic(...)`.

Verified commands: clean baseline `cargo test`; focused RED/GREEN `cargo test --test interpreter supports_atomic_pointer_type_specifiers -- --nocapture`; focused RED/GREEN `cargo test --test interpreter rejects_unsupported_atomic_pointer_suffixes_with_context -- --nocapture`; atomic regression filter; native `cc -std=c11 -Wall -Wextra -Werror` probe; `cargo test --test c_compat -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-12 void array/function type-name diagnostic run. The deterministic suffix matrix evaluated scalar, enum, aggregate, anonymous-aggregate, `_Atomic`, array-typedef, pointer-typedef, and `void` bases against pointer, array, and function suffixes across casts, `sizeof`, and `_Alignof`. It isolated the `void` early branches: `(void(void))0` fell through to a generic closing-parenthesis error, while `sizeof(void(void))` and `sizeof(void[2])` misleadingly reported only the bare `sizeof(void)` rejection. TDD RED captured the function-cast fallback. GREEN now checks void pointers first, then reports source-located shared function-type diagnostics or route-specific void-array diagnostics at the suffix, while preserving `(void)expr`, bare `sizeof(void)` / `_Alignof(void)`, and all existing void-pointer diagnostics.

Verified commands: clean baseline `cargo test`; focused RED/GREEN `cargo test --test interpreter rejects_void_array_and_function_type_names_with_context -- --nocapture`; focused void-pointer, bare-void type-query, and ordinary function-type regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.

All required verification passed after the 2026-07-12 pointer-to-array cast/type-query diagnostic run. The first unchecked TODO probe confirmed that direct array type names followed by `*`—including `(int[2]*)0`, `(struct Point[2]*)0`, `sizeof(int[2]*)`, and `_Alignof(struct Point[2]*)`—fell through to generic closing-parenthesis diagnostics even though array typedef aliases already reported Cust's intended pointer-to-array boundary. TDD RED captured `expected ')' after cast type, found Star`. GREEN now applies source-located `pointer-to-array casts/sizeof/_Alignof types are not supported` diagnostics after every direct scalar, named aggregate, and anonymous aggregate array suffix while preserving multidimensional-array, function-array, pointer-array, and array-typedef behavior.

Verified commands: baseline `cargo test`; focused RED/GREEN `cargo test --test interpreter rejects_pointer_to_array_cast_and_type_query_names_with_context -- --nocapture`; focused multidimensional-array, function-array, pointer-array cast, and pointer-array type-query regressions; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test`; `docker compose run --rm test`; and `docker compose run --rm cust`.
