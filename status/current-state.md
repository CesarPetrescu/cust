...
Work-package candidates were the first deterministic standalone integer object representation, union-backed raw storage, whole-aggregate layout, and further parser diagnostics. The integer slice was selected because it was first in both authoritative queues, completed all five intrinsics over a high-value scalar representation without host ABI inference, and had bounded acceptance tests. Whole aggregates and unions remain deferred because Cust does not model padding or union byte aliasing. The next package is bounded v0.25.0 release closure around this completed slice.
...
The next concrete package is a deterministic standalone `int` object-representation slice for bounded raw-memory intrinsics. It must define byte order and partial-write reconstruction explicitly, preserve Cust's fixed eight-byte integer model plus interpreter owner/lifetime/const/capacity checks, avoid host addresses and host ABI inference, and limit native-oracle assertions to portable relationships.
