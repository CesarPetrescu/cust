...
With release publication complete, the next concrete implementation package is deterministic whole-struct object bytes under Cust's existing no-padding aggregate size model. Union-backed storage remains a separate rejected boundary until Cust has shared member storage/aliasing semantics.
