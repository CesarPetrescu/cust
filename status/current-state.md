...
All required verification passed after the 2026-07-11 aggregate-initializer diagnostic run. Ideation evaluated clean-baseline failures (none), active blockers (none), the parser-trust continuation, conformance/fuzz coverage, and CLI/documentation work. A focused malformed-source probe found that an array-designator start in an aggregate compound literal (`(struct Point){[}`) fell through to generic `expected expression, found LBracket`. TDD RED added an exact-output interpreter regression and observed that failure; GREEN adds a narrow `parse_struct_initializer()` guard, reporting `expected initializer element in struct 'Point' initializer, found LBracket` at the bracket. Array designators remain valid only in array initializer contexts, while aggregate field designators remain dot-led.

Verified commands: focused RED/GREEN `cargo test --test interpreter rejects_array_designator_starts_in_struct_compound_literal_initializers_with_context -- --nocapture`; `cargo fmt --check`; `cargo clippy -- -D warnings`; `cargo test` (601 interpreter tests, all suites); `docker compose run --rm test`; and `docker compose run --rm cust`.
