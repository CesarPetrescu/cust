...
The bounded v0.32.0 package is prepared around completed direct scalar `double` aggregate fields. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and authoritative status ledgers are synchronized for `0.32.0`. Executable target listing independently reconciles 1,642 tests: 1,507 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. Exact CLI and Compose expectations failed against `0.31.0` before metadata moved to `0.32.0`, then passed; Cargo metadata and the built CLI report `cust 0.32.0`; local and remote annotated-tag preflight is empty. Independent review, the canonical gate, release-commit push, and annotated-tag publication remain pending, so `v0.32.0` is not yet claimed as published.

Candidate evaluation considered bounded v0.32.0 release closure, direct one-dimensional double arrays in aggregate fields, direct double pointers, and parser/property expansion. Release closure was selected because it is first in both authoritative queues, atomically packages the already reviewed aggregate-field slice, and has bounded consistency risk with exact executable version tests. Aggregate-field double arrays are preserved as the next coherent language package after publication; direct double pointers remain a larger safety and metadata expansion.
...
Candidate evaluation considered finishing the inherited direct-double-field package, immediate v0.32.0 release closure, extending double storage further, and parser/property expansion. Recovery priority and the queue-leading coherent language slice selected the inherited package; release closure is now first. Executable listing reconciles 1,642 tests: 1,507 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. The 47 focused `direct_double_` tests, actual compiler oracle, formatting, warning-denied Clippy, all 1,642 local tests, the rebuilt 1,642-test Docker gate, runtime output `10`, and `git diff --check` pass.
