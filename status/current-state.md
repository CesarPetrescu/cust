...
Latest published release is bounded v0.12.0: commit `6bcd1bd` and annotated tag `v0.12.0` are present on `origin/main`. The next package is v0.13.0 release synchronization/publication for `strcoll`/`strxfrm`; `strtol`/`strtoul` remain deferred behind safe pointer-to-pointer `endptr` support.
