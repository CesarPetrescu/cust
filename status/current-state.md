...
The bounded Cust v0.4.0 release package is prepared for atomic publication. Cargo package/lock metadata, exact CLI output, Docker Compose runtime/test image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized at `0.4.0`. The release captures the completed bounded preprocessing roadmap: object/function/variadic macros, definition lifetime, conditionals, physical-line splicing, secure project-relative quoted headers, stringification, and token pasting. The canonical local and Docker gates pass with 1,110 tests (990 interpreter, 98 fuzz-safety, 18 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test). The annotated `v0.4.0` tag is reserved for this verified release commit and must be published only after `origin/main` accepts it.
