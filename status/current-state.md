...
Candidate evaluation considered finishing the inherited aggregate-double package, bounded v0.33.0 release closure, direct `double *` support, shared-storage union bytes, and parser/property expansion. Recovery rules and correctness priority selected the existing package: it was already a substantial uncommitted vertical slice, while each alternative depends on landing it safely. Bounded v0.33.0 release closure is the next recommended package after this feature is verified and pushed.
