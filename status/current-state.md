...
Latest published release remains bounded v0.12.0: commit `6bcd1bd` and annotated tag `v0.12.0` are present on `origin/main`. The v0.13.0 release package is prepared above but is not yet published; `strtol`/`strtoul` remain deferred behind safe pointer-to-pointer `endptr` support.
