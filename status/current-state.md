...
The Cust v0.6.0 package is prepared for atomic publication. Cargo package/lock metadata, exact CLI output, both Docker Compose image tags, README support/limitations/roadmap text, changelog release notes, and all five status files are synchronized for `0.6.0`. The release packages C11 null directives, opened-identity `#pragma once`, and macro-expanded `_Pragma("once")`. Focused version assertions went RED on `0.5.0` and GREEN on `0.6.0`; the annotated `v0.6.0` tag is reserved for this verified release commit and must be created only after `origin/main` accepts it.

Release-preparation evidence: clean baseline `cargo test` (1,144 tests); focused CLI and Compose metadata RED/GREEN; `cargo check`; release-surface audit against v0.5.0; and local/remote non-conflicting tag preflight. Independent review and the canonical local/Docker gate are required before commit publication.
