...
- Top-level/local supported aggregate declarations can use brace initializers or same-type aggregate-returning call expressions such as `struct Point p = make_point(1, 2);` and `union Number n = make_number(5);`, preserving by-value copies and const binding enforcement after initialization.
