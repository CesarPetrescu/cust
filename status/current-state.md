...
Bounded Cust v0.27.0 release metadata is prepared around the completed two-dimensional non-character scalar-row object-byte package. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README central inventory/limitations/roadmap text, changelog notes, and the authoritative status ledgers are synchronized for `0.27.0`. Executable listing independently reconciles 1,479 tests: 1,344 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only exact CLI and Compose expectations first: both failed against package/image version `0.26.0`, then passed after Cargo/lock and both image tags moved to `0.27.0`. Cargo metadata reports `cust 0.27.0`; local and remote annotated-tag preflight found no conflicting `v0.27.0`. Work-package candidates were bounded v0.27.0 release closure, deterministic whole-struct object bytes, union-backed bytes, and further parser diagnostics. Release closure was selected because it was the sole unchecked roadmap item at selection time and atomically packages the already reviewed row slice with bounded consistency risk. Fresh independent complete-diff review returned `APPROVED`; formatting, warning-denied Clippy, all 1,479 local tests, doc tests, focused version/license/compiler-oracle tests, the rebuilt 1,479-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.27.0`, tag preflight, and `git diff --check` pass. The annotated tag remains reserved for creation only after the release commit reaches exact `origin/main`.

After release publication, the next concrete implementation package is deterministic whole-struct object bytes under Cust's existing no-padding aggregate size model. Union-backed storage remains a separate rejected boundary until Cust has shared member storage/aliasing semantics.
