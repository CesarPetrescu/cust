...
Bounded Cust v0.30.0 release preparation packages the completed direct `double` function-boundary slice. Cargo package/lock metadata, exact CLI output and tests, both Docker Compose image tags, README central support/limitations/roadmap text, changelog notes, and the authoritative status ledgers are synchronized for `0.30.0`. Executable target listing independently reconciles 1,594 tests: 1,459 interpreter, 99 deterministic fuzz-safety, 32 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD changed only the exact CLI and Compose expectations first: both failed against package/image version `0.29.0`, then passed after Cargo/lock and both image tags moved to `0.30.0`. Cargo metadata and the built CLI report `cust 0.30.0`; local and remote annotated-tag preflight found no conflicting `v0.30.0`. Candidate evaluation considered bounded v0.30.0 release closure, direct one-dimensional double arrays, shared-storage union bytes, and parser/fuzz expansion. Release closure was selected because it is first in both authoritative queues and atomically packages the independently reviewed function-boundary slice without widening runtime semantics. Direct one-dimensional double arrays are promoted as the next concrete language package.
