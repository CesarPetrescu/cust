...
Bounded Cust v0.16.0 is prepared for atomic publication around safe `char **` expression parity. Cargo package/lock metadata, exact CLI expectations/output, both Docker Compose image tags, README support/limitations/roadmap text, changelog notes, and all five status files are synchronized for `0.16.0`. The executable release inventory is 1,343 tests: 1,209 interpreter tests, 98 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

Release TDD evidence: exact CLI and Compose expectations first failed against package/image version `0.15.0`, then passed after Cargo/lock and both image tags moved to `0.16.0`. Local and remote annotated-tag preflight found no `v0.16.0`. Independent read-only review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 1,343 local tests, the rebuilt 1,343-test Docker gate, runtime output `10`, local/Cargo/container version `cust 0.16.0`, security scan, and `git diff --check` pass. The annotated tag remains reserved for this verified release commit and may be created only after `origin/main` accepts it; publication and remote peeled-target verification are not claimed by this preparation snapshot.
