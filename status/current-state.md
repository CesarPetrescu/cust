...
The inherited direct one-dimensional `double` aggregate-field package is not yet complete and remains uncommitted. This run used focused RED/GREEN to close the previously recorded reverse owner/index pointer-boundary and non-evaluating scalar-condition defects, then added const-root mutation, reverse address escape, wrapped-base, and non-evaluating unary/cast regressions. All 83 `direct_double_` tests, four focused reverse tests, the focused scalar-operator regression, and the actual compiler oracle pass. The final allowed independent review still found a distinct const-integrity bypass: assignment, compound assignment, and increment through conditional/comma/`_Generic` wrapped reverse bases can mutate both const aggregate arrays and pointer-to-const roots, including beneath `sizeof`. No canonical gate or feature commit/push is claimed. See `status/stuck.md` for exact reproductions and the safe continuation.
