...
Bounded v0.41.0 release preparation packages the completed aggregate-field binary64 object-byte slice. Version-first TDD changed only the exact CLI and Compose expectations: both failed against `0.40.0`, then passed after Cargo/lock and both image tags moved to `0.41.0`. Executable target-by-target listing independently reconciles 2,089 tests: 1,953 interpreter, 99 deterministic fuzz-safety, 33 CLI, 2 Docker metadata, 1 compiler-oracle harness, and 1 repository-license test. README central inventory/limitations/roadmap and changelog release notes distinguish supported scalar/one-dimensional-array double fields from still-unsupported two-dimensional, union-backed, pointer-object, unprovable-target, and whole-aggregate-with-double layouts. Fresh independent complete-diff review returned `APPROVED` with no findings. Formatting, warning-denied Clippy, all 2,089 local tests, a no-cache rebuild, all 2,089 rebuilt-Docker tests, runtime output `10`, exact local/runtime-image/test-image versions, and diff hygiene pass. The annotated `v0.41.0` tag remains reserved for creation only after the verified release commit reaches exact `origin/main`; publication and peeled-target verification are not claimed by this preparation diff.

Candidate evaluation considered bounded v0.41.0 release closure, selected two-dimensional binary64 rows, shared union-byte storage, and parser/property expansion. Release closure was selected because it is the authoritative queue-leading package and atomically packages an already reviewed ten-test feature slice with exact executable version assertions and bounded metadata risk. One selected row of supported two-dimensional `double` storage is the next concrete implementation package after release; shared union bytes remain deferred because they require true aliased storage.
...
Candidate evaluation considered this queue-leading aggregate-field binary64 package, selected two-dimensional binary64 rows, shared union-byte storage, parser/property expansion, and immediate release closure. Aggregate fields were selected because they reuse mature field provenance and deterministic serialization while completing all five intrinsics as one bounded vertical slice. A bounded v0.41.0 release closure is next; selected two-dimensional rows remain the adjacent implementation package after release, while union bytes still require architectural shared storage.
