...
All required verification passed after the 2026-07-20 outer-pointer-forwarded wrapped direct aggregate-array literal adjusted-parameter run. Two hundred sixteen generated scalar/named-aggregate two-writer/const-reader cases balance same-element, same-array-distinct, and separate-root relationships; conditional/comma wrappers; all three nonzero offsets; and one/two-hop `struct Item *` helpers before and after offset formation. A separate 36-case const-array-typedef matrix covers every wrapper/offset/`const struct Item *` helper combination. Inner forwarding, selected/unselected/comma markers, hidden roots, and copied slots preserve root/base/outer/inner identity. Fourteen panic-guarded inner/absolute bounds, const, cross-root, concrete type, and out-of-scope lifetime diagnostics pass. The warning-free fixture returns 49 under Cust, GCC, and Clang. The suite now has 51 fuzz-safety tests and 842 interpreter tests.
