...
Whole-object byte ranges for already admitted scalar-only unions are complete across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Whole unions use maximum-layout capacity and the deterministic full-width writable carrier; selected-member aliases stay synchronized, nested and aggregate-array-element routes retain canonical identity and isolation, and bounds/const/lifetime/overlap/zero-count/non-evaluation checks remain exact. Unsupported all-`_Bool`, pointer, `double`, array, nested-aggregate, and full-width-const-only layouts retain targeted diagnostics. TDD began with the exact whole-union rejection, then independent review drove RED/GREEN closure for nested writable ranges, typed `memchr` canonicalization, stale whole-struct boundary tests, and same-type const-member ordering. Fresh complete-diff re-review returned `APPROVED`. Formatting, warning-denied Clippy, all 2,121 local tests (1,985 interpreter + 99 deterministic fuzz-safety + 33 CLI + 2 Docker metadata + 1 compiler-oracle harness + 1 repository-license test), all 2,121 rebuilt-Docker tests, runtime output `10`, GCC/Clang warning-denied fixture execution, and diff hygiene pass. Bounded v0.45.0 release closure became next at that point and is published above.
