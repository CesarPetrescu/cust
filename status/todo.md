...
4. Consider additional expression-level C-subset features only with clear acceptance fixtures; assignment expressions, compound assignment, increment/decrement, the conditional operator, and do-while loops are now covered, so future expression/control work should avoid regressing exact delimiter diagnostics such as missing `]` inside index expressions.
