...
5. [ ] Add `sizeof` support as the next expression-level C-subset feature; acceptance: lexer/parser/interpreter handle `sizeof(int)`, `sizeof(char)`, `sizeof(pointer-expression)`, `sizeof scalar`, `sizeof array`, and `sizeof string literal` with defined Cust sizes and C compiler-oracle coverage where host sizes are stable, plus exact diagnostics for unsupported forms such as `sizeof(void)` if not implemented.
