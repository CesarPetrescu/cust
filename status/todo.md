...
9. Consider additional C-subset features only with clear acceptance fixtures; good candidates to design first include function prototypes/declarations, enums, or a deliberately scoped richer pointer milestone. Assignment expressions, arithmetic compound assignment, bitwise/shift compound assignment, increment/decrement, bitwise/shift operators, the conditional operator, the comma operator, switch statements, do-while loops, block comments, void helper functions, global variables, `sizeof`, and uninitialized scalar/pointer declarations are now covered, so future expression/control work should avoid regressing exact delimiter diagnostics such as missing `]` inside index expressions.
