...
1. Continue parser recovery/error-message expansion only for newly discovered malformed programs not already covered by the exact-error suite; acceptance: add focused exact-output parser tests before implementation for nested block/EOF diagnostics, pointer-parameter malformed-list diagnostics, array length edge diagnostics, or other delimiter/list boundary errors that still fall through to generic messages.
