...
1. Add release notes/tags once the current v0.1 surface is judged ready; acceptance: changelog/release notes summarize CLI flags, diagnostics, arrays/strings/pointers, conformance fixtures, and Docker verification.
2. Continue parser recovery/error-message expansion only for newly discovered malformed programs not already covered by the exact-error suite; acceptance: add focused exact-output parser tests before implementation for pointer-parameter malformed-list diagnostics, additional delimiter/list boundary errors, or other malformed programs that still fall through to generic messages.
