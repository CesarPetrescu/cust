...
- 2026-05-04: No external documentation was needed for first-pass C compatibility testing. Implementation uses the system `$CC` or discovers `gcc`, `clang`, or `cc`, compiles supported fixture programs as C11 with `-Wall -Wextra -Werror`, and compares the native process exit code with Cust's interpreter result; the Docker build stage now installs `gcc` and `libc6-dev` so the compatibility suite runs in `docker compose run --rm test`.
