...
- 2026-05-04: User clarified Cust must remain an interpreter-only project: verification should not compile user C programs with GCC/Clang/cc or rely on clangd. Removed native compiler compatibility tests and Docker compiler packages; future conformance tests should encode expected behavior directly against `cust::interpret` or the `cust` CLI.
