...
fn bounded_memory_scalar_object_bytes_preserve_safety_diagnostics() {
    let cases = [
        ...
        (
            "void *memset(void *, int, unsigned long int); struct Buffer { int values[2]; }; int main(void) { struct Buffer value = {{1, 2}}; memset(value.values + 1, 0, sizeof(value.values)); return 0; }",
...
#[test]
fn aggregate_non_character_scalar_fields_support_memory_operations() {
    let program = r#"
        void *memcpy(void *, const void *, unsigned long int);
        ...
        void *memset(void *, int, unsigned long int);
        void *memchr(const void *, int, unsigned long int);
        struct Pair { int left; int right; } value = {0x0102030405060708, 0x1112131415161718};
        int main(void) {
            if (memcpy(&value.right, &value.left, sizeof(value.left)) != &value.right) return 1;
            if (value.right != value.left) return 2;
            if (memcmp(&value.left, &value.right, sizeof(value.left)) != 0) return 3;
            if (memset(&value.right, 0, sizeof(value.right)) != &value.right) return 4;
            if (value.right != 0) return 5;
            void *found = memchr(&value.left, 0x06, sizeof(value.left));
            if (found == 0 || (char *)found - (char *)&value.left != 2) return 6;
            if (memmove(&value.right, &value.left, sizeof(value.left)) != &value.right) return 7;
...
#[test]
fn aggregate_scalar_object_byte_results_preserve_field_identity() {
    let program = r#"
        void *memchr(const void *, int, unsigned long int);
        struct Cell { int scalar; int values[2]; };
        struct Grid { struct Cell rows[2]; };
        int main(void) {
            struct Cell value = {0x22, {0, 0x22}};
            int *scalar = memchr(&value.scalar, 0x22, sizeof(value.scalar));
            if (scalar != &value.scalar) return 1;
            int *array = memchr(value.values, 0x22, sizeof(value.values));
            if (array != value.values + 1) return 2;
            char *interior = memchr(&value.scalar, 0, sizeof(value.scalar));
            if (interior == 0 || interior - (char *)&value.scalar != 1) return 3;
            interior[0] = 1;
            if (value.scalar != 0x0122) return 4;

            struct Grid grid = {{{0x11, {0, 0}}, {0x22, {0, 0}}}};
            int *embedded = memchr(&grid.rows[1].scalar, 0x22, sizeof(grid.rows[1].scalar));
            if (embedded != &grid.rows[1].scalar) return 5;
            return 0;
        }
    "#;
...
#[test]
fn aggregate_non_character_scalar_fields_preserve_memory_safety() {
    let cases = [
        (
            "void *memset(void *, int, unsigned long int); struct Inner { int scalar; }; struct Outer { struct Inner inner; }; int main(void) { const struct Outer value = {{1}}; memset(&value.inner.scalar, 0, sizeof(value.inner.scalar)); return 0; }",
            "cannot discard const qualifier from pointer target",
        ),
        (
            "void *memset(void *, int, unsigned long int); struct Cell { int scalar; }; struct Grid { struct Cell rows[1]; }; int main(void) { const struct Grid value = {{{1}}}; memset(&value.rows[0].scalar, 0, sizeof(value.rows[0].scalar)); return 0; }",
            "cannot discard const qualifier from pointer target",
        ),
        (
            "void *memcpy(void *, const void *, unsigned long int); struct Buffer { int values[3]; }; int main(void) { struct Buffer value = {{1, 2, 3}}; memcpy(value.values + 1, value.values, sizeof(int) * 2); return 0; }",
            "overlapping source and destination ranges passed to function 'memcpy'",
        ),
        (
            "void *memchr(const void *, int, unsigned long int); void *saved; struct Cell { int scalar; }; int main(void) { { struct Cell local = {1}; saved = memchr(&local.scalar, 1, sizeof(local.scalar)); } return memchr(saved, 1, 1) != 0; }",
            "pointer to out-of-scope variable 'local'",
        ),
        (
...
#[test]
fn sizeof_aggregate_scalar_object_byte_calls_is_non_evaluating() {
    let program = r#"
        void *memcpy(void *, const void *, unsigned long int);
        void *memset(void *, int, unsigned long int);
        struct Cell { int source; int destination; } value = {1, 2};
        int marker = 0;
        void *destination(void) { marker += 1; return &value.destination; }
        const void *source(void) { marker += 2; return &value.source; }
        int count(void) { marker += 4; return sizeof(value.source); }
        int main(void) {
            if (sizeof(memcpy(destination(), source(), count())) != sizeof(void *)) return 1;
            if (sizeof(memset(destination(), marker++, count())) != sizeof(void *)) return 2;
            return marker != 0 || value.destination != 2;
        }
    "#;
...
#[test]
fn aggregate_scalar_object_byte_routes_match_compiler_oracle_fixture() {
    let program =
        include_str!("fixtures/compat/valid/bounded_memory_aggregate_scalar_object_bytes.c",);
...
#[test]
fn character_pointer_object_reassignment_preserves_static_and_parameter_slot_semantics() {
    let program = r#"
char *global_first = 0;
char *global_second = 0;
char **global_output = 0;

void select_output(char **output, char **replacement, char *value) {
    output = replacement;
    *output = value;
}
...
int main(void) {
    char text[] = "abcd";
    global_output = &global_first;
    *global_output = text;
    select_output(global_output, &global_second, text + 2);

    return global_output == &global_first && *global_first == 'a' &&
           *global_second == 'c' && update_static(text) == 'a' + 'b' ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_objects_reassign_between_null_slots_and_objects() {
    let program = r#"
int main(void) {
    char text[] = "abcd";
    char *first_slot = 0;
    ...
    *output = text + 2;
    ...
    return output == 0 && !output && *first_slot == 'c' && *second_slot == 'b' ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_expressions_preserve_selected_identity_once() {
    let program = r#"
int markers = 0;
...
int main(void) {
    ...
    char **selected = (output = (mark(), &second_slot));

    set_output(output = first, text + 2);
    *selected = text + 1;
    return markers == 1 && output == first && selected == &second_slot &&
           *first_slot == 'c' && *second_slot == 'b' ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_results_support_indirect_reads_and_writes() {
    let program = r#"
int main(void) {
    char text[] = "abcd";
    char *slot = 0;
...
#[test]
fn character_pointer_object_assignment_results_normalize_in_bool_initializers() {
    let program = r#"
int main(void) {
    char *slot = 0;
    char **output = 0;

    _Bool non_null = (output = &slot);
    _Bool null_value = (output = 0);
    return non_null == 1 && null_value == 0 && output == 0 ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_sizeof_is_non_evaluating_and_pointer_sized() {
    let program = r#"
int markers = 0;
...
int main(void) {
    char *first_slot = 0;
    char *second_slot = 0;
    char **output = &first_slot;
    int size = sizeof(output = (mark(), &second_slot));

    return size == sizeof(char *) && markers == 0 && output == &first_slot ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_sizeof_does_not_observe_expired_targets() {
    let program = r#"
char **output = 0;
...
int main(void) {
    capture();
    return sizeof(output = output) == sizeof(char *) ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_condition_validation_preserves_runtime_order() {
    let program = r#"
char **dangling = 0;

void capture(void) {
    char *expired_slot = 0;
    dangling = &expired_slot;
}
...
int main(void) {
    ...
    output = ((dangling = &live_slot), dangling) ? &first_slot : &second_slot;
    return output == &first_slot &&
           sizeof(output = (dangling ? &first_slot : &second_slot)) == sizeof(char *) ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_comma_rhs_repairs_liveness_before_observation() {
    let program = r#"
char **dangling = 0;
...
int main(void) {
    ...
    output = (dangling = &live_slot, dangling);
    return output == &live_slot ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_assignment_in_strtol_sizeof_does_not_observe_liveness() {
    let program = r#"
long int strtol(const char *text, char **end, int base);
char **output = 0;
...
int main(void) {
    capture();
    return sizeof(strtol("7", output = output, 10)) == sizeof(long int) ? 0 : 1;
}
"#;
...
#[test]
fn character_pointer_object_indirect_assignment_sizeof_validates_conditional_pointer() {
    let program = r#"
int main(void) {
    char text[] = "x";
    char *slot = 0;
    char *ok = text;
    char **output = 0;
    return sizeof(*(1 ? (output = &slot) : 1) = ok);
}
"#;
    ...
    assert_eq!(
        err.to_string(),
        "function 'character pointer output dereference' parameter 'output' requires a char pointer slot address"
    );
}

#[test]
fn character_pointer_object_assignment_sizeof_validates_scalar_wrappers() {
    for expression in [
        "!(output = wrong)",
        "(output = wrong) == 0",
        "(int)(output = wrong)",
        ...
        let program = format!(
            "int main(void) {{ int value = 0; int *wrong = &value; char *slot = 0; char **output = &slot; return sizeof({expression}); }}\n"
        );
        let err = interpret(&program).unwrap_err();
        assert_eq!(
            err.to_string(),
            "character pointer object 'output' assignment requires null, another character pointer output object, or the address of a mutable char pointer variable",
...
#[test]
fn character_pointer_object_compound_updates_and_increment_are_rejected() {
    for operation in [
        "output += 1",
        "output++",
        "++output",
        ...
        let program = format!(
            "int main(void) {{ char *slot = 0; char **output = &slot; {operation}; return 0; }}\n"
        );
        let err = interpret(&program).unwrap_err();
        assert_eq!(
            err.to_string(),
...
#[test]
fn character_pointer_object_reassignment_rejects_incompatible_and_qualified_slots() {
    for (declarations, expression) in [
        ("int value = 0; int *wrong = &value;", "wrong"),
        ("int value = 0;", "&value"),
        ("const char *qualified = 0;", "&qualified"),
        ("char * const fixed = 0;", "&fixed"),
        ("volatile char *qualified = 0;", "&qualified"),
        ("char * volatile qualified = 0;", "&qualified"),
        ("char * restrict qualified = 0;", "&qualified"),
        ("_Atomic(char) *qualified = 0;", "&qualified"),
        ("char * _Atomic qualified = 0;", "&qualified"),
        ("char *ok = 0;", "1"),
    ] {
        for operation in [
            format!("output = {expression}"),
            format!("sizeof(output = {expression})"),
            format!("sizeof(*(output = {expression}))"),
            format!("sizeof(**(output = {expression}))"),
        ] {
            let program = format!(
                "int main(void) {{ char *slot = 0; char **output = &slot; {declarations} {operation}; return 0; }}\n"
            );
            let err = interpret(&program).unwrap_err();
            assert_eq!(
                err.to_string(),
...
#[test]
fn character_pointer_object_reassignment_rejects_expired_target_slots() {
    let program = r#"
char **output = 0;
...
"#;
    ...
    assert_eq!(
        err.to_string(),
        "pointer to out-of-scope variable 'local_slot'"
    );
}

#[test]
fn character_pointer_object_reassignment_observes_only_selected_conditional_target() {
    let valid_program = r#"
char **dangling = 0;
...
"#;
    ...
    let invalid_program = valid_program.replace("0 ? dangling", "1 ? dangling");
    let err = interpret(&invalid_program).unwrap_err();
    assert_eq!(
        err.to_string(),
        "pointer to out-of-scope variable 'expired_slot'"
    );
}
...
#[test]
fn character_pointer_output_parameter_reassignment_is_local_to_the_callee() {
    let program = r#"
int redirect_local(char **out, char **replacement, char *value) {
    out = replacement;
    *out = value;
    out = 0;
    return out == 0;
...
int main(void) {
    ...
    int redirected = redirect_local(output, &second, text + 1);
    return redirected && output == &first && first == 0 && *second == 'b' ? 0 : 1;
}
"#;
...
#[test]
fn adjusted_aggregate_parameter_wrapped_direct_compound_literal_alias_routes_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_wrapped_direct_compound_literal_alias_routes.c",
    );

    assert_eq!(interpret(program).unwrap(), 69);
}

#[test]
fn adjusted_aggregate_parameter_wrapped_direct_compound_literal_offset_routes_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_wrapped_direct_compound_literal_offset_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_post_forward_wrapped_direct_compound_literal_routes_match_fixture()
{
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_post_forward_wrapped_direct_compound_literal_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_post_wrapper_reforwarded_direct_compound_literal_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_post_wrapper_reforwarded_direct_compound_literal_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_mutable_to_const_reforwarded_direct_compound_literal_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_mutable_to_const_reforwarded_direct_compound_literal_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_wrapped_compound_literal_field_offset_routes_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_wrapped_compound_literal_field_offset_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_wrapped_compound_literal_field_outer_forwarding_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_wrapped_compound_literal_field_outer_forwarding_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_post_forward_wrapped_compound_literal_field_routes_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_post_forward_wrapped_compound_literal_field_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_mutable_to_const_reforwarded_compound_literal_field_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_mutable_to_const_reforwarded_compound_literal_field_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_derived_inner_pointer_const_promotion_routes_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_derived_inner_pointer_const_promotion_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_direct_literal_derived_inner_pointer_const_promotion_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_direct_literal_derived_inner_pointer_const_promotion_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_direct_literal_derived_inner_pointer_const_reforward_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_direct_literal_derived_inner_pointer_const_reforward_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_captured_field_derived_inner_pointer_const_reforward_routes_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_captured_field_derived_inner_pointer_const_reforward_routes.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_callee_internal_derived_inner_pointer_returns_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_callee_internal_derived_inner_pointer_returns.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_caller_reforwarded_derived_inner_pointer_returns_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_caller_reforwarded_derived_inner_pointer_returns.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_final_receiving_caller_reforwarded_derived_inner_pointers_match_fixture()
 {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_final_receiving_caller_reforwarded_derived_inner_pointers.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_const_parameter_return_selection_matches_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_const_parameter_return_selection.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_post_selection_wrappers_and_offsets_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_post_selection_wrappers_and_offsets.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_post_selection_reforward_return_boundaries_match_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_post_selection_reforward_return_boundaries.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_distinct_root_final_reselection_matches_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_distinct_root_final_reselection.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_distinct_root_selected_result_carry_matches_fixture() {
    let program = include_str!(
        "fixtures/valid/adjusted_aggregate_parameter_distinct_root_selected_result_carry.c",
    );
...
#[test]
fn adjusted_aggregate_parameter_fresh_distinct_root_reselection_matches_fixture() {
    let program = include_str!(
