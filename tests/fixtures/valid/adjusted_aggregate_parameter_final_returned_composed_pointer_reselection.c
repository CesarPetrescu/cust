...
const struct Point *reselect_point_twice(
    const struct Point *first,
    const struct Point *second,
    int select_first
) {
    aggregate_final_selection_calls = aggregate_final_selection_calls + 1;
    return reselect_point(first, second, select_first);
}
...
int main(void) {
    ...
    const int *int_reselected = reselect_int(int_returned, int_selected, 0);
    ...
    const struct Point *point_reselected = reselect_point_twice(
        point_returned,
        point_selected,
        1
    );
    ...
    return (*int_selected == 9) + (int_selected == int_final)
        ...
        + (int_reselected == int_selected) + (*int_reselected == 9)
        + (int_reselected - int_base == 1) + (int_returned == int_composed)
        + (int_selected == int_final) + (scalar_final_selection_calls == 1)
        + (point_reselected == point_returned) + (point_reselected->value == 9)
        + (point_reselected - point_base == 1)
        + (aggregate_final_selection_calls == 2);
}
