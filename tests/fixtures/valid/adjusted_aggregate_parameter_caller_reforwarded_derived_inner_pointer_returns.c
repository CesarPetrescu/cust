...
int d_first_comma;
int d_second_comma;
int a_caller_reforward_calls;
int b_caller_reforward_calls;
int c_caller_reforward_calls;
int d_caller_reforward_calls;
...
const int *caller_a_reforward_int(const int *value) {
    a_caller_reforward_calls = a_caller_reforward_calls + 1;
    return value;
}

const int *caller_b_reforward_int(const int *value) {
    b_caller_reforward_calls = b_caller_reforward_calls + 1;
    return value;
}

const int *caller_b_reforward_int_twice(const int *value) {
    b_caller_reforward_calls = b_caller_reforward_calls + 1;
    return caller_b_reforward_int(value);
}

const struct Point *caller_c_reforward_point(const struct Point *value) {
    c_caller_reforward_calls = c_caller_reforward_calls + 1;
    return value;
}

const struct Point *caller_d_reforward_point(const struct Point *value) {
    d_caller_reforward_calls = d_caller_reforward_calls + 1;
    return value;
}

const struct Point *caller_d_reforward_point_twice(const struct Point *value) {
    d_caller_reforward_calls = d_caller_reforward_calls + 1;
    return caller_d_reforward_point(value);
}
...
int main(void) {
    ...
    const int *a = (++caller_a_comma, caller_a_reforward_int(a_base)) + 1;

    const int *b_base = return_b_twice(holder.items + 1);
    const int *b = 1 + caller_b_reforward_int_twice(0
    ...
    const struct Point *c = caller_c_reforward_point(&(1
    ...
    const struct Point *d = &((++caller_d_comma,
        caller_d_reforward_point_twice(d_base))[1]);
    ...
    return (a == a_base + 1) + (*a == 11)
        ...
        + (direct_c_marker == 1) + (c_caller_reforward_calls == 1)
        + (d == d_base + 1) + (d->value == 11)
        + (d_first_comma == 1) + (d_second_comma == 1)
        + (caller_d_comma == 1) + (d_caller_reforward_calls == 2)
