...
struct Item *reforward_items(struct Item *items) {
    return items;
}

struct Item *reforward_items_twice(struct Item *items) {
    return reforward_items(items);
}

const struct Item *reforward_const_items(const struct Item *items) {
    return items;
}

const struct Item *reforward_const_items_twice(const struct Item *items) {
    return reforward_const_items(items);
}
...
int main(void) {
    ...
    score += probe(
        reforward_items_twice(
            1
            + (0
                   ? (anonymous_post_unselected++, forward_items(named->nested.primary))
    ...
    score += probe(
        &reforward_items_twice(
            (choice_post_comma++,
             forward_items_twice((choice_inner_comma++, choice->nested.primary)))
        )[1]
    );
    score += probe(
        reforward_items_twice(
                  ...
                  ? (cross_post_selected++,
                     forward_items_twice(
                         1
                             ...
                             : (cross_inner_unselected++, anonymous->nested.secondary)))
                  : (cross_post_unselected++,
                     forward_items_twice(anonymous->nested.secondary)))[1]
        )
    );
    score += read_const(
        reforward_const_items(
            1
