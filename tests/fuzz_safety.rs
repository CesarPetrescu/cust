...
#[test]
fn generated_post_promotion_inner_const_reforwarding_preserves_direct_literal_adjusted_parameter_identity_without_panics()
 {
    let mut kind_counts = [0; 2];
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
        for promotion_placement in InnerConstPromotionPlacement::ALL {
                        ...
                        for reforward_placement in InnerConstPromotionPlacement::ALL {
                            for post_wrapper in WrappedDirectLiteralRoute::ALL {
                                for post_offset in WrappedDirectLiteralOffsetRoute::ALL {
                                    for reforward_two_hop in [false, true] {
                                        kind_counts[kind_index] += 1;
                                        promotion_placement_counts[promotion_placement.index()] +=
                                            1;
                                        inner_wrapper_counts[inner_wrapper.index()] += 1;
                                        ...
                                        let source =
                                            direct_derived_inner_pointer_const_reforward_program(
                                                kind,
                                                promotion_placement,
                                                inner_wrapper,
                                                inner_offset,
                                        ...
                                        assert_interpretation(
                                            ...
                                            ExpectedInterpretation::Value(28),
                                            &format!(
                                                "post-promotion inner const re-forward case {case_index}, kind {kind:?}, promotion placement {promotion_placement:?}, inner wrapper {inner_wrapper:?}, inner offset {inner_offset:?}, promotion two hop {promotion_two_hop}, re-forward placement {reforward_placement:?}, post wrapper {post_wrapper:?}, post offset {post_offset:?}, re-forward two hop {reforward_two_hop}"
                                            ),
                                        );
                                        case_index += 1;
                                    }
            ...
            for reforward_placement in InnerConstPromotionPlacement::ALL {
                assert_interpretation(
                    &direct_derived_inner_pointer_const_reforward_discard_program(
                        kind,
                        promotion_placement,
                        reforward_placement,
                    ),
                    ...
                    &format!(
                        "post-promotion inner mutable rebinding, kind {kind:?}, promotion placement {promotion_placement:?}, re-forward placement {reforward_placement:?}"
                    ),
                );
                assert_interpretation(
                    &direct_derived_inner_pointer_const_reforward_write_program(
                        kind,
                        promotion_placement,
                        reforward_placement,
                    ),
                    ExpectedInterpretation::Error("cannot assign through pointer to const"),
                    &format!(
                        "post-promotion inner const write, kind {kind:?}, promotion placement {promotion_placement:?}, re-forward placement {reforward_placement:?}"
                    ),
                );
            }
        }
    ...
    assert_eq!(case_index, 5832);
    assert_eq!(kind_counts, [2916; 2]);
    assert_eq!(promotion_placement_counts, [1944; 3]);
    assert_eq!(inner_wrapper_counts, [1944; 3]);
    assert_eq!(inner_offset_counts, [1944; 3]);
    assert_eq!(promotion_hop_counts, [2916; 2]);
    assert_eq!(reforward_placement_counts, [1944; 3]);
    assert_eq!(post_wrapper_counts, [1944; 3]);
    assert_eq!(post_offset_counts, [1944; 3]);
    assert_eq!(reforward_hop_counts, [2916; 2]);
}
...
#[test]
fn generated_caller_reforwarded_inner_pointer_returns_preserve_adjusted_parameter_identity_without_panics()
 {
    let mut kind_counts = [0; 2];
    let mut root_counts = [0; 4];
    let mut promotion_placement_counts = [0; 3];
    ...
    let mut callee_reforward_wrapper_counts = [0; 3];
    let mut callee_reforward_offset_counts = [0; 3];
    let mut callee_reforward_hop_counts = [0; 2];
    let mut return_hop_counts = [0; 2];
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
                        ...
                        for two_return_hops in [false, true] {
                            for caller_wrapper in WrappedDirectLiteralRoute::ALL {
                                for caller_offset in WrappedDirectLiteralOffsetRoute::ALL {
                                    ...
                                    callee_reforward_wrapper_counts
                                        [callee_reforward_wrapper.index()] += 1;
                                    callee_reforward_offset_counts
                                        [callee_reforward_offset.index()] += 1;
                                    callee_reforward_hop_counts
                                        [usize::from(callee_reforward_two_hop)] += 1;
                                    return_hop_counts[usize::from(two_return_hops)] += 1;
                                    ...
                                    assert_interpretation(
                                        &caller_reforwarded_inner_pointer_return_program(
                                            kind,
                                            root,
                                            promotion_placement,
                                            promotion_wrapper,
                                        ...
                                        &format!(
                                            "caller re-forwarded inner-pointer return case {case_index}, kind {kind:?}, root {root:?}, promotion placement {promotion_placement:?}, promotion wrapper {promotion_wrapper:?}, promotion offset {promotion_offset:?}, promotion two hop {promotion_two_hop}, callee re-forward placement {callee_reforward_placement:?}, callee re-forward wrapper {callee_reforward_wrapper:?}, callee re-forward offset {callee_reforward_offset:?}, callee re-forward two hop {callee_reforward_two_hop}, two return hops {two_return_hops}, caller re-forward placement {caller_reforward_placement:?}, caller wrapper {caller_wrapper:?}, caller offset {caller_offset:?}, caller re-forward two hop {caller_reforward_two_hop}"
                                        ),
                                    );
                                    case_index += 1;
                                }
        ...
        assert_interpretation(
            &caller_reforwarded_inner_pointer_return_discard_program(kind),
            ExpectedInterpretation::Error("cannot discard const qualifier from pointer target"),
            &format!("caller re-forwarded inner return mutable rebinding, kind {kind:?}"),
        );
        assert_interpretation(
            &caller_reforwarded_inner_pointer_return_write_program(kind),
            ExpectedInterpretation::Error("cannot assign through pointer to const"),
            &format!("caller re-forwarded inner return const write, kind {kind:?}"),
        );
        assert_interpretation(
            &caller_reforwarded_inner_pointer_return_bounds_program(kind),
            ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
            &format!("caller re-forwarded inner return bounds, kind {kind:?}"),
        );
        assert_interpretation(
            &caller_reforwarded_inner_pointer_return_cross_root_program(kind),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("caller re-forwarded inner return cross-root identity, kind {kind:?}"),
        );
        assert_interpretation(
            &caller_reforwarded_inner_pointer_return_lifetime_program(kind),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("caller re-forwarded inner return lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &caller_reforwarded_inner_pointer_return_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "caller re-forwarded inner aggregate return type mismatch",
    );
    ...
    assert_eq!(callee_reforward_wrapper_counts, [864; 3]);
    assert_eq!(callee_reforward_offset_counts, [864; 3]);
    assert_eq!(callee_reforward_hop_counts, [1296; 2]);
    assert_eq!(return_hop_counts, [1296; 2]);
...
#[test]
fn generated_final_receiving_caller_reforwarded_inner_pointers_preserve_adjusted_parameter_identity_without_panics()
 {
    let mut kind_counts = [0; 2];
    let mut root_counts = [0; 4];
    let mut final_reforward_placement_counts = [0; 3];
    let mut final_wrapper_counts = [0; 3];
    let mut final_offset_counts = [0; 3];
    let mut final_reforward_hop_counts = [0; 2];
    let mut outer_return_hop_counts = [0; 2];
    let mut case_index = 0;
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
                                ...
                                for caller_offset in WrappedDirectLiteralOffsetRoute::ALL {
                                    ...
                                    let callee_reforward_offset =
                                        WrappedDirectLiteralOffsetRoute::ALL[(promotion_placement
                                            ...
                                            + caller_wrapper.index())
                                            % 3];
                                    let final_reforward_placement =
                                        InnerConstPromotionPlacement::ALL[(promotion_placement
                                            .index()
                                            + caller_reforward_placement.index()
                                            + caller_offset.index())
                                            % 3];
                                    let final_wrapper = WrappedDirectLiteralRoute::ALL
                                        [(promotion_placement.index()
                                            + caller_wrapper.index()
                                            + caller_offset.index())
                                            % 3];
                                    let final_offset = WrappedDirectLiteralOffsetRoute::ALL
                                        [(caller_reforward_placement.index()
                                            + caller_wrapper.index()
                                            + caller_offset.index())
                                            % 3];
                                    ...
                                    let callee_reforward_two_hop = case_index & 2 == 0;
                                    let two_outer_return_hops = case_index & 4 == 0;
                                    let final_reforward_two_hop = case_index & 8 == 0;

                                    kind_counts[kind_index] += 1;
                                    root_counts[root.index()] += 1;
                                    final_reforward_placement_counts
                                        [final_reforward_placement.index()] += 1;
                                    final_wrapper_counts[final_wrapper.index()] += 1;
                                    final_offset_counts[final_offset.index()] += 1;
                                    final_reforward_hop_counts
                                        [usize::from(final_reforward_two_hop)] += 1;
                                    outer_return_hop_counts[usize::from(two_outer_return_hops)] +=
                                        1;

                                    assert_interpretation(
                                        &final_receiving_caller_reforwarded_inner_pointer_program(
                                            kind,
                                            root,
                                            promotion_placement,
                                            promotion_wrapper,
                                            ...
                                            caller_reforward_two_hop,
                                            two_outer_return_hops,
                                            final_reforward_placement,
                                            final_wrapper,
                                            final_offset,
                                            final_reforward_two_hop,
                                        ),
                                        ExpectedInterpretation::Value(16),
                                        &format!(
                                            "final receiving caller inner-pointer case {case_index}, kind {kind:?}, root {root:?}, final placement {final_reforward_placement:?}, final wrapper {final_wrapper:?}, final offset {final_offset:?}, final two hop {final_reforward_two_hop}"
                                        ),
                                    );
                                    case_index += 1;
                                }
        ...
        assert_interpretation(
            &final_receiving_caller_inner_pointer_discard_program(kind),
            ExpectedInterpretation::Error("cannot discard const qualifier from pointer target"),
            &format!("final receiving caller mutable rebinding, kind {kind:?}"),
        );
        assert_interpretation(
            &final_receiving_caller_inner_pointer_write_program(kind),
            ExpectedInterpretation::Error("cannot assign through pointer to const"),
            &format!("final receiving caller const write, kind {kind:?}"),
        );
        assert_interpretation(
            &final_receiving_caller_inner_pointer_bounds_program(kind),
            ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
            &format!("final receiving caller bounds, kind {kind:?}"),
        );
        assert_interpretation(
            &final_receiving_caller_inner_pointer_cross_root_program(kind),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("final receiving caller cross-root identity, kind {kind:?}"),
        );
        assert_interpretation(
            &final_receiving_caller_inner_pointer_lifetime_program(kind),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("final receiving caller lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &final_receiving_caller_inner_pointer_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "final receiving caller aggregate type mismatch",
    );

    assert_eq!(case_index, 2592);
    assert_eq!(kind_counts, [1296; 2]);
    assert_eq!(root_counts, [648; 4]);
    assert_eq!(final_reforward_placement_counts, [864; 3]);
    assert_eq!(final_wrapper_counts, [864; 3]);
    assert_eq!(final_offset_counts, [864; 3]);
    assert_eq!(final_reforward_hop_counts, [1296; 2]);
    assert_eq!(outer_return_hop_counts, [1296; 2]);
}
...
#[test]
fn generated_nested_const_parameter_return_selection_preserves_final_received_inner_pointer_identity_without_panics()
 {
    let mut kind_counts = [0; 2];
    let mut root_counts = [0; 4];
    let mut first_selection_counts = [0; 2];
    let mut first_helper_hop_counts = [0; 2];
    let mut second_order_counts = [0; 2];
    let mut second_selection_counts = [0; 2];
    let mut second_helper_hop_counts = [0; 2];
    let mut ultimate_identity_counts = [0; 2];
    let mut case_count = 0;
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
                    ...
                    for swap_second_args in [false, true] {
                        for second_select_first in [false, true] {
                            for second_two_hop in [false, true] {
                                let ultimate_is_final = if second_select_first {
                                    first_select_final != swap_second_args
                                } else {
                                    first_select_final == swap_second_args
                                };
                                assert_interpretation(
                                    &nested_const_parameter_return_selection_program(
                                        kind,
                                        root,
                                        first_select_final,
                                        first_two_hop,
                                    ...
                                    ExpectedInterpretation::Value(16),
                                    &format!(
                                        "nested const-parameter return selection, kind {kind:?}, root {root:?}, first final {first_select_final}, first two hop {first_two_hop}, swap second {swap_second_args}, second first {second_select_first}, second two hop {second_two_hop}"
                                    ),
                                );
                                kind_counts[kind_index] += 1;
                                root_counts[root.index()] += 1;
                                first_selection_counts[usize::from(first_select_final)] += 1;
                                first_helper_hop_counts[usize::from(first_two_hop)] += 1;
                                second_order_counts[usize::from(swap_second_args)] += 1;
                                second_selection_counts[usize::from(second_select_first)] += 1;
                                second_helper_hop_counts[usize::from(second_two_hop)] += 1;
                                ultimate_identity_counts[usize::from(ultimate_is_final)] += 1;
                                case_count += 1;
                            }
                        }
                    }
        ...
        assert_interpretation(
            &nested_const_parameter_return_selection_write_program(kind),
            ExpectedInterpretation::Error("cannot assign through pointer to const"),
            &format!("nested const-parameter return selection write, kind {kind:?}"),
        );
        assert_interpretation(
            &nested_const_parameter_return_selection_cross_root_program(kind),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("nested const-parameter return selection cross-root, kind {kind:?}"),
        );
        assert_interpretation(
            &nested_const_parameter_return_selection_lifetime_program(kind),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("nested const-parameter return selection lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &nested_const_parameter_return_selection_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "nested const-parameter return selection aggregate type mismatch",
    );

    assert_eq!(case_count, 256);
    assert_eq!(kind_counts, [128; 2]);
    assert_eq!(root_counts, [64; 4]);
    assert_eq!(first_selection_counts, [128; 2]);
    assert_eq!(first_helper_hop_counts, [128; 2]);
    assert_eq!(second_order_counts, [128; 2]);
...
#[test]
fn generated_post_selection_reforward_and_return_boundaries_preserve_nested_const_parameter_return_identity_without_panics()
 {
    let mut kind_counts = [0; 2];
    let mut root_counts = [0; 4];
    let mut wrapper_counts = [0; 3];
    ...
    let mut placement_counts = [0; 2];
    let mut reforward_hop_counts = [0; 2];
    let mut return_boundary_counts = [0; 2];
    let mut selected_identity_counts = [0; 2];
    let mut returned_identity_counts = [0; 2];
    let mut case_count = 0;
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
                                        ...
                                        for placement in PostForwardWrapperPlacement::ALL {
                                            for reforward_two_hop in [false, true] {
                                                for two_return_boundaries in [false, true] {
                                                    ...
                                                    assert_interpretation(
                                                        &post_selection_reforwarded_nested_const_parameter_return_program(
                                                            kind,
                                                            root,
                                                            first_select_final,
                                                            first_two_hop,
                                                        ...
                                                        ExpectedInterpretation::Value(26),
                                                        &format!(
                                                            "post-selection re-forward/return, kind {kind:?}, root {root:?}, first final {first_select_final}, first two hop {first_two_hop}, swap second {swap_second_args}, second first {second_select_first}, second two hop {second_two_hop}, wrapper {wrapper:?}, offset {offset:?}, placement {placement:?}, re-forward two hop {reforward_two_hop}, two return boundaries {two_return_boundaries}"
                                                        ),
                                                    );
                                                    kind_counts[kind_index] += 1;
                                                    root_counts[root.index()] += 1;
                                                    ...
                                                    offset_counts[offset.index()] += 1;
                                                    placement_counts[placement.index()] += 1;
                                                    reforward_hop_counts
                                                        [usize::from(reforward_two_hop)] += 1;
                                                    return_boundary_counts
                                                        [usize::from(two_return_boundaries)] += 1;
                                                    selected_identity_counts
                                                        [usize::from(selected_is_final)] += 1;
                                                    returned_identity_counts
                                                        [usize::from(!selected_is_final)] += 1;
                                                    case_count += 1;
                                                }
                                            }
        ...
        assert_interpretation(
            &post_selection_reforwarded_nested_return_bounds_program(kind),
            ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
            &format!("post-selection re-forward/return bounds, kind {kind:?}"),
        );
        assert_interpretation(
            &post_selection_reforwarded_nested_return_cross_root_program(kind),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("post-selection re-forward/return cross-root, kind {kind:?}"),
        );
        assert_interpretation(
            &post_selection_reforwarded_nested_return_lifetime_program(kind),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("post-selection re-forward/return lifetime, kind {kind:?}"),
        );
        assert_interpretation(
            &post_selection_reforwarded_nested_return_write_program(kind),
            ExpectedInterpretation::Error("cannot assign through pointer to const"),
            &format!("post-selection re-forward/return const write, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &post_selection_reforwarded_nested_return_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "post-selection re-forward/return aggregate type mismatch",
    );

    assert_eq!(case_count, 18432);
    assert_eq!(kind_counts, [9216; 2]);
    assert_eq!(root_counts, [4608; 4]);
    assert_eq!(wrapper_counts, [6144; 3]);
    assert_eq!(offset_counts, [6144; 3]);
    assert_eq!(placement_counts, [9216; 2]);
    assert_eq!(reforward_hop_counts, [9216; 2]);
    assert_eq!(return_boundary_counts, [9216; 2]);
    assert_eq!(selected_identity_counts, [9216; 2]);
    assert_eq!(returned_identity_counts, [9216; 2]);
}

#[test]
fn generated_final_returned_composed_pointer_reselection_preserves_identity_without_panics() {
    let mut kind_counts = [0; 2];
    let mut root_counts = [0; 4];
    let mut argument_order_counts = [0; 2];
    let mut selection_counts = [0; 2];
    let mut selector_hop_counts = [0; 2];
    let mut selected_result_counts = [0; 2];
    let mut case_count = 0;
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
            ...
            for swap_final_args in [false, true] {
                for final_select_first in [false, true] {
                    for final_two_hop in [false, true] {
                        let reselected_is_returned = if final_select_first {
                            !swap_final_args
                        } else {
                            swap_final_args
                        };
                        assert_interpretation(
                            &final_returned_composed_pointer_reselection_program(
                                kind,
                                root,
                                swap_final_args,
                                final_select_first,
                            ...
                            &format!(
                                "final returned/composed reselection, kind {kind:?}, root {root:?}, swap final args {swap_final_args}, select first {final_select_first}, two hop {final_two_hop}"
                            ),
                        );
                        kind_counts[kind_index] += 1;
                        root_counts[root.index()] += 1;
                        argument_order_counts[usize::from(swap_final_args)] += 1;
                        selection_counts[usize::from(final_select_first)] += 1;
                        selector_hop_counts[usize::from(final_two_hop)] += 1;
                        selected_result_counts[usize::from(reselected_is_returned)] += 1;
                        case_count += 1;
                    }
                }
            }
        ...
        assert_interpretation(
            &final_returned_composed_pointer_reselection_bounds_program(kind),
            ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
            &format!("final returned/composed reselection bounds, kind {kind:?}"),
        );
        assert_interpretation(
            &final_returned_composed_pointer_reselection_cross_root_program(kind),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("final returned/composed reselection cross-root, kind {kind:?}"),
        );
        assert_interpretation(
            &final_returned_composed_pointer_reselection_lifetime_program(kind),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("final returned/composed reselection lifetime, kind {kind:?}"),
        );
        assert_interpretation(
            &final_returned_composed_pointer_reselection_write_program(kind),
            ExpectedInterpretation::Error("cannot assign through pointer to const"),
            &format!("final returned/composed reselection const write, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &final_returned_composed_pointer_reselection_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "final returned/composed reselection aggregate type mismatch",
    );

    assert_eq!(case_count, 64);
    assert_eq!(kind_counts, [32; 2]);
    assert_eq!(root_counts, [16; 4]);
    assert_eq!(argument_order_counts, [32; 2]);
    assert_eq!(selection_counts, [32; 2]);
    assert_eq!(selector_hop_counts, [32; 2]);
    assert_eq!(selected_result_counts, [32; 2]);
}

#[test]
fn generated_distinct_root_final_reselection_preserves_selected_provenance_without_panics() {
    let mut kind_counts = [0; 2];
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
            ...
            for swap_final_args in [false, true] {
                for final_select_first in [false, true] {
                    for final_two_hop in [false, true] {
                        ...
                        assert_interpretation(
                            &distinct_root_final_reselection_program(
                                kind,
                                primary_root,
                                swap_final_args,
                                final_select_first,
                            ...
                            ExpectedInterpretation::Value(17),
                            &format!(
                                "distinct-root final reselection, kind {kind:?}, primary root {primary_root:?}, swap final args {swap_final_args}, select first {final_select_first}, two hop {final_two_hop}"
                            ),
                        );
                        kind_counts[kind_index] += 1;
                        ...
                        selection_counts[usize::from(final_select_first)] += 1;
                        selector_hop_counts[usize::from(final_two_hop)] += 1;
                        selected_root_counts[usize::from(selected_primary)] += 1;
                        case_count += 1;
                    }
                }
            }
        ...
        for select_primary in [false, true] {
            assert_interpretation(
                &distinct_root_final_reselection_bounds_program(kind, select_primary),
                ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
                &format!(
                    "distinct-root final reselection bounds, kind {kind:?}, select primary {select_primary}"
                ),
            );
            assert_interpretation(
                &distinct_root_final_reselection_write_program(kind, select_primary),
                ExpectedInterpretation::Error("cannot assign through pointer to const"),
                &format!(
                    "distinct-root final reselection const write, kind {kind:?}, select primary {select_primary}"
                ),
            );
        }
        assert_interpretation(
            &distinct_root_final_reselection_cross_root_program(kind, false),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("distinct-root final reselection subtraction, kind {kind:?}"),
        );
        assert_interpretation(
            &distinct_root_final_reselection_cross_root_program(kind, true),
            ExpectedInterpretation::Error("cannot compare pointers to different arrays"),
            &format!("distinct-root final reselection ordering, kind {kind:?}"),
        );
        assert_interpretation(
            &distinct_root_final_reselection_lifetime_program(kind, false),
            ExpectedInterpretation::Value(17),
            &format!("distinct-root final reselection live lifetime, kind {kind:?}"),
        );
        assert_interpretation(
            &distinct_root_final_reselection_lifetime_program(kind, true),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("distinct-root final reselection dangling lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &distinct_root_final_reselection_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "distinct-root final reselection aggregate type mismatch",
    );

    assert_eq!(case_count, 64);
    assert_eq!(kind_counts, [32; 2]);
    assert_eq!(primary_root_counts, [16; 4]);
    assert_eq!(argument_order_counts, [32; 2]);
    assert_eq!(selection_counts, [32; 2]);
    assert_eq!(selector_hop_counts, [32; 2]);
...
#[test]
fn generated_final_reselected_distinct_root_results_survive_fresh_composition_without_panics() {
    let mut kind_counts = [0; 2];
    let mut primary_root_counts = [0; 4];
    let mut final_result_counts = [0; 2];
    let mut wrapper_counts = [0; 3];
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
        for primary_root in DerivedInnerReturnRoot::ALL {
            for selects_returned in [false, true] {
                for wrapper in WrappedDirectLiteralRoute::ALL {
                    for offset in PostSelectionOffsetRoute::ALL {
                        for placement in DistinctRootCarryPlacement::ALL {
                            for helper_two_hop in [false, true] {
                                for return_two_hop in [false, true] {
                                    assert_interpretation(
                                        &final_reselected_distinct_root_result_carry_program(
                                            kind,
                                            primary_root,
                                            selects_returned,
                                            wrapper,
                                        ...
                                        ExpectedInterpretation::Value(45),
                                        &format!(
                                            "final reselected distinct-root carry, kind {kind:?}, primary root {primary_root:?}, selects returned {selects_returned}, wrapper {wrapper:?}, offset {offset:?}, placement {placement:?}, helper two hop {helper_two_hop}, return two hop {return_two_hop}"
                                        ),
                                    );
                                    kind_counts[kind_index] += 1;
                                    primary_root_counts[primary_root.index()] += 1;
                                    final_result_counts[usize::from(selects_returned)] += 1;
                                    wrapper_counts[wrapper.index()] += 1;
                                    offset_counts[offset.index()] += 1;
                                    placement_counts[placement.index()] += 1;
                                    helper_hop_counts[usize::from(helper_two_hop)] += 1;
                                    return_hop_counts[usize::from(return_two_hop)] += 1;
                                    case_count += 1;
                                }
                            }
                        }
        ...
        for select_returned in [false, true] {
            assert_interpretation(
                &final_reselected_distinct_root_result_carry_bounds_program(kind, select_returned),
                ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
                &format!(
                    "final reselected distinct-root carry bounds, kind {kind:?}, select returned {select_returned}"
                ),
            );
            assert_interpretation(
                &final_reselected_distinct_root_result_carry_write_program(kind, select_returned),
                ExpectedInterpretation::Error("cannot assign through pointer to const"),
                &format!(
                    "final reselected distinct-root carry write, kind {kind:?}, select returned {select_returned}"
                ),
            );
        }
        assert_interpretation(
            &final_reselected_distinct_root_result_carry_cross_root_program(kind, false),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("final reselected distinct-root carry subtraction, kind {kind:?}"),
        );
        assert_interpretation(
            &final_reselected_distinct_root_result_carry_cross_root_program(kind, true),
            ExpectedInterpretation::Error("cannot compare pointers to different arrays"),
            &format!("final reselected distinct-root carry ordering, kind {kind:?}"),
        );
        assert_interpretation(
            &final_reselected_distinct_root_result_carry_lifetime_program(kind, false),
            ExpectedInterpretation::Value(17),
            &format!("final reselected distinct-root carry live lifetime, kind {kind:?}"),
        );
        assert_interpretation(
            &final_reselected_distinct_root_result_carry_lifetime_program(kind, true),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("final reselected distinct-root carry dangling lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &final_reselected_distinct_root_result_carry_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "final reselected distinct-root carry aggregate type mismatch",
    );

    assert_eq!(case_count, 1_152);
    assert_eq!(kind_counts, [576; 2]);
    assert_eq!(primary_root_counts, [288; 4]);
    assert_eq!(final_result_counts, [576; 2]);
    assert_eq!(wrapper_counts, [384; 3]);
    assert_eq!(offset_counts, [384; 3]);
    assert_eq!(placement_counts, [576; 2]);
    assert_eq!(helper_hop_counts, [576; 2]);
    assert_eq!(return_hop_counts, [576; 2]);
}

#[test]
fn generated_freshly_composed_distinct_root_results_survive_parameter_reselection_without_panics() {
    let mut kind_counts = [0; 2];
    let mut primary_root_counts = [0; 4];
    let mut fresh_root_counts = [0; 2];
    let mut argument_order_counts = [0; 2];
    let mut selection_counts = [0; 2];
    let mut selector_hop_counts = [0; 2];
    let mut selected_result_counts = [0; 2];
    ...
    for (kind_index, kind) in AdjustedParameterFieldKind::ALL.into_iter().enumerate() {
        for primary_root in DerivedInnerReturnRoot::ALL {
            for fresh_from_returned in [false, true] {
                for wrapper in WrappedDirectLiteralRoute::ALL {
                    for offset in PostSelectionOffsetRoute::ALL {
                        for placement in DistinctRootCarryPlacement::ALL {
                            for helper_two_hop in [false, true] {
                                for return_two_hop in [false, true] {
                                    for swap_args in [false, true] {
                                        for select_first in [false, true] {
                                            for selector_two_hop in [false, true] {
                                                let selects_fresh = if select_first {
                                                    !swap_args
                                                } else {
                                                    swap_args
                                                };
                                                assert_interpretation(
                                                    &freshly_composed_distinct_root_result_reselection_program(
                                                        kind,
                                                        primary_root,
                                                        fresh_from_returned,
                                                        wrapper,
                                                    ...
                                                    ExpectedInterpretation::Value(53),
                                                    &format!(
                                                        "freshly composed distinct-root reselection, kind {kind:?}, primary root {primary_root:?}, fresh from returned {fresh_from_returned}, wrapper {wrapper:?}, offset {offset:?}, placement {placement:?}, helper two hop {helper_two_hop}, return two hop {return_two_hop}, swap args {swap_args}, select first {select_first}, selector two hop {selector_two_hop}"
                                                    ),
                                                );
                                                kind_counts[kind_index] += 1;
                                                primary_root_counts[primary_root.index()] += 1;
                                                fresh_root_counts
                                                    [usize::from(fresh_from_returned)] += 1;
                                                argument_order_counts[usize::from(swap_args)] += 1;
                                                selection_counts[usize::from(select_first)] += 1;
                                                selector_hop_counts
                                                    [usize::from(selector_two_hop)] += 1;
                                                selected_result_counts
                                                    [usize::from(selects_fresh)] += 1;
                                                case_count += 1;
                                            }
                                        }
                                    }
        ...
        for select_fresh in [false, true] {
            assert_interpretation(
                &freshly_composed_distinct_root_result_reselection_bounds_program(
                    kind,
                    select_fresh,
                ),
                ExpectedInterpretation::Error(kind.inner_pointer_bounds_error()),
                &format!(
                    "freshly composed distinct-root reselection bounds, kind {kind:?}, select fresh {select_fresh}"
                ),
            );
            assert_interpretation(
                &freshly_composed_distinct_root_result_reselection_write_program(
                    kind,
                    select_fresh,
                ),
                ExpectedInterpretation::Error("cannot assign through pointer to const"),
                &format!(
                    "freshly composed distinct-root reselection write, kind {kind:?}, select fresh {select_fresh}"
                ),
            );
        }
        assert_interpretation(
            &freshly_composed_distinct_root_result_reselection_cross_root_program(kind, false),
            ExpectedInterpretation::Error("cannot subtract pointers to different arrays"),
            &format!("freshly composed distinct-root reselection subtraction, kind {kind:?}"),
        );
        assert_interpretation(
            &freshly_composed_distinct_root_result_reselection_cross_root_program(kind, true),
            ExpectedInterpretation::Error("cannot compare pointers to different arrays"),
            &format!("freshly composed distinct-root reselection ordering, kind {kind:?}"),
        );
        assert_interpretation(
            &freshly_composed_distinct_root_result_reselection_lifetime_program(kind, false),
            ExpectedInterpretation::Value(17),
            &format!("freshly composed distinct-root reselection live lifetime, kind {kind:?}"),
        );
        assert_interpretation(
            &freshly_composed_distinct_root_result_reselection_lifetime_program(kind, true),
            ExpectedInterpretation::Error("pointer to out-of-scope variable 'local'"),
            &format!("freshly composed distinct-root reselection dangling lifetime, kind {kind:?}"),
        );
    }

    assert_interpretation(
        &freshly_composed_distinct_root_result_reselection_type_mismatch_program(),
        ExpectedInterpretation::Error(
            "cannot convert pointer to struct 'Point' to pointer to struct 'Other'",
        ),
        "freshly composed distinct-root reselection aggregate type mismatch",
    );

    assert_eq!(case_count, 9_216);
    assert_eq!(kind_counts, [4_608; 2]);
    assert_eq!(primary_root_counts, [2_304; 4]);
    assert_eq!(fresh_root_counts, [4_608; 2]);
    assert_eq!(argument_order_counts, [4_608; 2]);
    assert_eq!(selection_counts, [4_608; 2]);
    assert_eq!(selector_hop_counts, [4_608; 2]);
    assert_eq!(selected_result_counts, [4_608; 2]);
}
...
fn derived_inner_pointer_const_reforward_helpers() -> &'static str {
    "const int *reforward_inner_int(const int *value) { return value; }\n\
     const int *reforward_inner_int_twice(const int *value) { return reforward_inner_int(value); }\n\
     const struct Point *reforward_inner_point(const struct Point *value) { return value; }\n\
     const struct Point *reforward_inner_point_twice(const struct Point *value) { return reforward_inner_point(value); }"
}

fn derived_inner_pointer_const_reforward_expression(
    kind: AdjustedParameterFieldKind,
    placement: InnerConstPromotionPlacement,
    wrapper: WrappedDirectLiteralRoute,
    offset: WrappedDirectLiteralOffsetRoute,
    ...
    let helper = match (kind, two_hop) {
        (AdjustedParameterFieldKind::Scalar, false) => "reforward_inner_int",
        (AdjustedParameterFieldKind::Scalar, true) => "reforward_inner_int_twice",
        (AdjustedParameterFieldKind::Aggregate, false) => "reforward_inner_point",
        (AdjustedParameterFieldKind::Aggregate, true) => "reforward_inner_point_twice",
    };

    match placement {
        InnerConstPromotionPlacement::BeforeWrapper => {
            let selected = format!("{helper}(promoted)");
            let unselected = format!("{helper}(fallback_raw)");
            offset.render(&post_forward_pointer_wrapper(
                wrapper,
                &selected,
                &unselected,
        ...
        InnerConstPromotionPlacement::AfterWrapper => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "promoted", "fallback_raw", "post");
            offset.render(&format!("{helper}({wrapped})"))
        }
        InnerConstPromotionPlacement::AfterOffset => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "promoted", "fallback_raw", "post");
...
#[allow(clippy::too_many_arguments)]
fn direct_derived_inner_pointer_const_reforward_program(
    kind: AdjustedParameterFieldKind,
...
fn direct_derived_inner_pointer_const_reforward_discard_program(
    kind: AdjustedParameterFieldKind,
    promotion_placement: InnerConstPromotionPlacement,
    reforward_placement: InnerConstPromotionPlacement,
) -> String {
...
fn direct_derived_inner_pointer_const_reforward_write_program(
    kind: AdjustedParameterFieldKind,
    promotion_placement: InnerConstPromotionPlacement,
    reforward_placement: InnerConstPromotionPlacement,
) -> String {
...
fn caller_inner_pointer_const_reforward_helpers() -> &'static str {
    "int caller_reforward_calls;\n\
     const int *caller_reforward_inner_int(const int *value) { caller_reforward_calls = caller_reforward_calls + 1; return value; }\n\
     const int *caller_reforward_inner_int_twice(const int *value) { caller_reforward_calls = caller_reforward_calls + 1; return caller_reforward_inner_int(value); }\n\
     const struct Point *caller_reforward_inner_point(const struct Point *value) { caller_reforward_calls = caller_reforward_calls + 1; return value; }\n\
     const struct Point *caller_reforward_inner_point_twice(const struct Point *value) { caller_reforward_calls = caller_reforward_calls + 1; return caller_reforward_inner_point(value); }"
}

fn caller_inner_pointer_const_reforward_expression(
    kind: AdjustedParameterFieldKind,
    placement: InnerConstPromotionPlacement,
    wrapper: WrappedDirectLiteralRoute,
    offset: WrappedDirectLiteralOffsetRoute,
    ...
    let helper = match (kind, two_hop) {
        (AdjustedParameterFieldKind::Scalar, false) => "caller_reforward_inner_int",
        (AdjustedParameterFieldKind::Scalar, true) => "caller_reforward_inner_int_twice",
        (AdjustedParameterFieldKind::Aggregate, false) => "caller_reforward_inner_point",
        (AdjustedParameterFieldKind::Aggregate, true) => "caller_reforward_inner_point_twice",
    };

    match placement {
        InnerConstPromotionPlacement::BeforeWrapper => {
            let selected = format!("{helper}(base)");
            let unselected = format!("{helper}(fallback)");
            offset.render(&post_forward_pointer_wrapper(
                wrapper,
                &selected,
                &unselected,
        ...
        InnerConstPromotionPlacement::AfterWrapper => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "base", "fallback", "caller");
            offset.render(&format!("{helper}({wrapped})"))
        }
        InnerConstPromotionPlacement::AfterOffset => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "base", "fallback", "caller");
...
#[allow(clippy::too_many_arguments)]
fn caller_reforwarded_inner_pointer_return_program(
    kind: AdjustedParameterFieldKind,
    root: DerivedInnerReturnRoot,
    promotion_placement: InnerConstPromotionPlacement,
    promotion_wrapper: WrappedDirectLiteralRoute,
    ...
    let return_helper = format!(
        "return_for_caller_reforward_{suffix}{}",
        if two_return_hops { "_twice" } else { "" },
    );
    let read_returned = kind.read("returned");
    let read_base = kind.read("base");
    ...
    format!(
        "{prelude}\n{promotion_helpers}\n{callee_reforward_helpers}\n{caller_reforward_helpers}\n\
         int inner_selected; int inner_unselected; int inner_comma;\n\
         int post_selected; int post_unselected; int post_comma;\n\
         {const_pointer_type}return_for_caller_reforward_{suffix}(struct Item items[]) {{\n\
             {pointer_type}raw = &items[0].nested.{field}[0];\n\
             {pointer_type}fallback_raw = raw;\n\
             {initialize_raw}; {initialize_first}; {initialize_second}; {initialize_third};\n\
             {const_pointer_type}promoted = {promoted_expression};\n\
         ...
         {const_pointer_type}return_for_caller_reforward_{suffix}_twice(struct Item items[]) {{\n\
             return return_for_caller_reforward_{suffix}(items);\n\
         }}\n\
         int main(void) {{\n\
             {declarations}\n\
             int caller_selected = 0; int caller_unselected = 0; int caller_comma = 0;\n\
             {const_pointer_type}base = {return_helper}({argument});\n\
             ...
             return ({read_returned} == 11) + (returned == base + 1)\n\
                 ...
                 + (base_copy == base) + (slot == base) + ({root_marker_check});\n\
         }}\n",
        promotion_helpers = derived_inner_pointer_const_promotion_helpers(),
...
fn caller_reforwarded_inner_pointer_return_discard_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let pointer_type = kind.pointer_type();
    ...
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         int main(void) {{ int root_marker = 0; {pointer_type}mutable_slot = caller_reforward_inner_{suffix}(return_inner_{suffix}_twice({argument})); return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn caller_reforwarded_inner_pointer_return_write_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         int main(void) {{ int root_marker = 0; {const_pointer_type}slot = caller_reforward_inner_{suffix}_twice(return_inner_{suffix}_twice({argument})); {write}; return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        suffix = kind.suffix(),
...
fn caller_reforwarded_inner_pointer_return_bounds_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let read = kind.read("slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         int main(void) {{ int root_marker = 0; {const_pointer_type}slot = caller_reforward_inner_{suffix}(return_inner_{suffix}_twice({argument})) + 2; return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        suffix = kind.suffix(),
...
fn caller_reforwarded_inner_pointer_return_cross_root_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}left = caller_reforward_inner_{suffix}(return_inner_{suffix}_twice((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }}));\n\
             {const_pointer_type}right = caller_reforward_inner_{suffix}_twice(return_inner_{suffix}_twice((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }}));\n\
             return left - right;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn caller_reforwarded_inner_pointer_return_lifetime_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let read = kind.read("slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         {const_pointer_type}dangling(void) {{ struct Item local[1]; return return_inner_{suffix}_twice(local); }}\n\
         int main(void) {{ {const_pointer_type}slot = caller_reforward_inner_{suffix}_twice(dangling()); return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        suffix = kind.suffix(),
...
fn caller_reforwarded_inner_pointer_return_type_mismatch_program() -> String {
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{ int root_marker = 0; const struct Other *slot = caller_reforward_inner_point_twice(return_inner_point_twice({argument})); return slot->value; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(
...
fn final_receiving_caller_reforward_helpers() -> &'static str {
    "int final_reforward_calls;\n\
     const int *final_reforward_inner_int(const int *value) { final_reforward_calls = final_reforward_calls + 1; return value; }\n\
     const int *final_reforward_inner_int_twice(const int *value) { final_reforward_calls = final_reforward_calls + 1; return final_reforward_inner_int(value); }\n\
     const struct Point *final_reforward_inner_point(const struct Point *value) { final_reforward_calls = final_reforward_calls + 1; return value; }\n\
     const struct Point *final_reforward_inner_point_twice(const struct Point *value) { final_reforward_calls = final_reforward_calls + 1; return final_reforward_inner_point(value); }"
}

fn final_receiving_caller_reforward_expression(
    kind: AdjustedParameterFieldKind,
    placement: InnerConstPromotionPlacement,
    wrapper: WrappedDirectLiteralRoute,
    offset: WrappedDirectLiteralOffsetRoute,
    ...
    let helper = match (kind, two_hop) {
        (AdjustedParameterFieldKind::Scalar, false) => "final_reforward_inner_int",
        (AdjustedParameterFieldKind::Scalar, true) => "final_reforward_inner_int_twice",
        (AdjustedParameterFieldKind::Aggregate, false) => "final_reforward_inner_point",
        (AdjustedParameterFieldKind::Aggregate, true) => "final_reforward_inner_point_twice",
    };
    ...
    match placement {
        ...
        InnerConstPromotionPlacement::AfterWrapper => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "base", "fallback", "final");
            offset.render(&format!("{helper}({wrapped})"))
        }
        InnerConstPromotionPlacement::AfterOffset => {
            let wrapped = post_forward_pointer_wrapper(wrapper, "base", "fallback", "final");
...
#[allow(clippy::too_many_arguments)]
fn final_receiving_caller_reforwarded_inner_pointer_program(
    kind: AdjustedParameterFieldKind,
    root: DerivedInnerReturnRoot,
    promotion_placement: InnerConstPromotionPlacement,
    promotion_wrapper: WrappedDirectLiteralRoute,
    ...
    caller_reforward_two_hop: bool,
    two_outer_return_hops: bool,
    final_reforward_placement: InnerConstPromotionPlacement,
    final_wrapper: WrappedDirectLiteralRoute,
    final_offset: WrappedDirectLiteralOffsetRoute,
    final_reforward_two_hop: bool,
) -> String {
    let (declarations, argument, root_marker_check) =
        derived_inner_const_pointer_callee_return_root(root);
    let promoted_expression = derived_inner_pointer_const_promotion_expression(
    ...
    let final_reforwarded_expression = final_receiving_caller_reforward_expression(
        kind,
        final_reforward_placement,
        final_wrapper,
        final_offset,
    ...
    let initialize_second = kind.write("(raw + 2)", 9);
    let initialize_third = kind.write("(raw + 3)", 11);
    let initialize_fourth = kind.write("(raw + 4)", 13);
    let callee_return_helper = format!(
        "return_for_final_caller_{suffix}{}",
        if two_callee_return_hops { "_twice" } else { "" },
    );
    let outer_return_helper = format!(
        "outer_return_for_final_caller_{suffix}{}",
        if two_outer_return_hops { "_twice" } else { "" },
    );
    let read_returned = kind.read("returned");
    let read_base = kind.read("base");
    let read_callee_base = kind.read("callee_base");
    let read_promoted = kind.read("promoted_copy");
    let promotion_marker_check = promotion_wrapper.marker_check("inner");
    let callee_reforward_marker_check = callee_reforward_wrapper.marker_check("post");
    let caller_marker_check = caller_wrapper.marker_check("caller");
    let final_marker_check = final_wrapper.marker_check("final");
    let caller_reforward_call_count = if caller_reforward_two_hop { 2 } else { 1 };
    let final_reforward_call_count = if final_reforward_two_hop { 2 } else { 1 };
    let prelude = captured_literal_field_offset_prelude().replace(
        "int values[3]; struct Point points[3];",
        "int values[5]; struct Point points[5];",
    );

    format!(
        "{prelude}\n{promotion_helpers}\n{callee_reforward_helpers}\n{caller_reforward_helpers}\n{final_reforward_helpers}\n\
         int inner_selected; int inner_unselected; int inner_comma;\n\
         int post_selected; int post_unselected; int post_comma;\n\
         int caller_selected; int caller_unselected; int caller_comma;\n\
         int final_selected; int final_unselected; int final_comma;\n\
         int outer_score;\n\
         {const_pointer_type}return_for_final_caller_{suffix}(struct Item items[]) {{\n\
             {pointer_type}raw = &items[0].nested.{field}[0];\n\
             {pointer_type}fallback_raw = raw;\n\
             {initialize_raw}; {initialize_first}; {initialize_second}; {initialize_third}; {initialize_fourth};\n\
             {const_pointer_type}promoted = {promoted_expression};\n\
             return {callee_reforwarded_expression};\n\
         }}\n\
         {const_pointer_type}return_for_final_caller_{suffix}_twice(struct Item items[]) {{\n\
             return return_for_final_caller_{suffix}(items);\n\
         }}\n\
         {const_pointer_type}outer_return_for_final_caller_{suffix}({const_pointer_type}base) {{\n\
             {const_pointer_type}base_copy = base;\n\
             {const_pointer_type}fallback = base - 1;\n\
             {const_pointer_type}returned = {caller_reforwarded_expression};\n\
             {const_pointer_type}slot = returned;\n\
         ...
         {const_pointer_type}outer_return_for_final_caller_{suffix}_twice({const_pointer_type}base) {{\n\
             return outer_return_for_final_caller_{suffix}(base);\n\
         }}\n\
         int main(void) {{\n\
             {declarations}\n\
             {const_pointer_type}callee_base = {callee_return_helper}({argument});\n\
             {const_pointer_type}base = {outer_return_helper}(callee_base);\n\
             {const_pointer_type}base_copy = base;\n\
             {const_pointer_type}fallback = base - 1;\n\
             {const_pointer_type}returned = {final_reforwarded_expression};\n\
             {const_pointer_type}promoted_copy = returned - 3;\n\
             {const_pointer_type}slot = returned;\n\
             slot = base;\n\
             return ({read_returned} == 13) + (returned == base + 1)\n\
                 + ({read_base} == 11) + ({read_callee_base} == 9) + ({read_promoted} == 7)\n\
                 + (callee_base == promoted_copy + 1) + (base == callee_base + 1)\n\
                 + ({promotion_marker_check}) + ({callee_reforward_marker_check})\n\
                 + ({caller_marker_check}) + ({final_marker_check})\n\
                 + (caller_reforward_calls == {caller_reforward_call_count})\n\
                 + (final_reforward_calls == {final_reforward_call_count})\n\
                 + (outer_score == 2) + (base_copy == base && slot == base)\n\
                 + ({root_marker_check});\n\
         }}\n",
        promotion_helpers = derived_inner_pointer_const_promotion_helpers(),
        callee_reforward_helpers = derived_inner_pointer_const_reforward_helpers(),
        caller_reforward_helpers = caller_inner_pointer_const_reforward_helpers(),
        final_reforward_helpers = final_receiving_caller_reforward_helpers(),
    )
}

fn final_receiving_caller_inner_pointer_discard_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let pointer_type = kind.pointer_type();
    let read = kind.read("mutable_slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         int main(void) {{ int root_marker = 0; {pointer_type}mutable_slot = final_reforward_inner_{suffix}_twice(outer_return_inner_{suffix}_twice({argument})); return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
...
fn final_receiving_caller_inner_pointer_write_program(kind: AdjustedParameterFieldKind) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let write = kind.write("slot", 11);
    let read = kind.read("slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         int main(void) {{ int root_marker = 0; {const_pointer_type}slot = final_reforward_inner_{suffix}(outer_return_inner_{suffix}_twice({argument})); {write}; return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        outer_helpers = outer_caller_return_helpers(kind),
...
fn final_receiving_caller_inner_pointer_bounds_program(kind: AdjustedParameterFieldKind) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let read = kind.read("slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         int main(void) {{ int root_marker = 0; {const_pointer_type}slot = final_reforward_inner_{suffix}(outer_return_inner_{suffix}_twice({argument})) + 2; return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        outer_helpers = outer_caller_return_helpers(kind),
...
fn final_receiving_caller_inner_pointer_cross_root_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
             {const_pointer_type}left = final_reforward_inner_{suffix}(outer_return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }}));\n\
             {const_pointer_type}right = final_reforward_inner_{suffix}_twice(outer_return_inner_{suffix}_twice((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }}));\n\
             return left - right;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_receiving_caller_inner_pointer_lifetime_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let read = kind.read("slot");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         ...
         int main(void) {{ {const_pointer_type}slot = final_reforward_inner_{suffix}_twice(dangling()); return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        outer_helpers = outer_caller_return_helpers(kind),
...
fn final_receiving_caller_inner_pointer_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{ int root_marker = 0; const struct Other *slot = final_reforward_inner_point_twice(outer_return_inner_point_twice({argument})); return slot->value; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        outer_helpers = outer_caller_return_helpers(kind),
...
fn nested_const_parameter_return_selection_program(
    kind: AdjustedParameterFieldKind,
    root: DerivedInnerReturnRoot,
    ...
    let ultimate_expected_pointer = if ultimate_is_final { "final" } else { "base" };
    let ultimate_value = if ultimate_is_final { 9 } else { 7 };
    let ultimate_offset = i64::from(ultimate_is_final);
    let first_ordering = if ultimate_is_final {
        "ultimate > base"
    } else {
        "ultimate < final"
    };
    let second_ordering = if ultimate_is_final {
        "base < ultimate"
    } else {
        "final > ultimate"
    };
    let read_ultimate = kind.read("ultimate");
    ...
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{final_helpers}\n{selection_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}ultimate = {second_helper}({second_first}, {second_second}, {second_selection});\n\
             return ({read_ultimate} == {ultimate_value}) + (ultimate == {ultimate_expected_pointer})\n\
                 + (ultimate - base == {ultimate_offset}) + (final - ultimate == {reverse_offset})\n\
...
#[allow(clippy::too_many_arguments)]
fn post_selection_reforwarded_nested_const_parameter_return_program(
    kind: AdjustedParameterFieldKind,
    root: DerivedInnerReturnRoot,
    first_select_final: bool,
    first_two_hop: bool,
    ...
    let mut program = post_selection_wrapped_nested_const_parameter_return_program(
        kind,
        root,
        first_select_final,
        first_two_hop,
    ...
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let reforward_helper = format!(
        "reforward_composed_{suffix}{}",
        if reforward_two_hop { "_twice" } else { "" },
    );
    let return_helper = format!(
        "return_composed_{suffix}{}",
        if two_return_boundaries { "_twice" } else { "" },
    );
    let helper_definitions = post_selection_reforward_and_return_helpers(kind);
    ...
    let return_start = program
        .rfind("return (")
        .expect("generated post-selection program should contain its main return");
    program.insert_str(
        return_start,
        &format!(
            "{const_pointer_type}reforwarded = {reforward_helper}(composed);\n\
             {const_pointer_type}returned = {return_helper}(reforwarded);\n"
        ),
    );
    let returned_read = kind.read("returned");
    let returned_is_final = !if second_select_first {
        first_select_final != swap_second_args
    } else {
        first_select_final == swap_second_args
    };
    let returned_value = if returned_is_final { 9 } else { 7 };
    let returned_offset = i64::from(returned_is_final);
    let reforward_calls = if reforward_two_hop { 2 } else { 1 };
    let return_calls = if two_return_boundaries { 2 } else { 1 };
    let return_end = program
        .rfind(';')
        .expect("generated post-selection program should end its main return");
    program.insert_str(
        return_end,
        &format!(
            "\n                 + (reforwarded == composed) + (returned == composed)\n\
             + ({returned_read} == {returned_value}) + (returned - base == {returned_offset})\n\
             + (composed_reforward_calls == {reforward_calls}) + (composed_return_calls == {return_calls})"
        ),
    );
    program
}

fn post_selection_reforward_and_return_helpers(kind: AdjustedParameterFieldKind) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "int composed_reforward_calls; int composed_return_calls; int final_selector_calls;\n\
         {const_pointer_type}reforward_composed_{suffix}({const_pointer_type}value) {{ composed_reforward_calls = composed_reforward_calls + 1; return value; }}\n\
         {const_pointer_type}reforward_composed_{suffix}_twice({const_pointer_type}value) {{ composed_reforward_calls = composed_reforward_calls + 1; return reforward_composed_{suffix}(value); }}\n\
         {const_pointer_type}return_composed_{suffix}({const_pointer_type}value) {{ composed_return_calls = composed_return_calls + 1; return value; }}\n\
         {const_pointer_type}return_composed_{suffix}_twice({const_pointer_type}value) {{ composed_return_calls = composed_return_calls + 1; return return_composed_{suffix}(value); }}\n\
         {const_pointer_type}reselect_composed_{suffix}({const_pointer_type}first, {const_pointer_type}second, int select_first) {{ final_selector_calls = final_selector_calls + 1; return select_first ? first : second; }}\n\
         {const_pointer_type}reselect_composed_{suffix}_twice({const_pointer_type}first, {const_pointer_type}second, int select_first) {{ final_selector_calls = final_selector_calls + 1; return reselect_composed_{suffix}(first, second, select_first); }}"
    )
}

fn final_returned_composed_pointer_reselection_program(
    kind: AdjustedParameterFieldKind,
    root: DerivedInnerReturnRoot,
    ...
    let first_select_final = root_index & 1 == 0;
    let swap_second_args = root_index >= 2;
    let second_select_first = kind == AdjustedParameterFieldKind::Scalar;
    let selected_is_final = if second_select_first {
        first_select_final != swap_second_args
    } else {
        first_select_final == swap_second_args
    };
    let mut program = post_selection_reforwarded_nested_const_parameter_return_program(
        kind,
        root,
        first_select_final,
        root_index < 2,
        ...
        second_select_first,
        root_index & 1 != 0,
        WrappedDirectLiteralRoute::ALL[root_index % WrappedDirectLiteralRoute::ALL.len()],
        PostSelectionOffsetRoute::ALL[(root_index
            + usize::from(kind == AdjustedParameterFieldKind::Aggregate))
            % PostSelectionOffsetRoute::ALL.len()],
        PostForwardWrapperPlacement::ALL
            [usize::from(kind == AdjustedParameterFieldKind::Aggregate)],
        root_index & 1 != 0,
        root_index >= 2,
    );
    ...
    let reselected_is_final = if reselected_is_returned {
        !selected_is_final
    } else {
        selected_is_final
    };
    let expected_pointer = if reselected_is_returned {
        "returned"
    } else {
        "selected"
    };
    let selected_expected_pointer = if selected_is_final { "final" } else { "base" };
    let expected_value = if reselected_is_final { 9 } else { 7 };
    let expected_offset = i64::from(reselected_is_final);
    let selected_offset = i64::from(selected_is_final);
    let ordering = if reselected_is_final {
        "reselected > base"
    } else {
        "reselected < final"
    };
    ...
    let return_start = program
        .rfind("return (")
        .expect("generated re-forwarded program should contain its main return");
    program.insert_str(
        return_start,
        &format!(
            "{const_pointer_type}reselected = {selector}({first}, {second}, {selection});\n\
             ",
            selection = i64::from(final_select_first),
        ),
    );
    let reselected_read = kind.read("reselected");
    let return_end = program
        .rfind(';')
        .expect("generated reselected program should end its main return");
    program.insert_str(
        return_end,
        &format!(
            "\n                 + (reselected == {expected_pointer}) + ({reselected_read} == {expected_value})\n\
             + (reselected - base == {expected_offset}) + ({ordering})\n\
             + (reselected - selected == {selected_delta}) + (final_selector_calls == {selector_calls})\n\
             + (returned == composed) + (selected == {selected_expected_pointer})",
            selected_delta = expected_offset - selected_offset,
        ),
    );
    program
}

fn final_returned_composed_pointer_reselection_bounds_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let read = kind.read("reselected");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
             {const_pointer_type}base = outer_return_inner_{suffix}({argument});\n\
             {const_pointer_type}selected = select_received_{suffix}_twice(base + 1, base, 1);\n\
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}_twice(reforward_composed_{suffix}(composed));\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}_twice(returned, selected, 1) + 2;\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_returned_composed_pointer_reselection_cross_root_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}left = outer_return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}right = outer_return_inner_{suffix}_twice((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}selected = select_received_{suffix}_twice(left + 1, left, 1);\n\
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}(reforward_composed_{suffix}_twice(composed));\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}(selected, returned, 0);\n\
             return reselected - right;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
...
fn final_returned_composed_pointer_reselection_lifetime_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    ...
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         {const_pointer_type}select_dangling_{suffix}(void) {{\n\
             struct Item local[1];\n\
             {const_pointer_type}base = outer_return_inner_{suffix}_twice(local);\n\
             {const_pointer_type}selected = select_received_{suffix}(base + 1, base, 1);\n\
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}_twice(reforward_composed_{suffix}(composed));\n\
             return reselect_composed_{suffix}_twice(returned, selected, 1);\n\
         }}\n\
         int main(void) {{\n\
             {const_pointer_type}reselected = select_dangling_{suffix}();\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_returned_composed_pointer_reselection_write_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("reselected", 11);
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
             ...
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}(reforward_composed_{suffix}_twice(composed));\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}(returned, selected, 0);\n\
             {write}; return 0;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_returned_composed_pointer_reselection_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
             const struct Point *base = outer_return_inner_point_twice({argument});\n\
             const struct Point *selected = select_received_point_twice(base + 1, base, 1);\n\
             const struct Point *composed = selected - 1;\n\
             const struct Point *returned = return_composed_point_twice(reforward_composed_point(composed));\n\
             const struct Point *reselected = reselect_composed_point_twice(selected, returned, 0);\n\
             const struct Other *mismatch = reselected;\n\
...
fn distinct_root_final_reselection_arguments(
    primary_root: DerivedInnerReturnRoot,
) -> (&'static str, &'static str, i64) {
    match primary_root {
        DerivedInnerReturnRoot::Direct => (
            "((struct Item[3]){{}, { .nested = { .values = {++direct_marker} } }, {}} + 1)",
            "named_right->nested.primary + 1",
            1,
        ),
        DerivedInnerReturnRoot::Named => (
            "named_left->nested.primary + 1",
        ...
        DerivedInnerReturnRoot::Anonymous => (
            "&anonymous_left->nested.primary[1]",
            "1 + choice_right->nested.primary",
            0,
        ),
        DerivedInnerReturnRoot::Union => (
            "choice_left->nested.primary + 1",
            "&anonymous_right->nested.primary[1]",
            0,
        ),
    }
}

fn distinct_root_return_helpers(kind: AdjustedParameterFieldKind) -> String {
    let pointer_type = kind.pointer_type();
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let field = kind.field_name();
    let initialize_raw = kind.write("raw", 15);
    let initialize_first = kind.write("(raw + 1)", 17);
    let initialize_second = kind.write("(raw + 2)", 19);
    format!(
        "{const_pointer_type}return_distinct_inner_{suffix}(struct Item items[]) {{\n\
             {pointer_type}raw = &items[0].nested.{field}[0];\n\
             {initialize_raw}; {initialize_first}; {initialize_second};\n\
             return raw + 1;\n\
         }}"
...
fn distinct_root_final_reselection_program(
    kind: AdjustedParameterFieldKind,
    primary_root: DerivedInnerReturnRoot,
    swap_final_args: bool,
    final_select_first: bool,
    ...
    let (primary_argument, alternate_argument, direct_marker_expected) =
        distinct_root_final_reselection_arguments(primary_root);
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let selector = format!(
        "reselect_composed_{suffix}{}",
    ...
    let (first, second) = if swap_final_args {
        ("alternate", "primary_returned")
    } else {
        ("primary_returned", "alternate")
    };
    let selected_primary = if final_select_first {
        !swap_final_args
    } else {
    ...
    let (expected, unselected, expected_value, selected_root) = if selected_primary {
        ("primary_returned", "alternate", 7, "primary_base")
    } else {
        ("alternate", "primary_returned", 17, "alternate")
    };
    let selected_read = kind.read("reselected");
    let primary_read = kind.read("primary_returned");
    let alternate_read = kind.read("alternate");
    let primary_selected_read = kind.read("primary_selected");
    let selector_calls = if final_two_hop { 2 } else { 1 };
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             {declarations}\n\
             int direct_marker = 0;\n\
             {const_pointer_type}primary_base = return_inner_{suffix}({primary_argument});\n\
             {const_pointer_type}primary_selected = select_received_{suffix}_twice(primary_base + 1, primary_base, 1);\n\
             {const_pointer_type}primary_composed = primary_selected - 1;\n\
             {const_pointer_type}primary_returned = return_composed_{suffix}_twice(reforward_composed_{suffix}(primary_composed));\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}({alternate_argument});\n\
             {const_pointer_type}reselected = {selector}({first}, {second}, {selection});\n\
             return (reselected == {expected}) + ({selected_read} == {expected_value})\n\
                 + (reselected != {unselected}) + (primary_returned == primary_base)\n\
                 + ({primary_read} == 7) + ({alternate_read} == 17)\n\
                 + (primary_selected == primary_base + 1) + ({primary_selected_read} == 9)\n\
                 + (primary_returned - primary_base == 0) + (reselected - {selected_root} == 0)\n\
                 + (primary_returned != alternate) + (parameter_return_calls == 2)\n\
                 + (composed_reforward_calls == 1) + (composed_return_calls == 2)\n\
                 + (final_selector_calls == {selector_calls}) + (marker == 6)\n\
                 + (direct_marker == {direct_marker_expected});\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn final_reselected_distinct_root_result_carry_helpers(kind: AdjustedParameterFieldKind) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "int final_carry_calls; int final_return_calls;\n\
         {const_pointer_type}carry_final_{suffix}({const_pointer_type}value) {{ final_carry_calls = final_carry_calls + 1; return value; }}\n\
         {const_pointer_type}carry_final_{suffix}_twice({const_pointer_type}value) {{ final_carry_calls = final_carry_calls + 1; return carry_final_{suffix}(value); }}\n\
         {const_pointer_type}return_final_{suffix}({const_pointer_type}value) {{ final_return_calls = final_return_calls + 1; return value; }}\n\
         {const_pointer_type}return_final_{suffix}_twice({const_pointer_type}value) {{ final_return_calls = final_return_calls + 1; return return_final_{suffix}(value); }}\n"
    )
}

#[allow(clippy::too_many_arguments)]
fn final_reselected_distinct_root_result_carry_program(
    kind: AdjustedParameterFieldKind,
    primary_root: DerivedInnerReturnRoot,
    selects_returned: bool,
    wrapper: WrappedDirectLiteralRoute,
    offset: PostSelectionOffsetRoute,
    placement: DistinctRootCarryPlacement,
    helper_two_hop: bool,
    ...
    let initial_delta = if primary_root.index() & 1 == 0 { 1 } else { -1 };
    let mut program = returned_distinct_root_result_reselection_program(
        kind,
        primary_root,
        false,
        true,
        primary_root.index() & 1 == 0,
        WrappedDirectLiteralRoute::ConditionalTrue,
        PostSelectionOffsetRoute::PointerPlusDelta,
        DistinctRootCarryPlacement::ComposeBeforeHelper,
        primary_root.index() & 2 == 0,
        primary_root.index() & 2 != 0,
        false,
        selects_returned,
        primary_root.index() & 1 != 0,
    ...
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let rejected_root = if selects_returned {
        "alternate"
    } else {
        "selected_returned"
    };
    let expected_root = if selects_returned {
        "primary_base"
    } else {
        "alternate"
    };
    let fresh_delta = if selects_returned {
        -initial_delta
    } else {
        initial_delta
    };
    let expected_root_delta = if selects_returned { 0 } else { fresh_delta };
    let expected_value = if selects_returned {
        7
    } else if fresh_delta == 1 {
        19
    } else {
        15
    };
    let carry_helper = format!(
        "carry_final_{suffix}{}",
        if helper_two_hop { "_twice" } else { "" },
    );
    let return_helper = format!(
        "return_final_{suffix}{}",
        if return_two_hop { "_twice" } else { "" },
    );
    let (stages, expected_carried) = match placement {
        DistinctRootCarryPlacement::ComposeBeforeHelper => {
            let wrapped =
                post_forward_pointer_wrapper(wrapper, "final_reselected", rejected_root, "fresh");
            let composed = offset.render(&wrapped, fresh_delta);
            (
                format!(
                    "{const_pointer_type}fresh_composed = {composed};\n\
                     {const_pointer_type}fresh_carried = {carry_helper}(fresh_composed);\n"
                ),
                "fresh_composed",
            )
        }
        DistinctRootCarryPlacement::ComposeAfterHelper => {
            let wrapped =
                post_forward_pointer_wrapper(wrapper, "fresh_carried", rejected_root, "fresh");
            let composed = offset.render(&wrapped, fresh_delta);
            (
                format!(
                    "{const_pointer_type}fresh_carried = {carry_helper}(final_reselected);\n\
                     {const_pointer_type}fresh_composed = {composed};\n"
                ),
                "final_reselected",
            )
    ...
    let helper_definitions = final_reselected_distinct_root_result_carry_helpers(kind);
    program = program.replacen(
        "int main(void) {",
        &format!("{helper_definitions}int main(void) {{"),
        1,
    ...
    let return_start = program
        .rfind("return (")
        .expect("generated final-reselected program should contain its main return");
    program.insert_str(
        return_start,
        &format!(
            "int fresh_selected = 0; int fresh_unselected = 0; int fresh_comma = 0;\n\
             {const_pointer_type}final_slot = final_reselected;\n\
             {const_pointer_type}returned_slot_after = selected_returned;\n\
             {const_pointer_type}unselected_slot_after = alternate;\n\
             {stages}\
             {const_pointer_type}fresh_returned = {return_helper}(fresh_composed);\n\
             "
        ),
    );
    let fresh_read = kind.read("fresh_returned");
    let marker_check = wrapper.marker_check("fresh");
    let carry_calls = if helper_two_hop { 2 } else { 1 };
    let return_calls = if return_two_hop { 2 } else { 1 };
    let return_end = program
        .rfind(';')
        .expect("generated final-reselected program should end its main return");
    program.insert_str(
        return_end,
        &format!(
            "\n                 + (fresh_returned == fresh_composed) + ({fresh_read} == {expected_value})\n\
             + (fresh_composed - final_reselected == {fresh_delta}) + (fresh_carried == {expected_carried})\n\
             + (final_slot == final_reselected) + (returned_slot_after == selected_returned)\n\
             + (unselected_slot_after == alternate) + ({marker_check})\n\
             + (final_carry_calls == {carry_calls}) + (final_return_calls == {return_calls})\n\
             + (fresh_returned - {expected_root} == {expected_root_delta})"
        ),
    );
    program
}

fn freshly_composed_distinct_root_result_reselection_helpers(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "int fresh_reselection_calls;\n\
         {const_pointer_type}reselect_fresh_{suffix}({const_pointer_type}first, {const_pointer_type}second, int select_first) {{ fresh_reselection_calls = fresh_reselection_calls + 1; return select_first ? first : second; }}\n\
         {const_pointer_type}reselect_fresh_{suffix}_twice({const_pointer_type}first, {const_pointer_type}second, int select_first) {{ fresh_reselection_calls = fresh_reselection_calls + 1; return reselect_fresh_{suffix}(first, second, select_first); }}\n"
    )
}

#[allow(clippy::too_many_arguments)]
fn freshly_composed_distinct_root_result_reselection_program(
    kind: AdjustedParameterFieldKind,
    primary_root: DerivedInnerReturnRoot,
    fresh_from_returned: bool,
    wrapper: WrappedDirectLiteralRoute,
    offset: PostSelectionOffsetRoute,
    placement: DistinctRootCarryPlacement,
    helper_two_hop: bool,
    ...
    swap_args: bool,
    select_first: bool,
    selector_two_hop: bool,
) -> String {
    let mut program = final_reselected_distinct_root_result_carry_program(
        kind,
        primary_root,
        fresh_from_returned,
        wrapper,
    ...
    let suffix = kind.suffix();
    let initial_delta = if primary_root.index() & 1 == 0 { 1 } else { -1 };
    let fresh_delta = if fresh_from_returned {
        -initial_delta
    } else {
        initial_delta
    };
    let rejected = if fresh_from_returned {
        "alternate"
    } else {
        "selected_returned"
    };
    let (first, second) = if swap_args {
        (rejected, "fresh_returned")
    } else {
        ("fresh_returned", rejected)
    };
    let selects_fresh = if select_first { !swap_args } else { swap_args };
    let (expected, unselected, expected_root, expected_delta, expected_value) = if selects_fresh {
        if fresh_from_returned {
            ("fresh_returned", rejected, "primary_base", 0, 7)
        } else {
            (
                "fresh_returned",
                rejected,
                "alternate",
                fresh_delta,
                if fresh_delta == 1 { 19 } else { 15 },
            )
        }
    } else if fresh_from_returned {
        (rejected, "fresh_returned", "alternate", 0, 17)
    } else {
        (
            rejected,
            "fresh_returned",
            "primary_base",
            initial_delta,
            if initial_delta == 1 { 9 } else { 5 },
        )
    };
    let selector = format!(
        "reselect_fresh_{suffix}{}",
        if selector_two_hop { "_twice" } else { "" },
    );
    let helper_definitions = freshly_composed_distinct_root_result_reselection_helpers(kind);
    program = program.replacen(
        "int main(void) {",
        &format!("{helper_definitions}int main(void) {{"),
        1,
    ...
    let return_start = program
        .rfind("return (")
        .expect("generated fresh-result program should contain its main return");
    program.insert_str(
        return_start,
        &format!(
            "{const_pointer_type}fresh_slot_after = fresh_returned;\n\
             {const_pointer_type}rejected_slot_after = {rejected};\n\
             {const_pointer_type}final_slot_after = final_reselected;\n\
             {const_pointer_type}fresh_reselected = {selector}({first}, {second}, {selection});\n\
             ",
            selection = i64::from(select_first),
        ),
    );
    let selected_read = kind.read("fresh_reselected");
    let selector_calls = if selector_two_hop { 2 } else { 1 };
    let return_end = program
        .rfind(';')
        .expect("generated fresh-result program should end its main return");
    program.insert_str(
        return_end,
        &format!(
            "\n                 + (fresh_reselected == {expected}) + (fresh_reselected != {unselected})\n\
             + ({selected_read} == {expected_value}) + (fresh_reselected - {expected_root} == {expected_delta})\n\
             + (fresh_slot_after == fresh_returned) + (rejected_slot_after == {rejected})\n\
             + (final_slot_after == final_reselected) + (fresh_reselection_calls == {selector_calls})"
        ),
    );
    program
}

fn freshly_composed_distinct_root_result_reselection_bounds_program(
    kind: AdjustedParameterFieldKind,
    select_fresh: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let read = kind.read("fresh_reselected");
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{carried_reselection_helpers}\n{fresh_helpers}\n{fresh_reselection_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}(primary, alternate, 1);\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}_twice(carry_final_{suffix}(final_reselected));\n\
             {const_pointer_type}fresh_reselected = reselect_fresh_{suffix}_twice(fresh_returned, alternate, {selection}) + 2;\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        ...
        boundary_helpers = post_selection_reforward_and_return_helpers(kind),
        carry_helpers = distinct_root_selected_result_carry_helpers(kind),
        carried_reselection_helpers = returned_distinct_root_result_reselection_helpers(kind),
        fresh_helpers = final_reselected_distinct_root_result_carry_helpers(kind),
        fresh_reselection_helpers = freshly_composed_distinct_root_result_reselection_helpers(kind),
        selection = i64::from(select_fresh),
    )
}

fn freshly_composed_distinct_root_result_reselection_write_program(
    kind: AdjustedParameterFieldKind,
    select_fresh: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("fresh_reselected", 23);
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{carried_reselection_helpers}\n{fresh_helpers}\n{fresh_reselection_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}_twice(primary, alternate, 1);\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}(carry_final_{suffix}_twice(final_reselected));\n\
             {const_pointer_type}fresh_reselected = reselect_fresh_{suffix}(fresh_returned, alternate, {selection});\n\
             {write}; return 0;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn freshly_composed_distinct_root_result_reselection_cross_root_program(
    kind: AdjustedParameterFieldKind,
    ordering: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{carried_reselection_helpers}\n{fresh_helpers}\n{fresh_reselection_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}(primary + 1, alternate, 1);\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}_twice(carry_final_{suffix}(final_reselected - 1));\n\
             {const_pointer_type}fresh_reselected = reselect_fresh_{suffix}_twice(alternate, fresh_returned, 0);\n\
             return {operation};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn freshly_composed_distinct_root_result_reselection_lifetime_program(
    kind: AdjustedParameterFieldKind,
    select_dangling: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{carried_reselection_helpers}\n{fresh_helpers}\n{fresh_reselection_helpers}\n\
         struct Item live_items[1];\n\
         {const_pointer_type}reselect_fresh_lifetime_{suffix}(int select_dangling) {{\n\
             struct Item local[1];\n\
             {const_pointer_type}dangling = return_inner_{suffix}(local);\n\
             {const_pointer_type}live = return_distinct_inner_{suffix}(live_items);\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}_twice(dangling, live, 1);\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}_twice(carry_final_{suffix}_twice(final_reselected));\n\
             return reselect_fresh_{suffix}_twice(fresh_returned, live, select_dangling);\n\
         }}\n\
         int main(void) {{\n\
             {const_pointer_type}fresh_reselected = reselect_fresh_lifetime_{suffix}({selection});\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn freshly_composed_distinct_root_result_reselection_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{carried_reselection_helpers}\n{fresh_helpers}\n{fresh_reselection_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             ...
             const struct Point *primary = return_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             const struct Point *alternate = return_distinct_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             const struct Point *final_reselected = reselect_carried_point_twice(primary, alternate, 1);\n\
             const struct Point *fresh_returned = return_final_point_twice(carry_final_point(final_reselected));\n\
             const struct Point *fresh_reselected = reselect_fresh_point_twice(alternate, fresh_returned, 0);\n\
             const struct Other *mismatch = fresh_reselected;\n\
             return mismatch->value;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_reselected_distinct_root_result_carry_bounds_program(
    kind: AdjustedParameterFieldKind,
    select_returned: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let read = kind.read("fresh_returned");
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{reselection_helpers}\n{fresh_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}returned = return_selected_{suffix}(carry_selected_{suffix}(primary));\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}(returned, alternate, {selection});\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}_twice(carry_final_{suffix}_twice(final_reselected)) + 2;\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_reselected_distinct_root_result_carry_write_program(
    kind: AdjustedParameterFieldKind,
    select_returned: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("fresh_returned", 23);
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{reselection_helpers}\n{fresh_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
...
fn final_reselected_distinct_root_result_carry_cross_root_program(
    kind: AdjustedParameterFieldKind,
    ordering: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{reselection_helpers}\n{fresh_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}returned = return_selected_{suffix}(carry_selected_{suffix}(primary + 1));\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}_twice(returned, alternate, 1);\n\
             {const_pointer_type}fresh_returned = return_final_{suffix}_twice(carry_final_{suffix}(final_reselected - 1));\n\
             return {operation};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn final_reselected_distinct_root_result_carry_lifetime_program(
    kind: AdjustedParameterFieldKind,
    select_dangling: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{reselection_helpers}\n{fresh_helpers}\n\
         struct Item live_items[1];\n\
         {const_pointer_type}carry_final_lifetime_{suffix}(int select_dangling) {{\n\
             struct Item local[1];\n\
             {const_pointer_type}dangling = return_inner_{suffix}(local);\n\
             {const_pointer_type}live = return_distinct_inner_{suffix}(live_items);\n\
             {const_pointer_type}returned = return_selected_{suffix}_twice(carry_selected_{suffix}(dangling));\n\
             {const_pointer_type}final_reselected = reselect_carried_{suffix}_twice(returned, live, select_dangling);\n\
             return return_final_{suffix}_twice(carry_final_{suffix}_twice(final_reselected));\n\
         }}\n\
         int main(void) {{\n\
             {const_pointer_type}fresh_returned = carry_final_lifetime_{suffix}({selection});\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn final_reselected_distinct_root_result_carry_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n{carry_helpers}\n{reselection_helpers}\n{fresh_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             ...
             const struct Point *primary = return_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             const struct Point *alternate = return_distinct_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             const struct Point *returned = return_selected_point(carry_selected_point(primary));\n\
             const struct Point *final_reselected = reselect_carried_point_twice(alternate, returned, 0);\n\
             const struct Point *fresh_returned = return_final_point_twice(carry_final_point(final_reselected));\n\
             const struct Other *mismatch = fresh_returned;\n\
             return mismatch->value;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn distinct_root_final_reselection_bounds_program(
    kind: AdjustedParameterFieldKind,
    select_primary: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}(primary, alternate, {selection}) + 2;\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn distinct_root_final_reselection_write_program(
    kind: AdjustedParameterFieldKind,
    select_primary: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("reselected", 23);
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             int left_marker = 0; int right_marker = 0;\n\
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}(primary, alternate, {selection});\n\
             {write}; return 0;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn distinct_root_final_reselection_cross_root_program(
    kind: AdjustedParameterFieldKind,
    ordering: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}primary = return_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}alternate = return_distinct_inner_{suffix}((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}reselected = reselect_composed_{suffix}(primary, alternate, 1);\n\
             return {operation};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn distinct_root_final_reselection_lifetime_program(
    kind: AdjustedParameterFieldKind,
    select_dangling: bool,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    ...
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n\
         struct Item live_items[1];\n\
         {const_pointer_type}select_lifetime_{suffix}(int select_dangling) {{\n\
             struct Item local[1];\n\
             {const_pointer_type}dangling = return_inner_{suffix}(local);\n\
             {const_pointer_type}live = return_distinct_inner_{suffix}(live_items);\n\
             return reselect_composed_{suffix}(dangling, live, select_dangling);\n\
         }}\n\
         int main(void) {{\n\
             {const_pointer_type}reselected = select_lifetime_{suffix}({selection});\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        primary_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn distinct_root_final_reselection_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{primary_helpers}\n{alternate_helpers}\n{boundary_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             ...
             const struct Point *primary = return_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++left_marker}} }} }}, {{}} }});\n\
             const struct Point *alternate = return_distinct_inner_point((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             const struct Point *reselected = reselect_composed_point_twice(primary, alternate, 0);\n\
             const struct Other *mismatch = reselected;\n\
             return mismatch->value;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn post_selection_reforwarded_nested_return_bounds_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let read = kind.read("returned");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
             ...
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}_twice(reforward_composed_{suffix}_twice(composed)) + 2;\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn post_selection_reforwarded_nested_return_cross_root_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}composed = selected - 1;\n\
             {const_pointer_type}returned = return_composed_{suffix}_twice(reforward_composed_{suffix}_twice(composed));\n\
...
fn post_selection_reforwarded_nested_return_lifetime_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    ...
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         ...
         int main(void) {{\n\
             {const_pointer_type}composed = select_dangling_{suffix}();\n\
             {const_pointer_type}returned = return_composed_{suffix}_twice(reforward_composed_{suffix}_twice(composed));\n\
             return {read};\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn post_selection_reforwarded_nested_return_write_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("returned", 11);
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
...
fn post_selection_reforwarded_nested_return_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n{boundary_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             ...
             const struct Point *composed = selected - 1;\n\
             const struct Point *returned = return_composed_point_twice(reforward_composed_point_twice(composed));\n\
             const struct Other *mismatch = returned;\n\
             return mismatch->value;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
...
fn nested_const_parameter_return_selection_write_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let write = kind.write("ultimate", 11);
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n\
         int main(void) {{\n\
             int root_marker = 0;\n\
             {const_pointer_type}base = outer_return_inner_{suffix}({argument});\n\
             {const_pointer_type}first_selected = select_received_{suffix}_twice(base + 1, base, 1);\n\
             {const_pointer_type}ultimate = select_received_{suffix}(base, first_selected, 0);\n\
             {write};\n\
             return 0;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
...
fn nested_const_parameter_return_selection_cross_root_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n\
         int main(void) {{\n\
             ...
             {const_pointer_type}right = outer_return_inner_{suffix}_twice((struct Item[2]){{ {{ .nested = {{ .values = {{++right_marker}} }} }}, {{}} }});\n\
             {const_pointer_type}first_selected = select_received_{suffix}_twice(left + 1, left, 1);\n\
             {const_pointer_type}ultimate = select_received_{suffix}(first_selected, right, 1);\n\
             return ultimate - right;\n\
         }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
...
fn nested_const_parameter_return_selection_lifetime_program(
    kind: AdjustedParameterFieldKind,
) -> String {
    let const_pointer_type = kind.const_pointer_type();
    let suffix = kind.suffix();
    let read = kind.read("ultimate");
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n\
         {const_pointer_type}select_nested_dangling_{suffix}(void) {{\n\
             struct Item local[1];\n\
             {const_pointer_type}base = outer_return_inner_{suffix}_twice(local);\n\
             {const_pointer_type}first_selected = select_received_{suffix}_twice(base + 1, base, 1);\n\
             return select_received_{suffix}(base, first_selected, 0);\n\
         }}\n\
         int main(void) {{ {const_pointer_type}ultimate = select_nested_dangling_{suffix}(); return {read}; }}\n",
        prelude = captured_literal_field_offset_prelude(),
        return_helpers = derived_inner_const_pointer_callee_return_helpers(kind),
        caller_helpers = caller_inner_pointer_const_reforward_helpers(),
        outer_helpers = outer_caller_return_helpers(kind),
...
fn nested_const_parameter_return_selection_type_mismatch_program() -> String {
    let kind = AdjustedParameterFieldKind::Aggregate;
    format!(
        "{prelude}\n{return_helpers}\n{caller_helpers}\n{outer_helpers}\n{selection_helpers}\n\
         struct Other {{ int value; }};\n\
         int main(void) {{\n\
             ...
             const struct Point *base = outer_return_inner_point_twice({argument});\n\
             const struct Point *first_selected = select_received_point_twice(base + 1, base, 1);\n\
             const struct Other *ultimate = select_received_point(base, first_selected, 0);\n\
