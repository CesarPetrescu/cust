...
fn wrapped_offset_direct_literal_adjusted_parameter_alias_program(
    ...
    let (arguments, marker_check) = if separate_roots {
        (
            format!(
                "{}, {}",
                wrapper.render_with_offset("struct Item[3]", "left", offset),
                wrapper.render_with_offset("struct Item[3]", "right", offset)
            ),
            format!(
                "({}) && ({})",
        ...
        (
            wrapper.render_with_offset("struct Item[3]", "root", offset),
            wrapper.marker_check("root"),
        )
    };
...
fn wrapped_captured_literal_field_offset_alias_program(
    ...
    format!(
        ...
        read_r = kind.read("r"),
        first_storage =
            wrapped_captured_literal_field_offset_argument(first.storage, wrapper, offset, "first"),
        second_storage = wrapped_captured_literal_field_offset_argument(
            second.storage,
            wrapper,
            offset,
            "second"
        ),
        reader_storage = wrapped_captured_literal_field_offset_argument(
            reader.storage,
            wrapper,
            offset,
            "reader"
