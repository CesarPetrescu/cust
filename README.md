...
> Status: **v0.13.0** — tested, Dockerized deterministic C-subset interpreter.
...
- Next milestone: implement the first safe two-level character-pointer slice required by C11 `strtol`/`strtoul` `endptr`, preserving interpreter-owned pointee identity, lexical lifetime, qualification, null handling, and exact non-evaluating type checks without exposing host addresses. The conversion intrinsics remain deferred until absent, null, and non-null `endptr` routes are all safe.
