...
The v0.29.0 package adds a bounded first `double` runtime-value slice. Decimal literals and direct local, file-global, and block-static scalar objects participate in assignment/update, mixed arithmetic and comparison, conditional/comma/truthiness, supported scalar call/return conversions, compound literals, scalar casts, `_Generic`, integer-constant casts, and deterministic eight-byte type queries. The release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded direct `double` scalar objects at local, file-global, and block-static scope, including replacement/compound assignment, prefix/postfix update, mixed arithmetic/comparison, conditional/comma forwarding, truthiness, scalar compound literals, `int`/`double`/`_Bool` casts, `_Generic`, integer-constant casts, conversion through supported scalar function signatures, and deterministic `sizeof(double) == 8` / `_Alignof(double) == 8`
...
- Next milestone: extend bounded `double` values through direct function parameter/return declarations, prototypes, definitions, and call results while preserving exact conversion, non-evaluation, and no-double-pointer boundaries.
