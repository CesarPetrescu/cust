...
The v0.39.0 package adds fixed two-dimensional `double` typedef aliases such as `typedef double Matrix[R][C]`, including chained and alias-carried `const` forms, across local, file-global, and block-static object storage with nested positional initialization and zero fill, double-index reads and scalar replacement/compound/prefix/postfix updates, one-time indexes, declaration lists, scalar `_Generic`, and deterministic full-object/row/element/alias `sizeof` plus alias `_Alignof` relationships. Current `main` additionally gives supported standalone scalar and one-dimensional-array `double` storage a deterministic eight-byte IEEE-754 binary64 little-endian object representation across the five bounded raw-memory intrinsics. Direct and alias-spelled two-dimensional double array parameters, array returns, two-dimensional aggregate fields and compound literals, pointer-to-row forms such as `double (*)[N]`, row addresses, row-pointer arithmetic, three-dimensional double arrays, and raw-memory views over aggregate-field, two-dimensional, or union-backed double storage remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: prepare bounded v0.40.0 around the completed standalone and one-dimensional-array `double` object-byte package, with exact package/CLI/Docker/docs/test-inventory metadata, independent review, canonical local/rebuilt-Docker verification, and release-commit-first tag publication.
