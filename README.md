...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers; `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage; and bounded unsigned-byte lexical `strcmp`, with one-time ordered argument evaluation, non-evaluating `sizeof`, deterministic scan limits, and exact pointer/string diagnostics
...
- Next milestone: scope bounded explicitly prototyped `strlen` over interpreter-owned NUL-terminated `char` storage, reusing the reviewed 4,096-byte traversal while preserving exact declaration/type/lifetime/null/termination boundaries and non-evaluating `sizeof`.
