...
> Status: **v0.18.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.18.0 package includes the bounded `_Generic` feature described below. The release procedure publishes the annotated tag only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: implement a bounded first `void *` object/conversion slice with null/equality/truthiness, object-pointer conversion, qualification preservation, non-evaluating type queries, and exact no-dereference/no-arithmetic boundaries before considering memory intrinsics.
