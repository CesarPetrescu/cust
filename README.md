...
The v0.25.0 package extends the bounded five-function C11 raw-memory family to deterministic object bytes of standalone `int`/`_Bool` scalars and one-dimensional arrays while preserving interpreter-owned provenance and avoiding host ABI inference. The release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: extend deterministic object-byte raw-memory operations to non-character scalar fields in supported struct storage while retaining field-local bounds, recursive const/lifetime checks, and exact whole-aggregate/union boundaries.
