...
> Status: **v0.35.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.35.0 executable inventory is 2,057 tests: 1,921 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.35.0 package extends safe one-level direct `double *` values to scalar and one-dimensional-array fields in supported structs. Direct, indexed, reverse-subscript, arrow, nested, compound-literal, and aggregate-valued expression routes retain Cust's owner, root/element/field path, lifetime, const, and bounds metadata through pointer fields, calls, returns, conditionals, comma expressions, and non-evaluating type queries. Deeper double pointers, pointer-to-row forms such as `double (*)[N]`, whole-array addresses, union-backed double field addresses/decay, typedef names for double types, multidimensional double arrays, and raw-memory views over double storage remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: add typedef aliases for supported direct `double` scalar, one-dimensional-array, function-boundary, aggregate-field, and one-level-pointer forms while preserving the existing deeper-pointer, pointer-to-row, multidimensional, union-backed-pointer, and raw-memory boundaries.
