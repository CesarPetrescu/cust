...
> Status: **v0.43.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.43.0 executable inventory is 2,111 tests: 1,975 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.43.0 package keeps Cust's deterministic eight-byte IEEE-754 binary64 little-endian object representation available to one selected row of supported fixed two-dimensional `double[R][C]` storage when bounded-memory calls are composed in short-circuit expressions. A row-returning `memset(*row, ...)` comparison may be the left operand of `||` with a row-byte `memcmp(*row, ...)` on the right while preserving source order, one-time argument evaluation, sequential-`if` parity, and adjacent-row isolation. Runtime alias analysis recognizes the live selected row directly instead of forcing it through unsupported scalar-pointer decay. Direct row decay, explicit row-pointer indexing/dereference, adjusted direct and typedef-backed parameters, and supported aggregate-field rows retain row-local capacity/overlap, checked row/element arithmetic, owner/lifetime/recursive const, aligned/interior identity, zero count, and direct/nested non-evaluating validation without flattening adjacent rows, exposing host addresses, or invoking host libc. Cross-row ranges, union-backed storage, whole two-dimensional objects and aggregate layouts containing double fields, row addresses, incompatible row casts/widths, and unprovable helper-returned roots retain targeted rejection. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: prepare bounded v0.44.0 around the completed scalar-only shared union object-byte slice with synchronized version/docs/inventory metadata, version-first tests, independent review, local/rebuilt-Docker verification, and release-commit-first tag publication.
