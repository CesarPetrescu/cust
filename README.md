...
> Status: **v0.28.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.28.0 executable inventory is 1,534 tests: 1,399 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.28.0 package extends the bounded five-function C11 raw-memory family to complete supported struct objects under Cust's deterministic field-order, no-padding layout. Scalar fields, one- and two-dimensional scalar arrays, nested structs, and embedded struct arrays retain interpreter-owned provenance and exact union/pointer-field boundaries without host ABI inference. The release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: design and implement a bounded first `double` runtime-value slice with literals, objects, arithmetic/comparison, scalar casts, deterministic type queries, and exact unsupported `float`/`long double` boundaries.
