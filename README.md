...
The v0.36.0 package adds typedef aliases for bounded direct `double` values. Scalar and const aliases, fixed one-dimensional array aliases with parameter adjustment, direct function parameter/return aliases, supported aggregate scalar/array/pointer fields, and one-level pointer aliases preserve value type, pointee qualification, pointer-slot const, owner, lifetime, bounds, and non-evaluating metadata. Deeper pointers, arrays of pointers, pointer-to-row forms such as `double (*)[N]`, multidimensional arrays, whole-array addresses, union-backed double field addresses/decay, atomic double-pointer aliases, double-array compound literals, and raw-memory views over double storage remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: add bounded direct and typedef-backed one-dimensional `double` array compound literals with hidden-root owner/lifetime/const metadata, decay and non-evaluating type-query coverage, while retaining exact pointer-to-row, multidimensional, union-backed, and raw-memory boundaries.
