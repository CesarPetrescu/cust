...
> Status: **v0.32.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.32.0 executable inventory is 1,642 tests: 1,507 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.32.0 extends `double` values through direct scalar fields in supported structs and unions, including positional/designated initialization, direct/indexed/arrow/nested reads and updates, aggregate copies and function boundaries, deterministic field queries, recursive const protection, and shared scalar-union bits. Direct double pointers/decay, double array parameters and double array compound literals, multidimensional double arrays, one-dimensional double arrays in aggregate fields, raw-memory views, mixed non-scalar union layouts, and non-`int` `main` remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded direct `double` scalar objects and one-dimensional arrays at local, file-global, and block-static scope plus scalar fields in supported structs and unions, including fixed/inferred array lengths, positional/designated initialization and zero fill, direct/reverse indexed and direct/indexed/arrow/nested field replacement/compound/prefix/postfix updates, mixed arithmetic/comparison, conditional/comma forwarding, truthiness, scalar compound literals, `int`/`double`/`_Bool` casts, `_Generic`, integer-constant casts, direct scalar parameter/return declarations and call results, aggregate copies and function boundaries, recursive const protection, shared scalar-union bits, non-evaluating `sizeof`, and deterministic eight-byte size/alignment relationships
...
- Next milestone: publish the prepared bounded v0.32.0 package; after publication, extend direct one-dimensional `double` arrays cautiously into supported aggregate fields without enabling double pointers or multidimensional double storage.
