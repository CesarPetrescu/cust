...
> Status: **v0.60.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.60.0 executable inventory is 2,596 tests: 2,444 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 8 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.60.0 packages fixed one-dimensional tracked scalar-output arrays (`T **outputs[N]`) across unqualified `char`, `int`, `_Bool`, and `double` pointees. Direct, inner-pointer-alias, complete-output-alias, and chained-alias element spellings work for automatic, file-global, and block-static objects with positional/designated or zero/default initialization, indexed reassignment and forwarding, typed indirect access, stable interpreter-owned slot identity, lexical owner/lifetime and const checks, and static-storage validation. Element `sizeof` yields Cust's fixed pointer size, full-array `sizeof` scales it by the element count, and both suppress side effects while still validating nested calls, initializers, assignments, and updates under deterministic depth/work limits. Arrays deliberately do not decay to scalar pointers; whole-array addresses, element addresses, tracked-output casts, element arithmetic/compound updates/increment/decrement, relational ordering, adjusted array parameters, aggregate-field arrays, deeper pointers, and multidimensional tracked-output arrays remain unsupported or retain targeted diagnostics. One hundred thirty-five focused interpreter regressions, generated alias-boundary parity, and one registered warning-free compiler-oracle fixture cover the package. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- fixed one-dimensional tracked scalar-output arrays such as `int **outputs[N]` preserve each element's typed output-slot identity across automatic, file-global, and block-static storage, direct and alias-backed declarations, positional/designated/default initialization, indexed assignment, forwarding, typed indirect access, and non-evaluating full-array/element `sizeof`; array decay, whole-array and element addresses, tracked-output casts, array-parameter adjustment, aggregate-field arrays, deeper pointers, multidimensional arrays, arithmetic, ordering, and update operators remain outside this bounded slice
...
- Post-v0.60.0 next milestone: implement a first safe tracked scalar-output aggregate-field array slice (`struct Box { T **outputs[N]; }`) across direct/indexed/nested/arrow object routes, preserving containing-object owner/lifetime, recursive const, indexed initialization/assignment/forwarding, non-evaluating type checks, and exact union/deeper-pointer/decay/address boundaries.
