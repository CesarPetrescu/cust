...
The v0.19.0 package includes the bounded first safe `void *` object/conversion feature described below. The release procedure publishes the annotated tag only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded one-level `void *` objects at local, file-global, block-static, `for`-initializer, and parameter scope, with null/equality/truthiness, ordinary assignment, conditional/comma forwarding, compatible object-pointer conversions, qualification preservation, and non-evaluating pointer-size queries while retaining interpreter-owned owner/lifetime/read-only identity
...
- Next milestone: extend the one-level `void *` model through explicit function return types, prototypes, and call results while preserving owner/lifetime/qualification metadata and exact unsafe-operation boundaries; this is the prerequisite for considering bounded memory intrinsics.
