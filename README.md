...
- manually declared C11 integer absolute-value library calls `abs`, `labs`, and `llabs`, executed by Cust with one-time argument evaluation, non-evaluating `sizeof`, and deterministic overflow diagnostics
...
- Next milestone: scope a bounded `atoi`/`atol`/`atoll` follow-up over interpreter-owned NUL-terminated `char` storage, including whitespace/sign/digit parsing, one-time pointer evaluation, bounded scanning, exact prototype/type/lifetime/termination/overflow boundaries, and warning-free native-oracle fixtures.
