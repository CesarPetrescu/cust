...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers; `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage; bounded unsigned-byte lexical `strcmp`/`strncmp`; bounded `strlen`; pointer-preserving bounded `strchr`/`strrchr`/`strpbrk`/`strstr`; initial-segment `strspn`/`strcspn`; and capacity-checked mutable `strcpy`, with one-time ordered argument evaluation, non-evaluating `sizeof`, deterministic scan/copy limits, and exact pointer/string diagnostics
...
- Next milestone: implement bounded explicitly prototyped C11 `strstr`, reusing two-sequence validation while returning the first matching haystack pointer with preserved owner/lifetime/read-only metadata.
