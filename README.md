...
> Status: **v0.41.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.41.0 executable inventory is 2,089 tests: 1,953 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.41.0 package extends Cust's deterministic eight-byte IEEE-754 binary64 little-endian object representation across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr` from standalone scalar and one-dimensional-array `double` storage to scalar and one-dimensional-array fields in supported non-union structs. Full and partial reads/writes preserve destination-aware typed coercion, field-local capacity and overlap, zero count, recursive const and lexical-lifetime safety, owner/path/interior identity, aligned/interior `memchr` results, one-time ordered argument evaluation, and direct/nested non-evaluating validation without exposing host addresses or invoking host libc. Two-dimensional, union-backed, pointer-object, whole-aggregate layouts containing double fields, pointer-field targets that cannot be proved to be supported storage, and other unprovable helper-returned double storage remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: extend deterministic binary64 object-byte access across all five bounded raw-memory intrinsics to one selected row of supported two-dimensional `double` storage, preserving row-local capacity/overlap, owner/lifetime/const metadata, aligned/interior identity, checked row-index arithmetic, and exact cross-row/union/whole-aggregate boundaries without flattening adjacent rows.
