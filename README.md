...
> Status: **v0.59.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.59.0 executable inventory is 2,449 tests: 2,297 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 8 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.59.0 packages safe tracked scalar-output aggregate fields across `char **`, `int **`, `_Bool **`, and `double **`. Direct, inner-pointer-alias, and complete-output-alias field declarations preserve the containing object's owner/lifetime, recursive const ancestry, scalar pointee type, qualification, and tracked slot identity through direct, indexed, nested, arrow, embedded-aggregate-array, reverse-subscript, aggregate-expression, conditional, comma, `_Generic`, assignment-result, forwarding, return, copy, and static-initializer routes without host addresses. Same-pointee direct unions synchronize one tracked identity; mixed-pointee, output/non-output, and nested-output union overlap remains rejected. Qualified output fields, output-field arrays, deeper pointers, tracked-output casts, address-taking of tracked outputs, arithmetic, relational ordering, increment/decrement, and compound updates retain targeted boundaries; scalar truth conversion such as `(_Bool)aggregate.output` remains supported. Runtime and non-evaluating callee analysis apply the same field conversion, static-storage, operation, and deterministic resource-limit checks, while recursive tracked-output return-call validation retains its exact 16-level callee-expression limit. One hundred thirty-five focused interpreter regressions, the generated output-parity suite, and one registered warning-free compiler-oracle fixture cover the package. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- Post-publication next milestone: implement a first safe fixed one-dimensional tracked scalar-output array slice (`T **outputs[N]`) across local, file-global, and block-static objects, with indexed initialization/assignment/forwarding, exact owner/lifetime/const behavior, non-evaluating full-array and element `sizeof`, and retained decay/address/deeper-pointer boundaries.
