...
> Status: **v0.50.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.50.0 executable inventory is 2,137 tests: 1,993 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.50.0 packages deterministic pointer classifier/evaluator parity. Sixty-nine stable pointer-valued route identities each cross compatible initializer, function-argument, function-return, and non-evaluating `sizeof` consumers plus incompatible initializer, argument, return, and assignment consumers for 552 generated programs; seven additional routes prove const preservation. Identity-keyed route/context/cell totals, panic guards, one-time markers, lexical owner/lifetime checks, and exact conversion diagnostics keep `expr_is_pointer_value()`, pointee and qualification metadata, runtime evaluation, and consumer dispatch aligned. Review-driven regressions also added aggregate-array element-address metadata, lexical owners for scalar-array compound literals, and metadata-only pointer-conversion checks before assignment and static-initializer side effects. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- Post-publication next milestone: generalize Cust's tracked two-level pointer model with a first safe unqualified `int **` object/output-parameter slice. Cover local/file-global/block-static objects, mutable `int *` slot addresses, null/default state, indirect reads/writes, parameter forwarding, owner/lifetime/const preservation, non-evaluating `sizeof`, and exact qualified/deeper/array/arithmetic boundaries without host addresses.
