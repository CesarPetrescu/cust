...
The v0.45.0 package extends shared object-byte semantics from selected scalar-union members to complete union-object ranges across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Every admitted member pointer and whole-object pointer resolves to one canonical aggregate root at offset zero; whole-object operations use the union's maximum layout size, while selected-member operations remain bounded by that member's own size. Deterministic little-endian `int`, `char`, and `_Bool` writes stay visible through aliasing members, including raw bytes beneath normalized `_Bool` values. Admission still requires a non-const `int` or `char` carrier as wide as the largest member; overlap, recursive const/lifetime, nested and array-element isolation, zero-count identity, typed `memchr` results, and direct/nested non-evaluation remain enforced. All-`_Bool` layouts, missing full-width writable carriers, and layouts containing arrays, pointers, `double`, or nested aggregates retain targeted rejection. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: add interpreter-owned persistent bytes for scalar unions, beginning with all-`_Bool` layouts that cannot use a non-normalizing full-width member as their carrier; preserve canonical selected/whole-object identity, raw bytes beneath normalized values, and exact const/lifetime/overlap/non-evaluation boundaries.
