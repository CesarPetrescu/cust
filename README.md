...
> Status: **v0.58.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.58.0 executable inventory is 2,309 tests: 2,157 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 8 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.58.0 packages safe tracked scalar-output function returns across `char **`, `int **`, `_Bool **`, and `double **`. Direct `T **`, inner-pointer-alias, complete-output-alias, and chained-alias return spellings preserve caller-owned output-slot identity through return chains, compatible assignment and arguments, typed indirect reads and writes, equality/truthiness, and non-evaluating `sizeof(call)`. Qualification, scalar pointee type, lexical owner/lifetime, static-storage validation, and callee-local escape rejection remain interpreter-owned; non-scalar, deeper-pointer, array, aggregate-field, tracked-output cast, arithmetic, relational-ordering, and compound-update forms retain targeted boundaries. Scalar truth conversion such as `(_Bool)output_call()` remains supported. Recursive non-evaluating tracked-output return-call validation is bounded to 16 callee-expression levels and reports a deterministic limit diagnostic. Seventy-three focused interpreter regressions, one generated alias-return parity test, and one registered warning-free compiler-oracle fixture cover the package without host addresses or a native runtime path. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- Post-publication next milestone: implement a first safe tracked scalar-output aggregate-field slice across `char **`, `int **`, `_Bool **`, and `double **`, preserving containing-object owner/lifetime and recursive const metadata through direct, indexed, nested, and arrow field routes while retaining targeted non-scalar, deeper-pointer, array, cast, arithmetic, ordering, and compound-update boundaries.
