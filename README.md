...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers; `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage; bounded unsigned-byte lexical `strcmp`/`strncmp`; bounded `strlen`; pointer-preserving bounded `strchr`/`strrchr`/`strpbrk`/`strstr`; initial-segment `strspn`/`strcspn`; capacity-checked mutable `strcpy`/`strcat`/`strncat`/`strncpy`; and stateful in-place `strtok` over tracked mutable storage, with one-time ordered argument evaluation, non-evaluating `sizeof`, deterministic scan/copy limits, and exact pointer/string diagnostics
...
- Next milestone: implement a bounded explicitly prototyped C11 `<ctype.h>` classification package with deterministic ASCII/C-locale classes, exact integer-call diagnostics, and native-oracle parity.
