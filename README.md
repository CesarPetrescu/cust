...
> Status: **v0.61.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.61.0 executable inventory is 2,652 tests: 2,495 interpreter tests, 101 deterministic fuzz-safety tests, 36 CLI tests, 6 pointer-classifier parity tests, 10 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.61.0 packages fixed one-dimensional tracked scalar-output arrays embedded in supported struct fields (`struct Box { T **outputs[N]; }`) across unqualified `char`, `int`, `_Bool`, and `double` pointees. Direct, inner-pointer-alias, complete-output-alias, and chained-alias element spellings work through direct, indexed, nested, arrow, embedded-containing-array, reverse-subscript, aggregate-valued temporary, copy, and return routes with positional/designated or zero/default initialization, replacement initialization, indexed reassignment and forwarding, typed indirect access, stable interpreter-owned slot identity, containing-object and referenced-pointee lifetime checks, recursive const ancestry, and static-storage validation. The array field requires its own nested braces (for example, `struct Box box = {{0}};`); flat aggregate initialization such as `struct Box box = {0};` is rejected with a targeted diagnostic. Element `sizeof` yields Cust's fixed pointer size, full-field-array `sizeof` scales it by the element count, and both suppress side effects while still validating nested calls, initializers, assignments, updates, and containing indexes under deterministic depth/work limits. Field arrays deliberately do not decay to scalar pointers; unions containing pointer-output arrays, whole-array or element addresses, tracked-output casts, element arithmetic/compound updates/increment/decrement, relational ordering, deeper pointers, flexible members, and multidimensional tracked-output arrays remain unsupported or retain targeted diagnostics. Fifty-one focused interpreter regressions, 160 generated route/boundary programs, three hostile CLI parser-depth regressions, and one registered warning-free compiler-oracle fixture cover the package. Ordinary unary/grouping syntax is capped at 40 levels and integer-constant syntax at 64 so mixed parser/type-query inputs fail with recoverable diagnostics on a 2 MiB host thread stack. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- Post-v0.61.0 next milestone: expand deterministic tracked-output aggregate-field-array classifier/evaluator parity across aggregate-valued temporary, copy/return, static, conditional, comma, assignment-result, and `_Generic` routes, with identity-keyed coverage assertions plus exact qualification, lifetime, const, union, decay, address, and deeper-shape boundaries.
