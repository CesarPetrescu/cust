...
> Status: **v0.39.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.39.0 executable inventory is 2,076 tests: 1,940 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
- Next milestone: define and implement deterministic IEEE-754 binary64 object-byte views for supported standalone and one-dimensional-array `double` storage across the five exactly prototyped bounded raw-memory intrinsics, with byte-order, partial-write, typed-coercion, const/lifetime/bounds/overlap, non-evaluating, and exact unsupported aggregate/two-dimensional/union boundaries covered by focused tests.
