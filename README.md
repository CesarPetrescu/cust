...
> Status: **v0.44.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.44.0 executable inventory is 2,117 tests: 1,981 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.44.0 package adds shared object-byte semantics for selected members of scalar-only unions across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Every admitted member pointer resolves to one canonical aggregate root at offset zero; deterministic little-endian `int`, `char`, and `_Bool` writes remain visible through aliasing members, including raw bytes beneath normalized `_Bool` values. Admission requires a non-const `int` or `char` carrier as wide as the largest member, while each operation remains bounded by the selected member's own size and preserves overlap, const/lifetime, array-element isolation, zero-count identity, and direct/nested non-evaluation. Whole-union ranges, all-`_Bool` layouts, missing full-width writable carriers, and layouts containing arrays, pointers, `double`, or nested aggregates retain targeted rejection. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: extend the bounded raw-memory model from selected members to whole-object byte ranges for the already admitted scalar-only union layouts, preserving maximum-layout capacity, alias synchronization, const/lifetime/overlap rules, and exact unsupported-layout boundaries.
