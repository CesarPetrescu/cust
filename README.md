...
> Status: **v0.56.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.56.0 executable inventory is 2,231 tests: 2,084 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 3 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.56.0 packages typedef aliases of complete tracked scalar output types across `char`, `int`, `_Bool`, and `double` objects and function parameters. Direct `T **`, inner-alias `typedef T *ValuePtr; ValuePtr *output`, and complete-alias `typedef ValuePtr *Output; Output output` spellings now lower to the same interpreter-owned tracked representation, including chained complete aliases, first and later comma declarators, local/file-global/block-static storage, forwarding, typed indirect reads and writes, and non-evaluating `sizeof`. Dedicated semantic alias metadata preserves the scalar pointee without exposing host addresses. Alias pointee or output-slot qualification, non-scalar complete aliases, deeper pointers, pointer arrays, aggregate fields, complete-alias casts and function returns, address-taking, arithmetic, ordering, and compound updates retain targeted boundaries. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- Post-publication next milestone: add a deterministic complete-output-alias parity matrix across all four scalar pointees, direct/inner-alias/complete-alias/chained-alias spellings, local/file-global/block-static/parameter routes, and initializer/assignment/argument/equality/truthiness/non-evaluating consumers, with identity-keyed exact totals plus qualification, lifetime, and retained-boundary cells.
