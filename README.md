...
- Next milestone: design true shared-member byte storage for supported unions, then implement a first bounded scalar-only union object-byte slice with aliased member views, deterministic capacity, const/lifetime/overlap checks, non-evaluating validation, exact unsupported-layout boundaries, focused regressions, and ABI-independent compiler-oracle relationships.
