...
> Status: **v0.55.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.55.0 executable inventory is 2,223 tests: 2,076 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 3 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.55.0 packages pointer typedef aliases as the inner type of tracked scalar output objects and parameters across `char`, `int`, `_Bool`, and `double`. Direct `T **` and alias-backed `typedef T *ValuePtr; ValuePtr *output` spellings now lower to the same interpreter-owned tracked representation in function parameters, first object declarators, and later comma declarators. Local, file-global, and block-static storage, chained aliases, forwarding, indirect typed writes, non-evaluating `sizeof`, owner/lifetime identity, and static-storage rules remain shared with the direct spelling. Alias pointee qualification, alias pointer-slot qualification, qualified outer slots, deeper pointers, pointer arrays, aggregate fields, non-scalar pointer aliases, pointer returns, address-taking, arithmetic, ordering, and compound updates retain targeted boundaries. No host addresses or native runtime path are introduced. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- pointer typedef aliases may provide the inner one-level scalar pointer for each tracked output family, so `typedef T *ValuePtr; ValuePtr *output` shares the direct `T **output` representation across parameters and local/file-global/block-static objects, including chained aliases and comma declarators; qualification on either alias dimension or the outer slot remains rejected
...
- Post-publication next milestone: support typedef aliases of the complete tracked scalar output type (for example `typedef int *IntPtr; typedef IntPtr *IntOutput; IntOutput output`) across objects and function parameters while preserving qualification, owner/lifetime identity, non-evaluating classification, and exact non-scalar/deeper/array/aggregate/return boundaries.
