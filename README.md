...
> Status: **v0.33.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.33.0 executable inventory is 1,715 tests: 1,580 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.33.0 extends direct one-dimensional `double` arrays into supported aggregate fields with positional/designated initialization and zero fill, direct/indexed/arrow/nested/reverse element reads and updates, aggregate pointer-field element routes, aggregate copies and function boundaries, conditional/comma/`_Generic` wrappers, deterministic full-field and element queries, recursive const protection, and one-time index evaluation. Direct double pointers and ordinary decay, double array parameters and double array compound literals, multidimensional double arrays, raw-memory views over double storage, mixed non-scalar union layouts, and non-`int` `main` remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: after publishing bounded v0.33.0, scope a safe one-level direct `double *` slice over interpreter-owned scalar and one-dimensional-array storage without enabling deeper pointers or multidimensional double storage.
