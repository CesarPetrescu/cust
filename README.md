...
> Status: **v0.38.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.38.0 executable inventory is 2,073 tests: 1,937 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.38.0 package adds direct fixed two-dimensional `double[R][C]` objects at local, file-global, and block-static scope with nested positional initialization and zero fill, double-index reads and scalar replacement/compound/prefix/postfix updates, one-time indexes, declaration lists, scalar `_Generic`, and deterministic full-object/row/element `sizeof` plus `_Alignof` relationships. Two-dimensional double typedef aliases, parameters, aggregate fields, compound literals, pointer-to-row forms such as `double (*)[N]`, row addresses, row-pointer arithmetic, three-dimensional double arrays, and raw-memory views over double storage remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: add bounded fixed two-dimensional `double` typedef aliases such as `typedef double Matrix[R][C]` across direct object storage, nested initialization and zero fill, double-index scalar lvalues, deterministic type queries, exact adjusted-parameter/pointer-to-row/aggregate-field/compound-literal boundaries, focused fixtures, and warning-free ABI-independent compiler-oracle relationships.
