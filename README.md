...
> Status: **v0.29.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.29.0 executable inventory is 1,592 tests: 1,457 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.29.0 package adds a bounded first `double` runtime-value slice. Current `main` additionally carries `double` values through direct function parameters and returns, prototypes, definitions, recursion, call-result type metadata, scalar conversions, `_Generic`, and non-evaluating `sizeof(call)`, while retaining exact pointer/array/row-pointer boundaries. The release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: package the completed direct `double` function-boundary slice as bounded v0.30.0 with synchronized executable version, Docker, documentation, status, and release-tag evidence.
