...
Current `main` adds a safe one-level direct `double *` slice over interpreter-owned scalar and one-dimensional-array storage. Local/global/static objects, address/decay, indexing/arithmetic/equality/truthiness, pointer fields, function forwarding, qualification-preserving `void *` conversion, and non-evaluating type queries retain Cust's owner, lifetime, const, and bounds metadata. Deeper double pointers, aggregate-field double addresses/decay, typedef names for double types, multidimensional double arrays, raw-memory views over double storage, mixed non-scalar union layouts, and non-`int` `main` remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- Next milestone: prepare bounded v0.34.0 around the completed safe one-level direct `double *` slice.
