...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers; `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage; bounded unsigned-byte lexical `strcmp`/`strncmp`; bounded `strlen`; and pointer-preserving bounded `strchr`/`strrchr`, with one-time ordered argument evaluation, non-evaluating `sizeof`, deterministic scan/count limits, and exact pointer/string diagnostics
...
- Next milestone: scope bounded explicitly prototyped `strpbrk` as the next standard-library slice, preserving interpreter-owned pointer identity, left-to-right one-time evaluation, and exact argument-specific null/type/lifetime/bounded-search diagnostics across both character sequences.
