...
- Next milestone: scope bounded explicitly prototyped `strchr` as the next standard-library slice, preserving interpreter-owned pointer identity, const/read-only metadata, one-time source-ordered evaluation, and exact null/type/lifetime/bounded-search diagnostics for found and not-found results.
