...
> Status: **v0.57.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.57.0 executable inventory is 2,235 tests: 2,084 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 7 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.57.0 packages deterministic classifier/evaluator parity for complete tracked scalar output aliases across `char`, `int`, `_Bool`, and `double`. An identity-keyed 384-program matrix crosses direct `T **`, inner-alias, complete-alias, and chained-complete-alias spellings with local, file-global, block-static, and parameter routes plus initializer, assignment, argument, equality, truthiness, and non-evaluating `sizeof` consumers. Seventy-two supplemental programs preserve qualification, split non-observing/evaluated lifetime behavior, and non-scalar, deeper-pointer, array, aggregate-field, cast, and return boundaries. The matrix confirms that every supported spelling shares the same interpreter-owned identity, one-time evaluation, backing-slot mutation, null behavior, owner/lifetime, and const metadata without changing the production runtime path or exposing host addresses. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`.
...
- pointer typedef aliases may provide either the inner one-level scalar pointer or the complete tracked output type for each family, so `typedef T *ValuePtr; ValuePtr *output` and `typedef ValuePtr *Output; Output output` share the direct `T **output` representation across parameters and local/file-global/block-static objects, including chained complete aliases and comma declarators; qualification on the pointee, output slot, or alias chain remains rejected
...
- Post-publication next milestone: implement the first safe tracked scalar-output function-return slice across `char **`, `int **`, `_Bool **`, and `double **`, including direct, inner-alias, complete-alias, and chained-alias return spellings; preserve caller-owned identity, qualification, lifetime/static-storage validation, forwarding, and non-evaluating `sizeof(call)` while retaining exact callee-local escape, non-scalar, deeper-pointer, array, aggregate-field, cast, arithmetic, ordering, and compound-update boundaries.
