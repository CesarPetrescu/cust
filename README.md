...
> Status: **v0.15.0** — tested, Dockerized deterministic C-subset interpreter.
...
- narrowly typed unqualified `char **` objects at local, file-global, and block-static scope plus standard-library-style output parameters, preserving mutable `char *` slot identity, pointee owner/lifetime/read-only metadata, null/default state, indirect reads/writes, forwarding, equality/truthiness, and non-evaluating type checks without exposing host addresses
...
- Next milestone: extend safe `char **` expression parity with conditional/comma object-value forwarding, qualified function-parameter slot-address rejection, and exact relational-comparison boundaries without exposing host addresses or deeper pointers.
