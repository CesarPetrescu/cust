...
The v0.30.0 package extends bounded `double` values through direct function parameters and returns, prototypes, definitions, recursion, call-result type metadata, integer/double conversions, casts, `_Generic`, and non-evaluating `sizeof(call)`. Direct double pointers, arrays, aggregate fields, pointer-to-row forms, and non-`int` `main` remain unsupported. The release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded direct `double` scalar objects at local, file-global, and block-static scope, including replacement/compound assignment, prefix/postfix update, mixed arithmetic/comparison, conditional/comma forwarding, truthiness, scalar compound literals, `int`/`double`/`_Bool` casts, `_Generic`, integer-constant casts, direct parameter/return declarations and call results, recursive forwarding, non-evaluating `sizeof(call)`, and deterministic `sizeof(double) == 8` / `_Alignof(double) == 8`
...
- Next milestone: extend bounded `double` storage to direct one-dimensional arrays with initialization, indexing, assignment/update, scalar conversion, deterministic type queries, exact pointer/decay boundaries, and warning-free ABI-independent native parity.
