...
> Status: **v0.34.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.34.0 executable inventory is 1,977 tests: 1,841 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
The v0.34.0 package adds a safe one-level direct `double *` slice over interpreter-owned scalar and one-dimensional-array storage. Local/global/static objects, address/decay, indexing/arithmetic/equality/truthiness, pointer fields, function forwarding, qualification-preserving `void *` conversion, and non-evaluating type queries retain Cust's owner, lifetime, const, and bounds metadata. Deeper double pointers, pointer-to-row double forms such as `double (*)[N]`, aggregate-field double addresses/decay, typedef names for double types, multidimensional double arrays, raw-memory views over double storage, mixed non-scalar union layouts, and non-`int` `main` remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded direct `double` scalar objects and one-dimensional arrays at local, file-global, and block-static scope plus scalar and one-dimensional-array fields in supported structs and unions, including fixed/inferred array lengths, positional/designated initialization and zero fill, direct/reverse indexed and direct/indexed/arrow/nested field replacement/compound/prefix/postfix updates, aggregate pointer-field element routes, mixed arithmetic/comparison, conditional/comma/`_Generic` forwarding, truthiness, scalar compound literals, `int`/`double`/`_Bool` casts, integer-constant casts, direct scalar parameter/return declarations and call results, aggregate copies and function boundaries, recursive const protection, shared scalar-union bits, one-time index evaluation, non-evaluating full-field/element `sizeof`, and deterministic eight-byte size/alignment relationships
- safe one-level direct `double *` objects over standalone scalar and one-dimensional-array storage, including local/file-global/block-static declarations, address and array decay, indexed reads and updates, bounded arithmetic, same-array comparison, truthiness/equality, pointer fields, parameter/return forwarding, qualification-preserving `void *` conversion, and non-evaluating `sizeof`/`_Alignof`/`_Generic` classification with interpreter-owned owner/lifetime/const metadata
...
- Next milestone: extend the direct `double *` model to scalar and one-dimensional-array fields in supported aggregates, with direct/indexed/arrow/nested address/decay routes, exact recursive const/lifetime/bounds behavior, non-evaluating queries, and warning-free native parity.
