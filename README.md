...
> Status: **v0.9.0** — tested, Dockerized deterministic C-subset interpreter.
...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers; `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage; deterministic ASCII/C-locale `isalnum`/`isalpha`/`isblank`/`iscntrl`/`isdigit`/`isgraph`/`islower`/`isprint`/`ispunct`/`isspace`/`isupper`/`isxdigit`; bounded unsigned-byte lexical `strcmp`/`strncmp`; bounded `strlen`; pointer-preserving bounded `strchr`/`strrchr`/`strpbrk`/`strstr`; initial-segment `strspn`/`strcspn`; capacity-checked mutable `strcpy`/`strcat`/`strncat`/`strncpy`; and stateful in-place `strtok` over tracked mutable storage, with one-time ordered argument evaluation, non-evaluating `sizeof`, deterministic scan/copy limits, and exact pointer/string diagnostics
...
- Next milestone: add bounded explicitly prototyped C11 `tolower` and `toupper` intrinsics with deterministic ASCII/C-locale conversion, one-time scalar evaluation, exact EOF/unsigned-character-domain handling, user-definition precedence, non-evaluating `sizeof`, and warning-free native-oracle coverage.
