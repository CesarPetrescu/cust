...
> Status: **v0.31.0** — tested, Dockerized deterministic C-subset interpreter.
...
The v0.31.0 executable inventory is 1,598 tests: 1,463 interpreter tests, 99 deterministic fuzz-safety tests, 32 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.
...
Bounded v0.31.0 extends `double` values through direct one-dimensional arrays with fixed or inferred lengths, positional/designated initialization and zero fill, direct and reverse indexed scalar reads/writes/updates, conversion, `_Generic`, and deterministic type queries. Direct double pointers/decay, array parameters/compound literals, multidimensional double arrays, aggregate fields, and non-`int` `main` remain unsupported. Release procedure publishes annotated tags only after independent review, the canonical gate, and release-commit acceptance on `origin/main`. Cust currently supports this C subset:
...
- bounded direct `double` scalar objects and one-dimensional arrays at local, file-global, and block-static scope, including fixed/inferred lengths, positional/designated initialization and zero fill, direct/reverse indexed replacement/compound/prefix/postfix update, mixed arithmetic/comparison, conditional/comma forwarding, truthiness, scalar compound literals, `int`/`double`/`_Bool` casts, `_Generic`, integer-constant casts, direct scalar parameter/return declarations and call results, recursive forwarding, non-evaluating `sizeof`, and deterministic eight-byte size/alignment relationships
...
- Next milestone: publish the prepared bounded v0.31.0 package; after publication, extend `double` cautiously into direct aggregate scalar fields without enabling double pointers, double aggregate-array fields, or multidimensional double storage.
