...
- manually declared C11 integer library calls: `abs`, `labs`, and `llabs` over Cust integers plus `atoi`, `atol`, and `atoll` over interpreter-owned NUL-terminated `char` storage, with one-time argument evaluation, non-evaluating `sizeof`, bounded scans, and deterministic overflow diagnostics
...
- Next milestone: scope a bounded explicitly prototyped `strcmp` slice over two interpreter-owned NUL-terminated `char` pointers, including left-to-right one-time argument evaluation, independent scan bounds, lexical unsigned-byte ordering, exact type/lifetime/null/termination boundaries, and warning-free native-oracle fixtures.
