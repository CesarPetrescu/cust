...
- Added deterministic classifier/evaluator parity for complete tracked scalar output aliases across `char`, `int`, `_Bool`, and `double`. The identity-keyed primary matrix executes 384 programs from four pointee kinds × four direct/inner-alias/complete-alias/chained-alias spellings × four local/file-global/block-static/parameter routes × six initializer/assignment/argument/equality/truthiness/non-evaluating `sizeof` consumers.
- Added 72 supplemental programs: 16 qualification cells, 32 split non-observing/evaluated lifetime programs, and 24 retained non-scalar, deeper-pointer, array, aggregate-field, cast, and return boundary cells. Direct file-global operands, route-specific backing-slot checks, null branches, and observable `sizeof` side-effect witnesses resist false-positive coverage.
- Confirmed shared interpreter-owned identity, one-time evaluation, typed backing-slot mutation, qualification, owner/lifetime, and null behavior across every supported spelling. This release adds conformance coverage without changing the production runtime path or exposing host addresses.

### CLI, packaging, and verification

- Versioned the Cargo package, exact CLI output, and Docker Compose runtime/test images as `0.57.0`.
- Reconciled an executable inventory of 2,235 tests: 2,084 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 7 tracked pointer-output parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

### Known limitations

- Cust remains a deterministic educational C subset, not a full C implementation or native-ABI emulator.
- Tracked scalar outputs remain limited to unqualified object and parameter families. Qualified pointees or output slots, non-scalar complete aliases, deeper pointers, pointer arrays, aggregate fields, tracked-output casts and function returns, address-taking of tracked two-level objects, arithmetic, relational ordering, and compound updates remain unsupported or retain targeted diagnostics.
...
- Added the first safe tracked unqualified `double **` object/output-parameter slice without exposing host addresses. Local, file-global, and block-static objects preserve mutable `double *` slot identity through null/default initialization, compatible forwarding, indirect pointer/scalar reads and writes, equality, truthiness, and non-evaluating `sizeof`.
- Preserved caller-owned pointee owner, lifetime, constness, array identity, and static-storage rules. Indirect scalar reads and writes reuse Cust's deterministic binary64 storage path.

### Diagnostics and verification

- Added eight focused interpreter regressions plus one registered warning-free native fixture. Exact boundary coverage includes qualified/incompatible slots, deeper pointers, pointer arrays, aggregate fields, pointer returns, tracked-object address-taking, arithmetic, ordering, compound updates, callee-local/parameter escapes, and unrelated calls that must not observe dangling-but-unused values.

### CLI, packaging, and verification

- Versioned the Cargo package, exact CLI output, and Docker Compose runtime/test images as `0.53.0`.
- Reconciled an executable inventory of 2,216 tests: 2,072 interpreter tests, 101 deterministic fuzz-safety tests, 33 CLI tests, 6 pointer-classifier parity tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

### Known limitations

- Cust remains a deterministic educational C subset, not a full C implementation or native-ABI emulator.
- Tracked `double **` support is deliberately limited to unqualified local/file-global/block-static objects and function parameters over mutable `double *` slots. Qualified or incompatible slots, deeper pointers, pointer arrays, aggregate fields, pointer-return types, address-taking of tracked two-level objects, arithmetic, relational ordering, and compound updates remain unsupported. Shared union object-byte and other existing subset boundaries remain unchanged.
...
- Extended shared scalar-union object bytes from selected members to complete union-object ranges across exactly prototyped bounded `memcpy`, `memmove`, `memcmp`, `memset`, and `memchr`. Whole-object pointers reuse the canonical aggregate root at offset zero and expose the union's maximum layout size through the existing deterministic non-const full-width `int` or `char` carrier.

### Diagnostics and verification

- Preserved selected-member-local capacity, alias synchronization, nested and aggregate-array-element identity/isolation, recursive const and lexical-lifetime checks, overlap, zero-count identity, typed aligned/interior `memchr` results, and direct/nested non-evaluation. All-`_Bool` unions, layouts without a non-const carrier as wide as the largest member, and layouts containing arrays, pointers, `double`, or nested aggregates retain targeted rejection. Four focused interpreter tests and the expanded warning-free ABI-independent compiler-oracle fixture cover the release delta.

### CLI, packaging, and verification

- Versioned the Cargo package, exact CLI output, and Docker Compose runtime/test images as `0.45.0`.
- Reconciled an executable inventory of 2,121 tests: 1,985 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

### Known limitations

- Cust remains a deterministic educational C subset, not a full C implementation or native-ABI emulator.
- Shared union object bytes remain limited to scalar-only non-`double` layouts with a non-const full-width `int` or `char` carrier. All-`_Bool` layouts, layouts without a full-width writable carrier, and union layouts containing arrays, pointers, `double`, or nested aggregates remain unsupported. Selected-member operations retain that member's own capacity; complete-object operations use the union's maximum layout size. General pointer levels beyond the narrow tracked unqualified `char **` model, variable-length arrays, arrays with more than two dimensions, flexible array members, bit-fields, `goto`, system headers, and unsupported pragma semantics also remain outside the release.
...
- Versioned the Cargo package, exact CLI output, and Docker Compose runtime/test images as `0.40.0`.
- Reconciled an executable inventory of 2,079 tests: 1,943 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

### Known limitations

- Cust remains a deterministic educational C subset, not a full C implementation or native-ABI emulator.
- The bounded raw-memory slice accepts standalone scalar and one-dimensional-array `double` storage under Cust's fixed eight-byte little-endian IEEE-754 binary64 representation. Aggregate-field, two-dimensional, union-backed, pointer-object, and unprovable helper-returned double storage remain unsupported. The broader bounded `double` language slice still excludes `float`, `long double`, hexadecimal/suffixed/non-finite floating literals, deeper double pointers, arrays of double pointers, pointer-to-row forms such as `double (*)[N]`, whole-array and row addresses, union-backed double field addresses/decay, atomic double-pointer aliases, direct one-dimensional double-array parameter declarators, direct and alias-spelled two-dimensional double array parameters, array returns, two-dimensional double aggregate fields/compound literals, and three-dimensional double arrays. Complex runtime values, host floating ABI/promotion behavior, general pointer levels beyond the narrow tracked unqualified `char **` model, variable-length arrays, arrays with more than two dimensions, flexible array members, bit-fields, `goto`, system headers, and other pragma semantics also remain outside the release.
...
- Added direct fixed two-dimensional `double[R][C]` objects at local, file-global, and block-static scope with nested positional initialization and zero fill, double-index reads and scalar replacement/compound/prefix/postfix updates, one-time row and column index evaluation, declaration lists, scalar `_Generic`, and deterministic full-object/row/element `sizeof` plus `_Alignof` relationships.

### Diagnostics and verification

- Retained targeted const, row/column bounds, incomplete-rank, pointer-to-row, parameter, aggregate-field, compound-literal, three-dimensional, and raw-memory boundaries in evaluated and non-evaluating contexts. Checked integer-constant folding, dimension multiplication, size calculation, and fallible backing-storage reservation convert source-controlled overflow or allocation failure into Cust diagnostics instead of Rust host panics. Six focused interpreter tests and one registered warning-free GCC/Clang compiler-oracle fixture cover the package.

### CLI, packaging, and verification

- Versioned the Cargo package, exact CLI output, and Docker Compose runtime/test images as `0.38.0`.
- Reconciled an executable inventory of 2,073 tests: 1,937 interpreter tests, 99 deterministic fuzz-safety tests, 33 CLI tests, 2 Docker metadata tests, 1 compiler-oracle harness, and 1 repository-license test.

### Known limitations

- Cust remains a deterministic educational C subset, not a full C implementation or native-ABI emulator.
- The bounded `double` slice supports direct and typedef-backed scalar/function/one-dimensional-array/aggregate-field storage, direct and typedef-backed one-dimensional array compound literals, safe one-level pointers to standalone and supported struct-field scalar/one-dimensional-array storage, and direct fixed two-dimensional objects. `float`, `long double`, hexadecimal/suffixed/non-finite floating literals, deeper double pointers, arrays of double pointers, pointer-to-row forms such as `double (*)[N]`, whole-array and row addresses, union-backed double field addresses/decay, atomic double-pointer aliases, direct double-array parameter declarators, two-dimensional double typedef aliases/aggregate fields/compound literals, three-dimensional double arrays, and raw-memory operations over double storage remain unsupported. Complex runtime values, host floating ABI/promotion behavior, general pointer levels beyond the narrow tracked unqualified `char **` model, variable-length arrays, arrays with more than two dimensions, flexible array members, bit-fields, `goto`, system headers, and other pragma semantics also remain outside the release.
