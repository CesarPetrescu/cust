...
Initial recommended work package if no blocker exists: finish the next unchecked item in status/missing-features.md or status/todo.md, currently pointer parameter/array/string decay support, interpreter-only conformance fixtures, fuzz/property tests, CLI flags, README examples, release notes/tags, and license work. Prefer implementing code and tests over merely describing improvements. Do not add GitHub Actions or external CI unless the user explicitly asks later; this project relies on local Docker verification run by the autonomous agent. Do not add tests or tooling that compile user C programs with GCC, Clang, cc, or clangd; Cust is the interpreter and must execute programs itself.
```
