...
- Top-level, local, static-local, by-value parameter, return-value, and one-dimensional array variables: `union Number n;`, `union Number n = {5};`, `union Number make_number(int value);`, `union Number values[2] = {{1}, {2}};`
- One-level union pointers and pointer fields inside unions: `union Number *p = &n;`, `union Number *p = &values[1];`, `p->value`, `(*p).tag`, `union Node { int value; union Node *next; };`, and scalar pointer fields such as `union External { int *external; };`
...
- Pointer arithmetic on union pointers, arrays of pointer fields, and pointer-to-pointer union fields remain unsupported unless a later milestone extends the safe pointer model deliberately.
...
- `tests/fixtures/compat/valid/unions.c`, `tests/fixtures/compat/valid/nested_union_fields.c`, `tests/fixtures/compat/valid/union_pointers.c`, and `tests/fixtures/compat/valid/union_return_functions.c` compare supported scalar union behavior against a native C compiler as an external oracle.
